...
| Versión | — | 3.0 |
| Fecha | — | 2026-08-17 |
| Estado | — | **Aprobado** (confirmado por el Product Owner el 2026-08-16). Migraciones 6.0 → 8.6, 8.6 → 8.11 y 8.11 → 9.9 **cerradas** |
...
**Reescrita por la fase M5 de la migración 8.11 → 9.9, el 2026-08-17.** La emisión anterior declaraba
el conjunto **8.11**, cerrado por la M5 de la migración 8.6 → 8.11 el 2026-08-16. **Este salto sí
alcanzó artefactos de este destino, a diferencia del anterior**: las once reglas de categoría
aplicadas, las cuatro transversales, `Root-Rules` y las dos plantillas **cambiaron todas de versión**.

**Y aun así el trabajo fue de siete documentos, porque la mayor parte del salto ya estaba aplicada
antes de que la migración empezara.** Lo que la 8.17 y la 9.0 traían para el corpus —la cabecera de
nivel de todo documento generado y el renombre del artefacto de la categoría `05`— lo reparó la salida
A del orquestador de reanudación el mismo 2026-08-17, con residuo cero verificado, antes de esta
migración. La lista de qué cambió artefacto por artefacto, con su severidad y con las tres superficies
que quedaban vivas, está en `SDD/Docs/Audit/Plan-Migracion-8.11-a-9.9.md` §2, §3 y §4.

**Las versiones de esta tabla se leyeron del snapshot `_legacy/9.9/` del repositorio del framework**,
y no de sus archivos vivos: el framework publicó **9.10** durante la corrida, y el plan había
congelado el objetivo en **9.9** declarando que no se reabre. La procedencia dice contra qué se migró
efectivamente, que sigue siendo cierto aunque exista una versión posterior.

| Artefacto del framework | Versión |
|---|---|
| Framework SDD (conjunto) | **9.9** |
| `Master-Prompt` | 8.4 |
| `Master-Prompt-Migracion` | 2.7 |
| `Master-Prompt-Reanudacion` | 1.6 |
| `Root-Rules` | 6.1 |
| `Rules-Contexto` | 4.3 |
| `Rules-Necesidades-Negocio` | 4.2 |
| `Rules-Especificacion-Funcional` | 5.3 |
| `Rules-UX-UI-DX` | 5.3 |
| `Rules-Arquitectura-Tecnica` | 4.3 |
| `Rules-Backlog-Tecnico` | 4.3 |
| `Rules-Plan-Sprint` | 5.3 |
| `Rules-Calidad-Y-Pruebas` | 4.4 |
| `Rules-Devops` | 4.5 |
| `Rules-Examples` | 6.3 |
| `Rules-Documentacion` | 5.3 |
| Reglas transversales aplicadas | `Intake-Rules` 4.1, `Vocabulario-Rules` 3.1, `Maqueta-Rules` 4.2 y `Deriva-Rules` 5.2 (Fase B2 confirmada para la unidad de entrega `GeometriaFactory-Web`). **`Migracion-Rules` 3.7 sí aplica**: este árbol atravesó **tres** migraciones normativas, 6.0 → 8.6, 8.6 → 8.11 y 8.11 → 9.9, con sus informes en `SDD/Docs/Audit/` |
