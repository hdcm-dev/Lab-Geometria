...
| Versión | — | 3.1 |
| Fecha | — | 2026-08-17 |
| Estado | — | **Aprobado** (confirmado por el Product Owner el 2026-08-16). Migraciones 6.0 → 8.6, 8.6 → 8.11, 8.11 → 9.9 y 9.9 → 9.10 **cerradas** |
...
**Reescrita por la fase M5 de la migración 9.9 → 9.10, el 2026-08-17.** Es la **cuarta** migración
normativa de este destino y la primera con **alcance documental cero**: **veintiuno de los veintidós
artefactos del framework no cambiaron de versión**, y el único que se movió —`Migracion-Rules`
3.7 → 3.8— agrega reglas de **cómo consolidar al fundir árboles**, que gobiernan la corrida y no la
forma de ningún documento del destino.

**Se verificó sobre el árbol y no se supuso:** cero carpetas `_fusion/`, ninguna fila del salto
anterior clasificada «regenerar», y la única consolidación del producto —la de la migración
6.0 → 8.6— cerrada y auditada bajo su propia normativa. La verificación artefacto por artefacto está
en `SDD/Docs/Audit/Plan-Migracion-9.9-a-9.10.md` §2 y §3, **escrita antes de tocar esta tabla**, que
es la condición que `Master-Prompt-Reanudacion.md` §4 exige para actualizar la procedencia sin migrar
documentos.

| Artefacto del framework | Versión |
|---|---|
| Framework SDD (conjunto) | **9.10** |
| `Master-Prompt` | 8.4 |
...
| Reglas transversales aplicadas | `Intake-Rules` 4.1, `Vocabulario-Rules` 3.1, `Maqueta-Rules` 4.2 y `Deriva-Rules` 5.2 (Fase B2 confirmada para la unidad de entrega `GeometriaFactory-Web`). **`Migracion-Rules` 3.8 sí aplica**: este árbol atravesó **tres** migraciones normativas, 6.0 → 8.6, 8.6 → 8.11, 8.11 → 9.9 y 9.9 → 9.10, con sus informes en `SDD/Docs/Audit/` |
