...
**Las tres muestras `S-1`, `S-2` y `S-3` no son el conjunto de las carpetas de `/samples`** [PRECISADO 2026-08-11]. Son las tres **demostraciones nombradas** que este documento distingue por su papel: la que prueba el punto de extensión, la que ejercita la API y la que provee los datos. §16.1 asigna carpeta a **seis** de los siete proyectos, y esas carpetas **contienen** a las tres muestras además de otras. Confundir las dos listas fue lo que hizo que §16.1 y §18 se contradijeran durante veintitrés versiones sobre `Infrastructure`, a quien §18 asignaba `S-3` mientras §16.1 le negaba carpeta.
