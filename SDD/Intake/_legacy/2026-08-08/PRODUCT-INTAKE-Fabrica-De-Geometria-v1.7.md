...
- [Parte A — Negocio del producto](#parte-a--negocio-del-producto)
  ...
  - [§4 Alcance funcional pretendido (MoSCoW)](#4-alcance-funcional-pretendido-moscow)
    - [§4.1 Reglas de negocio declaradas (RN-01 a RN-09)](#41-reglas-de-negocio-declaradas-rn-01-a-rn-09)
...
### §4.1 Reglas de negocio declaradas (RN-01 a RN-09)
...
**Dos reglas nuevas que el circuito de revisión introduce** y que no existían en RF §7, declaradas por el Product Owner el 2026-08-08:
...
| RN-12 | **El reseteo de contraseña conserva la cuenta y sus trabajos.** El administrador fija una contraseña **provisoria**; la cuenta queda marcada como *con cambio de contraseña pendiente* y **conserva su estado de habilitación**, su papel, su identidad y **todos sus trabajos con sus estados y comentarios**. Resetear no es dar de baja: no dispara RN-07 | Un alumno con tres trabajos —uno en `Borrador`, uno en `Rechazado` y uno en `Finalizado`— conserva los tres, con sus comentarios, después del reseteo. Ingresa con la provisoria y los ve [DECISIÓN 2026-08-09] |
| RN-13 | **Mientras la contraseña provisoria no se cambie, la cuenta no llega a ninguna otra parte del sistema.** Ingresa, y lo único que puede hacer es cambiar la contraseña; cualquier otra ruta la devuelve al cambio. Al cambiarla, la marca se levanta y la cuenta opera con normalidad. La contraseña nueva la elige el alumno y **el administrador no la conoce** | Un alumno reseteado que intenta abrir el listado de trabajos o enviar uno termina en el cambio de contraseña, sin haber leído ni escrito nada. Después de cambiarla, la misma navegación funciona [DECISIÓN 2026-08-09] |
...
| CL-7 | ¿Qué pasa si el alumno olvida su contraseña? | **El administrador se la resetea** desde su panel (F-26): le fija una contraseña provisoria, se la comunica, y el alumno **está obligado a cambiarla en su próximo ingreso**. La cuenta y todos sus trabajos **se conservan**. No hay recuperación autónoma, porque no hay correo | RF §8, F-26 [DECISIÓN 2026-08-09] |
...
|---|---|---|
| GeometriaFactory-Api | `rest-api` | Colección de peticiones HTTP reproducible con los escenarios E-1 a E-7 como cuerpo: alta de trabajo, envío con JSON válido e inválido, y aprobación y rechazo por el administrador, con los códigos de respuesta esperados |
...
| INV-09 | Una cuenta con la marca de **cambio de contraseña pendiente** no ejerce ninguna capacidad del sistema salvo cambiar su propia contraseña. La marca la pone **únicamente** el reseteo del administrador, y la levanta **únicamente** el cambio efectivo hecho por la propia cuenta | RN-12, RN-13, F-26 [DECISIÓN 2026-08-09]. Es lo que hace que la provisoria sea provisoria: sin este invariante, una clave que el administrador conoce quedaría sirviendo indefinidamente para operar como el alumno |
...
**Ocho invariantes desde el 2026-08-09.** Los invariantes no son reglas distintas de las de §4.1: son las mismas vistas desde el dominio. La regla declara qué decidió el negocio; el invariante declara qué condición sobre los datos no puede romperse nunca. Las tres reglas que no tienen invariante asociado —RN-07 baja física, RN-08 texto íntegro, RN-09 error con ubicación— no lo tienen porque describen comportamientos y no condiciones permanentes sobre el estado. RN-11 tampoco: es una regla de alcance de consulta, no una condición sobre los datos.
...
**El JSON del alumno no lleva fechas y no se le agrega ninguna** [DECISIÓN 2026-08-09]. El texto que produce `Describir()` contiene únicamente figuras con sus dimensiones y sus valores calculados —ver los siete escenarios de §20—, y el producto **no lo modifica** (RN-08). Los sellos viven en la fila del trabajo, nunca dentro del texto conservado.
...
4. **La distinción de responsabilidad**: el visor informa por qué no dibujó una pieza; **decidir si el trabajo pasa a `Pendiente` es del validador**, no del bundle (RA-02). Este escenario no prescribe el desenlace del envío.
...
| Dimensión no legible en el visor | **E-8** | La pieza no se dibuja y se enumera con índice y código | Etapa `g` [DECISIÓN 2026-08-09] |
...
| A-2 | Los targets de las cuatro métricas de negocio (7 de 7 etapas, ≥ 80 % de alumnos que entregan, 100 % de entregas revisadas, ≥ 1 advertencia por alumno) | §8 | Ninguna fuente fija métricas de negocio; los criterios de los que se derivan sí están declarados | Sólo cambia §8 y lo que la categoría 01 derive de ahí |
