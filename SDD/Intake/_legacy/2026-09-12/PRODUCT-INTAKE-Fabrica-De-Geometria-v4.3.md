...
| X-9 | Pasarela de reenvío `/api/*` en el front | [DECISIÓN, RT §2.4] Hoy ningún JavaScript del navegador toca la API (RA-01), de modo que la pasarela sólo consumiría el recurso más escaso del plan gratuito. Queda **especificada** para adoptarla sin rediseño. **Reevaluada el 2026-09-12 por `E-02`**: la API pasa a ofrecerse a clientes externos (§17.1.P.3 · GeometriaFactory-Api), pero esos clientes hablan directo con la API, no a través del front — la condición de RA-01 que motiva esta exclusión sigue sin darse para `GeometriaFactory.Web`, de modo que la pasarela **sigue sin hacer falta** | Si aparece descarga de archivos, carga directa desde el navegador o migración a WebAssembly, o si el cliente externo de `D-01` resulta ser JavaScript de navegador que el front deba intermediar |
...
| Quién la consume | `GeometriaFactory.Web`, servidor a servidor (RA-01, sin cambios para el front propio), **y clientes externos** [DECISIÓN 2026-09-12, PO, mesa `Mesa-2026-09-12-ciclo-2.md`]: «la API debe ser expuesta públicamente; la idea es ofrecerla para otros clientes». Cambio de alcance sobre lo declarado hasta la versión 4.2. **Qué cliente en concreto es queda diferido** (ítem diferido `D-01`, `Mesa-2026-09-12.md` §8): el PO no lo tiene declarado todavía —«podría ser otro front, una aplicación MAUI, aún no sé»—; el diseño toma el caso general: credenciales **por cliente** (API key o client-credentials, RFC 6749 §4.4) además del ROPC de alumnos (§17.1.P.5) |
...
| CORS | No hace falta **para `GeometriaFactory.Web`** (RT §2.2, RA-01 intacta para el front propio). **Diferido para clientes externos** hasta que `D-01` se cierre con un cliente que sea JavaScript de navegador de otro origen; si el cliente que aparezca es servidor a servidor, CORS no llega a hacer falta nunca |
| Endpoint de autenticación | `POST /auth/token` con correo y contraseña, para alumnos y administrador (ROPC, sin cambios). **Los clientes externos no usan este endpoint**: su vía de autenticación es la de la fila «Quién la consume» |
...
**Autenticación de clientes externos, aparte del ROPC de alumnos** [DECISIÓN 2026-09-12, PO + mesa `Mesa-2026-09-12-ciclo-2.md` §4 punto 3, resolviendo `E-02`]: ROPC no sirve para un cliente externo real, porque obligaría a ese cliente a manejar la contraseña de un alumno, que es exactamente lo que OAuth 2.1 desaconseja fuera del propio sistema. Un cliente externo se autentica por **clave de API o `client_credentials`** (RFC 6749 §4.4), una credencial **por cliente**, con revocación propia; una clave inválida rechaza con `401` y una válida autentica sin tocar ninguna cuenta de alumno. El ROPC de alumnos **no se toca**. Sin límite de tasa declarado hasta hoy (`RS-OPE-01`): un cliente sin cuota agotaría el único escritor de SQLite (§17.1.P.4), por lo que el plan de `Roadmap-Producto.md` fila `k` ordena el límite de tasa **antes** de habilitar rutas versionadas a terceros.
