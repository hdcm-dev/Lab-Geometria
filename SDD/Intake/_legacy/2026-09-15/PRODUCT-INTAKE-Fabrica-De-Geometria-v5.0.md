...
- [Parte A — Negocio del producto](#parte-a--negocio-del-producto)
  ...
  - [§4 Alcance funcional pretendido (MoSCoW)](#4-alcance-funcional-pretendido-moscow)
    - [§4.1 Reglas de negocio declaradas (RN-02001 a RN-02016)](#41-reglas-de-negocio-declaradas-rn-01-a-rn-16)
...
- [Parte C — Técnica por unidad de entrega](#parte-c--técnica-por-unidad-de-entrega)
...
|---|---|---|
| **RF** | `PROMPTs/03-Ejecutar-Prompt-Integrador-Documento-Intake/INPUTs/Requerimientos-Funcionales.md` | Decisiones funcionales y de planeamiento cerradas: RF-01 a RF-20, RN-02001 a RN-02009, estados, mapa de navegación, etapas `a` a `g` |
...
Las capacidades Must Have son la traducción directa de los requerimientos RF-01 a RF-20, que la fuente declara cerrados. Las Should, Could y Won't salen de RF §10 (etapa `i` y siguientes, que la fuente numeraba `h`) y de RF §8 (fuera de alcance). Las capacidades que **no vienen de RF** son decisión del Product Owner y están fechadas en su fila: las **cuatro** del circuito de revisión del 2026-08-08 (F-21, F-22, F-23, F-24) y las **dos** del 2026-08-09, F-26 y F-25. F-13 sí viene de RF y lo que cambió el 2026-08-10 fue su prioridad, no su origen.

| ID | Capacidad | MoSCoW | Origen |
|---|---|---|---|
| F-01 | Configurar la cuenta de administrador en el primer arranque, y sólo mientras no exista ninguna | Must Have | RF-01, RF-02, RN-02001 |
| F-02 | Registro de alumno con correo, nombre y apellido, sin elegir contraseña | Must Have | RF-03, RF-04 |
| F-03 | Habilitar, bloquear, rehabilitar y dar de baja física cuentas de alumno desde el panel del administrador | Must Have | RF-05, RF-08, RN-02007 |
| F-04 | Establecer contraseña en el primer ingreso efectivo del alumno, sin envío de correo. **El alumno se identifica con la contraseña provisoria que el sistema produce al habilitarlo** —el mismo mecanismo del reseteo (F-26)— y la cambia por el camino que RN-02013 ya fija | Must Have | RF-06, RF-07, RN-02016 [PRECISADO 2026-08-09] |
...
| F-07 | Reeditar y eliminar el trabajo **sólo mientras está en `Borrador`**, que es el estado en el que queda cuando el JSON no verifica | Must Have | RF-15, RF-17, RN-02004 [DECISIÓN 2026-08-08] |
...
| F-09 | Validar el JSON con la tolerancia de claves del emisor real y reportar errores con índice de figura y campo | Must Have | RF-13, RN-02009, RT §6.3 |
| F-10 | Verificar `Area` y `Volumen` recalculándolos desde las dimensiones y emitir advertencias que no bloquean | Must Have | RF-20, RN-02005, RT §6.5 |
...
| F-14 | Despliegue real: front y backend en contenedores del servidor propio de i7infra, publicados por túnel bajo dominio propio (`geometria.aplicada.stream`), con PT-05 medida desde la facultad. **[DECISIÓN 2026-09-06]**: realiza `X-10` —el dominio propio apareció y el túnel satisface mejor la razón que originó la partición front/hosting externo—. El canal de FTP a somee.com (`.github/workflows/deploy-front-ftp.yml`) **se conserva como alternativa documentada y antecedente de despliegue, sin disparador automático**, no como canal vigente | Should Have | RF §10 etapa `h`, hoy etapa `i`, RT §13, §14 |
| F-15 | Panel de resumen del administrador: cantidad de trabajos por alumno y por estado | Could Have | RF §10 etapa `h`, hoy etapa `i` |
| F-16 | Exportar el trabajo (JSON original y captura de la escena) | Could Have | RF §10 etapa `h`, hoy etapa `i`, AN §12.5 P2 |
...
| F-19 | Múltiples administradores, roles configurables y permisos finos | Won't Have v1 | RF §8, RN-02001 |
| F-20 | Edición o corrección del JSON del alumno desde la aplicación | Won't Have v1 | RN-02008, RF §8 |
...
### §4.1 Reglas de negocio declaradas (RN-02001 a RN-02016)

> **Las reglas se renumeraron a cinco dígitos por la migración M2/M9, el 2026-08-16.** `RN-01` a
> `RN-16` pasan a **`RN-02001` a `RN-02016`**, con el mismo enunciado y el mismo orden: es un cambio
> de forma del identificador, no de contenido. Se hizo porque `Root-Rules.md` §9.2 fija cinco dígitos
> uniformes y **enumera `RN` entre las familias alcanzadas**, y porque el árbol migrado ya numeraba
> estas mismas reglas así: hasta hoy convivían `RN-15` y `RN-02015` para la misma regla, sin que nada
> dijera que eran la misma. **377 citas reconectadas** desde
> `SDD/Docs/Audit/Migracion-M9-Registro-Citas-RN.json`.


Las nueve reglas están cerradas por RF §7 y se transcriben acá **completas**, porque la versión anterior de este intake las citaba por identificador sin enunciarlas y dos de ellas —RN-02002 y RN-02006— no aparecían en ninguna sección. Sin enunciado, ninguna categoría aguas abajo puede derivarlas.

| Id | Enunciado | Cómo se verifica |
|---|---|---|
| RN-02001 | Existe **exactamente un** administrador. Su alta sólo es posible mientras no exista ninguno | Intentar la ruta de alta inicial con administrador existente redirige a inicio de sesión |
| RN-02002 | El correo del alumno es **único** | Registrar dos veces el mismo correo se rechaza con mensaje explícito |
| RN-02003 | Un alumno **sólo ve y opera sus propios trabajos** | Pedir el trabajo de otro alumno por su identificador devuelve «no encontrado», no «no autorizado» |
| RN-02004 | El alumno elimina sus trabajos **sólo en `Borrador`**. El **administrador elimina cualquier trabajo que ve**, en cualquier estado, con borrado físico | La acción no se ofrece al alumno en otros estados **y** se rechaza en el backend si se fuerza. El borrado del administrador se verifica sobre un trabajo en `Pendiente` [DECISIÓN 2026-08-08] |
| RN-02005 | Un trabajo **no pasa a `Pendiente` con errores de interpretación** del JSON. Las advertencias **sí** lo permiten | Enviar un trabajo con tipo desconocido lo deja en `Borrador`; enviar uno con advertencias de valor lo pasa a `Pendiente` [DECISIÓN 2026-08-08: el corte se adelanta del cierre al envío] |
| RN-02006 | Una cuenta `Pendiente` o `Bloqueada` **no obtiene sesión** | Intentar ingresar con una cuenta recién registrada devuelve el motivo, sin sesión |
| RN-02007 | La **baja física** de una cuenta elimina la cuenta y **todos sus trabajos**, y exige confirmación explícita escribiendo el correo de la cuenta | Se verifica que no quede ningún trabajo del alumno dado de baja |
| RN-02008 | El **JSON original del alumno se conserva íntegro** y nunca se reescribe | El texto guardado es idéntico al pegado |
| RN-02009 | Los mensajes de error de validación indican **índice de figura y campo**, nunca un texto genérico | Cargar un tipo desconocido produce el índice y el campo `Tipo` |

**Reglas nuevas que no existían en RF §7**, declaradas por el Product Owner. Las dos primeras —**RN-02010** y **RN-02011**— las introduce el **circuito de revisión** el 2026-08-08. Las cuatro siguientes —**RN-02012** a **RN-02015**— las introduce el **reseteo de contraseña** (F-26) el 2026-08-09. La séptima —**RN-02016**— la introduce la **identificación en el primer ingreso** el 2026-08-09, y unifica la credencial inicial con el mecanismo del reseteo. Se listan en el orden en que se decidieron. [ROTULADO CORREGIDO **dos veces** el 2026-08-09: primero decía «Dos reglas nuevas» encabezando **seis** filas, y tras corregirlo volvió a quedar corto al entrar RN-02016, con lo cual encabezaba **siete**. El rótulo enumera tandas y por eso envejece cada vez que entra una: si entra una octava regla, esta frase hay que tocarla otra vez.]

| Id | Enunciado | Cómo se verifica |
|---|---|---|
| RN-02010 | **Sólo el administrador aprueba o rechaza** un trabajo, y sólo desde `Pendiente`. `Finalizado` y `Rechazado` son terminales: ninguna transición sale de ellos | Un alumno que fuerce la transición contra el backend es rechazado |
| RN-02011 | El administrador **no ve los trabajos en `Borrador`**: no forman parte de su flujo de trabajo | El listado del administrador sobre un alumno con un borrador y un pendiente devuelve sólo el pendiente |
| RN-02014 | **La contraseña provisoria la produce el sistema, no la escribe el administrador**, y la superficie se la muestra para que se la comunique. La provisoria **no es adivinable** y **no se repite** entre cuentas ni entre reseteos de la misma cuenta | Dos reseteos consecutivos sobre la misma cuenta producen provisorias distintas, y ninguna es derivable del nombre, del correo ni de la fecha [DECISIÓN 2026-08-09: si la escribe el docente, en la práctica termina siendo la misma clave para toda la comisión, y el panel cargaría con un campo de contraseña ajena] |
| RN-02015 | **Resetear no exige que la cuenta esté habilitada.** Procede sobre `Pendiente`, `Habilitado` y `Bloqueado`: es una operación sobre la credencial y **no una transición de la máquina de estados de la cuenta**, de modo que no altera el estado ni requiere un orden previo. Sigue sin admitirse sobre la cuenta de administrador (INV-08) | Resetear la contraseña de una cuenta `Bloqueado` funciona y la deja `Bloqueado`; habilitar y resetear la contraseña en cualquiera de los dos órdenes termina igual [DECISIÓN 2026-08-09: el administrador no tiene que acordarse de una secuencia] |
| RN-02012 | **El reseteo de contraseña conserva la cuenta y sus trabajos.** El sistema produce una contraseña **provisoria**; la cuenta queda marcada como *con cambio de contraseña pendiente* y **conserva su estado de habilitación**, su papel, su identidad y **todos sus trabajos con sus estados y comentarios**. Resetear no es dar de baja: no dispara RN-02007 | Un alumno con tres trabajos —uno en `Borrador`, uno en `Rechazado` y uno en `Finalizado`— conserva los tres, con sus comentarios, después del reseteo. Se autentica con la provisoria, cambia la contraseña y los ve [DECISIÓN 2026-08-09] |
| RN-02016 | **Habilitar una cuenta produce una contraseña provisoria**, con las mismas propiedades y el mismo tratamiento que la del reseteo (RN-02014): el sistema la produce, la pantalla se la muestra al administrador para que se la comunique, y la cuenta queda con **cambio de contraseña pendiente** (INV-09). En consecuencia **no existe ninguna escritura anónima de credencial**: toda operación que fija o cambia una contraseña ocurre con la cuenta ya autenticada. [PRECISADO 2026-08-09: la redacción de 1.13 decía «ninguna escritura anónima **en el sistema**», que era falso —el **registro de cuenta** de RF-03 es anónimo por diseño y así debe seguir: es como el alumno entra al laboratorio—. Lo que RN-02016 elimina es la escritura anónima **de contraseña**, que es la que abría el agujero] | Habilitar a un alumno y entrar con la provisoria lleva al cambio obligatorio y a ninguna otra ruta. No hay ningún punto de acceso que acepte un correo y una contraseña nueva sin credencial [DECISIÓN 2026-08-09: la alternativa —un punto anónimo con correo y contraseña elegida— dejaba que cualquiera que conociera un correo habilitado le fijara la contraseña a esa cuenta antes que su dueño. Además unifica: un solo mecanismo de credencial inicial en vez de dos] |
| RN-02013 | **Mientras la contraseña provisoria no se cambie, la cuenta no llega a ninguna otra parte del sistema.** **Se autentica pero no obtiene sesión de trabajo**: el sistema reconoce la credencial provisoria y, en lugar de admitirla, la deriva al cambio de contraseña, que es lo único que puede hacer. Una vez cambiada, entra con normalidad. [PRECISADO 2026-08-09: la versión 1.7 decía «ingresa», que se leía como que obtenía sesión. Emitir sesión a una cuenta que por INV-09 no ejerce ninguna capacidad es contradictorio, y la diferencia es observable en la API; se resuelve del lado de no emitirla, que además es el paralelo exacto del primer ingreso con contraseña no fijada.] Al cambiarla, la marca se levanta y la cuenta opera con normalidad. La contraseña nueva la elige el alumno y **el administrador no la conoce** | Un alumno con la contraseña reseteada que intenta abrir el listado de trabajos o enviar uno termina en el cambio de contraseña, sin haber leído ni escrito nada. Después de cambiarla, la misma navegación funciona [DECISIÓN 2026-08-09]  **Con qué se autentica el cambio forzado** [PRECISADO 2026-08-14]: la operación de cambio de contraseña admite **dos formas de autenticarse** — con **sesión de trabajo**, que es el cambio corriente que cualquier cuenta puede hacer cuando quiera, incluida la del administrador; y con **la contraseña actual**, que es el caso del cambio forzado. El alumno reseteado **entra con la provisoria que el administrador le comunicó**, y esa misma provisoria es la que autentica el cambio. Sin esta precisión la pantalla del cambio forzado era **inalcanzable**: la operación quedaba detrás de la guardia de sesión y la cuenta marcada no obtiene sesión, de modo que el alumno reseteado no podía llegar a cambiar su clave. Es la misma forma que ya tiene el establecimiento de la primera contraseña, y no afloja `RN-02013`: sigue sin haber sesión de trabajo hasta que la contraseña se cambie. ### §4.2 Modelo de estados del trabajo|

```mermaid
stateDiagram-v2
    [*] --> Borrador: el alumno crea el trabajo
    Borrador --> Borrador: reedita, o envía y el JSON no verifica
    Borrador --> Pendiente: envía y el JSON verifica (RN-02005)
...
|---|---|---|---|---|---|
| `Borrador` | El JSON todavía no verifica, o el trabajo recién se creó | **Sí** | **Sí** | **No** (RN-02011) | — |
...
4. Como **alumno**, quiero que el sistema me diga en qué figura y en qué campo está el problema, para no tener que adivinar dónde falla mi salida (RN-02009).
...
7. Como **administrador**, quiero ver los trabajos de la comisión agrupados y filtrados por alumno, sin los borradores que los alumnos todavía están armando, para revisar la entrega de una sola vez (RF-18, RN-02011).
...
| CL-3 | ¿Qué pasa si el JSON llega mal formado o incompleto? | El trabajo **queda en `Borrador`** con su texto crudo conservado y no pasa a `Pendiente`: el envío es una sola acción y el estado lo decide la interpretación del JSON. El mensaje indica índice de figura y campo, nunca un texto genérico. El alumno corrige y vuelve a enviar | RF-15, RN-02005, RN-02009 [DECISIÓN 2026-08-08: el corte pasa del cierre al envío] |
| CL-10 | ¿Qué pasa si el administrador rechaza un trabajo y el alumno quiere corregirlo? | **Carga un trabajo nuevo.** `Rechazado` es terminal: el alumno no lo edita ni lo elimina, y queda como registro del intento. Sólo el administrador puede quitarlo | [DECISIÓN 2026-08-08], RN-02004, RN-02010, INV-07 |
...
| CL-4 | ¿Qué pasa si el valor calculado que trae el JSON está mal? | **No se corrige ni se rechaza: se señala** como advertencia, y el trabajo **pasa a `Pendiente` igual**. Es deliberado, y es el mayor valor didáctico del servicio. Que el administrador después lo apruebe o lo rechace es una decisión suya y no del validador | RF-20, RT §6.5, RN-02005 |
| CL-5 | ¿Qué pasa si un alumno pide por URL el trabajo de otro? | Devuelve «no encontrado», no «no autorizado»: no se confirma la existencia del recurso ajeno. Se verifica por pertenencia en el backend, no sólo ocultando el botón en la interfaz | RN-02003, INV-02 |
| CL-6 | ¿Qué pasa si alguien quiere borrar su cuenta o sus datos? | La baja física la ejecuta el administrador, elimina la cuenta **y todos sus trabajos**, y exige confirmación escribiendo el correo de la cuenta | RF-08, RN-02007 |
...
|---|---|---|---|---|
| Avance del producto | Etapas cerradas con OK explícito del agente humano en su punto de control, **sobre todas las etapas comprometidas** (`a` a `h`) | **8 de 8** [ASUNCIÓN en el target; DECISIÓN 2026-08-09: se cuentan todas las comprometidas, no siete. Con siete, el producto podía declararse completo con una etapa comprometida sin construir] | Sin plazo calendario: RT §1 declara «sin plazo; el avance se mide por etapas cerradas» | RF §10, RT §1 |
| **Entrega** del alumno | Alumnos habilitados que llegan a tener al menos un trabajo en `Pendiente` o posterior, sobre el total de alumnos habilitados | ≥ 80 % [ASUNCIÓN] | Al cierre de la cursada en que se use por primera vez | RF §4.2. **Partida el 2026-08-08**: mide lo que depende del alumno, y `Pendiente` es el primer estado que expresa una entrega efectiva, porque exige JSON interpretado sin errores (RN-02005) |
...
| Valor didáctico efectivamente entregado | Advertencias de valor declarado contra derivado que el sistema muestra sobre trabajos reales de alumnos | ≥ 1 advertencia visible por alumno que cargue un cubo de Ejemplo1 o un ortoedro [ASUNCIÓN] | Primera entrega de la cursada | RF §3.3 y AN §9.2: los defectos D3 y D4 están presentes en el 100 % de esas figuras, de modo que la métrica mide que el sistema las **muestre**, no que existan |
...
| ~~X-2~~ | ~~Recuperación de contraseña olvidada~~ | **Exclusión retirada el 2026-08-09.** Su justificación era que sin correo no hay canal de recuperación, y su salida declarada —dar de baja y volver a dar de alta— **destruía todos los trabajos del alumno** (RN-02007). El Product Owner incorporó **F-26**: el administrador resetea la contraseña desde su panel, con una provisoria que el alumno debe cambiar en su próximo ingreso. **Sigue excluida la recuperación autónoma por correo**, que es lo que X-1 impide | — |
| X-3 | Múltiples administradores, roles configurables, permisos finos | La aplicación es **básica** [DECISIÓN, RF §1]: el modelo es de dos roles fijos y de un único administrador (RN-02001, INV-05) | No previsto en este alcance |
| X-4 | Corrección o edición del JSON del alumno desde la aplicación | RN-02008: el JSON original se conserva íntegro y nunca se reescribe. El formato es premisa fija (AN §12.1) y es la única fuente fiel del trabajo | No previsto: contradice la premisa |
...
| X-6 | Modo despiece, exportación de imágenes y URL compartible del visor | Propuestas de AN §12.5 que no entraron en este alcance | Candidatos declarados de la etapa `h` de RF §10, **hoy etapa `i`** |
| X-7 | Ambientación a un problema real (depósito, costos, capacidades) | Propuesta de AN §12.3; no forma parte de esta aplicación | Candidato de la etapa `h` de RF §10, **hoy etapa `i`** |
| X-8 | Segundo factor de autenticación | No fue pedido; la elección de autenticación (ROPC) no lo bloquea | No previsto |
| X-9 | Pasarela de reenvío `/api/*` en el front | [DECISIÓN, RT §2.4] Hoy ningún JavaScript del navegador toca la API (RA-01), de modo que la pasarela sólo consumiría el recurso más escaso del plan gratuito. Queda **especificada** para adoptarla sin rediseño. **Reevaluada el 2026-09-12 por `E-02`**: la API pasa a ofrecerse a otras aplicaciones propias del producto (§17.1.P.3 · GeometriaFactory-Api), pero esas aplicaciones hablan directo con la API, no a través del front — la condición de RA-01 que motiva esta exclusión sigue sin darse para `GeometriaFactory.Web`, de modo que la pasarela **sigue sin hacer falta** | Si aparece descarga de archivos, carga directa desde el navegador o migración a WebAssembly, o si una aplicación propia de otro origen resulta ser JavaScript de navegador que el front deba intermediar (`D-01` cerrado por `ADR-00009`: el cliente es una aplicación propia, de la forma que sea) |
| X-10 | Túnel saliente con dominio propio para el backend | **[DECISIÓN, RT §15.1] REALIZADA el 2026-09-06** (ver F-14): apareció el dominio propio (`geometria.aplicada.stream`, cloudflare, i7infra) y se ejecutó el túnel para las dos piezas. Resuelve R-02, R-04 y R-05; el front deja de necesitar el hosting externo, que se conserva como canal alternativo | Reevaluar si aparece un dominio propio o si PT-01 obliga a mover el front — **cumplida** |
...
| RN-B3 | **El validador se escribe sin leer el análisis** y rechaza el dato real de los alumnos: es la falla que más veces se repite, porque el JSON del alumno no es JSON estrictamente válido | Alta si no se controla | **Alto**: la aplicación no sirve para el dato que existe | Batería obligatoria de **diez** casos de prueba con datos verificados —los nueve de RT §11 más el décimo de §21—, y los escenarios **E-1 a E-8** de la Parte D como fixtures. RF §10 etapa `f` lo marca como el criterio que más se rompe |
...
| ~~RN-B6~~ | ~~El alumno pierde su trabajo por olvidar la contraseña, sin canal de recuperación~~ | — | — | **Riesgo cerrado el 2026-08-09.** Se apoyaba en las exclusiones X-1 y **X-2**, y X-2 fue retirada por **F-26**: el administrador resetea la contraseña, la cuenta y todos sus trabajos se conservan (RN-02012) y no hace falta dar de baja a nadie. La mitigación que declaraba —advertir al alumno antes de la baja— **quedó sin objeto**, porque la baja dejó de ser el remedio del olvido. Lo que sobrevive de X-1 es la ausencia de recuperación **autónoma**: el alumno depende del administrador, y eso está declarado en CL-7, no acá |
| RN-B7 | **Las claves de protección de datos del front viven en un bind mount sin respaldo declarado.** Perder ese directorio invalida las sesiones abiertas de toda la comisión en medio de una clase (`Mesa-2026-09-12.md` §9, `E-05`) | Media | Bajo: un re-login, no hay datos en juego | **Aceptado por escrito** [DECISIÓN 2026-09-12, PO]: «front — riesgo aceptado y declarado (perder la clave DataProtection cuesta un re-login a los alumnos, no hay datos)». Sin respaldo de las claves del front; no se adopta mitigación de código |
| RN-B8 | **El archivo SQLite de `lab-geometria-api/data` no tiene respaldo declarado.** Es el único almacén de datos del producto (escritor único, §17.1.P.4): perderlo pierde todos los trabajos de la comisión, a diferencia de RN-B7 | Media | **Alto**: a diferencia del front, acá sí hay datos | **Requisito escrito, mecanismo fuera de este intake** [DECISIÓN 2026-09-12, PO]: «backend — se respalda el SQLite de `lab-geometria-api/data`». El requisito es que exista un respaldo restaurable de ese archivo; **el mecanismo lo implementa el Product Owner en su despliegue** (`~/docker/lab-geometria`, fuera del alcance de este repositorio), no un guion de este árbol. Criterio de verificación: una restauración de prueba desde el respaldo reproduce un archivo SQLite legible con las migraciones aplicadas. Ítem diferido `Mesa-2026-09-12.md` §8 hasta que el mecanismo quede declarado y corriendo |
...
| **Aprobar / Rechazar** | Las dos decisiones del administrador sobre un trabajo en `Pendiente`, y su facultad exclusiva. Aprobar equivale a **cerrar** el trabajo | F-23, RN-02010 [DECISIÓN 2026-08-08] |
...
| **Error de validación** | Defecto que impide interpretar el JSON como figuras. **Impide que el trabajo pase a `Pendiente`**: el trabajo queda en `Borrador` y el alumno corrige y vuelve a enviar | RF §12, RN-02005 [DECISIÓN 2026-08-08] |
...
|---|---|---|
| **RA-01** | Ningún JavaScript del navegador invoca la API | Es lo que sostiene las tres propiedades de la topología: sin contenido mixto, sin CORS y sin exposición de la IP del servidor propio (RT §2.2). Romperla en un solo proyecto de código las reabre las tres. **Sigue vigente para `GeometriaFactory.Web`** tras `E-02` [DECISIÓN 2026-09-12]: la exposición pública de la API es hacia otras aplicaciones propias del producto, no una habilitación del navegador del propio front; se reevalúa sólo si una aplicación propia que aparezca es JavaScript de navegador de otro origen (`D-01` cerrado por `ADR-00009`), caso en el que hace falta CORS **para esa ruta ajena**, no para el circuito de `Web` |
...
**`F-25` y `F-26` quedan asignadas** [INCORPORADO el 2026-08-12 por la pasada de estabilización]. Las dos son `Must Have` desde el 2026-08-09 y **ninguna fila de esta tabla las nombraba**: entraron en las versiones 1.5 y 1.7 y la tabla que enumera no se movió. La asignación no se inventa acá — la derivó y la fundamentó [`../Docs/00-Contexto/Roadmap-Producto.md`](../Docs/00-Contexto/Roadmap-Producto.md) §3 y §4, que ya declara que «el intake **no le asigna etapa**» y resuelve las dos:

| Capacidad | Etapa | Fundamento, de `Roadmap-Producto.md` |
|---|---|---|
| **F-26** reseteo de la contraseña de un alumno | `d` | Toda la superficie que necesita la levanta `d` —el panel de cuentas del administrador y la contraseña provisoria del primer ingreso— y ninguna etapa anterior la tiene |
| **F-25** movimiento automático de la escena | `g` | Es un agregado sobre la superficie de `g` y no existe antes: gobierna el movimiento de la escena tridimensional, que `g` es la etapa que la construye |
...
├── GeometriaFactory.sln                agrupador: los proyectos .NET, los veinte samples (nueve construidos, once nodos sin construcción) y el nodo del visor
├── src/
...
│   ├── geometriafactory-visor.csproj  nodo sin construcción para el agrupador (§13.2, §13.3)
│   ├── package.json
│   ├── webpack.config.js
│   ├── src/
...
Dos bloques, uno por cada unidad de entrega vigente de §13.1, en el orden de integración. Cada uno es autocontenido, y las entradas de sus proyectos de código componentes viven adentro nombrando el proyecto.
...
| INV-02 | Un alumno sólo accede a sus propios trabajos. No existe consulta que devuelva trabajos de otro alumno a un rol de alumno | RN-02003 |
| INV-03 | Un trabajo **eliminado por un alumno** estaba en `Borrador` y le pertenecía | RN-02004 [DECISIÓN 2026-08-08: acotado al alumno, porque el borrado del administrador alcanza cualquier estado y el enunciado anterior habría quedado falso] |
| INV-04 | Un trabajo `Finalizado` tiene JSON interpretado sin errores (puede tener advertencias) | RN-02005 |
| INV-05 | Existe exactamente un administrador configurado; su alta sólo es posible mientras no exista ninguno | RN-02001 |
| INV-06 | Un alumno en estado `Pendiente` o `Bloqueado` no obtiene token | RN-02006 |
| INV-07 | Un trabajo en `Finalizado` o en `Rechazado` no cambia de estado ni de contenido | RN-02010 [DECISIÓN 2026-08-08: nuevo, sostiene la terminalidad de los dos estados de cierre] |
| INV-09 | Una cuenta con la marca de **cambio de contraseña pendiente** no ejerce ninguna capacidad del sistema salvo cambiar su propia contraseña. La marca la ponen **únicamente** las dos operaciones que producen una contraseña provisoria —el **reseteo** (RN-02014) y la **habilitación** (RN-02016)— y la levanta **únicamente** el cambio efectivo hecho por la propia cuenta [CORREGIDO el 2026-08-09: decía «únicamente el reseteo», redacción de 1.7 que quedó falsa al entrar RN-02016 en 1.13, que pone la misma marca al habilitar y cita a este invariante al hacerlo] | RN-02012, RN-02013, F-26 [DECISIÓN 2026-08-09]. Es lo que hace que la provisoria sea provisoria: sin este invariante, una clave que el administrador conoce quedaría sirviendo indefinidamente para operar como el alumno |
| INV-08 | La cuenta con papel `Administrador` está **siempre** `Habilitado`: nace habilitada, ninguna operación la lleva a `Pendiente` ni a `Bloqueado`, y no admite baja. Toda cuenta con papel `Alumno` nace `Pendiente` | RN-02001, RN-02006 [DECISIÓN 2026-08-09: **adoptado**. No proviene de las fuentes; lo propuso la categoría 02 de `GeometriaFactory-Domain` después de que la familia de defectos que su ausencia habilita se abriera **dos veces por puertas distintas** —dejar nacer `Pendiente` a esa cuenta, y permitir bloquearla después—, las dos terminando con la instancia sin nadie capaz de habilitar, desbloquear ni revisar. Cierra la familia en lugar de tapar cada puerta] |

**Nueve invariantes desde el 2026-08-09.** Los invariantes no son reglas distintas de las de §4.1: son las mismas vistas desde el dominio. La regla declara qué decidió el negocio; el invariante declara qué condición sobre los datos no puede romperse nunca. **Dieciséis reglas desde el 2026-08-09: diez con invariante asociado y seis sin él.** Las seis que no lo tienen —RN-02007 baja física, RN-02008 texto íntegro, RN-02009 error con ubicación, **RN-02011** alcance de la consulta del administrador, **RN-02014** producción de la provisoria y **RN-02015** independencia del estado— no lo tienen porque describen comportamientos, o alcances de consulta, y no condiciones permanentes sobre el estado. **RN-02012, RN-02013 y RN-02016 sí lo tienen, y es INV-09**, que es lo que la columna de ese invariante declara. [CORREGIDO el 2026-08-09: esta lista nombraba a **RN-02012** en el lugar de RN-02011, con lo cual la prosa contradecía a su propia tabla y a `GeometriaFactory-Domain`, que cuenta nueve reglas con invariante y seis sin él.]
...
| Quién la consume | `GeometriaFactory.Web`, servidor a servidor (RA-01, sin cambios para el front propio), **y toda otra aplicación propia del producto** [DECISIÓN 2026-09-12, PO, mesa `Mesa-2026-09-12-ciclo-2.md`]: «la API debe ser expuesta públicamente; la idea es ofrecerla para otros clientes». Cambio de alcance sobre lo declarado hasta la versión 4.2. **Qué cliente es queda decidido** [DECISIÓN 2026-09-12, PO, `ADR-00009`, cierra `D-01`]: «no hay tercero — es re simple: un administrador, y luego usuarios generales, que pueden ser cualquiera, entre estos alumnos». El cliente es cualquier aplicación propia —otro front, una aplicación MAUI— que actúa en nombre de una de las dos personas (administrador o usuario) y se autentica **como ella** por `POST /auth/token` (§17.1.P.5); su forma no cambia la autenticación. **No hay credenciales por cliente** |
...
| CORS | No hace falta **para `GeometriaFactory.Web`** (RT §2.2, RA-01 intacta para el front propio). **Diferido** hasta que aparezca un cliente propio que sea JavaScript de navegador desde otro origen (`D-01` cerrado por `ADR-00009`: el cliente es una aplicación propia, de la forma que sea); si las aplicaciones propias que aparezcan son servidor a servidor o móviles, CORS no llega a hacer falta nunca |
| Endpoint de autenticación | `POST /auth/token` con correo y contraseña, para alumnos y administrador (ROPC, sin cambios). **Toda aplicación propia usa este endpoint** con las credenciales de la persona en cuyo nombre actúa [DECISIÓN 2026-09-12, PO, `ADR-00009`]: no hay otra vía de autenticación |
| Salud | Endpoint de salud, consumido por la página de salud del front y por el `healthcheck` del `compose.yaml` |
| Versionado del contrato | [DECISIÓN 2026-09-12, PO + mesa `Mesa-2026-09-12-ciclo-2.md` §3-§4, resolviendo `E-04`] **Dos ejes independientes.** El **producto** sigue versionado con **SemVer 2.0.0** (`vMAJOR.MINOR.PATCH`, sin prerelease), sin cambios respecto de `Estrategia-Versionado.md`. El **contrato REST expuesto públicamente** se versiona **en la ruta, sólo el `MAJOR`** (`/v{MAJOR}/`), compartiendo esa cifra con la del producto — no un ensamblado de contratos aparte para clientes externos. **Evento de etiqueta**: toda fusión a `main` que cambie código de producción, calculada por Conventional Commits con MinVer (`D-03`, default aceptado), y no «el cierre de etapa del roadmap» que dejó 129 fusiones sin etiquetar desde `v0.8.0`. **Deprecación**: al publicar `/v{N+1}/`, `/v{N}/` se mantiene un plazo mínimo de **un cuatrimestre** (`D-02`, default aceptado) con cabecera `Deprecation` (draft-ietf-httpapi-deprecation-header). **Quién reescribe la superficie de esta decisión**: `ADR-00008` (que hoy fija «sin versionado de rutas» sobre la premisa «no hay clientes de terceros», ya no cierta) y `Estrategia-Versionado.md` §1.1 —ítems 1 y 10 del plan de `Roadmap-Producto.md` fila `k`, **no de esta entrada**: este intake asienta la decisión y no reescribe esos dos documentos |

#### §17.1.P.3 · GeometriaFactory-Contracts

Es **el** contrato de comunicación del producto: define el payload JSON que viaja entre `GeometriaFactory.Web` y `GeometriaFactory.Api` por HTTP con `Bearer` (RT §4.1, §9.1). Política de cambios incompatibles: como los dos extremos se compilan contra el mismo ensamblado, un cambio incompatible **rompe la compilación** antes de romper el runtime. La regla operativa es que Api y Web se despliegan juntos ante un cambio de contrato. **Para el tramo `Web`↔`Api` esto no cambia** [DECISIÓN 2026-09-12]: la afirmación «no hay clientes de terceros», cierta hasta la versión 4.2, **deja de serlo** — la API se ofrece públicamente a clientes externos (ver §17.1.P.3 · GeometriaFactory-Api, fila «Quién la consume»), y la superficie pública versiona sus rutas por `/v{MAJOR}/`, no el ensamblado de contratos.
...
|---|---|---|
| `CONTRATO_OPERACION_EXCLUSIVA_DEL_ADMINISTRADOR` | **El papel no alcanza para la operación pedida, fuera del desenlace de un trabajo**: gobernar las cuentas de la comisión (F-03), resetear la contraseña de un alumno (F-26) y ver el listado de trabajos de la comisión (F-12) | El único código de facultad del conjunto, `CONTRATO_DESENLACE_EXCLUSIVO_DEL_ADMINISTRADOR`, está **acotado por su enunciado** a aprobar o rechazar un trabajo (F-23, RN-02010), y la capa de aplicación emite ese mismo rechazo en las otras tres. Sin código propio, las tres caen en el genérico `CONTRATO_ERROR_NO_CLASIFICADO` y **el front no puede distinguir «no tenés permiso» de «algo salió mal»**, que son dos mensajes distintos para el alumno |
| `CONTRATO_ESTADO_NO_PERMITE_MODIFICAR` | **Se pidió enviar o reeditar un trabajo que no está en `Borrador`.** Un trabajo en `Pendiente`, `Finalizado` o `Rechazado` es de **sólo lectura** para el alumno (F-07, RN-02004, INV-07) | El código análogo existente, `CONTRATO_ESTADO_NO_PERMITE_ELIMINAR`, está **acotado a la eliminación** y al camino del alumno, y no cubre las otras dos escrituras que el mismo estado prohíbe. Sin él, forzar un envío o una reedición contra la API sobre un trabajo ya entregado no tiene código propio |
...
El modelo de datos es el de RT §7.1: `ALUMNO`, `TRABAJO`, `PIEZA`, `COMPONENTE` y `OBSERVACION`, con `OriginalJson` conservado íntegro y nunca reescrito (RN-02008).

**Los valores de conjunto cerrado pasan a identificador inglés con etiqueta castellana** [DECISIÓN del Product Owner 2026-08-12, `F-02`, §13.1]. Alcanza a los **diez valores** de los cuatro conjuntos —papel, estado de cuenta, estado del trabajo y especie de observación—, que esta sección guarda **como texto y por su nombre**. Dos consecuencias que hay que anotar acá porque son de persistencia: **el costo de esquema es cero mientras no haya base poblada**, y hoy no la hay, de modo que la transformación inicial nace ya en inglés en lugar de necesitar una segunda; y **la colisión de `Pendiente` se deshace**, porque el estado de cuenta y el del trabajo dejan de compartir literal. La correspondencia, con su etiqueta, está en `Norma-De-Nomenclatura.md` §6.7. **Esta versión no renombra ningún valor**: la ejecución es el tramo `R-4` de la norma §8.
...
**Autenticación de las aplicaciones propias** [DECISIÓN 2026-09-12, PO, `ADR-00009`; supera el párrafo «Autenticación de clientes externos» de la 4.3 y cierra `D-01`]: la API autentica **personas, no aplicaciones**. Hay dos identidades y nada más —administrador y usuario— y el cliente es cualquier aplicación propia del producto que actúa en nombre de una de ellas: «no hay tercero — es re simple: un administrador, y luego usuarios generales, que pueden ser cualquiera, entre estos alumnos». Toda aplicación propia obtiene el acceso por `POST /auth/token` con las credenciales de la persona (ROPC), igual que el front; la premisa de la nota de seguridad de arriba —el intermediario es del propio sistema— se extiende a todo cliente propio y deja de estar atada al front Blazor. **No hay claves de API ni `client_credentials`**: no existe una tercera clase de identidad. «Alumno» es la etiqueta del papel `Student` en este despliegue; el papel es «usuario», y ningún uso se renombra (`ADR-00009` §2 punto 4). Sin límite de tasa declarado hasta hoy (`RS-OPE-01`): una aplicación sin cuota agotaría el único escritor de SQLite (§17.1.P.4), por lo que el plan de `Roadmap-Producto.md` fila `k` ordena el límite de tasa —por persona autenticada o por dirección de origen— **antes** de habilitar las rutas versionadas.

#### §17.1.P.5 · GeometriaFactory-Contracts

No implementa autenticación, pero **es donde se decide qué se expone**. Regla: ningún DTO incluye el hash de contraseña, la clave de firma ni ninguna dirección de servicio interno (RA-03). El DTO de respuesta de error lleva texto neutro y, cuando corresponde, índice de figura y campo (RN-02009), nunca la dirección del servicio que falló.
...
**Cobertura mínima: 90 % de líneas y 85 % de ramas** [ASUNCIÓN]. Es el proyecto de código con el número más alto del producto porque no tiene dependencias que dificulten la prueba y porque sus invariantes son la última defensa de las reglas RN-02001 a RN-02016.
...
2. **La verificación de valores produce observaciones de dos niveles**, `Error` y `Advertencia`, y sólo el primero impide que el trabajo pase a `Pendiente` (RT §6.5, RN-02005).
...
2. **El JSON original se conserva íntegro** (RN-02008), lo que permite reprocesar si el validador mejora.
...
ASP.NET Core sobre **.NET 10** con **Blazor**, páginas **Interactive Server** [DECISIÓN, RT §3]. Componentes de interfaz: ~~**MudBlazor**, cuya versión exacta **se ancla al crear el andamiaje y se registra en RT §3 en ese momento** —la fuente la deja explícitamente **[A VERIFICAR]** porque no se puede contrastar contra el registro de paquetes en este entorno—.~~ **NINGUNO, y la marca queda SIN OBJETO el 2026-08-31.** La biblioteca **nunca se introdujo**: `src/GeometriaFactory.Web/GeometriaFactory.Web.csproj` declara una sola referencia de proyecto y lleva el **apartamiento declarado** de la etapa `b`, con tres razones medidas sobre la maqueta aprobada —el sistema visual adoptado no es el de ninguna biblioteca, la etapa no construye comportamiento, y la regla de anclaje seguía sin poder cumplirse—. `Web/05` §11 `PA-01` lo cerró **por lectura el 2026-08-20**. **Condición de reapertura:** si una etapa introduce comportamiento sobre las superficies, entra **tematizada contra los tokens de la maqueta** o no entra. Dependencias core: `GeometriaFactory.Contracts` y el bundle de `GeometriaFactory-Visor`, que llega como archivo a `wwwroot/js/`.
...
Es **el** contrato de comunicación del producto: define el payload JSON que viaja entre `GeometriaFactory.Web` y `GeometriaFactory.Api` por HTTP con `Bearer` (RT §4.1, §9.1). Política de cambios incompatibles: como los dos extremos se compilan contra el mismo ensamblado, un cambio incompatible **rompe la compilación** antes de romper el runtime. La regla operativa es que Api y Web se despliegan juntos ante un cambio de contrato. **Para el tramo `Web`↔`Api` esto no cambia** [DECISIÓN 2026-09-12, reescrito en la 4.5 por `BT-00027`]: la afirmación «no hay versionado de endpoints en este alcance porque no hay clientes de terceros», cierta hasta la versión 4.2, **deja de serlo** — hay aplicaciones propias además del front (§17.1.P.3 · GeometriaFactory-Api, fila «Quién la consume»; `ADR-00009`), y la superficie pública de la API versiona sus rutas por `/v{MAJOR}/` (§17.1.P.3 · GeometriaFactory-Api, fila «Versionado del contrato»; `ADR-00010`), no el ensamblado de contratos.
...
**Los nombres definitivos quedan fijados el 2026-08-12** [DECISIÓN del Product Owner, `F-01`, §13.1]. Lo que esta sección dejaba abierto —«los nombres definitivos a fijar en la etapa que la implementa»— **deja de estar abierto**: los seis pasan a inglés y su correspondencia es la de `Norma-De-Nomenclatura.md` §6.6, que es su fuente única. La tabla de arriba **conserva sus nombres castellanos hasta la tanda de renombre** (norma §8, tramo `R-3`), para que la decisión y su ejecución no se mezclen en la misma versión.
...
Servidor: hosting gratuito somee.com con servidor de información, HTTPS y dominio público. ~~**La versión de .NET que soporta el hosting está [A VERIFICAR]**~~ **RESUELTA el 2026-08-13, midiendo: el hosting soporta `net10.0`.** Es `PT-01.a`, y **pasa**: la ruta pública responde **200**, confirmado además desde el panel de la cuenta. **No hizo falta bajar la versión objetivo del front**, que era la salida declarada para el caso contrario (RT §12). Evidencia en [`../Docs/Audit/Reporte-Despliegue-Somee.md`](../Docs/Audit/Reporte-Despliegue-Somee.md) §2 y [`../Docs/00-Contexto/Compatibilidad-Plataformas.md`](../Docs/00-Contexto/Compatibilidad-Plataformas.md). **La marca sobrevivió dieciocho días a su propia respuesta**, y el control de cambios `1.33` de este documento declaró el 2026-08-14 haberla retirado sin retirarla — ver §22, donde queda registrado.
...
| S-2 | **Colección de peticiones HTTP de la API**: alta de trabajo, envío que verifica y envío que no, y aprobación y rechazo por el administrador, con **los cuerpos de los ocho escenarios `E-1` a `E-8`** y los códigos de respuesta esperados [DECISIÓN 2026-08-12] | GeometriaFactory-Api | Media | Golpea los endpoints reales de `src/GeometriaFactory.Api/` con los DTOs de `GeometriaFactory.Contracts` |
| S-3 | **Juego de datos de los ocho escenarios** de la Parte D, en archivos sueltos, listos para pegar en el formulario de carga o para usar como cuerpo en S-2 | GeometriaFactory-Infrastructure (validador) | Baja | Es la entrada de las **diez** pruebas de la batería obligatoria —las nueve de RT §11 más la décima de §21— y de los guiones de demostración de las etapas `f` y `g` |

**El alcance de la colección de peticiones son los ocho escenarios `E-1` a `E-8`** [DECISIÓN 2026-08-12]. §16.1 ya lo declaraba así para el `/samples` de `GeometriaFactory-Api` mientras esta tabla decía «los cuerpos de E-2 y E-5»: **la divergencia queda resuelta a favor de los ocho**. El fundamento es lo que cada alcance demuestra: con dos, la colección demuestra que **la API responde**; con ocho, **ejercita el validador contra todos los datos reales por HTTP**, incluido el cilindro de `E-1` cuya diferencia de exactamente 0.01 funda el operador estricto de tolerancia (§17.3 P.10) y el `E-8` de la coma decimal, que es el modo de falla más probable del producto. Un validador que sólo se ejercita en proceso no está probado del lado por el que le llegan los datos.
...
**Las tres muestras `S-1`, `S-2` y `S-3` no son el conjunto de las carpetas de `/samples`** [PRECISADO 2026-08-11]. Son las tres **demostraciones nombradas** que este documento distingue por su papel: la que prueba el punto de extensión, la que ejercita la API y la que provee los datos. §16.1 asigna carpeta a **los siete** proyectos, y esas carpetas **contienen** a las tres muestras además de otras. [CORREGIDO 2026-08-11: la redacción de 1.25 decía «seis de los siete», contando mal la propia tabla que acababa de completarse. Lo levantó la corrección de la Fase G, que fue a contar las filas.] Confundir las dos listas fue lo que hizo que §16.1 y §18 se contradijeran durante veintitrés versiones sobre `Infrastructure`, a quien §18 asignaba `S-3` mientras §16.1 le negaba carpeta.
...
Las fuentes aportan ejemplos de instancia completos, de modo que esta parte es obligatoria y los transcribe enteros. Los ocho escenarios están citados desde el cuerpo (§6, §11 RN-B3, §16.1, §17.2 P.10, §17.3 P.4, §17.3 P.6, §17.3 P.10, §17.7 P.6, §17.7 P.8, §18) y ninguno queda huérfano. [CORREGIDO 2026-08-12: la lista nombraba §7 y §17.5 P.6, que **no citan ningún escenario**, y omitía §11, §17.3 P.4 y §17.7 P.8, que sí. Se recontó yendo a buscar las citas.]
...
6. Ninguna observación es de severidad `Error`, de modo que el trabajo **pasa a `Pendiente` al enviarlo** (RN-02005).
...
7. El trabajo **pasa a `Pendiente` con la advertencia asociada** (RN-02005).
...
4. El trabajo **no se rechaza** ni se corrige el valor: RN-02008 exige que el JSON original se conserve íntegro.
...
4. **El trabajo queda en `Borrador`** con su texto conservado (RF-15) y **no pasa a `Pendiente`** (RN-02005).
...
**Contexto.** Es el borde del validador: la figura degenerada que un alumno produce cuando su programa recibe un parámetro sin cargar. Aparece en la batería obligatoria de **diez** casos —las nueve de RT §11 más la décima de §21—, no en el guion de demostración.
...
3. El trabajo pasa a `Pendiente`, porque no hay error de interpretación (RN-02005).
...
4. **Todo esto ocurre sin backend**, con el bundle cargado en una página estática y **la estructura de piezas ya reconstruida** puesta a mano: interpretar el texto del alumno es del laboratorio y no del bundle [DECISIÓN 2026-08-16, `ADR-08006`]. Lo que este escenario ejercita en el bundle es **el mapeo de los seis tipos a malla**; la tolerancia de claves —`Bases` contra `Tapas`— dejó de ser suya y la verifica la batería del validador.
...
2. La pieza del índice 1 **no llega al visor** [DECISIÓN 2026-08-16, `ADR-08006`]: el validador no la reconstruye, emite su observación de especie error de validación con **índice 1** y campo `Largo`, y el visor recibe **sólo el ortoedro**. La condición `DIMENSION_NO_LEGIBLE` sigue declarada en el contrato de la fachada para la pieza que su anfitrión no pueda usar, y **deja de ser el camino de este escenario**.
3. **La confusión que este escenario detecta cambia de par y no desaparece.** Ya no es entre dos códigos del visor —`DIMENSION_NO_LEGIBLE` contra `JSON_INVALIDO`— sino entre **no poder dibujar y no poder interpretar**: el texto es JSON sintácticamente válido, lo que falla es la lectura de un valor, y quien lo declara es el validador. Tratarlo como texto inválido diría que nada se pudo leer, cuando el ortoedro se leyó entero.
...
|---|---|
| RN-02005 — un trabajo no pasa a `Pendiente` con errores de interpretación; las advertencias sí lo permiten | E-5 (rechaza), E-1, E-2, E-3 (aceptan con advertencias) |
| RN-02008 — el JSON original se conserva íntegro y nunca se reescribe | **Los ocho**: ningún escenario espera que el texto guardado difiera del pegado. **Corregido el 2026-08-08**: esta fila atribuía la regla a INV-04, que enuncia otra cosa —un trabajo `Finalizado` tiene JSON interpretado sin errores, §17.1.P.2 · GeometriaFactory-Domain—. RN-02008 no tiene invariante asociado |
| RN-02009 — todo mensaje de error indica índice de figura y campo | E-5 |
...
- [x] §4 tiene al menos un ítem en cada categoría MoSCoW y el Must Have es el mínimo razonable: son las **diecinueve** capacidades Must Have —contadas sobre la tabla de §4 el 2026-08-12—: las **doce** que traducen RF-01 a RF-20 —sin las cuales el trabajo no se carga, no se valida ni se ve—, las **cuatro** del circuito de revisión incorporadas el 2026-08-08 (F-21, F-22, F-23, F-24), sin las cuales la entrega no tiene desenlace, y **tres más que subieron después**: F-26 (reseteo de contraseña) y F-25 (movimiento automático), incorporadas el 2026-08-09, y F-13 (sincronización árbol ⇄ escena), que subió de `Should` el 2026-08-10 por estar citada por PT-02.
- [x] §5 tiene 9 historias en formato `Como/quiero/para`, cubriendo los dos roles, incluida la de revisión del administrador.
- [x] §7 lista **11** casos límite (CL-1 a CL-11) con su respuesta del cliente.
...
- [x] §9 lista **8** exclusiones vigentes con justificación (X-1, X-3, X-4, X-6 a X-10), más **las dos retiradas** con el registro de su retiro y de la condición de reingreso que se cumplió: **X-5** el 2026-08-08 y **X-2** el 2026-08-09.
...
- [x] §11 lista **7** riesgos vigentes (RN-B1 a RN-B5, RN-B7 y RN-B8) con probabilidad, impacto y mitigación, más **RN-B6, cerrado el 2026-08-09** por F-26 y conservado con el registro de su cierre. RN-B7 y RN-B8 entran en la versión **4.3** (`E-05`, decisión del Product Owner del 2026-09-12).
...
- [x] §17 está completo para cada unidad de entrega **vigente** de §13.1 (sus **dos** tablas de identidad + P.1 a P.12). **No hay un bloque §17 por proyecto de código**: los proyectos aparecen dentro del bloque de cada entrega que componen.
- [x] Cada bloque §17: P.6 declara cobertura mínima numérica o el gate numérico equivalente cuando no hay código de producción que cubrir (Contracts, Web, Visor); P.7 adopta SemVer 2.0.0 y Conventional Commits; P.8 enumera quality gates bloqueantes; P.9 declara plataformas y versiones mínimas o el requisito por capacidad cuando la fuente no fija número; P.10 expresa NFR con métricas numéricas.
...
**A-1 ya no es una asunción**: la identidad del producto la decidió el Product Owner el 2026-08-08 y la fila queda como registro de la decisión y de la acción que dispara sobre las fuentes.

> **`A-2`, `A-3`, `A-4` y los umbrales de `A-5` fueron CONFIRMADOS por el Product Owner el 2026-08-26**,
> por la decisión **`D1`** ([`../Docs/Audit/D1-Confirmacion-De-Asunciones.md`](../Docs/Audit/D1-Confirmacion-De-Asunciones.md) §0,
> [`../Docs/Audit/A3-Decisiones-Del-Product-Owner.md`](../Docs/Audit/A3-Decisiones-Del-Product-Owner.md) §2).
> **Queda viva una sola cosa de toda la sección: el caudal de 20 peticiones por minuto de `A-5`**, excluido
> expresamente de la confirmación.
>
> **Qué produjo `D1`:** cerró **doce filas vencidas** en cinco documentos y volvió **bloqueantes** a diez
> puertas de calidad —`QG-03`, `QG-04` y `QG-13` de `-Api`, `QG-03` y `QG-07` de `-Domain`, `QG-03` y
> `QG-10` de `-Application`, y `QG-05`, `QG-06` y `QG-14` de `-Infrastructure`—. `scripts/coverage.sh`
> transcribe hoy esos umbrales citando la confirmación.
>
> **Lo que la confirmación no compra**, y `D1` §0 lo declara: los umbrales **no quedan medidos**. Confirmarlos
> los volvió exigibles, no verdaderos.
>
> **Por qué este aviso se agrega recién el 2026-08-31, cinco días después.** Porque `D1` se propagó a los
> documentos que **usan** los umbrales y **no a la fuente que los declara**. Durante cinco días esta sección
> —la que cualquiera abre para saber si una asunción sigue viva— dijo que había cuatro asunciones vivas
> cuando quedaba **un fragmento de una**. Es el hueco que el **reporte 21** al framework describe: **no hay
> matriz de propagación** que obligue a que una decisión vuelva sobre el artefacto del que salió la pregunta.
...
| A-2 | **CONFIRMADA por `D1` el 2026-08-26.** Los targets de las cuatro métricas de negocio (**8 de 8 etapas** [corregido el 2026-08-09: decía «7 de 7», residuo de antes de que §8 pasara a contar todas las etapas comprometidas], ≥ 80 % de alumnos que entregan, 100 % de entregas revisadas, ≥ 1 advertencia por alumno) | §8 | Ninguna fuente fija métricas de negocio; los criterios de los que se derivan sí están declarados | Sólo cambia §8 y lo que la categoría 01 derive de ahí |
| A-3 | **CONFIRMADA por `D1` el 2026-08-26.** Coberturas mínimas: 90/85 en Domain, 85/80 en Application, 85/80 con 95 en el validador de Infrastructure, 75/70 en Api, **y la pirámide invertida de 60 % integración / 40 % unitarias de la Api** [incorporada el 2026-08-12: llevaba el rótulo **[ASUNCIÓN]** en §17.5 P.6 y no tenía fila acá] | §17 P.6 | RT §11 declara qué se prueba pero no con qué umbral. Los números son altos donde la fuente señala criticidad | Cambia el gate del pipeline de esos proyectos de código |
| A-4 | **CONFIRMADA por `D1` el 2026-08-26.** Gates no basados en cobertura de líneas para Contracts, Web y Visor (100 % de DTOs ejercitados, 100 % de pasos de guion, cero llamadas de red) | §17 P.6 | Son proyectos de código sin lógica propia que cubrir, o cuya verificación la fuente define de otra forma | Cambia la forma del gate, no su carácter bloqueante |
| A-5 | **CONFIRMADA por `D1` el 2026-08-26 EN SUS UMBRALES, con una excepción declarada: el caudal.** NFR de §17 P.10: los numéricos —500 ms de validación en Application, 200 ms en el validador, p99 de 500 ms y **20 peticiones por minuto** en la Api —**este valor es el único que sigue abierto de toda la sección**—, 30 s de arranque en frío, 10 s de la batería de dominio— **y el estructural de Contracts**: que el payload de listado no lleve el `OriginalJson` ni los componentes de las piezas | §17 P.10 | RT §12 define puertas técnicas medidas pero no umbrales de latencia ni de throughput. Los valores se eligieron acordes al uso previsto: una comisión durante una clase | Cambia lo que la categoría 08 verifique como NFR-tests. **Sobre el caudal, que es lo único vivo:** su valor sale de **`PT-05`**, que se mide en la **fase `i`**, y depende además de **`D5`** —el volumen de la comisión—, que es una decisión del Product Owner todavía sin tomar. `QG-14` de `-Api` pasó a bloqueante **sólo en su percentil** y sigue condicionado en su caudal |
...
**Marcas [A VERIFICAR]**, que no son asunciones de este documento sino incógnitas declaradas. **Son CINCO, y esta sección enumeraba cuatro.** Clasificadas contra el árbol el **2026-08-31** en [`../Docs/Audit/Inventario-Marcas-A-Verificar-2026-08-31.md`](../Docs/Audit/Inventario-Marcas-A-Verificar-2026-08-31.md), sobre **71 apariciones** vivas del corpus:

| Incógnita | Origen | Cómo se cierra | Estado |
|---|---|---|---|
| Las capacidades del **hosting gratuito** | PT-01 | Midiendo | **RESUELTA el 2026-08-13.** `PT-01.a` pasa con **200** y el hosting soporta `net10.0` |
| La versión exacta de **MudBlazor** | RT §3 | Midiendo | **SIN OBJETO el 2026-08-31.** La biblioteca nunca se introdujo, y su ausencia es una decisión declarada en el `.csproj` |
| Construir la **imagen en destino desde Git** | RT §13 | Midiendo | **VIVA.** Se prueba una vez en la **fase `i`**, con `BT-00026` |
| Un **dominio propio** para el túnel saliente | RT §15.1 | Midiendo | **VIVA.** Atada a `X-10`, exclusión vigente |
| **El volumen de la comisión** | Las fuentes de `-Web` | **DECIDIENDO** | **CERRADA POR INCOGNOSCIBLE el 2026-08-20**: el dato no se sabe ni se puede saber, y no se fija número. *Incorporada el 2026-08-31, y corregida el mismo día* |

> **CORREGIDO el 2026-08-31, en la emisión 3.4 y el mismo día que la 3.3.** La emisión 3.3 incorporó esta quinta marca declarándola **viva**. **No lo está: `D5` se cerró el 2026-08-20 POR INCOGNOSCIBLE**, por decisión del Product Owner —commit `b4a4804`—, en cinco documentos que `A3` nunca reflejó. Lo que sobrevive **no es la pregunta sino su consecuencia**: el **caudal** de `A-5` se derivaba de «una comisión operando durante una clase» y **sin tamaño conocido ese fundamento se cayó**; el número queda **provisorio** hasta que `PT-05` lo mida. **La incorporación de la marca al §22 sigue siendo correcta** —faltaba, y su ausencia tenía un motivo que vale registrar—; lo que estaba mal era su estado.

**El volumen de la comisión entra hoy, y conviene decir por qué faltaba.** Está rotulado `[A VERIFICAR]` en **nueve** lugares —`Web/05` §11 `PA-06`, `Web/06` `PA-08`, `BT-10022`, `Experiencia-De-Uso.md`, los wireframes del listado y `Handoff-Checkout.md` `A-19`, entre otros— y **no tenía fila acá**. No es inocuo: **los dos listados no incorporan paginación** y el fundamento escrito es que suponen decenas y no cientos; además es lo que destraba el **caudal** de `A-5`, que quedó fuera de la confirmación `D1`, y `A3` §3 lo pone **primero** en el orden recomendado, por delante de `D1`.

**Y su incorporación obliga a matizar la frase que esta sección venía repitiendo.** Se decía que las marcas *«se resuelven midiendo, no decidiendo»*, y eso es cierto para las cuatro que se enumeraban. **La quinta se decide.** La sección estaba organizada por *cómo se cierra* el ítem, y el ítem que se cerraba distinto **se quedó afuera de la enumeración por no encajar en la generalización que la encabezaba** — que es la forma más silenciosa de perder un pendiente: no se omite por descuido, se omite porque no entra en la frase.

> **Constancia sobre el control de cambios de este documento, que se corrige emitiendo y no editando.** La fila **1.33**, del 2026-08-14, declara en su punto **(b)** que la marca de la versión de plataforma dejaba de estar rotulada. **El cambio nunca se aplicó**: el commit que emitió la 1.33 tocó la celda de versión, un párrafo de la corrección (a) y la propia fila del control de cambios, **y ninguna línea con la marca**. La marca sobrevivió **diecisiete días más**, hasta hoy. La fila **no se reescribe** —el registro histórico dice lo que se afirmó, no lo que hubiera correspondido—, y además **ya fue alterada una vez**: citaba `§17.6.P.9` y hoy cita `§17.2.P.9`, reescrita por alguna renumeración posterior. **Una fila de control de cambios que se edita deja de ser control de cambios.**
...
| §20–§21 anexos de datos | `02-Especificacion-Funcional/`, `08-Calidad-Y-Pruebas/`, `10-Examples/` | Modelo conceptual con ejemplos, fixtures de las **diez** pruebas de la batería obligatoria (§21), contratos de verificación |
...
| 4.3 | 2026-09-12 | **El Product Owner asienta, fuera de toda corrida del orquestador, las tres decisiones que contestan las escaladas `E-02`, `E-04` y `E-05` de `Mesa-2026-09-12.md` §9, convertidas en plan por la mesa de ciclo 2** (`SDD/Docs/Audit/Mesa-2026-09-12-ciclo-2.md`), `Master-Prompt.md` §13.1: el intake es documento humano y esta escritura no pasa por los casos (a)/(b) de §13. **`E-02`** — «la API debe ser expuesta públicamente; la idea es ofrecerla para otros clientes»: §17.1.P.3 (`GeometriaFactory-Api`, fila «Quién la consume», fila CORS, y `GeometriaFactory-Contracts`) deja de afirmar «no hay clientes de terceros»; §17.1.P.5 suma autenticación por cliente (API key / `client_credentials`) sin tocar el ROPC de alumnos; `X-9` y `RA-01` se reevalúan y **siguen sin darse** para `GeometriaFactory.Web` — la exposición es hacia clientes externos, no una habilitación del navegador del propio front. **Quién es el cliente queda diferido** como ítem `D-01` (`Mesa-2026-09-12.md` §8), con CORS diferido en la misma condición. **`E-04`** — «buscar una forma de nomenclar el versionado, según el estándar de la industria»: §17.1.P.3, fila «Versionado del contrato», adopta versión de ruta `/v{MAJOR}/` para la superficie pública, MAJOR compartido con el producto (que sigue en SemVer 2.0.0 sin cambios), evento de etiqueta = fusión a `main` con cambio de código de producción vía Conventional Commits + MinVer (`D-03`), y plazo de convivencia de un cuatrimestre (`D-02`). **No reescribe `ADR-00008` ni `Estrategia-Versionado.md`**: esa reescritura es ítems 1 y 10 del plan de diez ítems que abre `Roadmap-Producto.md` fila `k`, trabajo del ciclo que el roadmap abre y no de esta entrada. **`E-05`** — el estado durable: §11 suma **RN-B7** (claves de protección de datos del front, riesgo aceptado por escrito: «perder la clave DataProtection cuesta un re-login a los alumnos, no hay datos») y **RN-B8** (requisito de respaldo del SQLite de `lab-geometria-api/data`, con criterio de verificación de restauración; el mecanismo lo implementa el Product Owner en su despliegue, fuera de este repositorio). §19 se corrige de **5** a **7** riesgos vigentes. **Es el evento de `Rules-Backlog-Tecnico.md` §3.6 y `Rules-Contexto.md` §3.5**: `Roadmap-Producto.md` suma la fila `k` con el plan de diez ítems. Sube **minor**: ninguna de las tres reescribe una sección aprobada contradiciéndola de raíz; las tres actualizan filas existentes con el resultado de decisiones del Product Owner y agregan lo que antes no existía (autenticación por cliente, convención de versionado, dos riesgos). Estado anterior en [`_legacy/2026-09-12/PRODUCT-INTAKE-Fabrica-De-Geometria-v4.2.md`](_legacy/2026-09-12/PRODUCT-INTAKE-Fabrica-De-Geometria-v4.2.md). Séptima reanudación del destino. | Product Owner, con el Orquestador de reanudación SDD |
| 4.2 | 2026-09-12 | **El Product Owner asienta, fuera de toda corrida del orquestador, la decisión de topología de despliegue del 2026-09-06** (`Master-Prompt.md` §13.1: el intake es documento humano y esta escritura no pasa por los casos (a)/(b) de §13 porque no ocurre durante una corrida). §14 (`F-14`) y §9 (`X-10`) se reescriben: el túnel con dominio propio (`geometria.aplicada.stream`, i7infra) **realiza** `X-10` —su condición de reevaluación, «si aparece un dominio propio», se cumplió— y las dos piezas corren en contenedores del servidor propio; el hosting externo de somee.com **no se retira**: `deploy-front-ftp.yml` se conserva como **canal alternativo y antecedente de despliegue, sin disparador automático** (ver `changelog.md` de la misma fecha y `.github/workflows/deploy-front-ftp.yml`). **Es el evento de `Rules-Backlog-Tecnico.md` §3.6 y `Rules-Contexto.md` §3.5**: modifica el contenido de la fila `i` de `Roadmap-Producto.md` §2.1/§5.2, que se reabre con este mismo evento. Sube **minor**: no reescribe ninguna sección ya aprobada, sólo actualiza F-14 y X-10 con el resultado de una condición que ellas mismas declaraban. | Product Owner, asistido por agente (séptima reanudación) |
| 4.1 | 2026-09-12 | **§16, en la misma operación que la 4.0 y como entrada aparte por la regla de atomicidad de `Master-Prompt.md` §13:** el árbol suma `visor/geometriafactory-visor.csproj` (el nodo sin construcción de §13.2) y anota en `GeometriaFactory.sln` qué agrupa desde hoy. Sube **minor**: agrega un archivo al árbol sin mover ninguno. | Product Owner, asistido por agente |
| 4.0 | 2026-09-12 | **Reescritura de §13.2 y §13.3 por decisión del Product Owner del 2026-09-11 (respuesta a la escalada `ESC-001` de la mesa evaluadora de la Feature 20 del framework): todos los proyectos de código quedan bajo el árbol de solución, aunque sea bajo carpetas virtuales.** El visor pasa de «Ninguna» solución de código a `GeometriaFactory.sln` como **nodo sin construcción**; el párrafo que lo declaraba fuera del agrupador «para que no se estorben» se reescribe con la forma elegida —que no estorba, porque no ejecuta nada— y con quién genera el bundle desde ahora (`GeometriaFactory.Web.csproj`, `Web ADR-10008`). §13.3 suma la tercera consecuencia de la excepción: el nombre del archivo de proyecto. La ruta `visor/`, la identidad npm y la clase «activo de construcción» de la arista **no cambian**. Caso (a) de `Master-Prompt.md` §13; sube **major** porque reescribe una sección ya aprobada; el manifiesto se re-deriva en la misma operación (6.0). Estado anterior en `_legacy/2026-09-12/`. | Product Owner, asistido por agente |
| 3.4 | 2026-08-31 | **Corrige la emisión 3.3 el mismo día: la quinta marca del §22 —el volumen de la comisión— NO está viva.** `D5` se **cerró por INCOGNOSCIBLE el 2026-08-20**, por decisión del Product Owner registrada en el commit `b4a4804` sobre **cinco documentos**: el dato no se sabe ni se puede saber de antemano y **no se fija número**, que es mejor desenlace que inventar uno. La 3.3 lo leyó de `A3-Decisiones-Del-Product-Owner.md`, **que nunca registró ese desenlace** — el mismo defecto de propagación que esta sección venía documentando, esta vez con la fuente stale siendo un documento de `Audit` y no el intake. **La incorporación de la marca al §22 se mantiene**: faltaba de verdad, y el motivo de su ausencia —que es la única que se decide y no se mide— sigue siendo correcto. Lo que se corrige es su **estado**. **Y lo que sobrevive de `D5` no es la pregunta sino su consecuencia**, que toca directamente a `A-5`: el caudal de 20 peticiones por minuto **se derivaba de «una comisión operando durante una clase»**, y sin tamaño conocido **ese fundamento se cayó**. El número se conserva como referencia, queda **provisorio**, y su valor definitivo sale de `PT-05` en la fase `i`. Eso explica, además, por qué `D1` no pudo confirmarlo el 2026-08-26: no le faltaba una decisión, **le faltaba el fundamento que `D5` había retirado seis días antes**. | Orquestador SDD |
| 3.3 | 2026-08-31 | **Cierre de las dos incógnitas `[A VERIFICAR]` que ya no tenían pregunta**, sobre el inventario [`Inventario-Marcas-A-Verificar-2026-08-31.md`](../Docs/Audit/Inventario-Marcas-A-Verificar-2026-08-31.md), que clasificó las **71** apariciones vivas del corpus en **cinco** incógnitas. **(a) La versión de plataforma del hosting quedó RESUELTA el 2026-08-13, midiendo**: `PT-01.a` pasa con **200** y el hosting soporta `net10.0`, confirmado desde el panel; no hizo falta bajar la versión objetivo del front. **(b) La versión de la biblioteca de componentes queda SIN OBJETO**: la biblioteca nunca se introdujo y su ausencia es una decisión declarada en el `.csproj` — `PA-01` de `Web/05` §11 **ya lo había cerrado por lectura el 2026-08-20** y el desenlace no bajó. **Ninguna de las dos se decide acá: las dos se leen.** **Y §22 pasa de cuatro marcas a CINCO**: entra **el volumen de la comisión**, rotulado `[A VERIFICAR]` en nueve lugares del corpus y sin fila acá. Es la única de las cinco que **se decide y no se mide** —es `D5`, del Product Owner—, y por eso se había quedado afuera: la sección estaba organizada por *cómo se cierra* el ítem y la frase que la encabezaba decía «se resuelven midiendo, no decidiendo». **Se omitió por no encajar en su propia generalización.** Se deja además constancia de que el punto **(b)** de la fila **1.33** declaró retirada una marca que **nunca se retiró** —el commit no tocó ninguna línea con la marca—, y de que esa misma fila **fue alterada después** (citaba `§17.6.P.9` y hoy cita `§17.2.P.9`). **No se reescribe: se corrige emitiendo.** **Ningún umbral, ningún contrato y ninguna decisión cambian.** | Orquestador SDD |
| 3.2 | 2026-08-31 | **`D1` llega, cinco días tarde, al documento del que salió la pregunta: §22 estampa la confirmación del 2026-08-26 y la cuenta de asunciones vivas pasa de cuatro a un fragmento.** El Product Owner **confirmó `A-2`, `A-3`, `A-4` y los umbrales de `A-5` el 2026-08-26**. La decisión cerró **doce filas vencidas** en cinco documentos y volvió **bloqueantes** diez puertas de calidad, y `scripts/coverage.sh` ya transcribe esos umbrales **citando la confirmación**. **Queda vivo un solo valor de toda la sección: el caudal de 20 peticiones por minuto de `A-5`**, excluido expresamente. Su fila registra ahora de dónde sale —`PT-05`, que se mide en la fase `i`— y de qué depende —`D5`, el volumen de la comisión, sin decidir—, y que `QG-14` de `-Api` pasó a bloqueante **sólo en su percentil**. **Lo que este asiento no debe dejar pasar es por qué hizo falta.** `D1` se propagó a los documentos que **usan** los umbrales y **no a la fuente que los declara**. Durante cinco días, la sección que cualquiera abre para saber si una asunción sigue viva **decía que había cuatro cuando quedaba una**, y ninguna comprobación del método lo detecta: la decisión existía, estaba fechada, estaba registrada y era correcta —sólo que no volvió sobre el artefacto que planteó la pregunta—. Es exactamente el hueco que el **reporte 21** al framework describe: no hay matriz de propagación. Se anota en §22 con esa cita para que el próximo lector no lo lea como un descuido aislado. **Ninguna decisión, ningún contrato, ningún umbral y ningún caso de prueba cambian**: este asiento registra una confirmación ya tomada, no toma ninguna. Se emite con `Handoff-Checkout.md` **1.8**, que leía de acá su recuento. **Las marcas `[A VERIFICAR]` de esta misma sección quedan deliberadamente sin tocar**: se tratan en su propia tanda, después de clasificarlas. | Orquestador SDD |
| 3.1 | 2026-08-27 | **Parche `P-04` de la mesa de evaluación del 2026-08-27** ([`../Docs/Audit/Mesa-2026-08-27.md`](../Docs/Audit/Mesa-2026-08-27.md), hallazgo **H-04**, ancla **E2**, nivel **P2**). La línea 3 declaraba «plantilla de referencia **3.0**» y **ninguna fuente sostenía ese número**: este intake se emitió sobre la **3.4** y la procedencia vigente del manifiesto declara la **3.5**. El 3.0 era la versión **de este documento**, copiada al campo equivocado. Se corrige con las dos fuentes citadas y con la advertencia de no volver a confundirlas. **Ninguna sección del intake cambia**: es la cabecera, no el contenido. | Orquestador de reanudación SDD |
| 3.0 | 2026-08-17 | **Migración normativa 8.11 → 9.9, fase M2** (`Master-Prompt-Migracion.md` 2.7 §6), sobre la plantilla **3.4**. **Ninguna sección se mueve, se parte, se colapsa ni se agrega, y ningún campo cambia de dueño**: el cuerpo ya estaba migrado a los dos ejes por la 2.0. Lo que se corrige son **siete residuos de aquella migración**, que sobrevivieron alrededor del cuerpo migrado y que en dos casos **afirmaban algo falso**: §16 declaraba la documentación «por proyecto de código bajo `Proyectos/`» —el layout anterior a la 8.0, que este árbol no tiene— y condicionaba las maquetas a que «algún proyecto de código» ejecutara la Fase B2, cuando `requiere_maqueta` se evalúa por unidad de entrega; la **Parte C** y el encabezado de su checklist seguían titulándose «por proyecto de código»; su intro declaraba **«siete bloques, uno por cada proyecto de código»** cuando **§17 tiene dos** desde la 2.0; el checklist de §19 verificaba **«§17 completo para los 7 proyectos de código»** —un recuento que no existe— y pasa a pedir las **dos tablas de identidad** por unidad de entrega vigente; y su segundo ítem pasa de «cada proyecto de código» a «cada bloque §17». **Ninguna regla de negocio, invariante, escenario, métrica, exclusión ni decisión del Product Owner cambia de enunciado.** Batería M2: **sin preguntas**; las veintiuna secciones de la plantilla están presentes y §22 y §23 conservan su apartamiento declarado. **Nada se rellenó.** El estado anterior queda archivado en `_legacy/2026-08-17/`. Sube **major** porque el flujo de escritura estructural de `Master-Prompt.md` §13 regla 2 caso (b) lo exige como tercera condición, no por el volumen del cambio, y se declara acá para que no se lea como una reescritura que no fue. **Observación aguas arriba:** la plantilla 3.4 se contradice —su §15 y su checklist §19 dicen «por unidad de entrega» y su propio encabezado de Parte C sigue diciendo «por proyecto de código»—; se emite como hallazgo y no se copia. |
| 2.2 | 2026-08-16 | **Absorbe la decisión del Product Owner registrada en [`ADR-08006`](../Docs/Producto/Adrs/ADR-08006-El-Visor-Recibe-Piezas-Reconstruidas-Y-No-El-Texto.md): el visor recibe las piezas ya reconstruidas y no el texto del alumno.** Se reescriben **dos puntos de dos escenarios**, y nada más. **§20.E-7 punto 4**: la página estática recibe la estructura de piezas en lugar del JSON pegado a mano, y se declara qué sigue ejercitando el bundle —el mapeo de los seis tipos a malla— y qué dejó de ser suyo —la tolerancia de claves, que ahora verifica la batería del validador—. **§20.E-8 puntos 2 y 3**: la pieza del índice 1 **no llega al visor**, porque el validador no la reconstruye y emite su error con índice y campo; la condición `DIMENSION_NO_LEGIBLE` sigue declarada en el contrato de la fachada y deja de ser el camino de este escenario, y **la confusión que el escenario detecta cambia de par**: ya no es entre dos códigos del visor sino entre no poder dibujar y no poder interpretar. **El punto 5 no se toca**: el desenlace del envío sigue siendo error con el trabajo en `Borrador`, y se cumple igual. **Ningún otro escenario, ninguna capacidad, ninguna regla de negocio y ninguna sección de §17 cambia.** Sube minor. | Product Owner |
| 1.34 | 2026-08-14 | **Con qué se autentica el cambio forzado de contraseña**, precisado por el Product Owner. Dos fuentes se contradecían y la contradicción **dejaba una pantalla inalcanzable**: `Api CU-01` §6 declara que la cuenta con cambio pendiente **no obtiene acceso**, mientras `Definicion-Superficie-HTTP.md` §3 pone la operación de cambio **bajo la guardia de sesión** — sin acceso emitido, el alumno reseteado no podía llegar a cambiar su clave y quedaba fuera del laboratorio. Se resuelve declarando que la operación admite **dos formas de autenticarse**: con **sesión**, para el cambio corriente que cualquier cuenta puede hacer —incluida la del administrador—, y con **la contraseña actual**, para el cambio forzado, donde la provisoria que el administrador comunicó es la que autentica. **`RN-02013` no se afloja**: sigue sin haber sesión de trabajo hasta que la contraseña se cambie. El Product Owner declaró además el flujo completo: cada usuario cambia su propia contraseña, y si un alumno la pierde el administrador se la resetea y **le acerca la provisoria**; los flujos más automatizados exigirían correo o mensajes, que este producto no tiene (X-1). Sube minor y archiva: 1.33 ya fue citada como insumo. | Product Owner (decisión) · Orquestador SDD (registro) |
...
| 1.15 | 2026-08-09 | **Tres defectos de la propia fuente**, levantados por `Coherencia-Corpus-r1.md` y por la corrección que lo siguió. **(a)** El título y el ancla de §4.1 decían «RN-02001 a **RN-02009**» sobre una sección de **dieciséis** reglas: el rango quedó congelado en la emisión y nunca se movió, pese a siete reglas nuevas. **(b) El más importante**: RN-02016 afirmaba que «no existe ninguna escritura anónima **en el sistema**», y eso **es falso** — el **registro de cuenta** de RF-03 es anónimo por diseño y así debe seguir, porque es como el alumno entra al laboratorio. Lo que RN-02016 elimina es la escritura anónima **de credencial**. La afirmación de más se propagó a los siete proyectos antes de que nadie la mirara, que es el riesgo de enunciar una garantía más ancha que la decisión que la sostiene. **(c)** El rótulo de la segunda tabla de §4.1 volvió a quedar corto al entrar RN-02016 —encabezaba siete filas enumerando seis—, el mismo defecto que 1.11 ya había corregido una vez; se reescribe declarando que enumera tandas y que envejece con cada una. Ninguna decisión cambia. Sube minor y archiva: 1.14 ya fue citada como insumo. | Orquestador SDD |
| 1.14 | 2026-08-09 | **Dos contradicciones internas que dejó la entrada de RN-02016**, levantadas por la propagación de 1.13 a los siete proyectos. **(a)** El enunciado de **INV-09** en §17.1.P.2 · GeometriaFactory-Domain seguía diciendo que la marca de cambio pendiente «la pone **únicamente** el reseteo del administrador» —redacción de 1.7— mientras RN-02016 declara que **habilitar también la pone**, y cita a ese mismo invariante al hacerlo. La fuente se contradecía a sí misma en el punto exacto que la propagación tenía que transcribir; los proyectos propagaron la decisión y no la letra, dejando constancia. Corregido: la marca la ponen las **dos** operaciones que producen provisoria. **(b)** La prosa de §17.1.P.2 · GeometriaFactory-Domain cerraba en **quince** reglas con «seis sin invariante»; con RN-02016 son **dieciséis: diez con invariante y seis sin él**. Ninguna decisión cambia: los dos son defectos de transcripción de la fuente. Sube minor y archiva: 1.13 ya fue citada como insumo. | Orquestador SDD |
| 1.13 | 2026-08-09 | **Identificación de la cuenta en el primer ingreso**, resuelta por el Product Owner. La emisión de la Fase B de `GeometriaFactory-Api` levantó que establecer la contraseña del primer ingreso era **la única escritura de la superficie que ocurría sin credencial**, y que ninguna fuente declaraba cómo viaja la identidad en esa operación: un punto anónimo que aceptara correo y contraseña nueva habría permitido fijarle la contraseña a cualquier cuenta habilitada antes que su dueño. Se resuelve con **RN-02016**: habilitar produce una contraseña provisoria, con el mismo mecanismo y el mismo tratamiento que el reseteo de F-26, y el alumno la cambia por el camino que RN-02013 ya fija. Dos consecuencias: **no queda ninguna escritura anónima en el sistema**, y el producto tiene **un solo mecanismo de credencial inicial** en lugar de dos. F-04 se precisa en consecuencia. Sube minor y archiva: 1.12 ya fue citada como insumo. | Product Owner (decisión) · Orquestador SDD (registro) |
...
| 1.11 | 2026-08-09 | **Tres residuos de forma que dejó la incorporación de F-26**, levantados por `F26-Propagacion-r2.md` y por la revisión del manifiesto. **(a)** El rótulo de §4.1 decía «**Dos** reglas nuevas que el circuito de revisión introduce» encabezando **seis** filas: no se actualizó cuando entraron las cuatro del reseteo, y atribuía al circuito de revisión reglas que introdujo F-26. Reescrito distinguiendo las dos tandas por fecha y por origen. **(b)** La prosa de §17.1.P.2 · GeometriaFactory-Domain nombraba a **RN-02012** entre las reglas sin invariante, cuando la que corresponde ahí es **RN-02011**: la prosa contradecía a su propia tabla —la columna de INV-09 asigna RN-02012 y RN-02013— y a `GeometriaFactory-Domain`, que cuenta nueve con invariante y seis sin él. La auditoría había diagnosticado el defecto como un error de conteo; era un identificador equivocado. **(c)** §18 enumeraba **cinco** funciones de la fachada al describir el punto de extensión, contra las **seis** que §17.7 P.3 declara desde 1.6. Ninguna decisión cambia: los tres son defectos de transcripción. Sube minor y archiva: 1.10 ya fue citada como insumo. | Orquestador SDD |
...
| 2.1 | 2026-08-16 | **Cierra el hallazgo M-09 del informe de migración, en la parte determinable.** La familia **`RN` se renumera a cinco dígitos**: `RN-01` a `RN-16` pasan a `RN-02001` a `RN-02016`, con el mismo enunciado, el mismo orden y **377 citas reconectadas** desde registro. No es una elección de numeración: el árbol migrado **ya numeraba estas mismas reglas así**, y hasta hoy convivían `RN-15` y `RN-02015` para la misma regla. `Root-Rules.md` §9.2 fija cinco dígitos y enumera `RN` entre las familias alcanzadas. **Las demás familias que este intake acuña —`INV`, `F`, `E`, `A`, `X`, `R`, `CL`, `CP`, `RF` y `RA`— conservan su ancho de origen y quedan declaradas en `ADR-14002`**, porque no tienen numeración de destino en el árbol y renumerarlas es una decisión sin efecto verificable. Ninguna regla, invariante, escenario ni decisión cambia de enunciado. Sube minor. |
