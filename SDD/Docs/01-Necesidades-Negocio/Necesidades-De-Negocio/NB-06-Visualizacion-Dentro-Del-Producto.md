...
| Trazabilidad upstream | PRODUCT-INTAKE 1.9 §1 (idea y problema), §3 (diferenciadores D-3 y D-4), §4 (capacidades F-11 y F-25, `Must Have`, y F-13, `Should Have`), §4.1 (regla RN-11), §6 (flujos 2, 2.1 y 3), §17.7 P.3 (la sexta función de la fachada, que gobierna los dos movimientos), §17.7 P.10 (el movimiento automático no altera la disposición), §20 (escenarios E-1 y E-7); `Vision-Producto.md` §1, §3 y §9; `Alcance-Producto.md` §4.1 —donde vive F-25 desde su promoción—, §4.2 —donde vive F-13— y §8; `Roadmap-Producto.md` §3 y §5.2 |
...
Los criterios octavo y noveno tienen el plazo de la etapa `g` porque es la etapa sobre cuya superficie vive la capacidad, y desde `PRODUCT-INTAKE` 1.7 **son además bloqueantes**: promovida F-25 a `Must Have`, la transición `g` → `h` de `Roadmap-Producto.md` §5.2 incorpora el gobierno independiente de los dos movimientos como **séptimo criterio**, incorporado allá el 2026-08-09 con la versión 1.4 de ese documento; y `Alcance-Producto.md` §8 incorpora el criterio de aceptación de producto correspondiente. El mecanismo de diferimiento que este apartado describía para estos dos criterios **ya no aplica**; sigue aplicando para F-13, que continúa siendo `Should Have`.
