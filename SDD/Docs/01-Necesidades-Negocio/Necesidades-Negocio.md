...
| NB-02 | F-02, F-04, F-05, F-26 | El trabajo del alumno no tiene dueño, y la ausencia de canal de correo condiciona las cuatro capacidades por igual: ninguna credencial se transporta. F-26 es la cuarta cara del mismo dolor —cómo llega el alumno a tener una credencial propia, y cómo la recupera cuando la pierde— y no un dolor nuevo: la provisoria tampoco viaja por ningún canal, y por RN-13 no sirve para operar sino sólo para cambiarla. Su fundamento completo, incluido por qué no cae en NB-01 pese a ejercerse desde el panel del administrador, está en NB-02 §9 |
...
**Punto abierto que este catálogo arrastraba, y que ya está resuelto.** El objetivo de avance del producto declaraba «7 de 7 etapas (`a` a `g`)» mientras el intake declaraba ocho etapas comprometidas. **El Product Owner lo resolvió** el 2026-08-09 en PRODUCT-INTAKE §8: se cuentan las ocho comprometidas, `a` a `h`. `Vision-Producto.md` §5 y §6 y `Alcance-Producto.md` §3 lo recogen, y §5.2 de este catálogo también. Como ninguna NB usaba ese target como criterio de éxito propio, la resolución no obligó a reescribir ningún criterio de acá. Subsiste un residuo **de la fuente y no de esta categoría**: la fila A-2 de PRODUCT-INTAKE §22 todavía transcribe «7 de 7 etapas», y corregirla es del Product Owner sobre su propio documento.
