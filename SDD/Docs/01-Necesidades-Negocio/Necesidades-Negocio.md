...
Las dieciséis capacidades Must Have del intake se agrupan en ocho necesidades (NB-01 a NB-07 más NB-09), las capacidades Should Have F-13 y F-25 se absorben en NB-06 por compartir etapa y dolor con F-11, la Could Have F-15 se absorbe en NB-07 por la misma razón, y la Should Have F-14 recibe necesidad propia porque su dolor —que el laboratorio esté disponible en el aula— es de naturaleza distinta y tiene su propio riesgo declarado.
...
Las dieciséis capacidades Must Have están cubiertas: ninguna quedó sin necesidad que la articule. La correspondencia entre capacidad y etapa se lee de `Alcance-Producto.md` §4.1 y de `Roadmap-Producto.md` §3, y no se decide acá. Un caso merece nota: **el intake no asigna etapa a F-25** —su §15 sigue declarando `g` con F-11 y F-13—, de modo que la etapa `g` de esa fila la fundamenta `Roadmap-Producto.md` §3 como decisión de planificación, y esta categoría la deriva de ahí. Que F-25 sea `Should Have` y viva en una etapa comprometida no la compromete: no es criterio de transición de ninguna fase ni criterio de aceptación del producto.
...
Veintiocho casos de uso previstos, todos con estado `a generar`. La numeración es una previsión de esta categoría y la confirma la categoría 02 al redactarlos.
...
| --- | --- | --- |
| CU | 02-Especificacion-Funcional | Las 28 CU previstas de §5.3 y el problema de negocio que cada una debe resolver |
