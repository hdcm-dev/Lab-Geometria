...
| --- | --- | --- |
| F-13 | **Must Have** desde el intake 1.19 | Absorbida en NB-06, identificada en su cuarto criterio —disposición estable entre procesados—, en su quinto —sincronización entre el árbol y la escena— y en CU-17. Su promoción a `Must Have` **no cambia el agrupamiento** —la prioridad decide qué se difiere primero, no dónde vive la necesidad—, pero sí cambia el trato: los dos criterios dejan de ser diferibles y NB-06 §7 lo declara. El fundamento de la promoción es que `PRODUCT-INTAKE` §17.7 P.8 ya incluía las dos propiedades entre lo que la puerta técnica `PT-02` mide antes de comprometer la etapa `g` |
