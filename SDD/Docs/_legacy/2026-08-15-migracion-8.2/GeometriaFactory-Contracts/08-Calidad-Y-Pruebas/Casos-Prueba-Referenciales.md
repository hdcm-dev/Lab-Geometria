...
#### TC-08001 — Respuesta-De-Sesion-Con-Cuatro-Campos

| Campo | Valor |
| --- | --- |
| Tipo | Integración e inspección de superficie |
| Cubre | `CU-08001`; `RT-01`, `RT-10`; `US-08001`; NFR «Campos de la respuesta de sesión» |
...
#### TC-08002 — Las-Condiciones-Que-Impiden-Operar-Viajan-Como-Error

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08001`, `CU-08006`, `CU-08008`; `RT-10`; `US-08001`, `US-08014`, `US-08022`; `RN-08006`, `RN-08013`, `RN-08016` de `GeometriaFactory-Domain` |
...
| Salida esperada | Ninguna respuesta de sesión emitida en los tres casos; dos respuestas con código idéntico. Es `CA-08` de `CU-08006` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-08003 — Registro-De-Cuenta-De-Alumno

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08002`; `US-08002`; `RN-08002` de `GeometriaFactory-Domain` |
...
#### TC-08004 — Listado-De-Cuentas-Del-Panel-Del-Administrador

| Campo | Valor |
| --- | --- |
| Tipo | Integración e inspección de superficie |
| Cubre | `CU-08002`; `RT-01`; `US-08003` |
...
#### TC-08005 — Cambio-De-Situacion-De-La-Cuenta

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08002`; `US-08004`; `RN-08016` de `GeometriaFactory-Domain` |
| Setup | Una cuenta `Pendiente` y una `Habilitado` |
| Pasos | Given la orden de habilitar, When se la envía, Then el resultado **devuelve la provisoria producida**, porque habilitar la produce (`RN-08016`). Given la orden de bloquear y la de rehabilitar, Then el resultado declara la situación alcanzada. Given la configuración de un segundo administrador, Then `CONTRATO_ADMINISTRADOR_YA_CONFIGURADO` |
| Salida esperada | Tres órdenes con su resultado y un rechazo. **Ningún tipo de establecimiento anónimo de contraseña participa**: `RN-08016` lo eliminó |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-08006 — Baja-Con-Su-Confirmacion-Escrita

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08002`; `US-08005`; `RN-08007` de `GeometriaFactory-Domain` |
...
#### TC-08007 — El-Texto-Original-Viaja-Como-Cadena-Sin-Interpretarse

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08003`; `RT-03`, `RT-08`; `US-08006`, `US-08019`; `RN-08008` de `GeometriaFactory-Domain` |
...
#### TC-08008 — Solicitud-Unica-De-Eliminacion-Para-Los-Dos-Papeles

| Campo | Valor |
| --- | --- |
| Tipo | Integración e inspección de superficie |
| Cubre | `CU-08003`; `US-08007`; `RN-08004`, `RN-08003`, `RN-08011` de `GeometriaFactory-Domain` |
...
#### TC-08009 — La-Proyeccion-De-Listado-No-Arrastra-El-Detalle

| Campo | Valor |
| --- | --- |
| Tipo | Inspección de superficie e integración |
| Cubre | `CU-08004`; `RT-04`; `US-08008`, `US-08020`; NFR «Carga útil del listado» |
...
| Salida esperada | Tres recuentos en 0, y el estado presente. Es la restricción que da sentido a la separación entre `CU-08004` y `CU-08005` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-08010 — El-Alcance-Del-Listado-Cambia-Segun-El-Papel

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08004`; `US-08009`; `RN-08011`, `RN-08003` de `GeometriaFactory-Domain` |
...
#### TC-08011 — Resumen-Por-Alumno-Y-Por-Estado

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08004`; `US-08010` |
...
| Salida esperada | Recuentos coherentes con el listado. **Este caso de prueba está fuera del tramo comprometido**: `US-08010` es `Could` y deriva de `F-15`, capacidad `Could Have` de la fase `i…`. Se declara para que la cobertura de las veintidós historias quede completa, y **no se compromete** |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente`, fuera del tramo comprometido |

#### TC-08012 — El-Detalle-Transporta-Piezas-Componentes-Y-Texto

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08005`; `RT-03`; `US-08011`, `US-08012` |
...
#### TC-08013 — La-Observacion-Lleva-Severidad-Y-El-Par-De-Valores

| Campo | Valor |
| --- | --- |
| Tipo | Integración e inspección de superficie |
| Cubre | `CU-08005`; `US-08013`; `RN-08009` de `GeometriaFactory-Domain` |
...
#### TC-08014 — El-Comentario-Es-Bloque-Propio-Y-Nunca-Observacion

| Campo | Valor |
| --- | --- |
| Tipo | Inspección de superficie e integración |
| Cubre | `CU-08005`, `CU-08007`; `RT-09`; `US-08018`, `US-08020` |
...
#### TC-08015 — El-Tipo-De-Error-Tiene-Cuatro-Campos-Y-Ninguno-Filtra

| Campo | Valor |
| --- | --- |
| Tipo | Inspección de superficie e integración |
| Cubre | `CU-08006`; `RT-01`, `RT-02`; `US-08014`; NFR «Campos capaces de transportar una dirección de servicio, una ruta de datos o un secreto» |
...
| Salida esperada | Cuatro campos, tres recuentos en 0, y un texto que no revela cuál de los dos datos de ingreso falló. Son `CA-01`, `CA-03` y `CA-04` de `CU-08006` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-08016 — El-Conjunto-Cerrado-Tiene-Diecisiete-Codigos-Vivos

| Campo | Valor |
| --- | --- |
| Tipo | Inspección de superficie |
| Cubre | `CU-08006`; `US-08014`, `US-08016`; NFR «Códigos de error del conjunto cerrado» |
...
| Salida esperada | 17 vivos, 3 retirados sin reciclar, 20 emitidos en total, 0 códigos para las dos causas que `RN-08016` volvió imposibles, y el cierre del conjunto verificado. Es `CA-09` de `CU-08006` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-08017 — El-Detalle-De-Ubicacion-Lleva-Indice-Y-Campo

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-08006`, `CU-08005`; `RT-02`; `US-08015`; `RN-08009` de `GeometriaFactory-Domain` |
...
| Salida esperada | Índice 1 y campo `Tipo` en la observación de `E-5`; estado `Borrador` con observación localizada en `E-8`; un detalle con el campo señalado en el error de forma. **El índice 1 y no 0** es lo que prueba que la ubicación se calcula y no se informa siempre la primera figura. Es `CA-02` de `CU-08006` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-08018 — El-Desenlace-Es-Cerrado-Y-No-Tiene-Vuelta

| Campo | Valor |
| --- | --- |
| Tipo | Inspección de superficie e integración |
| Cubre | `CU-08007`; `RT-08`; `US-08017`; `RN-08010`, `RN-08011` de `GeometriaFactory-Domain` |
...
#### TC-08020 — Cero-Referencias-Hacia-El-Dominio

| Campo | Valor |
| --- | --- |
| Tipo | Inspección de superficie |
| Cubre | `RT-05`; NFR «Referencias hacia `GeometriaFactory-Domain`»; `BT-08002`; `QG-02`; `DXC-01` |
...
#### TC-08021 — Cien-Por-Ciento-De-Los-Tipos-Ejercitados

| Campo | Valor |
| --- | --- |
| Tipo | Inspección sobre la matriz |
| Cubre | `RT-07`; NFR «Tipos ejercitados por prueba de integración» **[ASUNCIÓN del intake §17.4.P.6, sobre la forma del gate]**; `BT-08016`; `QG-05`, **bloqueante** |
...
#### TC-08022 — Ningun-Tipo-Habilita-Al-Navegador-A-Invocar-El-Servicio
...
| Magnitud | Valor |
| --- | --- |
| Casos de prueba declarados | **22**, `TC-08001` a `TC-08022`, serie contigua |
...
| Historias de usuario cubiertas | **22 de 22**, una de ellas —`US-08010`— fuera del tramo comprometido y declarada así |
...
| Códigos del conjunto cerrado verificados | **17 vivos** y **3 retirados sin reciclar**, sobre **20** emitidos, agregados en `TC-08016` y desplegados en los casos funcionales |
| Escenarios del intake §20 alcanzados | **8 de 8** |
| Casos de prueba sin upstream declarado | **0** |

**Verificación de la cobertura de los ocho escenarios, uno por uno:** `E-1`, `E-6` y `E-7` en `TC-08012`; `E-2` en `TC-08007`; `E-3` y `E-4` en `TC-08013`; `E-5` y `E-8` en `TC-08017`. Ninguno se sustituye por datos sintéticos.

## 4. Control de cambios

| Versión | Fecha | Descripción |
| --- | --- | --- |
| 1.1 | 2026-08-11 | **`H-02`.** El campo «Cubre» de `TC-08021` califica ahora el rótulo [ASUNCIÓN] de `QG-05` como referido a **la forma del gate** y declara el gate **bloqueante**, según §17.4.P.6 y la fila `A-4` del intake §22. **Ningún caso de prueba, paso ni salida esperada cambia.** Corrige contra [`../../../Audit/E-08-Calidad-Siete-Proyectos-r1.md`](../../../Audit/E-08-Calidad-Siete-Proyectos-r1.md) 1.0 y contra el texto vivo del intake **1.20**. |
| 1.0 | 2026-08-11 | Emisión inicial. Declara **veintidós** casos de prueba referenciales, `TC-08001` a `TC-08022`, cada uno con tipo, upstream por identificador, setup, pasos en Given-When-Then, salida esperada, salida observada y estado. Diecinueve cubren los **ocho** contratos de uso y las **veintidós** historias; los tres últimos son inspecciones de superficie sobre las referencias hacia el dominio, el ejercicio de los tipos por integración y la prohibición de que el navegador invoque el servicio. Los **ocho** escenarios del intake §20 quedan alcanzados, con la precisión de qué parte de cada uno le toca a un ensamblado que transporta y no interpreta. `TC-08011` se declara fuera del tramo comprometido con su motivo. Todas las salidas observadas dicen «Sin ejecutar» y todos los estados `Pendiente`. |
