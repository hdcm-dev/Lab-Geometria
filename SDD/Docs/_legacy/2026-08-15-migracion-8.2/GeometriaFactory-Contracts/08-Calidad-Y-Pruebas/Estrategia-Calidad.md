...
| QG-03 | **0** campos capaces de transportar el hash de la contraseña, la clave de firma, una dirección de servicio interno, una ruta de archivo de datos o una traza | Prueba de inspección de superficie pública, que es `CA-01` de `CU-08006` (`TC-08015`) | **Se rechaza aunque compile** (`DXC-05`) |
| QG-04 | El conjunto cerrado tiene exactamente **17** códigos vivos y se producen **0** códigos fuera de él | Prueba de inspección del conjunto, que es `CA-09` de `CU-08006` (`TC-08016`) | Se rechaza aunque compile (`DXC-03`) |
| QG-05 | **100 %** de los tipos de transferencia ejercitados por al menos una prueba de integración **[ASUNCIÓN del intake §17.4.P.6, sobre la forma del gate y no sobre su carácter]** | Matriz tipo contra prueba de [`Matriz-Cobertura-Pruebas.md`](Matriz-Cobertura-Pruebas.md) §6, sobre `BT-08016` | **Bloquea la fusión, y no es condicionado.** Lo sujeto a confirmación es **la forma**, ver §3.1 |
| QG-06 | La proyección de listado no lleva texto original, ni componentes de pieza, ni comentario **[ASUNCIÓN derivada del intake §17.4.P.10]** | Inspección de la superficie de la familia de listado (`TC-08009`) | **Condicionado**, ver §3.1 |
| QG-07 | La respuesta de sesión declara exactamente **4** campos y **0** que transporten una condición que impida operar | Inspección de la superficie pública, restricción `RT-10` (`TC-08001`, `TC-08002`) | Se rechaza aunque compile (`DXC-14`) |
...
| QG-09 | **Ningún tipo permite salir de un estado terminal** y ningún tipo habilita a que el navegador invoque el servicio de datos | Inspección de la superficie, restricciones `RT-08` y `RT-11` (`TC-08018`, `TC-08022`) | Se rechaza aunque compile (`DXC-10`) |
