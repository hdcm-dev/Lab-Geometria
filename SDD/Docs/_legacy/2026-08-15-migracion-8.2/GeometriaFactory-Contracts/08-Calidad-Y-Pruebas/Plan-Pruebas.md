...
**Qué cubre.** Los **veintidós** casos de prueba de [`Casos-Prueba-Referenciales.md`](Casos-Prueba-Referenciales.md), repartidos entre las **siete** etapas comprometidas que este proyecto de código toca —`a`, `c`, `d`, `e`, `f`, `g` y `h`—, más `TC-08011`, que cae en la fase `i…` y **no se compromete**.
...
- [ ] `BT-08001` está cerrada: el ensamblado compila y **no declara ninguna biblioteca de serialización**.
- [ ] `BT-08004` está cerrada para la familia de la etapa: sus nombres de tipos, de campos y de espacios de nombres están decididos y registrados en el punto de control de esa etapa.
- [ ] `BT-08006` y `BT-08007` están cerradas antes de cualquier etapa posterior a la `c`: el tipo de error existe con sus cuatro campos y el conjunto cerrado de **diecisiete** códigos está declarado. **Las siete familias restantes dependen de la familia de error**.
...
| --- | --- | --- | --- | --- |
| RQ-01 | Que aparezca una referencia hacia `GeometriaFactory-Domain` y el acoplamiento vuelva por esa vía | Alto | Media | `TC-08020` en **cada** etapa, no sólo en la `a`; `QG-02` bloquea la fusión |
| RQ-02 | **Que un campo nuevo transporte una dirección de servicio o una traza, sin que nadie lo note porque compila** | Alto | Media, y es la forma habitual en que este defecto entra | `TC-08015`, más la cadencia de revisión **por cambio de superficie** de [`Estrategia-Calidad.md`](Estrategia-Calidad.md) §5 |
| RQ-03 | Que el listado incorpore un campo del detalle «porque hace falta en una pantalla» | Medio | **Alta**: es la presión natural de la capa de presentación | `TC-08009`, con sus tres recuentos en 0, ejecutado en cada etapa que toque la familia de listado |
| RQ-04 | Que un identificador de código retirado se recicle para otra condición | Medio | Baja, **pero con precedente**: ya hay tres retirados | `TC-08016`, que compara los diecisiete vivos contra los tres retirados |
...
| RQ-06 | Que aparezca un tipo pensado para que el navegador invoque el servicio de datos | Alto | Baja | `TC-08022` |
...
| --- | --- | --- | --- |
| `a` | EP-08001 Esqueleto ambulante | `TC-08020`: cero referencias hacia el dominio. `QG-01` medido por primera vez | Todo lo demás: no hay tipos todavía |
| `c` | EP-08002 Identidad del administrador y sesión | `TC-08001` en su tramo de inspección, `TC-08015`, `TC-08016`, `TC-08022` | `TC-08002`, y el tramo de integración de `TC-08001` y `TC-08015` |
| `d` | EP-08003 Ciclo de vida de la cuenta de alumno | `TC-08019` en su tramo de inspección, `TC-08004` en su tramo de inspección | `TC-08003`, `TC-08005`, `TC-08006`, y los tramos de integración de `TC-08004` y `TC-08019` |
| `e` | EP-08004 Gestión del trabajo | `TC-08008` y `TC-08009` en sus tramos de inspección | `TC-08007`, `TC-08010`, y los tramos de integración de `TC-08008` y `TC-08009`. **`TC-08011` no entra acá**: cae en la fase `i…` |
| `f` | EP-08005 Interpretación y verificación | `TC-08013` y `TC-08014` en sus tramos de inspección | `TC-08012`, `TC-08017`, y los tramos de integración de `TC-08013` y `TC-08014` |
| `g` | EP-08006 Visualización del trabajo | Nada nuevo de superficie: la familia de detalle ya existe | Reejecución de `TC-08012` verificando que el texto original que el árbol despliega sigue llegando íntegro |
| `h` | EP-08007 Desenlace de la entrega | `TC-08018` en su tramo de inspección | El tramo de integración de `TC-08018` y la reejecución de `TC-08014` |
| `i…` | EP-08008 Capacidades de prioridad menor | — | `TC-08011`. **Fuera del tramo comprometido**, se declara y no se compromete |
| Al cerrar cada etapa | — | `TC-08021`: la matriz tipo contra prueba, con lo ejercitado hasta ese momento | — |

**La suma cubre los veintidós casos de prueba.** `TC-08021` aparece en todas las etapas porque es la medición acumulativa del gate equivalente a la cobertura.
