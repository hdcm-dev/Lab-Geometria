...
**Trazabilidad upstream:** `Especificacion-Funcional.md` (índice maestro de esta sección); `00-Contexto/Vision-Producto.md` §9; `01-Necesidades-Negocio/Necesidades-Negocio.md` §2 y §5.3; `PRODUCT-INTAKE` §17.4, §13 y §14
...
- [2. Los seis casos de uso](#2-los-seis-casos-de-uso)
...
| [`Casos-De-Uso/`](Casos-De-Uso/) | Un contrato de uso por archivo, `CU-01` a `CU-06`, con las once secciones obligatorias más la §17 de compatibilidad de versión pública, que es la opcional propia de `library` | Propuesto |
...
1. **`Especificacion-Funcional.md`** — primero siempre. Sin su §2 y su §5, los seis casos de uso se leen como una lista de tipos sin la decisión que los ordena.
2. **CU-06** — conviene leerlo temprano, aunque sea el último del catálogo: los otros cinco lo referencian en todos sus caminos de error.
...
5. **CU-04 y CU-05** — las dos lecturas, que se leen de a pares: la proyección de listado existe precisamente para no ser el detalle.
6. **`Glosario-Funcional.md`** — se puede leer suelto, y conviene abrirlo antes que nada si el lector viene de otra categoría, por los tres referentes de «contrato».
...
La omisión de las `RN-XX` es la que explica que la columna RN de la matriz del índice maestro esté vacía en sus ocho filas. Esa columna se mantiene en la matriz, con el motivo declarado, en lugar de suprimirse.

## 5. Notas de uso de esta sección

- **Autoridad.** Ningún caso de uso de esta sección origina una capacidad, una prioridad ni una exclusión: todo se deriva del `PRODUCT-INTAKE`, de `00-Contexto/` y de las ocho `NB-XX` de `01-Necesidades-Negocio/`, y traza a su sección de origen.
- **Actor.** El actor de los seis casos de uso es el código que compila contra el ensamblado de contratos. No hay actor humano: las personas del producto aparecen en la especificación funcional de `GeometriaFactory-Web`.
...
- **Numeración.** Los identificadores `CU-XX` y `US-XX` son locales a este proyecto de código; la decisión está declarada en `Especificacion-Funcional.md` §3.2 y la correspondencia con las veintidós previsiones de `Necesidades-Negocio.md` §5.3, una por una, en su §4.2. Los seis casos de uso numeran su única sección opcional como **§17**, que es el número que `Rules-Especificacion-Funcional.md` §4.3 le asigna a la compatibilidad de versión pública para `library`; el hueco entre §11 y §17 es deliberado y está declarado en el encabezado de esa sección.
- **Vocabulario.** Los términos del dominio no se redefinen acá: están en `Vision-Producto.md` §9. La palabra «proyecto» a secas no se usa, «pieza» en su referente de artefacto desplegable se escribe siempre calificada, y «contrato» tiene tres referentes declarados en `Glosario-Funcional.md` §3.1.
