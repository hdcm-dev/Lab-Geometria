...
**Autor:** DX Lead (AG-03)
**Variante:** DX
**Trazabilidad upstream:** `02-Especificacion-Funcional/Glosario-Funcional.md` §2 (veintidós términos acuñados), §3.1, §3.2, **§3.3** (`Pendiente`) y §4 (veinticuatro términos referenciados); `00-Contexto/Vision-Producto.md` §9.1 (Estado del trabajo, Enviar, Aprobar / Rechazar, Comentario) y §9.2 (`Pendiente`, forma calificada obligatoria); `02-Especificacion-Funcional/Especificacion-Funcional.md` §4.2 (correspondencia con la previsión de 01, con el prefijo `P·`) y §6; `00-Contexto/Vision-Producto.md` §9 (glosario raíz de la cadena: §9.1, §9.2 y §9.3); `Vocabulario-Rules.md` §2, §4 y §9; `Rules-UX-UI-DX.md` §3.3; `PRODUCT-INTAKE` §12.1, §15, §16, §17.4 P.3, P.5 y P.8
...
- [3. Términos con más de un referente](#3-términos-con-más-de-un-referente)
  - [3.1 Error](#31-error)
  - [3.2 Contrato, pieza y `Pendiente`: resueltos aguas arriba](#32-contrato-pieza-y-pendiente-resueltos-aguas-arriba)
...
- `02-Especificacion-Funcional/Glosario-Funcional.md` es el glosario de la categoría anterior de este mismo proyecto de código, con veintidós términos acuñados y **tres** términos con más de un referente: «contrato», «pieza» y `Pendiente`.
...
| --- | --- | --- |
| El defecto que impide interpretar el texto del alumno, especie de «observación» junto con la advertencia | **«error de validación»**, siempre calificado. Declarado en `Vision-Producto.md` §9.1 y referenciado en `Glosario-Funcional.md` §4 | `DX-Error-Messages.md` §3.3, en la causa de las señales `DXT-N2` y `DXT-N3`; §1 principio 3 y §4 |
...
Evidencia de que los contextos colisionan, verificable dentro de la §3 de `DX-Error-Messages.md`: `DXT-N3` de §3.3 es una **señal** —explícitamente no un error transportado— cuya causa es un **error de validación** del texto del alumno, y que lleva identificador de la serie transportada; `DXT-14` de §3.2 es un **error transportado**; y `DXC-11` de §3.1 es un **error de construcción**. Los tres sentidos conviven en la misma sección y a pocas líneas de distancia, de modo que la forma desnuda «error» sin calificar no permite decidir de cuál se habla. La colisión se lee además condensada en una sola oración de `DX-Error-Messages.md` §1 principio 3 y en la fila de diagnóstico de `Guia-Onboarding-Developer.md` §4 que empieza «Se busca el código de error del texto que no verifica».
...
### 3.2 Contrato, pieza y `Pendiente`: resueltos aguas arriba

Los **tres** términos con más de un referente que esta sección hereda ya están resueltos y **no se vuelven a declarar acá**: se cumplen.
...
- **`Pendiente`** tiene dos referentes, declarados en `Vision-Producto.md` §9.2 y resueltos en `Glosario-Funcional.md` §3.3: el estado de una **cuenta** y el de un **trabajo**. Va **siempre calificado**, con las dos excepciones que la resolución declara —las enumeraciones del conjunto cerrado y los identificadores literales de código—, donde calificar sería el falso positivo de `Vocabulario-Rules.md` §9.1.
...
| Desenlace | `Glosario-Funcional.md` §2 | La decisión del administrador que resuelve un trabajo en estado `Pendiente`, dentro de un conjunto cerrado de dos valores. Es uno de los conjuntos cerrados que `DXC-03` enumera, y el origen de `DXT-14` y `DXT-15` |
| Estado terminal | `Glosario-Funcional.md` §2 | `Finalizado` y `Rechazado`, de los que no sale ninguna transición. Es lo que `DXC-10` protege: el contrato no declara ningún tipo que permita salir de ellos, y esa ausencia es deliberada |
| Señal declarada que no es error | `Glosario-Funcional.md` §2 | Un código que un contrato de uso nombra en su §6.1 y que no produce respuesta de error ni pertenece al conjunto cerrado. Son tres, y son las tres entradas `DXT-Nx` de `DX-Error-Messages.md` §3.3. **El término lo acuña 02 y esta sección lo usa sin redefinirlo** |
| Estado del trabajo | `Vision-Producto.md` §9.1, registrado en `Glosario-Funcional.md` §2 | Conjunto cerrado de cuatro valores, con dos terminales. Agregar uno es incompatible de hecho (`DXC-03`) |
| Enviar | `Vision-Producto.md` §9.1 | La única acción de guardado del alumno. Es el motivo por el que `CONTRATO_TEXTO_NO_INTERPRETABLE` dejó de ser código de error: ninguna operación falla por un texto que no verifica. Ver `DXT-N3` |
| Aprobar / Rechazar | `Vision-Producto.md` §9.1 | Las dos decisiones del administrador, que el contrato transporta como los dos valores del desenlace |
| Comentario | `Vision-Producto.md` §9.1 | Texto libre y opcional del administrador. **No es calificación y no es observación.** Es el término alrededor del cual giran `RT-09`, `DXC-11` y `DXC-12`, y la quinta pregunta del tramo de 30 minutos del recorrido |
| `Pendiente`, forma calificada obligatoria | `Vision-Producto.md` §9.2, resuelto en `Glosario-Funcional.md` §3.3 | Nombra dos estados distintos —el de una cuenta y el de un trabajo— y los dos cruzan este contrato. Se escribe «cuenta `Pendiente`» o «trabajo en estado `Pendiente`». **Las enumeraciones del conjunto cerrado y los identificadores literales de código quedan exceptuados**, por declaración de aguas arriba |
...
| 1.1 | 2026-08-09 | **Actualización por contenido nuevo aguas arriba**: `PRODUCT-INTAKE` 1.3 incorporó el circuito de revisión del administrador y 00 dio de alta en su glosario raíz los cuatro estados del trabajo, «enviar», «aprobar / rechazar» y «comentario». Sube minor y archiva el estado anterior por `Master-Prompt.md` §5. **§4 pasa de veintitrés a treinta y una entradas**, todas **referenciadas y no redefinidas**: «desenlace», «estado terminal» y «señal declarada que no es error» del glosario de 02, y «estado del trabajo», «enviar», «aprobar / rechazar», «comentario» y la forma calificada obligatoria de `Pendiente` del glosario raíz. **No se acuñó ningún término nuevo en §2 y no se agregó ninguna entrada a §3**: los términos nuevos ya están declarados aguas arriba, y la única polisemia nueva de la cadena —`Pendiente`— está resuelta en `Glosario-Funcional.md` §3.3, de modo que acá se referencia y se cumple, sin volver a decidirla. §1 y la cabecera actualizan los conteos del glosario de 02. **Corrección de la ronda 3 de auditoría (`Audit/B-02-03-GeometriaFactory-Contracts-r3.md`), absorbida en esta misma versión sin subir a 1.2 y sin snapshot nuevo, por `Master-Prompt.md` §5 y por el punto 5 de §8 del informe. H-04**: la evidencia de colisión de «error» en §3.1 se apoyaba en `DXT-09`, que la propia pasada convirtió en fila de retiro; se reapoya en un trío vigente y verificable dentro de la §3 de `DX-Error-Messages.md` —`DXT-N3` señal con causa de error de validación, `DXT-14` error transportado y `DXC-11` error de construcción—, y la columna «Dónde se lee» del primer referente pasa a las dos señales de §3.3. **Barrido de la misma especie**: §3.2 seguía diciendo «los dos términos que esta sección hereda» cuando desde esta versión son tres; se corrige el conteo, se suma el bullet de `Pendiente` con sus dos excepciones y se ajusta el título y su entrada de la tabla de contenido. | DX Lead (AG-03) |
| 1.2 | 2026-08-09 | **Actualización por contenido nuevo aguas arriba**: `PRODUCT-INTAKE` **1.7** incorpora la capacidad **F-26** y la categoría 02 emite **CU-08008**. **No se acuña ningún término nuevo en §2 y no se agrega ninguna entrada a §3**: «contraseña provisoria» y «cambio de contraseña pendiente» los declara `Glosario-Funcional.md` §2 de la categoría 02, de modo que acá se referencian y no se redefinen, y no introducen ninguna polisemia nueva dentro de esta sección. La evidencia de colisión de «error» de §3.1 no cambia: el trío `DXT-N3`, `DXT-14` y `DXC-11` sigue vigente. Se registra la revisión para que el conteo de códigos transportados de [`DX-Error-Messages.md`](DX-Error-Messages.md), que pasó de catorce a **dieciséis**, no se lea como un término nuevo de este glosario. | DX Lead (AG-03) |
| 1.3 | 2026-08-09 | **Cierra la parte del hallazgo `F26-28`** del informe de auditoría `SDD/Docs/Audit/F26-Propagacion-r1.md` 1.0 que alcanza a este archivo. El informe lo registra sobre `DX-Developer-Experience.md` y `DX-Error-Messages.md`; **este archivo tiene el mismo defecto** y se corrige en la misma pasada para que la carpeta quede pareja. **Cierra la parte del hallazgo `F26-28`** del informe de auditoría `SDD/Docs/Audit/F26-Propagacion-r1.md` 1.0 que alcanza a este archivo: las filas de este control de cambios estaban **fuera de orden cronológico** (1.1, 1.0, 1.0, 1.2) y se reordenan por versión, **sin tocar el texto de ninguna**. **Ningún término cambia de definición, de forma obligatoria ni de criterio de inclusión.** Sube minor: repara el orden de esta tabla sin alterar lo que sus filas dicen. | DX Lead (AG-03) |
