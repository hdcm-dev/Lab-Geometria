...
**Autor:** DX Lead (AG-03)
**Variante:** DX
**Trazabilidad upstream:** `02-Especificacion-Funcional/Especificacion-Funcional.md` §1, §2, §6 (`RT-01` a `RT-11`); `CU-08001` §10 y §17, `CU-08002` §17, `CU-08003` §1, CA-01, CA-02, §6.1 y §17, `CU-08004` CA-01, §6.1, §10 y §17, `CU-08005` CA-05, CA-06, CA-07, CA-08, §6.1 y §17, `CU-08006` §6, CA-01, §10 y §17, `CU-08007` §6, CA-03, CA-05, §10 y §17; `00-Contexto/Vision-Producto.md` §9.1 (Estado del trabajo, Enviar, Aprobar / Rechazar, Comentario) y §9.2; `00-Contexto/Alcance-Producto.md` §2.2 y §8; `01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00004-Interpretacion-Fiel-Del-Dato-Del-Alumno.md` §1; `NB-00009-Desenlace-Explicito-De-La-Entrega.md` §1; `PRODUCT-INTAKE` **1.14** §4.1, §4.2, §17.4 P.3, P.5, P.6, P.8, P.10 y P.11, §15, §16, §16.1
...
Y una segunda frase, que en este contrato se paga casi tan cara: **el comentario del administrador no es una observación**. Los dos viajan en el mismo tipo de detalle, los dos son texto sobre el trabajo, y ahí termina el parecido. La observación la emite el producto al interpretar el texto, es una colección y lleva severidad, índice de figura, campo señalado y par de valores; el comentario lo escribe una persona, es a lo sumo uno y no lleva ninguno de esos campos. Fusionarlos compila sin protestar (`RT-09`, `CU-08005` CA-08).
...
| --- | --- | --- |
| ¿Dónde viajan el texto original completo y el comentario del administrador? | Los dos, **sólo en el detalle** del trabajo interpretado. Es el único tipo del ensamblado que los transporta; el elemento de listado trae el **estado**, que es lo que expresa el desenlace | `CU-08005` §10; `CU-08004` CA-01 y §10 |
| ¿Por qué el elemento de listado no los trae? | Porque la proyección de listado existe precisamente para **no** ser el detalle: declara 0 campos de texto original, 0 de componente de pieza y 0 de comentario, para que el listado no arrastre texto libre de cada trabajo | `RT-04` en su enunciado ampliado; `CU-08004` CA-01 y §10 |
| ¿Cuántos campos tiene la respuesta de error, y cuántos pueden transportar una dirección de servicio? | Exactamente cuatro —código, texto, detalles y momento— y **cero** de la segunda clase | `CU-08006` CA-01 |
| ¿Qué recibe una persona recién habilitada, que canja con la contraseña provisoria de su primer ingreso? | Una **respuesta de error** con el código `CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` y su motivo —**el mismo** que recibe una cuenta con la contraseña reseteada—. **No** una respuesta de sesión con una marca: la respuesta de sesión sigue declarando cuatro campos y ninguno agregado para este caso. Desde `PRODUCT-INTAKE` 1.13, **RN-08016** hace que el primer ingreso y el cambio posterior a un reseteo sean el mismo camino, y `CONTRATO_CONTRASENA_NO_ESTABLECIDA` salió del conjunto cerrado | `CU-08001` FA-02, CA-05 y §10; `CU-08006` §6 y §10; `CU-08008` §1 |
| ¿En qué se diferencia el comentario del administrador de una observación, si los dos viajan en el detalle? | En **cuatro planos, y no comparten ni un campo**: cardinalidad —el comentario es a lo sumo uno, las observaciones son una colección—; origen —lo escribe una persona, las emite el producto al interpretar el texto—; forma —el comentario no lleva severidad, ni índice de figura, ni campo señalado, ni par de valores, y la observación no lleva autoría humana—; y ubicación —el comentario es un bloque propio del detalle, nunca un elemento de la colección de observaciones—. Además **no es una calificación**: no lleva nota ni escala | `RT-09`; `CU-08005` CA-07 y CA-08; `CU-08007` CA-05 |
...
1. Agregar un campo opcional a la respuesta de sesión de `CU-08001`.
2. Agregar una situación de cuenta al conjunto admitido de `CU-08002` —por ejemplo, una cuarta además de pendiente, habilitada y bloqueada—.
3. Agregar el texto original al elemento de listado de `CU-08004`, para ahorrarse una solicitud al abrir el detalle.
4. Hacer que el comentario del administrador viaje como un elemento más de la colección de observaciones de `CU-08005`, con una severidad que lo distinga.
...
- **Reference** — los ocho contratos de uso `CU-08001` a `CU-08008`, y [`../02-Especificacion-Funcional/Especificacion-Funcional.md`](../02-Especificacion-Funcional/Especificacion-Funcional.md) §6 para las once restricciones transversales. Es la descripción normativa de la superficie; esta sección no la duplica.
