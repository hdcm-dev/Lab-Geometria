...
**Variante:** UX/UI
**Trazabilidad upstream:** `../02-Especificacion-Funcional/Casos-De-Uso/CU-10005-Enviar-Un-Trabajo-Y-Ver-El-Resultado-De-La-Interpretacion.md` íntegro —§4, FA-01 a FA-06, §6, §6.1 y CA-01 a CA-08—; `../02-Especificacion-Funcional/Especificacion-Funcional.md` §6 (RT-03, RT-04, RT-05, RT-07, RT-08, RT-11) y §7; `../../GeometriaFactory-Visor/02-Especificacion-Funcional/Definicion-Contrato-De-Fachada.md` §4.1, §4.2, §4.5 y §6; `../../../01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00004-Interpretacion-Fiel-Del-Dato-Del-Alumno.md` §1, §5 (tercero, cuarto, quinto y sexto criterio); `NB-00003` §1, §5 (primero, segundo y quinto criterio); `NB-00005` §5 (segundo y tercer criterio); `PRODUCT-INTAKE-Fabrica-De-Geometria.md` §4 (F-06, F-09, F-10, F-22), §4.1 (RN-10005, RN-10008, RN-10009), §4.2, §6 (flujos 2 y 4), §7 (CL-3, CL-4), §17.6 P.11 punto 4 y punto 5, §20.E-1 y §20.E-2; `Design-Rules-Web-Generico.md` §3, §4.4, §4.6, §4.9, §5, §7, §8; `Design-Rules-Blazor-Mudblazor.md` §2, §4, §5
...
| Acción de previsualizar | Base §4.9, secundaria | Dibujar sin enviar | «Previsualizar» | **Pide la interpretación al servicio de datos y le entrega las piezas al visor** ([`ADR-08006`](../../../Producto/Adrs/ADR-08006-El-Visor-Recibe-Piezas-Reconstruidas-Y-No-El-Texto.md)). **Sigue sin guardar nada**: enviar es la única acción de guardado |
| Nota de alcance de la previsualización | Base §5 | Declarar que el dibujo **no verifica** | Texto fijo | Inerte. Es la nota que evita la malinterpretación principal de la superficie, **y sigue siendo cierta con `ADR-08006`**: que el laboratorio haya podido reconstruir las piezas para dibujarlas no dice que el trabajo verifique. Un trabajo con advertencias se dibuja entero |
...
| Previsualizar | Acción secundaria | Se pide la interpretación del texto al servicio de datos **sin guardar**, se crea la instancia sobre el elemento de dibujo y se le entregan las piezas reconstruidas. **La escena se dibuja en el navegador y el visor no habla con nadie**: las piezas se las da esta superficie | El área de texto no está vacía |
| Girar, acercar y encuadrar | Interacción con la escena | La cámara se mueve. **Cero tráfico de circuito** | Hay instancia viva |
| Previsualizar otra vez tras editar el texto | Acción secundaria | Se vuelve a pedir la interpretación del texto nuevo y la carga **reemplaza por completo** lo dibujado, liberando lo anterior | Ídem |
...
| CU origen | [`CU-10005`](../02-Especificacion-Funcional/Casos-De-Uso/CU-10005-Enviar-Un-Trabajo-Y-Ver-El-Resultado-De-La-Interpretacion.md) íntegro, con [`CU-10006`](../02-Especificacion-Funcional/Casos-De-Uso/CU-10006-Consultar-El-Listado-Propio-Y-Operar-Sobre-El-Borrador.md) FA-01 como vía de llegada al curso de reedición |
| Reglas de negocio relevantes | `RN-10005` (sin errores de validación no hay paso a estado `Pendiente`; corta hoy en el envío), `RN-10008` (texto conservado íntegro), `RN-10009` (observación con posición y campo), `RN-10003` |
...
| US a generar en 06 | `US-10011`, `US-10012`, `US-10013`, `US-10014` |
...
| 1.0 | 2026-08-09 | Correcciones absorbidas del audit `B-02-03-GeometriaFactory-Web-r1.md` (ronda 1), **sin subir versión** por `Master-Prompt.md` §5, que lo admite mientras el documento está en estado `Propuesto`. **H-06**: las `NB-00004`, `NB-00003` y `NB-00005` de la cabecera pasan a citarse con sección y criterio numerado. **H-08**: §3 reformula la especificación tipográfica de la fila «Área de texto del trabajo» como **avance uniforme por carácter, para que el texto del alumno se lea sin desalineación**, en lugar de nombrar una clase de fuente, que es detalle del sistema visual y no de un wireframe. |
