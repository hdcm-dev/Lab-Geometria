...
| 3 | Pedir el índice 6, que no existe en un trabajo de seis piezas | Se informa `INDICE_FUERA_DE_RANGO`, **la selección vigente se conserva** y no se resalta nada por aproximación | `CU-12003` CA-03 |
...
| 6 | Invocar `destruir` y después cualquier otra función con el mismo identificador, incluida `establecerMovimiento` | Cada invocación posterior informa `INSTANCIA_DESCONOCIDA` y nada se rompe | `CU-12005` CA-02; `CU-12007` CA-04 |
