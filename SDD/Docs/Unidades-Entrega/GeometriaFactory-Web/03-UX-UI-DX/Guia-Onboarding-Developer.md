...
**Trazabilidad upstream:** `DX-Developer-Experience.md` §1 (roles de intervención), §2 (tramos) y §3 (quick-start); `../02-Especificacion-Funcional/Definicion-Contrato-De-Fachada.md` §3.1, §3.2, §4 —con §4.6, la sexta función—, §5.5 y §6; `../02-Especificacion-Funcional/Casos-De-Uso/CU-12001` a `CU-12007`, en su orden de lectura —`CU-12001` a `CU-12005`, después `CU-12007` y por último `CU-12006`, el recorrido de integración completo sin backend (`Especificacion-Funcional.md` §3.2)—; `../../../00-Contexto/Compatibilidad-Plataformas.md` §2.2 y §2.3; `../../../00-Contexto/Alcance-Producto.md` §4.1 (capacidad F-11); `../../../01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00006-Visualizacion-Dentro-Del-Producto.md` §5; `PRODUCT-INTAKE-Fabrica-De-Geometria.md` §16 (estructura de repositorio y guiones), §16.1, §17.7 P.6, P.7, P.8 y P.10, §18 (sample S-1), §20 E-1 y E-7
...
| Un navegador con **capacidad gráfica tridimensional** | Se comprueba en el paso 3 de §3.1: si falta, `inicializar` informa `GRAPHICS_CAPABILITY_MISSING` | La combinación está declarada no soportada (`Compatibilidad-Plataformas.md` §4). Se cambia de navegador; no hay repliegue |
...
| --- | --- | --- | --- |
| 1 | Elegir el elemento de índice 3 en el árbol | Queda resaltada esa pieza en la escena, y **ninguna otra**: el resaltado es exclusivo | `CU-12003` CA-02 |
| 2 | Elegir el mismo índice otra vez | Sigue resaltada la misma pieza, sin doble resaltado ni parpadeo: la operación es idempotente | `CU-12003` CA-04 |
| 3 | Pedir el índice 6, que no existe en un trabajo de seis piezas | Se informa `INDEX_OUT_OF_RANGE`, **la selección vigente se conserva** y no se resalta nada por aproximación | `CU-12003` CA-03 |
| 4 | Cambiar el tamaño del elemento de dibujo e invocar `redimensionar` | La escena se ajusta, las piezas conservan su proporción y la pieza resaltada sigue siendo la misma | `CU-12004` CA-01 y CA-02 |
| 5 | Con la pieza de índice 3 todavía resaltada, invocar `establecerMovimiento(id, opciones)` prendiendo los dos movimientos | Los dos movimientos corren, **la pieza de índice 3 sigue siendo la única resaltada**, la disposición no se movió, el identificador sigue valiendo y el retorno declara el **estado efectivo de los dos** | `CU-12007` CA-01 y CA-03 |
| 6 | Invocar `destruir` y después cualquier otra función con el mismo identificador, incluida `establecerMovimiento` | Cada invocación posterior informa `UNKNOWN_INSTANCE` y nada se rompe | `CU-12005` CA-02; `CU-12007` CA-04 |
...
Para cambiar el estado con la instancia **ya cargada** hay una función y una sola, la sexta: `establecerMovimiento` (`Definicion-Contrato-De-Fachada.md` §4.6, contrato de uso en `CU-12007`). Una línea, sin reconstruir nada.
...
4. **Su única condición es `UNKNOWN_INSTANCE`**, la misma de siempre, que con esta función pasa a presentarse en **cinco**. **No hay ningún código nuevo**: los del contrato siguen siendo **siete** (entrada `E-VIS-13` del catálogo).
...
| Todas las funciones informan `UNKNOWN_INSTANCE` | Se está usando un identificador de una instancia ya destruida, o se guardó mal el que devolvió `inicializar` | Volver a inicializar y conservar el identificador nuevo. Una instancia liberada no vuelve | E-VIS-03 a E-VIS-06 |
...
| `establecerMovimiento` informa `UNKNOWN_INSTANCE` | El control de movimiento del anfitrión siguió vivo en la página después de que la instancia se destruyó | Desactivar el control en el mismo momento en que se invoca `destruir`, igual que se desactiva la interacción del árbol. No mostrarlo como error: es un pedido llegado tarde | E-VIS-13 |
...
| Necesidad de negocio | `NB-00006` §5, criterios primero (3 de 3 piezas), segundo (6 de 6 tipos), tercero (10 de 10 recorridos de ida y vuelta) y cuarto (disposición estable), que son los que verifican los tramos |
| CU origen | `CU-12001` a `CU-12005` para cada comprobación de §3.2 y §3.3; `CU-12007` para la comprobación de movimiento de §3.2; `CU-12006` para el recorrido de integración completo sin backend. Ése es también el **orden de lectura** de los siete: `CU-12001` a `CU-12005`, después `CU-12007` y por último el transversal `CU-12006`, que los recorre juntos (`Especificacion-Funcional.md` §3.2). `CU-12007` lleva número más alto que `CU-12006` porque se emitió después, con la sexta función, y **no se renumera** |
