...
**Variante:** UX/UI
**Trazabilidad upstream:** `../02-Especificacion-Funcional/Casos-De-Uso/CU-10007-Abrir-Un-Trabajo-Y-Explorarlo-En-Escena-Y-Arbol.md` íntegro —§4 pasos 4 a 13, FA-01 a FA-06, §6 y CA-01 a CA-10—; `../02-Especificacion-Funcional/Especificacion-Funcional.md` §6 (RT-03, RT-04, RT-05, RT-07, RT-10, RT-11) y §7; `../../GeometriaFactory-Visor/02-Especificacion-Funcional/Definicion-Contrato-De-Fachada.md` §3.1, §3.2, §4, §5.2 y §6; `../../../01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00006-Visualizacion-Dentro-Del-Producto.md` §1, §4, §5 (los siete criterios); `NB-00005` §5 (tercer criterio); `NB-00004` §5 (sexto criterio); `NB-00007` §5 (quinto criterio); `NB-00009` §5 (sexto criterio); `PRODUCT-INTAKE-Fabrica-De-Geometria.md` §4 (F-11, F-13), §6 (flujo 3), §14 (RA-02, RA-03), §17.6 P.3, P.10, **P.11 punto 4 y punto 5**; `Design-Rules-Web-Generico.md` §3, §4, §5, §7, §8; `Design-Rules-Blazor-Mudblazor.md` §2, §4, §5
...
| Bloque de la escena | — | Alojar el elemento de dibujo | La escena de la instancia del visualizador | Es el **componente anfitrión**: provee el elemento, invoca las **seis** funciones, opera el ciclo de vida y es quien **consulta la preferencia de movimiento reducido y manda los dos valores de verdad** del movimiento automático |
...
**Regla de aislamiento, y es una restricción de diseño y no sólo de implementación.** La escena se opera **exclusivamente** por las **seis** funciones de la fachada —las cinco del ciclo de vida y la selección, más la que gobierna los dos movimientos automáticos—. Ninguna interacción de esta superficie superpone marcas sobre la escena, lee su contenido, la captura ni toca su interior. La lista de piezas no dibujadas está **al lado** y no encima, y la alternativa textual del elemento de dibujo se compone con lo que la fachada devolvió, no con lo que la escena muestra.
...
1. **La escena no es la única vía a la información.** El árbol presenta el mismo contenido, se recorre con flechas, se activa con la barra o el ingreso, y su selección va en los dos sentidos por el mismo índice. Quien no puede operar la escena tiene el árbol completo.
   **Y además la escena se opera por teclado.** El árbol cubre la *elección* de pieza, no la *manipulación de la vista*, y una vista que sólo se puede orbitar y acercar con el puntero es una barrera. El elemento de dibujo **recibe foco** y responde a las **flechas** —orbitar—, a **`+` y `-`** —acercar y alejar— y a **Inicio** —volver a la vista de partida—. La rueda del puntero **sólo se captura con el elemento enfocado**: si no, la página se desplaza normalmente, para no atrapar el desplazamiento de quien apenas pasa por encima.
...
| CU origen | [`CU-10007`](../02-Especificacion-Funcional/Casos-De-Uso/CU-10007-Abrir-Un-Trabajo-Y-Explorarlo-En-Escena-Y-Arbol.md) íntegro. `CU-10009` aporta el bloque de decisión, documentado aparte |
| Reglas de negocio relevantes | `RN-10003` (trabajo ajeno indistinguible de inexistente), `RN-10008` (texto conservado íntegro), `RN-10009` (observación con posición y campo), `RN-10011` (el administrador no ve los borradores) |
| Restricciones transversales | `RT-03`, `RT-04`, `RT-05`, `RT-07`, `RT-10`, `RT-11`, **`RT-13`** |
| Contrato de fachada | **Las seis funciones**, con sus siete códigos de condición, que **no aumentan** con el gobierno del movimiento |
...
| US a generar en 06 | `US-10018`, `US-10019`, `US-10020`, `US-10021` |
...
| 1.0 | 2026-08-09 | Correcciones absorbidas del audit `B-02-03-GeometriaFactory-Web-r1.md` (ronda 1), **sin subir versión** por `Master-Prompt.md` §5, que lo admite mientras el documento está en estado `Propuesto`. **H-06**: las `NB-00006`, `NB-00005`, `NB-00004`, `NB-00007` y `NB-00009` de la cabecera pasan a citarse con sección y criterio numerado. |
...
| 1.0 | 2026-08-09 | Correcciones absorbidas de la auditoría `B2-Maqueta-GeometriaFactory-Web-r1.md`, **sin subir versión** por `Master-Prompt.md` §5, que lo admite mientras el documento está en estado `Propuesto`. **`AB2-08`**: §7 declara el **equivalente de teclado de la manipulación de la vista** —foco sobre el elemento de dibujo, flechas para orbitar, `+` y `-` para acercar, Inicio para volver a la vista de partida, y rueda capturada sólo con el elemento enfocado—, porque el árbol cubre la elección de pieza y no la manipulación de la vista; y declara que el rol y el estado de cada nodo del árbol los lleva **un solo elemento**, con un único punto de entrada tabable por árbol. **`AB2-02`**: §7 pasa de cinco a **seis** funciones de la fachada. **`AB2-05`**: §7 precisa la frontera —esta superficie consulta la preferencia de movimiento reducido y le manda a la fachada **dos valores de verdad**; la fachada no consulta nada y con opciones ausentes o parciales arranca con los dos movimientos apagados—. |
| 1.1 | 2026-08-09 | **Propagación del `PRODUCT-INTAKE` 1.7**, decisión **(b)**: **F-25 sube a `Must Have`** y su frontera queda fijada. La superficie ya declaraba en §7 lo que 1.7 confirma —el anfitrión manda **dos valores de verdad**, la fachada **no guarda nada y no consulta nada**, y la preferencia de movimiento reducido la consulta esta superficie—, de modo que **no había nada que corregir en el fondo**. Lo que se corrige son tres lugares donde el documento se contradecía con eso: §4 y §8 declaraban **cinco** funciones de fachada y son **seis** desde el intake 1.6, que incorporó el gobierno del movimiento; y §3 y §7 decían que los movimientos «arrancan destildados cuando el sistema declara preferencia de movimiento reducido» **sin decir quién la lee**, lo que admitía la lectura equivocada de que la consultaba el visor. §8 suma **RT-13**. Sube minor: precisa la frontera y corrige dos recuentos, sin cambiar ningún componente, estado ni interacción. |
