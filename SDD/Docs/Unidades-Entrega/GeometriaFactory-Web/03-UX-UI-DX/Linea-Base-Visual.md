...
Los ocho últimos estados de esta tabla, más `EST-10025` a `EST-10028`, son los que materializan las **siete condiciones del contrato de fachada** del proyecto de código del visor, con sus dos cursos de `INVALID_CANVAS_ELEMENT`. Sus códigos son los de `Definicion-Contrato-De-Fachada.md` §6 y no se renombran.
...
> **NOTA DE APLICACIÓN DE `EST-10011`, obligatoria desde el 2026-08-31.** El estado `reconectando` **existe y sigue validado**: no baja ningún recuento de esta línea de base. Lo que cambia es **en cuántas superficies se sensa**. Sin circuito no hay corte de circuito, y `ADR-10001` §2.1 declara que **sólo seis de las catorce superficies son interactivas**. Para cruzar esta fila hay que hacer dos cosas y no una: leer `ADR-10001` §2.1 **y** el mapa superficie→componente, porque `Credencial-Propia` tiene **tres** componentes y uno de ellos —`OwnCredentialForcedChange`— es estático: la superficie es **parcialmente aplicable**. `MI-07` de [`../../../Audit/Mesa-2026-08-31-B.md`](../../../Audit/Mesa-2026-08-31-B.md).
