...
| Documento a retroalimentar | `Definicion-Contrato-De-Fachada.md` §5.3 y §6, y `CU-10002` §5, §6 y §8, del proyecto de código del visor: el cero como dimensión legible no estaba declarado en ninguna parte |
...
| --- | --- |
| **`H-6` alcanzaba a `00-Contexto` y a `01-Necesidades-Negocio`** | **Ya no está sin resolver.** La regla de corte de `Maqueta-Rules.md` §3.6 obligó a detenerse, informar el alcance real y pedir confirmación, y eso se hizo: informado el alcance, **el humano confirmó y las dos categorías se tocaron el 2026-08-09**. `Alcance-Producto.md` y `Roadmap-Producto.md` pasaron a **1.2** con su 1.1 archivada en `00-Contexto/_legacy/2026-08-09/`; `Necesidades-Negocio.md` y `NB-00006` pasaron a **1.2**, con `Necesidades-Negocio-v1.1.md` archivada en `01-Necesidades-Negocio/_legacy/2026-08-09/`. Las cuatro entradas de control de cambios citan la Fase B2 |
| **`H-5`, la condición sin escenario de datos propio** | **Resuelto por el Product Owner.** El `PRODUCT-INTAKE` **1.7** incorpora el escenario **`E-8`** para `UNREADABLE_DIMENSION`. El punto quedó abierto en 1.5 y cerrado en 1.7 |
...
| **La propagación del intake 1.7 y 1.9 a `00-Contexto` y a `01-Necesidades-Negocio`** | **Ya no está sin resolver, y esta fila registra qué pasó porque la versión 1.1 de esta bitácora lo declaró mal.** Decía que `Roadmap-Producto.md` §3 punto 4 y `NB-00006` §5 y §9 «siguen escritos contra el intake 1.5», cuando esos dos documentos **se habían corregido en el mismo commit** que subió esta bitácora a 1.1; y no nombraba lo que sí seguía pendiente, que era `Alcance-Producto.md` y `Necesidades-Negocio.md`. Lo detectó la auditoría `SDD/Docs/Audit/F26-Propagacion-r1.md` como hallazgo **`F26-12`**. El estado real, verificado el 2026-08-09: los cuatro documentos están propagados —`Roadmap-Producto.md` 1.4, `Alcance-Producto.md` 1.3, `Necesidades-Negocio.md` 1.3 y `NB-00006` 1.4—, F-25 figura como `Must Have` comprometida y **F-26 entró al nivel producto** con su etapa, su fila de alcance y su necesidad de negocio, que es `NB-00002`. La propagación fue del orquestador, no de esta fase |
