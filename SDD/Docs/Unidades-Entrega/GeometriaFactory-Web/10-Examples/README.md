...
| Cuál es el punto de extensión del producto | El **contrato de la fachada del visor**, con sus **seis** funciones (`PRODUCT-MANIFEST` §5, `PRODUCT-INTAKE` §17.2.P.3 · GeometriaFactory-Visor). Vive en otro proyecto de código y ya tiene sus samples, **consolidados el 2026-08-16 dentro de esta misma categoría** —ver §1— cuando el árbol de `GeometriaFactory-Visor` se absorbió en esta unidad de entrega |
...
| 2.1 | 2026-08-16 | **Reconexión de un puntero colgado**, encontrada por la verificación mecánica de la fase M5 de la migración 8.6 → 8.11. La fila del punto de extensión del producto enlazaba a `../../GeometriaFactory-Visor/10-Examples/`, **un árbol que la consolidación M-10 absorbió dentro de esta misma categoría el mismo día**: el enlace no resolvía. Pasa a declarar dónde están hoy esos samples, que es acá, remitiendo a §1. **Ningún contenido de la categoría cambia**, no se agrega ni se retira ningún sample y ninguna comprobación de la tabla se reescribe: es reconexión de puntero por `Migracion-Rules.md` §4.3.1 y no reemisión, de modo que no corresponde archivado previo. Sube minor. |
