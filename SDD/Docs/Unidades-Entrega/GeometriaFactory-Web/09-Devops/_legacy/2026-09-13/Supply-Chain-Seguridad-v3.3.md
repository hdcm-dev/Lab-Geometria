...
| --- | --- | --- |
| **Plataforma** | ~~La biblioteca de componentes de interfaz, cuya versión la fuente deja **[A VERIFICAR]** y se ancla al crear el andamiaje;~~ **No hay biblioteca de componentes** —decisión declarada en el `.csproj`, `PA-01` cerrado el 2026-08-20—, de modo que esta cadena se reduce a los tipos de `GeometriaFactory-Contracts` compilados adentro, que **no tienen dependencias** | Es la única cadena que un inventario convencional del ecosistema de la plataforma vería |
...
| Integridad del artefacto respecto de su fuente | **Cumplido para el bundle**, que es la parte generada: `QG-12002` exige que se genere en el mismo flujo y `QG-12009` de `GeometriaFactory-Visor` prohíbe editarlo a mano | [`Pipeline-CI-CD.md`](Pipeline-CI-CD.md) §2.2 |
...
| El artefacto no fue alterado a mano | `QG-12009` y `CV-12030`, con objetivo **0** ediciones manuales |
...
| --- | --- | --- | --- |
| Bibliotecas de guion agregadas que consulten servicios por su cuenta | **0** | `QG-10006`, con `TC-10030`, inspección del árbol de fuentes y de las dependencias de guion | **Bloqueante** |
...
| ~~Versión de la biblioteca de componentes de interfaz~~ **SIN OBJETO el 2026-08-31** | ~~**Anclada y registrada** al crear el andamiaje~~ **No hay biblioteca que anclar** | `BT-10002` queda sin objeto; `PA-01` de `05` §11 cerrado **el 2026-08-20**: el `.csproj` declara que la etapa `b` decide **no introducir** MudBlazor | ~~Bloqueante como tarea de la etapa `a`~~ **Deja de ser bloqueante: no hay tarea** |
...
| --- | --- | --- |
| Estático | **Existe y bloquea**: el gate de construcción es «sin advertencias» | Intake §17.2.P.8 · GeometriaFactory-Web; `QG-10001` |
| Estático de estructura | **Existe, bloquea, y es la verificación característica de este proyecto de código**: `QG-10006` sobre el árbol de fuentes y las dependencias de guion, `QG-10008` sobre el traductor de condiciones y `QG-10009` sobre las invocaciones al bundle | [`../08-Calidad-Y-Pruebas/Estrategia-Calidad.md`](../08-Calidad-Y-Pruebas/Estrategia-Calidad.md) §3 |
| **Dinámico** | **Existe, y acá sí tiene sujeto**, a diferencia de las cinco bibliotecas del producto: `QG-10005`, `QG-10007` y `QG-10010` se miden **sobre el producto corriendo en un navegador**, contando peticiones, leyendo el almacenamiento y observando el tráfico del circuito | `Estrategia-Calidad.md` §3; `Plan-Pruebas.md` §3 |
| Condición de las mediciones de ausencia | **`QG-10005` se mide con los dos movimientos automáticos prendidos.** Un conteo con los movimientos apagados daría cero sin haber ejercitado el bucle de dibujo | `Estrategia-Calidad.md` §3, `QG-10005` |
...
| Exposición de la dirección del servidor propio | Rotación del secreto, republicación y **revisión de por dónde se filtró**: `QG-10008` mide **0** mensajes que la expongan, sobre los **diecisiete** códigos vivos y el camino de ausencia de respuesta | Es `RA-03`; `RI-05` de [`../../../Producto/Vista-Producto.md`](../../../Producto/Vista-Producto.md) §7 |
...
| --- | --- | --- | --- |
| **`RA-01`** · ningún guion del navegador invoca el servicio de datos | Una biblioteca de guion agregada que consulte por su cuenta, o una llamada de red escrita en el navegador | `QG-10005` (**0** peticiones, con los movimientos prendidos) y `QG-10006` (**1** salida, **0** bibliotecas) | Reaparecen las tres propiedades que la topología cierra: contenido mixto, intercambio entre orígenes y **exposición de la dirección del servidor propio** |
| **`RA-02`** · el bundle es un visualizador puro | Invocar el interior del bundle en lugar de su fachada, o pasarle configuración por un camino lateral | `QG-10009` (**0** invocaciones al interior; **6 de 6** funciones como única vía; **0** accesos al elemento de dibujo fuera del anfitrión) | `RI-04` de [`../../../Producto/Vista-Producto.md`](../../../Producto/Vista-Producto.md) §7: el bundle adquiere capacidades y `RA-02` deja de ser cierta sin que nadie lo note |
| **`RA-03`** · nada expone direcciones de servicios internos | Un mensaje de error que incluya la dirección, la ruta del almacén o una traza | `QG-10008` (**0** sobre los **diecisiete** códigos vivos **y** sobre el camino de ausencia de respuesta) | `RI-05`: la violación directa de `RA-03`, en el último tramo antes de llegar a la persona |
...
| **Se verifican con un recuento, no con un juicio** | Los tres gates tienen umbral **0**, y `QG-10009` además **6 de 6** |
| **Una medición de ausencia sin su condición no cuenta** | `QG-10005` medido con los movimientos apagados daría cero sin haber ejercitado el bucle de dibujo. Es el criterio que el producto aplica en las dos puntas de la fachada |
