...
**Un artefacto, y sólo uno.** `Rules-Devops.md` §2.2 admite un `openapi` versionado como artefacto secundario para servicios; acá no aplica: esta unidad no expone contrato a nadie. *(Hasta el 2026-09-12 esta frase agregaba que tampoco aplicaría en el backend porque «no hay versionado de rutas porque no hay clientes de terceros»; esa premisa quedó superada —la superficie pública de la API se versiona por `/v{MAJOR}/`, [`ADR-00010`](../../GeometriaFactory-Api/05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md)— y lo que diga el backend sobre su propio artefacto lo declara su categoría 09, no ésta.)*
...
| ~~**Versión de la biblioteca de componentes de interfaz anclada**~~ **SIN OBJETO el 2026-08-31** | ~~También **[A VERIFICAR]** en la fuente; se ancla al crear el andamiaje~~ **No hay biblioteca de componentes**: el `.csproj` declara que la etapa `b` decide no introducirla | Intake §17.2.P.1 · GeometriaFactory-Web, **marca retirada**; `PA-01` de `05` §11, **cerrado el 2026-08-20**; `BT-10002` sin objeto |
