...
| CV-36 | **El peso del documento de las dos superficies de mayor volumen no crece más de un 10 % sobre su línea de base**, por corte. Se registran **dos columnas distintas**: el **marcado emitido** —lo que el componente produce— y los **bytes transferidos** —lo que cruza el cable—. **No se promedian y no son lo mismo**: hasta el 2026-08-31 el instrumento reportaba sólo el primero bajo el rótulo «Documento» y lo comparaba contra los bytes HTTP del servicio, que sí son bytes. `N-1` de la mesa | ≤ **110 %** de la base, por superficie y por corte |
