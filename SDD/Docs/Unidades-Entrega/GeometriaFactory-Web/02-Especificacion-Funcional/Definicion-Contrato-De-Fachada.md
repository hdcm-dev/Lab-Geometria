...
**Qué pasa con la condición `UNREADABLE_DIMENSION`.** Era el motivo con el que esta función
...
| Qué no hace | **No toca la escena más allá del movimiento**: no la reconstruye, no la recarga, no vuelve a leer el texto, no altera la disposición ni la selección, y no invalida el identificador. No roza el determinismo: la posición de cada pieza sigue derivada de su índice (G-6). **No emite condición nueva**: con un identificador que no corresponde a una instancia viva informa `UNKNOWN_INSTANCE`, que ya existe, y los códigos de §6 siguen siendo siete. No dibuja controles, no consulta la preferencia de movimiento reducido del sistema (G-3) y no conserva la preferencia (G-2). No hace red (G-1), no lee configuración propia y no sabe quién mira |
...
3. Un identificador que no corresponde a ninguna instancia viva produce la condición `UNKNOWN_INSTANCE` y ninguna otra consecuencia: no crea instancias, no dibuja y no rompe la instancia que sí exista.
...
- Una pieza de un tipo que no está en esos seis **no se dibuja** y queda enumerada con la condición `NON_DRAWABLE_TYPE`. La fachada no la califica de error del trabajo: eso lo decide el backend.
...
8. **Ninguno de los dos emite una condición.** No hay código nuevo: la lista de §6 sigue cerrada en siete. Un movimiento que no arranca porque la instancia no existe es `UNKNOWN_INSTANCE` y nada más, y `establecerMovimiento` no agrega ninguno.
