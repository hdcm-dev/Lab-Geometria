...
**Trazabilidad downstream:** [`../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md`](../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md), que toma las cuatro sondas `VER-XX`; `11-Documentacion` cuando se emita, que referencia estos samples sin duplicar su código
...
- [4. El alcance de la colección, resuelto](#4-el-alcance-de-la-colección-resuelto)
...
**Desde el 2026-09-13 hay un cuarto markdown y una cuarta carpeta**, y entraron directamente en la **pasada de ejecución**: [`ejemplo-04-cliente-http-basico-api.md`](ejemplo-04-cliente-http-basico-api.md) gobierna [`04-cliente-http-basico/`](../../../../../samples/api/04-cliente-http-basico/), el cliente de referencia de una aplicación propia que corre **sin el código fuente** contra `/v1/` (`BT-00034`), con su `evidencia` completa y `VER-00004` en `Verificado`. Los tres párrafos de arriba describen la emisión inicial de los tres primeros y no se reescriben.
...
| [`ejemplo-04-cliente-http-basico-api.md`](ejemplo-04-cliente-http-basico-api.md) | Básico, por capacidad (`cliente-http-basico`) | 2-5 min | CU-00022, CU-00026, CU-00028 | `/samples/api/04-cliente-http-basico/` |
...
**El cuarto es un sample adicional por capacidad** (`Rules-Examples.md` §2.2, «samples adicionales para cubrir capacidades extra»), y su tiempo de setup es el menor porque **no necesita el repositorio**: se copia la carpeta, se exportan las credenciales de una persona y corre contra la superficie publicada. Los casos de uso que ilustra ya están cubiertos por los tres anteriores; lo que agrega no es cobertura sino **lugar**: es el único que ejerce `/v1/` desde afuera, con `curl`, como lo haría una aplicación propia ([`ADR-00009`](../05-Arquitectura-Tecnica/Adrs/ADR-00009-La-Api-Autentica-Personas-No-Aplicaciones.md), [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) §2.2 regla 5). Los recuentos de cobertura de abajo son de los tres primeros y no cambian con él; los identificadores `CU-000NN` de esas filas son los de la numeración con que se emitieron, previa al renumerado a `CU-00021`–`CU-00029`, y el cuarto usa la vigente.

**Cobertura de los doce casos de uso: 12 de 12.** Verificación uno por uno: `CU-00001`, `CU-00002` y `CU-00009` en el 01; `CU-00003`, `CU-00004`, `CU-00005`, `CU-00006`, `CU-00007`, `CU-00008` y `CU-00012` en el 02; `CU-00010` y `CU-00011` en el 03. Sin repeticiones y sin huecos: 3 + 7 + 2 = 12.

**Cobertura de los quince puntos de acceso, en dos recuentos que no hay que confundir.** El **sample 02** ejercita **13 de 15**, que es exactamente lo que `CU-00012` §8 `CA-08` declara para la colección, y los dos que quedan fuera de su archivo son los que esa misma sección nombra: **`A-08`**, la baja física de una cuenta, porque ejercitarla dejaría a la colección sin el alumno con el que sigue el recorrido, y **`A-16`**, el punto de salud. **Entre los tres samples el recuento es 14 de 15**, porque el sample 03 sí ejercita `A-16`: es su acto de arranque. **El único que ningún sample ejercita es `A-08`**, y su verificación vive en las pruebas de integración, tal como `CU-00012` §10 lo declara. 14 + 1 = 15. El identificador `A-04` está **retirado** de la superficie desde `PRODUCT-INTAKE` 1.13 y no se recicla: ni se ejercita ni se cuenta.

**Un residuo de la fuente aguas arriba, que esta categoría declaró y que ya quedó cerrado en su origen.** La §9 de `CU-00012` **1.2** escribía «Puntos de acceso que ejercita | **13 de 16**» y «Los tres que no, en §10», mientras que su propia §8 `CA-08` y su §10 decían **13 de 15** y **dos** puntos no ejercitados —`A-08` y `A-16`— más una precisión sobre un tercero. **La cuenta viva es 13 de 15**, coherente con [`../02-Especificacion-Funcional/Definicion-Superficie-HTTP.md`](../02-Especificacion-Funcional/Definicion-Superficie-HTTP.md) §3, que declara **quince** puntos desde su versión 1.1. **`CU-00012` 1.3 corrigió esa fila** al cerrar el hallazgo `B-API-02` de [`../../../Audit/B-02-03-GeometriaFactory-Api-r1.md`](../../../Audit/B-02-03-GeometriaFactory-Api-r1.md) 1.0; el recuento de esta categoría no cambia, porque **siempre escribió 15**.
...
| `VER-00004` | [`ejemplo-04-cliente-http-basico-api.md`](ejemplo-04-cliente-http-basico-api.md) | CU-00022, CU-00026, CU-00028; US-00001, US-00017, US-00019, US-00021 — y `Contratos-REST.md` §3.1 y §4.1 | `API_EMAIL=... API_PASSWORD=... bash samples/api/04-cliente-http-basico/run.sh` | **Verificado** — CONFORME 6/6 | 2026-09-13, contra el servicio de la rama en `127.0.0.1:5081` |

**Tres sondas, ninguna redundante**: los conjuntos de casos de uso y de historias que verifican son disjuntos, y entre las tres alcanzan a los doce casos de uso y a las treinta historias. **La cuarta se superpone a propósito** con la primera y la segunda en lo que verifica del canje, el envío y el listado, porque su objeto no es cubrir un caso de uso que las otras no cubran sino **verificar dos cosas que ninguna de las tres puede**: que el contrato se descubre y se cuenta desde `/openapi/v1.json` sin el código (`Contratos-REST.md` §3.1: dieciséis bajo `/v1/` y una exenta) y que la cuota responde `429` con `Retry-After` y sin cuerpo (§4.1). Es la única de las cuatro en `Verificado`. Las tres entran a [`../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md`](../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md) con estado `Sin verificar`, que es lo que `Deriva-Rules.md` §2.4 declara para un contrato en `No verificado — sin código`.

**`VER-00002` es la sonda que la categoría 08 anticipó.** [`../08-Calidad-Y-Pruebas/README.md`](../08-Calidad-Y-Pruebas/README.md) §3 declaró, al omitir la matriz de sensado, que este proyecto de código «tiene un candidato natural, la **colección de peticiones reproducible** de `CU-00012`». Éste es ese candidato, y la matriz se abre con él y con los otros dos.

**Qué relación tiene la colección con las pruebas, dicho con precisión.** `CU-00012` §9 declara que **la colección no reemplaza a las pruebas de integración y no se cuenta como cobertura**, y [`../08-Calidad-Y-Pruebas/Estrategia-Testing.md`](../08-Calidad-Y-Pruebas/Estrategia-Testing.md) §3 la registra como «no es una prueba automatizada: es la forma de demostración que el intake declara». **Eso no cambia con esta emisión.** Lo que `VER-00002` agrega es una **aserción evaluable** sobre esa demostración, para que su resultado sea un veredicto y no una impresión. Lo que comparten con las pruebas de integración son los datos, y nada más.

**Qué queda fuera de las tres sondas, y por qué no es un hueco.** Los **diecisiete** códigos de contrato vivos recorridos uno por uno en las dos direcciones, la tabla de traducción, los umbrales de latencia y de caudal, la forma de la pirámide y la cobertura no los verifica ningún sample: los verifica la batería de `tests/GeometriaFactory.Integration.Tests` y el pipeline de `09-Devops`. Un sample que los duplicara sería el anti-patrón de `Rules-Examples.md` §4.5.

## 4. El alcance de la colección, resuelto

El `PRODUCT-INTAKE` describía la colección **en dos lugares con alcances distintos**, y ninguno de los dos estaba envejecido:

| Lugar de la fuente | Qué decía | Alcance |
| --- | --- | --- |
...
**El Product Owner resolvió la divergencia a favor de los ocho** (`PRODUCT-INTAKE` **1.29** §18, 2026-08-12). El fundamento es lo que cada alcance demuestra: con dos, la colección demuestra que **la API responde**; con ocho, **ejercita el validador contra todos los datos reales por HTTP**, incluido el cilindro de `E-1` cuya diferencia de 0.01 funda el operador estricto de tolerancia y el `E-8` de la coma decimal. §18 `S-2` pasa a decir lo mismo que §16.1 ya decía.

**Esta categoría no cambia nada, y eso es el resultado esperado.** `CU-00012` §10 había adoptado **los ocho** con su fundamento —«`E-8` es el modo de falla que el propio intake llama el más probable de todos», porque lo produce la configuración regional de la máquina del alumno y no un error de programación— y esta categoría heredaba esa lectura. **La decisión la confirma**: el sample 02 conserva sus ocho cuerpos, `VER-00002` conserva sus aserciones de recuento y la fila `SD-00002` de la matriz de sensado no se toca. El punto abierto que la registraba, `PA-06` de [`../05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md`](../05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md) §11, queda **cerrado con su desenlace y su fecha**.
...
- **Ningún dato de prueba inventado.** Los cuerpos salen de los escenarios del intake §20 y las identidades son valores evidentemente ficticios, declarados como tales. Es una regla de delivery del producto (`PRODUCT-INTAKE` §15), medida como NFR con umbral **0** y verificada por `TC-00035`.
- **Ninguna clave de firma, ninguna contraseña real y ninguna dirección de servidor de producción**, que es `CA-07` de `CU-00012` §8 y `RA-03`. La dirección del servicio llega del entorno.
- **`RA-01` no se toca.** Estos samples **no son el navegador**: corren servidor a servidor dentro del entorno contenido. La superficie no recibe peticiones del navegador y por eso no hay configuración de intercambio de origen cruzado, que es lo que `TC-00036` verifica.
...
- **Los samples no acuñan códigos.** Los **diecisiete** códigos de contrato vivos tienen fuente única en `GeometriaFactory-Contracts` y los **diez** códigos de respuesta en [`../02-Especificacion-Funcional/Definicion-Superficie-HTTP.md`](../02-Especificacion-Funcional/Definicion-Superficie-HTTP.md).
...
| --- | --- | --- |
| `rest-api` | `/samples/01-cliente-http-basico/`, `/samples/02-postman-collection/`, `/samples/03-sdk-tipado-generado/` | `/samples/api/01-basico/`, `/samples/api/02-intermedio/`, `/samples/api/03-avanzado/`, `/samples/api/04-cliente-http-basico/` |

Cuatro desvíos, los cuatro declarados acá y ninguno de nomenclatura por dominio:
...
4. **El cuarto lleva el slug que §2.3 reserva para el primero, y no es un renombre.** `04-cliente-http-basico/` es el «cliente HTTP básico» que la matriz de `rest-api` pone en la primera posición: el que un integrador corre desde afuera. Este producto lo tiene recién desde la fase `k`, cuando la superficie se publicó para aplicaciones propias, y lo agrega como carpeta extra —lo que §2.3 admite— sin renombrar `01-basico/`, que corre adentro del entorno contenido y demuestra otra cosa. Su nivel implícito es Básico (§3.2).
...
Los tres samples ejercitan la superficie que declara [`../05-Arquitectura-Tecnica/Contratos-REST.md`](../05-Arquitectura-Tecnica/Contratos-REST.md) y no invocan nada de adentro del proceso. **`CU-00012` no tiene componente de tiempo de ejecución y es correcto que no lo tenga** ([`../05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md`](../05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md) §3.3): la colección no implementa, demuestra. **La forma del archivo de la colección es de la categoría 05**, y su contenido vive acá; [`ADR-00008`](../05-Arquitectura-Tecnica/Adrs/ADR-00008-Sin-Versionado-De-Rutas-Y-Despliegue-Conjunto.md) agrega la convención de que la colección se actualiza **en la misma intervención** en que cambia la superficie.
...
| 2.1 | 2026-09-13 | **Entra el cuarto sample**, [`ejemplo-04-cliente-http-basico-api.md`](ejemplo-04-cliente-http-basico-api.md) 1.0 con `VER-00004` en `Verificado` (`BT-00034`, fase `k`): el cliente de referencia de una aplicación propia contra `/v1/`, sin el código fuente, autenticándose como una persona. Filas nuevas en §2 y §3, párrafo en §1, desvío 4 en §6 (el slug `cliente-http-basico` de §2.3, agregado como carpeta extra y no como renombre) y cabecera con cuatro sondas. **Los recuentos de cobertura de los tres primeros no cambian** y sus párrafos de emisión inicial no se reescriben; se declara que sus `CU-000NN` son de la numeración previa al renumerado. Sube minor. |
