...
| 2.0 | 2026-09-12 | **Ejecución y cierre → Done**, rama `fase-k/bt-00030-cors` sobre `main` = `8e5e2f9`. Tipo `indagación`: **sin cambio en `src/`**. **Hecho medido**, reproduciendo `Audit/Mesa-2026-09-12-ciclo-2.md` §2.2 el 2026-09-13 02:51 UTC: (1) `git grep -niE "cors" -- src/` → sin resultados, salida de `1`, **0 ocurrencias**; (2) `curl -si -X OPTIONS https://api-geometria.aplicada.stream/salud -H "Origin: https://evil.example.com"` → `HTTP/2 405`, `allow: GET`, `content-length: 0`, **sin `Access-Control-Allow-Origin`** ni ninguna cabecera `Access-Control-*`; el contenedor `lab-geometria-api` consultado directo en `:8080` responde igual (`405 Method Not Allowed`, `Allow: GET`, `Server: Kestrel`). **Veredicto del criterio 1: cumplido.** [`Arquitectura-Unidad-Entrega.md`](../../05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md) **3.12** §9.1, fila del riesgo de CORS/navegador, declara en su celda de mitigación por qué CORS no aplica hoy —ningún cliente propio es JavaScript de navegador desde otro origen ([`ADR-00009`](../../05-Arquitectura-Tecnica/Adrs/ADR-00009-La-Api-Autentica-Personas-No-Aplicaciones.md); el front es Blazor Server y llama servidor a servidor; MAUI no es un navegador)—, con la medición y con la condición que lo reabre; entra **§9.1.1** con los comandos, su salida y la forma de la política si la condición se diera: **por origen explícito, nunca `AllowAnyOrigin` con credenciales** (OWASP CORS Cheat Sheet). **Veredicto del criterio 2: no se dispara** —ningún cliente propio de navegador desde otro origen apareció en la etapa `k`—, y la caja temporal se cierra por la primera de sus dos vías, la declaración. [`Definicion-Superficie-HTTP.md`](../../02-Especificacion-Funcional/Definicion-Superficie-HTTP.md) **1.12** §7, fila «CORS», se alinea con `PRODUCT-INTAKE` 4.4 y remite a `05` §9.1. Ancho de la tabla de `05` §9.1 sin cambio (4 columnas). Sube **major** por cambio de estado. |
