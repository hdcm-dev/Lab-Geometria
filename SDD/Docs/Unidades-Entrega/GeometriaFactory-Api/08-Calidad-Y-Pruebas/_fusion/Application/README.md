...
**Los dos gates condicionados dependen de valores rotulados [ASUNCIÓN] en el intake §22** —asunción `A-3` para la cobertura y `A-5` para los 500 ms—: se miden y se registran, y no bloquean la fusión hasta que el Product Owner los confirme (`BT-04018`).
...
| Sondas `VER-XX` en la matriz de sensado | **3** | [`Matriz-Sensado-Deriva.md`](Matriz-Sensado-Deriva.md) §3 |
...
| 1.1 | 2026-08-11 | Actualiza la tabla de artefactos: [`Matriz-Cobertura-Pruebas.md`](Matriz-Cobertura-Pruebas.md) sube a **1.1**. Es de trazabilidad: la matriz declaraba que ningún `TC-XX` deja de referenciar un `CU-XX`, una `RN-XX`, un `INV-XX`, una comprobación o un NFR, y `TC-04031` traza a `QG-11`, a `ADR-04006` y a un riesgo de `05` §9 sin fila en ninguna de las cinco tablas. **Ningún gate, umbral, caso ni recuento de esta sección cambia.** Corrige contra [`../../../Audit/E-08-Calidad-Siete-Proyectos-r1.md`](../../../../../Audit/E-08-Calidad-Siete-Proyectos-r1.md) 1.0 y contra el texto vivo del intake **1.20**. |
