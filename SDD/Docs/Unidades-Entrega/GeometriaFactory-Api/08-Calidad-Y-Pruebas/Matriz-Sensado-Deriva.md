...
| `SD-00004` | `VER-00004` de [`../10-Examples/ejemplo-04-cliente-http-basico-api.md`](../10-Examples/ejemplo-04-cliente-http-basico-api.md) §9 | Un cliente propio **sin el código fuente** recorre `/v1/` en **5** pasos con **0** datos inventados: el documento generado en `/openapi/v1.json` describe **17** operaciones, **16** bajo `/v1/` y **una** exenta (`/salud`); el canje de las credenciales de **una persona** responde **200** con acceso firmado y papel; el listado responde **200** con una lista; el envío de `E-1` responde **201** con el trabajo en **`Pendiente`**, **2** advertencias y **0** errores; y la petición que excede la cuota de la persona responde **429** con `Retry-After` entre **1** y **60** y **sin cuerpo**. El acceso firmado no aparece en la salida | El comando del contrato: `API_EMAIL=... API_PASSWORD=... bash samples/api/04-cliente-http-basico/run.sh`, desde una copia de la carpeta fuera del repositorio | Campo `evidencia` de `VER-00004`, con su fecha | **Mayor, sin gradación** en tres tramos: una operación bajo `/v1/` de más o de menos, o una cuarta ruta exenta (`Contratos-REST.md` §3.1, `ADR-00010`); un `429` que no llega, que llega con cuerpo o sin `Retry-After` (§4.1, `BT-00029`); y el acceso firmado impreso. **Mayor** además si el `criterio_aceptacion` falla, si el sample pasa a necesitar algo del repositorio para correr, si `E-1` deja de quedar en `Pendiente` con dos advertencias, o si el canje admite una identidad que no sea una persona (`ADR-00009`). **Menor** en el resto | `Verificado` | 2026-09-13 · CONFORME 6/6 contra el servicio de la rama `fase-k/bt-00034-sample-onboarding` en `127.0.0.1:5081`, umbrales por omisión |

**Cuatro filas, una por contrato de verificación, sin contratos huérfanos ni filas sin contrato que las respalde**, que es lo que exige `Deriva-Rules.md` §6. La correspondencia es uno a uno: `SD-00001`↔`VER-00001`, `SD-00002`↔`VER-00002`, `SD-00003`↔`VER-00003`, `SD-00004`↔`VER-00004`. La cuarta entró el 2026-09-13 directamente en `Verificado`, porque su contrato nació en la pasada de ejecución con la evidencia hecha.

**El método de verificación de las cuatro es el comando declarado en su contrato, sin desvío.** No hay ninguna fila resuelta por inspección, porque `Deriva-Rules.md` §2.4 declara que una sonda `VER-XX` «trae su propio comando y su propia aserción».

### 3.2 `GeometriaFactory-Domain`

| ID | Elemento de línea de base | Afirmación a verificar | Método de verificación | Evidencia esperada | Umbral de deriva | Estado | Última verificación |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `SD-02001` | `VER-02001` de [`../10-Examples/ejemplo-01-basico.md`](../10-Examples/ejemplo-01-basico-dominio.md) §9 | El ciclo de vida de la cuenta cierra entero: la admisibilidad devuelve los **tres** desenlaces de `CU-02004` sobre la misma cuenta —`ACCOUNT_PENDING`, `PASSWORD_CHANGE_PENDING` y admisible—, y las **9** operaciones invocadas producen **2** rechazos tipados y **0** excepciones | El comando del contrato: `dotnet run --project samples/domain/01-basico` | Campo `evidencia` de `VER-02001`, con su fecha | **Menor**: cambia el texto de una línea de salida sin cambiar su semántica. **Mayor**: el `criterio_aceptacion` falla, cambia el comando sin actualizar el contrato, aparecen precondiciones no declaradas, o alguno de `CU-02001`, `CU-02002`, `CU-02003`, `CU-02004` o `CU-02012` deja de estar cubierto | **`Verificado`** el **2026-08-29**, en la Fase I, incremento 1 | El sample corrió y su `--verificar` devolvió **CONFORME · las 10 líneas coinciden**, con exit 0. La deriva sensada en esta pasada fue **Mayor** y **está reparada**: el `criterio_aceptacion` incumplía cuatro de sus cinco `stdout_contiene` porque §6 del contrato citaba los códigos de condición en la forma que `F-03` retiró el 2026-08-12. Reparado en el documento, no en el código |
| `SD-02002` | `VER-02002` de [`../10-Examples/ejemplo-02-intermedio.md`](../10-Examples/ejemplo-02-intermedio-dominio.md) §9 | Los **seis** escenarios reales del intake §20 recorren el ciclo del trabajo: `E-1` adopta **3** piezas y **2** advertencias y pasa a `Pendiente`; `E-3` produce la advertencia con el par 36.00 y 54.00 y `E-4` produce **0** observaciones; `E-5` y `E-8` quedan retenidos en `Borrador` con la observación localizada en el **índice 1**; el recuento final es **4** envíos a `Pendiente` y **2** retenidos | El comando del contrato: `dotnet run --project samples/domain/02-intermedio` | Campo `evidencia` de `VER-02002`, con su fecha | **Menor**: cambia el texto de una línea sin cambiar su semántica. **Mayor**: el `criterio_aceptacion` falla, un escenario se sustituye por datos sintéticos, el índice reportado pasa a ser 0, o alguno de `CU-02005` a `CU-02008` deja de estar cubierto | **`Verificado`** el **2026-08-29**, en la Fase I, incremento 2 | El sample corrió y su `--verificar` devolvió **CONFORME**, con exit 0. **Cero deriva sensada**: el snapshot de §6 se escribió antes que el código y el código lo reprodujo **a la primera**, sin tocar el contrato |
...
| --- | --- | --- |
| 2.3 | 2026-09-13 | **Entra `SD-00004`** en §3.1, sonda de `VER-00004` ([`../10-Examples/ejemplo-04-cliente-http-basico-api.md`](../10-Examples/ejemplo-04-cliente-http-basico-api.md) 1.0, `BT-00034`): el cliente propio sin el código fuente contra `/v1/`. Entra en **`Verificado`** con fecha, porque el contrato se declaró y se corrió en la misma intervención (CONFORME 6/6 contra el servicio de la rama). Los tres párrafos de §3.1 pasan de tres a cuatro filas. Ninguna otra fila cambia. Sube minor. |
