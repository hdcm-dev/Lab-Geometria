...
| **La superficie pública se versiona en la ruta, sólo el `MAJOR`: `/v{MAJOR}/`** [DECISIÓN 2026-09-12]. Hasta el intake 4.2 decía «no hay versionado de rutas, porque no hay clientes de terceros»; la premisa **dejó de ser cierta** (hay aplicaciones propias además del front, `ADR-00009`). Las rutas de §3 se publican con el prefijo desde `BT-00032` | Intake **4.3** §17.1.P.3 · GeometriaFactory-Api, fila «Versionado del contrato»; [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) |
...
| A-18 | **Interpretar un texto sin guardar nada**, para la previsualización | `POST` | `/interpretaciones` **[derivado]** | `Alumno` | `200`, `400`, `401`, `403` | CU-00026 |

**Por qué entra A-18, y por qué no guarda.** Lo exige
[`ADR-08006`](../../../Producto/Adrs/ADR-08006-El-Visor-Recibe-Piezas-Reconstruidas-Y-No-El-Texto.md):
el visor pasó a recibir **las piezas reconstruidas** en lugar del texto del alumno, de modo que
previsualizar necesita que alguien las reconstruya, y quien sabe hacerlo es el laboratorio. **Es la
contrapartida declarada de esa decisión.**

**No escribe, y la ruta lo dice.** El producto tiene **una sola acción de guardado** y es enviar
(`A-10` y `A-11`). Por eso este punto **no cuelga de `/trabajos`**: colgarlo de ahí insinuaría que
hay un segundo camino por el que un trabajo entra al almacén. Recibe un texto, devuelve la
interpretación y **no deja nada**: ni trabajo, ni piezas, ni observaciones, ni registro del texto.

**Devuelve lo mismo que el envío informa, menos lo que sólo el envío tiene.** Las piezas
reconstruidas —que son las que el visor dibuja— y las observaciones. **No devuelve estado de
trabajo**, porque no hay trabajo: resolver el estado es del dominio sobre un trabajo que existe, y
devolver un estado acá afirmaría una entrega que no ocurrió.

**Es de papel `Alumno` y exige acceso firmado**, con el mismo criterio que `A-10`: interpretar
consume el motor del laboratorio, y un punto anónimo que lo consume es una superficie de abuso
gratuita sobre la única pieza cara del producto.

**Diecisiete puntos de acceso.** El recuento que importa y que se puede comprobar sobre la tabla: **cinco no exigen acceso firmado** —A-01, A-02, A-03, A-16 y A-17—, **A-05 admite las dos formas** y **los diez restantes exigen acceso firmado y quedan bajo la guardia de `CU-00002`**. Cinco más uno más diez son dieciséis. **Ningún punto queda con su forma de identificación abierta**, y ésa es la diferencia con la emisión 1.0.

**Por qué entra A-17, y por qué su ausencia no era una omisión de forma sino un hueco funcional.** `Web ADR-00003` §2 declara **cuatro guardianes de ruta** para la pieza pública, y el primero dice: «mientras no exista la cuenta de administrador, cualquier ruta pedida desvía al aprovisionamiento inicial; una vez que existe, esa ruta **deja de armar formulario para siempre** y desvía de forma neutra, sin explicar por qué». Ese guardián **nunca se construyó**, y la causa está de este lado de la frontera: **esta superficie no tenía ningún punto con el que un visitante anónimo pudiera preguntar si el laboratorio ya tiene administrador**. Revisada punto por punto: **A-03** configura —es escritura, y contestar preguntando sería configurar—, **A-16** responde por la salud del servicio y **A-06** lista cuentas pero exige ser `Administrador`. Ninguno le sirve a quien todavía no se identificó, que es exactamente quien pasa por el guardián. La consecuencia observada: con un administrador ya configurado, la pantalla de aprovisionamiento **seguía sirviendo su formulario** —cinco campos, dos de contraseña— a cualquier anónimo. **No era un agujero de seguridad** —`ADMINISTRATOR_ALREADY_CONFIGURED` sigue cerrando A-03 y está probado en las tres capas— sino un defecto **de superficie**, y no se podía cerrar sin agregar este punto.
...
| `OPERATION_ADMIN_ONLY` | `403` | Es una negativa de facultad **fuera del desenlace** —gobierno de cuentas, listado de la comisión y reseteo— y, como la del desenlace, **no tiene nada que ocultar**: el recurso no es ajeno, lo que no alcanza es el papel |
| `STATE_FORBIDS_UPDATE` | `409` | El estado del trabajo no habilita la escritura pedida. La respuesta **declara el estado actual** y no ofrece forma de volver a `Borrador`, porque no existe |
...
| `SERVICE_UNAVAILABLE` | **Ninguno** | **No lo produce esta capa.** El ensamblado de contratos lo declara «el único que el contrato admite que produzca la propia pieza pública, porque describe la ausencia de respuesta de la otra pieza». Una respuesta de esta superficie con este código sería una contradicción: si hay respuesta, el servicio respondió |
...
| **Versionado de rutas** — **ausencia superada el 2026-09-12** | Hasta el intake 4.2, no había clientes de terceros: los dos extremos compilaban contra el mismo ensamblado y un cambio incompatible **rompía la compilación** antes que el tiempo de ejecución. Desde `PRODUCT-INTAKE` **4.3** hay aplicaciones propias que no compilan contra el ensamblado, y la superficie se versiona por `/v{MAJOR}/` ([`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md)) | El **despliegue conjunto** de las dos piezas que compilan contra el ensamblado **subsiste** (`ADR-00008` regla 2); el prefijo lo implementa `BT-00032` |
...
1. **CERRADO — El código del contrato para una operación de administrador pedida por quien no lo es**, fuera del desenlace. Lo resolvió el Product Owner incorporando `OPERATION_ADMIN_ONLY` al conjunto cerrado (`PRODUCT-INTAKE` **1.29** §17.4 P.3, 2026-08-12); su fila de traducción está en §6 con destino `403`. `OUTCOME_ADMIN_ONLY` **no cambia de enunciado**.
2. **CERRADO — El código del contrato para un envío o una reedición forzados fuera de `Borrador`.** Lo resolvió la misma decisión con `STATE_FORBIDS_UPDATE`, con destino `409` en §6. `STATE_FORBIDS_DELETE` **sigue acotado a la eliminación**.
...
| 1.8 | 2026-08-16 | **Entra `A-18`, el punto que interpreta un texto sin guardar nada**, exigido por [`ADR-08006`](../../../Producto/Adrs/ADR-08006-El-Visor-Recibe-Piezas-Reconstruidas-Y-No-El-Texto.md): con el visor recibiendo piezas reconstruidas en lugar del texto, la previsualización necesita quien las reconstruya. La superficie pasa de **dieciséis a diecisiete** puntos. **No cuelga de `/trabajos`** y es deliberado: el producto tiene una sola acción de guardado y colgarlo de ahí insinuaría un segundo camino de entrada al almacén. **No devuelve estado de trabajo**, porque no hay trabajo: devolverlo afirmaría una entrega que no ocurrió. Exige acceso firmado y papel `Alumno`, con el mismo criterio que `A-10`. El recuento de puntos sin acceso firmado **no cambia**: siguen siendo cinco. | Product Owner (decisión) · Orquestador SDD |
