...
**Habilitar una cuenta produce una contraseña provisoria**, con las mismas propiedades y el mismo tratamiento que la del reseteo (RN-02014): el sistema la produce, la superficie se la muestra al administrador para que se la comunique, y la cuenta queda con **cambio de contraseña pendiente** (INV-09). En consecuencia **no existe ninguna escritura anónima de credencial**: toda operación que fija o cambia una contraseña ocurre con la cuenta ya autenticada. **Lo que la regla no toca es el registro de la cuenta**, que es anónimo por diseño y así debe seguir: es la puerta por la que el alumno entra al laboratorio (`CU-02001`, F-02) [PRECISADO por `PRODUCT-INTAKE` **1.15** §4.1: la redacción de 1.13 decía «ninguna escritura anónima **en el sistema**», y era falsa].

## 2. Justificación

El producto tenía **dos** mecanismos de credencial inicial que hacían lo mismo. El primer ingreso del alumno recién habilitado fijaba una contraseña sin que nadie hubiera probado quién era; el reseteo la producía y obligaba a cambiarla. Con RN-02016 son el mismo camino, y el que queda es el que ya estaba probado.
...
Lo que la regla **no** cambia es quién elige la contraseña definitiva. El alumno la sigue eligiendo: lo que cambia es que llega a elegirla **identificado**, por el camino que RN-02013 ya fija, en lugar de llegar anónimo.

## 3. Ámbito de aplicación

- Se evalúa en **toda habilitación de una cuenta de alumno** (`CU-02002`): la transición a `Habilitado` no se admite sin la credencial derivada provisoria aportada, y deja puesta la marca de cambio de contraseña pendiente.
...
- **La marca la ponen dos actos y no uno**: la habilitación de `CU-02002` y el reseteo de `CU-02013`. La levanta, como antes, **únicamente** el reemplazo hecho por la propia cuenta (`CU-02003` FA-04). Rehabilitar una cuenta `Bloqueado` es también una habilitación a estos efectos y produce provisoria nueva.
- **No alcanza a la cuenta de administrador**, que nace `Habilitado` con su credencial aportada en el mismo acto (`CU-02012`) y a la que INV-08 le cierra las cuatro operaciones de `CU-02002`.
- **Su invariante es INV-09**, que ya sostenía a RN-02012 y RN-02013 y que esta regla amplía en su origen: la marca deja de tener una sola fuente.
- **Lo que la regla no declara es el mecanismo.** Cómo se produce un valor no adivinable y no repetido es de RN-02014 y de `GeometriaFactory-Infrastructure`; el dominio recibe la credencial **ya derivada** y nunca la contraseña en claro.

## 4. Consecuencia si se viola

Violarla tiene dos formas, y las dos son silenciosas.

La primera es **dejar habilitar sin credencial**: la cuenta queda en el estado que el producto declara admisible sin tener con qué autenticarse, y el único camino para darle una sería reponer el punto anónimo que la regla suprime. `CU-02002` la cierra con un rechazo propio.

La segunda es **habilitar sin poner la marca**: la provisoria que el administrador conoce queda sirviendo indefinidamente para operar como el alumno, que es exactamente lo que INV-09 existe para impedir y lo que RN-02013 declara para el reseteo. No produce ningún error; sólo una prueba lo detecta.
