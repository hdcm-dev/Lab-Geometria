...
| `Modelo-Datos/Modelo-Conceptual.md` y sus `RC-XX` | La regla de la categoría los omite para `library`, y el flag `tiene_persistencia` de este proyecto de código es false: el intake declara «no aplica directamente» en §17.1.P.4 · GeometriaFactory-Application. Esta capa declara el puerto de repositorio y el alcance de la unidad de trabajo, no el modelo de datos. El modelo del dominio vive en `Definicion-Modelo-De-Dominio.md` de `GeometriaFactory-Domain` |
