...
>
> Copia archivada de `Glosario-Funcional.md` en su versión **1.1**, tomada el 2026-08-09 por el orquestador SDD antes de que la versión vigente la superara (`Master-Prompt.md` §5 y §5.1).
>
> - **Estado:** `Superado`
...
| Papel | Atributo del alumno que vale `Alumno` o `Administrador`. Es un conjunto cerrado de dos valores, sin permisos configurables | `Definicion-Modelo-De-Dominio.md`, CU-01, CU-02, CU-04, CU-10, CU-11, RN-01, RN-10 | «Rol» en las fuentes técnicas. **Se usa «papel»**, y «rol» queda reservado al encabezado normativo de la tabla de actores: ver §3.4 |
| Estado de cuenta | Atributo del alumno que vale `Pendiente`, `Habilitado` o `Bloqueado`, con transiciones declaradas. **No se confunde con el estado del trabajo**, que es otro conjunto cerrado: ver §3.3 | `Definicion-Modelo-De-Dominio.md`, CU-01 a CU-04, RN-01, RN-06, RN-07 | — |
| Credencial derivada | Valor derivado de la contraseña del alumno, que el dominio recibe ya derivado y nunca en claro. Sin valor hasta el primer ingreso efectivo | `Definicion-Modelo-De-Dominio.md`, CU-03, CU-04, RN-06 | «Hash de contraseña» en las fuentes técnicas |
...
| Consumidor de la biblioteca | El proyecto de código que usa la superficie pública de este dominio por referencia de proyecto de código: `GeometriaFactory-Application` o `GeometriaFactory-Infrastructure`. **Es el actor primario de todos los casos de uso de esta categoría** | Los once CU | «Capa consumidora» |
| Sujeto de la regla | La persona sobre la que recae una regla que el dominio hace cumplir —el alumno, el administrador—, que **no** es actor de ningún caso de uso de esta categoría | Los once CU | — |
...
Los dos referentes conviven **en la misma tabla y hasta en la misma celda** —la tabla de actores de los once casos de uso—, de modo que colisionan en el sentido de `Vocabulario-Rules.md` §9.2 y se desambiguan. El segundo referente no lo eligió este proyecto de código: viene impuesto por `Rules-Especificacion-Funcional.md` §4.2 punto 2, que fija el encabezado de esa tabla.
...
| Función que un actor cumple dentro de un caso de uso | **«rol»**, y **sólo** como encabezado de la columna de la tabla de actores, que es donde la regla lo impone | §2 «Actores» de los once casos de uso |
