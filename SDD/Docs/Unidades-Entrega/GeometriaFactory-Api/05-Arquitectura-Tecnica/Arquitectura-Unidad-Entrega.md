...
| Alternativa | A favor | En contra | Resolución |
| --- | --- | --- | --- |
| Repositorio genérico sobre el conjunto de entidades | Un solo tipo para las cinco entidades, sin escribir un adaptador por puerto | Diluye las consultas que sí importan —el listado del administrador agrupado por alumno—, y obliga a que el recorte se arme del lado del consumidor, que es justo lo que `QUERY_WITHOUT_DECLARED_SCOPE` viene a impedir | **Descartada** por `PRODUCT-INTAKE` §17.1.P.2 · GeometriaFactory-Infrastructure |
...
| Componente | Responsabilidad | Entradas | Salidas | Dependencias |
| --- | --- | --- | --- | --- |
| Guarda de autorización | Ejercer las **cuatro** comprobaciones —cambio de contraseña pendiente, pertenencia, facultad y alcance del administrador— en el orden fijo de [`ADR-04004`](Adrs/ADR-04004-Orden-Fijo-De-Las-Cuatro-Comprobaciones.md) **2.0**. **Es una responsabilidad con cuatro domicilios y no un componente con uno**: la marca vive en un intermediario de la superficie, la facultad en el caso de uso, y la pertenencia y el alcance en la entidad. Lo que se garantiza es **un solo lugar por comprobación**, y la tabla de §2 de esa ADR dice cuál es cada uno | Identidad ya resuelta afuera, papel, marca de cambio pendiente y la entidad pedida | Autorizado, o la condición que lo impidió | Declaración de puertos, `GeometriaFactory-Domain` |
...
1. **Ningún adaptador depende de otro adaptador.** El único par acoplado son los dos motores, y en una sola dirección: la verificación de valores **exige las piezas ya reconstruidas** y por eso `PIECE_SET_NOT_REBUILT` existe. La dirección inversa no existe: la interpretación no consulta la verificación.
...
- **Escritor único, por restricción del motor y no por elección.** El almacén no admite escrituras concurrentes y el intake acepta esa limitación por escrito a cambio de un despliegue sin servicio de base de datos aparte (`PRODUCT-INTAKE` §17.1.P.4 · GeometriaFactory-Infrastructure y §17.1.P.12 · GeometriaFactory-Infrastructure). La escritura que llega mientras otra tiene el almacén tomado termina en `CONCURRENT_WRITE_REJECTED`, que es **terminación degradada y no espera activa**.
...
- **Arranque detenido: la única forma de terminación que ninguna otra parte del producto tiene.** Si la preparación del almacén no se completa, el servicio **no atiende ninguna petición** (`MIGRATION_NOT_APPLICABLE` y `STORE_PATH_UNAVAILABLE`). No hay modo de sólo lectura ni arranque parcial: un servicio que atiende sobre un almacén equivocado es peor que un servicio que no arranca.
...
| Secretos | La clave de firma **se provee o se genera en el primer arranque y vive fuera del repositorio de código y fuera de la imagen** (`PRODUCT-INTAKE` §17.1.P.5 · GeometriaFactory-Infrastructure). Este proyecto de código **la recibe y no la busca**: si no llega, `SIGNING_KEY_MISSING` |
| Ciclo de construcción | Dentro del contenedor de desarrollo, porque el equipo anfitrión no tiene el kit de desarrollo instalado (`PRODUCT-INTAKE`, encabezado de la Parte C) |
...
| Configuración | **Se recibe, no se busca.** La ubicación del almacén, la clave de firma y los parámetros de derivación llegan desde la composición de raíz. Este proyecto de código no lee archivos de configuración ni variables de entorno por su cuenta, y por eso `STORE_PATH_UNAVAILABLE` y `SIGNING_KEY_MISSING` son condiciones y no valores por defecto | [`ADR-06007`](Adrs/ADR-06007-Transformaciones-Al-Arrancar-Con-Linaje-Inmutable.md) |
...
| Caudal sostenido | **20 peticiones por minuto** [ASUNCIÓN del intake], **provisorio desde el 2026-08-20**, derivado del uso previsto —una comisión operando durante una clase— y de la limitación de escritor único del almacén | Prueba de carga acotada en la batería de integración. **Desde `BT-00029` (2026-09-13) el NFR tiene mecanismo que lo hace cumplir**: límite de tasa por persona autenticada o por dirección de origen ([`Contratos-REST.md`](Contratos-REST.md) §4.1; 60 por persona, 1200 por origen y 600 canjes por origen por minuto, más 10 fallos por cuenta cada 900 segundos, por omisión y configurables; los topes por origen toleran una comisión detrás de una dirección desde [`ADR-00011`](Adrs/ADR-00011-El-Canje-Se-Protege-Por-Cuenta-Y-El-Origen-Tolera-Una-Comision.md), porque 120 y 30 dejaban afuera a la clase, `RN-B1`), verificado por `RateLimitingTests` con los umbrales reales, incluida la ráfaga de la cuota entera de una persona en escritura contra el único escritor | [`ADR-00005`](Adrs/ADR-00005-Sin-Paginacion-Con-Condicion-De-Reingreso-Declarada.md) |
...
| Que se agregue un punto de acceso pensado para el navegador, o se configure el intercambio de origen cruzado | **Muy alto**: reabre las tres propiedades de la topología —contenido mixto, intercambio de origen cruzado y exposición de la dirección del servidor propio— y rompe `RA-01`, que es regla de nivel producto | Baja, pero el costo de equivocarse es de rediseño | **CORS no aplica hoy, y se declara por qué** (§9.1.1): ningún cliente propio es JavaScript de navegador desde otro origen —el front es Blazor Server y llama servidor a servidor; una aplicación MAUI no es un navegador— ([`ADR-00009`](Adrs/ADR-00009-La-Api-Autentica-Personas-No-Aplicaciones.md)). **Medido**: 0 ocurrencias de `cors` en `src/`, y `OPTIONS /salud` con un `Origin` ajeno responde `405` sin `Access-Control-Allow-Origin`. **Se reabre** sólo cuando aparezca ese cliente, y entonces con política **por origen explícito**, nunca `AllowAnyOrigin` con credenciales. Siguen valiendo la ausencia declarada en la superficie de 02 §7, con lo que la repone escrito, y el hecho de que el único cliente legítimo esté declarado en el manifiesto y en el grafo |
...
### 9.1.1 CORS: por qué no aplica hoy

La fila anterior nombra el riesgo; esta subsección deja escrita la razón por la que **hoy no hay política de intercambio de origen cruzado y no falta ninguna** (tarea `BT-00030`, etapa `k`).

**Por qué no aplica.** CORS es un mecanismo del navegador: rige cuando un script que corre en una página de un origen pide recursos a otro origen. Por [`ADR-00009`](Adrs/ADR-00009-La-Api-Autentica-Personas-No-Aplicaciones.md), el cliente de esta API es **cualquier aplicación propia del producto que actúe en nombre de una persona**, y ninguna de las conocidas o previstas es JavaScript de navegador desde otro origen: el front `GeometriaFactory-Web` es **Blazor Server** y llama a la API **servidor a servidor** (`RA-01` en §10.4); otro front servidor haría lo mismo; una aplicación MAUI o un script del docente son procesos nativos, y **un proceso nativo no es un navegador**, así que no está sujeto a la política de mismo origen. Configurar CORS sería declarar que la superficie recibe peticiones del navegador, que es exactamente lo que `RA-01` prohíbe y lo que la ausencia de `02` §7 repone escrito.

**Hecho medido** (2026-09-13 02:51 UTC, `main` = `8e5e2f9`, reproduciendo `Audit/Mesa-2026-09-12-ciclo-2.md` §2.2):

```text
$ git grep -niE "cors" -- src/
(sin resultados; 0 ocurrencias)

$ curl -si -X OPTIONS https://api-geometria.aplicada.stream/salud -H "Origin: https://evil.example.com"
HTTP/2 405
allow: GET
content-length: 0
(sin Access-Control-Allow-Origin ni ninguna otra cabecera Access-Control-*)
```

El contenedor `lab-geometria-api`, consultado directo en `:8080` y sin el proxy adelante, responde lo mismo: `405 Method Not Allowed`, `Allow: GET`, `Server: Kestrel`, sin cabecera de origen cruzado. Un navegador que intentara una petición con verificación previa desde otro origen la vería rechazada por el propio navegador, sin que la API haya tenido que decidir nada.

**Condición que lo reabre, y con qué forma.** Aparece un cliente propio que corra **en navegador desde otro origen** —un front en Blazor WebAssembly o en cualquier otra ejecución en el navegador, servido desde un origen distinto al de la API—. En ese caso, y **antes** de habilitar origen alguno, entra una política **por origen explícito**: la lista cerrada de orígenes propios, los verbos y las cabeceras que ese cliente usa, y nada más. **Nunca `AllowAnyOrigin`**, y nunca un origen reflejado desde la petición, cuando la respuesta viaja con credenciales: con `Access-Control-Allow-Credentials: true` la especificación exige un origen concreto, y una API que acepta un `Bearer` de una persona es una API con credenciales (OWASP *CORS Cheat Sheet*, la guía que la ficha cita; Fetch Standard, «CORS protocol»). Ese cambio es del ensamblado `GeometriaFactory-Api` —composición de raíz, [`ADR-00001`](Adrs/ADR-00001-Host-Delgado-Con-Composicion-De-Raiz-Unica.md)— y reabre `RA-01` **para esa ruta ajena**, no para el circuito de `Web`, tal como `PRODUCT-INTAKE` **4.4** §14 y §17.1.P.3 (fila «CORS») lo dejan escrito.
...
| ADRs que lo gobiernan | ADR-00001 a ADR-00010, las **diez**; `ADR-00008` superada parcialmente por [`ADR-00010`](Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) desde el 2026-09-12 (regla 1 derogada; reglas 2 a 5 subsisten) |
...
| INV-05 Exactamente un administrador | Responder si ya existe una cuenta con papel `Administrador`, y sostenerlo con la restricción del almacén, que emite `ADMINISTRATOR_UNIQUENESS_VIOLATED` | Adaptador de repositorio de cuentas |
...
| 3.3 | 2026-08-20 | **Segunda pasada del paso `A2`**: **2** punto(s) abierto(s) cerrados **por lectura del árbol**, sobre las familias que `Audit/A3-Decisiones-Del-Product-Owner.md` §1 dejó verificadas. Cada uno cita **el archivo que ya tenía la decisión** — el motor de dibujo anclado en `three 0.169.0`, `PBKDF2` en `PasswordDerivation.cs`, el `@media` de 768 px en `app.css`, `EmailIdentity.Normalize`, los 18 puntos de acceso, las herramientas de cada stage en los guiones, y **la biblioteca de componentes, que no existe porque la etapa `b` decidió no introducirla** y su `.csproj` lo declara como apartamiento. **Ninguno se cerró por criterio propio** y **ningún enunciado de punto abierto se tocó**. Sube minor. | Orquestador SDD |
| 3.4 | 2026-08-20 | **1** punto(s) abierto(s) **cerrados por decisión del Product Owner** del 2026-08-20, sobre `Audit/A3-Decisiones-Del-Product-Owner.md`: el **volumen de la comisión** queda cerrado **por incognoscible** —el dato no se sabe ni se puede saber de antemano, y no se fija número—; el **límite de tamaño del cuerpo** adopta **el valor por omisión del servidor**, con la obligación derivada de declararlo explícitamente cuando se toque la composición; y el **mutation score** se cierra **con un no**, dejando `CV-19` declarado sin medir. **El caudal de 20 peticiones por minuto de §8 pasa a provisorio**, y es la consecuencia declarada de cerrar el volumen de la comisión por incognoscible: ese número **se derivaba de «una comisión operando durante una clase»**, de modo que al no haber tamaño conocido **pierde su fundamento**. Se conserva como referencia y **su valor definitivo sale de lo que `PT-05` mida sobre el uso real**. **Ningún enunciado de punto abierto se tocó.** Sube minor. | Orquestador SDD |
| 3.5 | 2026-08-26 | **`A2c` · cierre por lectura de `D3`, la vigencia del acceso firmado.** `A3-Decisiones-Del-Product-Owner.md` §4 —«**`D3` puede estar fijada y no la encontré.** Busqué en la composición de la API y no en la configuración. **Si aparece, pasa a la lista de §1 y deja de ser tuya**»— dejó autorizado este cierre, y **apareció**: el valor en efecto es **480 minutos** por omisión de `SigningOptions.cs:25`, y **nada lo sobreescribe**. La fila **deja de afirmar que no hay número**, que era falso, y **deja de estar vencida**: su evento pasa a la **fase `i`**, que es cuando la configuración del ambiente se declara y **no ocurrió**. **Lo que queda abierto es otra pregunta y es del Product Owner**: si ocho horas cumplen el criterio de `ADR-00003` §5 —«que caduque **dentro de la sesión de trabajo de una clase**»—, porque **ninguna fuente declara cuánto dura una clase** y sin ese dato la comparación no se puede hacer leyendo. Se suma la **obligación derivada** con el mismo criterio con que el Product Owner cerró `D4`: el valor **se declara explícitamente**. Sube **minor**: reformula un punto abierto y no cambia ninguna decisión. |
| 3.6 | 2026-08-26 | **`D1` confirmada por el Product Owner el 2026-08-26**, sobre `Audit/D1-Confirmacion-De-Asunciones.md` 1.0 y el intake §22. Se confirman los valores rotulados `[ASUNCIÓN]` —umbrales de latencia y arranque (`A-5`), coberturas y forma de la pirámide (`A-3`), gates que no son de líneas (`A-4`) y métricas de negocio (`A-2`)—, con lo que **cuatro filas vencidas de este documento quedan cerradas**. **Con una excepción declarada: el caudal de 20 peticiones por minuto queda FUERA de la confirmación.** Su fundamento se cayó al cerrarse `D5` —el volumen de la comisión, incognoscible— y su valor sale de **`PT-05`**; sigue **provisorio**. **Y una constancia que la confirmación no compra**: los umbrales se eligieron altos donde la fuente señala criticidad, ninguna fuente los fija, y **confirmarlos no los vuelve medidos**. Sube **minor**: cierra puntos abiertos y no cambia ninguna decisión de contenido. |
| 3.7 | 2026-08-29 | **Tramo `R-3c` del renombre `F-03`**, reactivado por el Product Owner el 2026-08-29 y registrado en [`../../../Producto/Norma-De-Nomenclatura.md`](../../../Producto/Norma-De-Nomenclatura.md) §8. **16 línea(s)** pasan los códigos de condición de la forma castellana a la vigente, con el mapeo de **§6.8** —101 pares— y **sin elegir ninguno acá**. Se respeta **§4.1**: no se tocan las filas de control de cambios, ni lo que está entre «…», ni los informes de `Audit/`. **Ninguna palabra de prosa cambia**, verificado con el control de diff del tramo. |
| 3.8 | 2026-08-30 | **`E-02` y `E-04` de la mesa del 2026-08-27, contestadas por el Product Owner el 2026-08-30.** **`E-02`, opción (c)**: la fila del `PA-04` de §11.2 —la herramienta que calcula la versión— **reasigna su evento a la fase `i` · Despliegue real**, que es cuando aparece el primer release real que hay que versionar; estaba **vencida** desde el cierre de la etapa `a` el 2026-08-13. Lo mismo la `PA-06` de §11.3. **`E-04`, opción (a) en bloque**: **cuatro filas** que declaraban «falta declarar el evento» —peor que estar vencidas, porque **sin evento nada las podía vencer nunca**— pasan a **punto de control de la fase `i`**, el último que queda y que `PRODUCT-INTAKE` §10 y §15 declaran bloqueante. **Ningún enunciado cambia y ninguna fila se retira.** Sube minor. |
| 3.9 | 2026-08-31 | **§3.1 corrige la descripción del componente «Guarda de autorización».** Decía que ejerce las cuatro comprobaciones como componente único; son **cuatro domicilios** —la marca en un intermediario de la superficie, la facultad en el caso de uso, y la pertenencia y el alcance en la entidad— y lo que se garantiza es **un solo lugar por comprobación**. Lo declara [`ADR-04004`](Adrs/ADR-04004-Orden-Fijo-De-Las-Cuatro-Comprobaciones.md) **2.0**, que pasa de `Propuesto` a `Aprobado` corrigiendo el ADR y no el código. **El nombre del componente NO se cambia**, y es deliberado: nombra una responsabilidad que sí existe, lo tocan unas cuarenta citas del corpus, y renombrarlo dispararía un barrido de propagación por un problema que era de descripción. Sube minor. |
| 3.10 | 2026-09-12 | **§10.1, fila «ADRs que lo gobiernan», pasa de «ADR-00001 a ADR-00008, las ocho» a las diez**, con `ADR-00009` (ya vigente y no reflejada acá) y `ADR-00010` (versión en la ruta para la superficie pública, `Aceptado`), y anota que `ADR-00008` queda superada parcialmente. Tarea `BT-00027`. Una celda. Sube minor. |
| 3.11 | 2026-09-12 | **`BT-00033` · `PA-04` de §11.2 (Domain) y `PA-06` de §11.3 (Application) pasan a Cerrado**, los dos con el mismo desenlace: la herramienta que calcula la versión es **MinVer 8.0.0**, decisión `D-03` del Product Owner en `Audit/Mesa-2026-09-12-ciclo-2.md` §8 y adoptada en `Directory.Build.props` con `MinVerTagPrefix=v`; la declaración vive en `09-Devops/Estrategia-Versionado.md` **5.0** §3.1 a §3.4. **Ningún enunciado cambia y ninguna fila se retira**; sólo la columna «Estado» de las dos filas. Los párrafos de recuento de §11.2 y §11.3 no se tocan: ya no coincidían con las columnas «Estado» antes de esta emisión (`PA-01` y `PA-02` figuran cerradas en las dos tablas y contadas como abiertas), y corregirlos es otra corrida. Sube minor. |
| 3.13 | 2026-09-13 | **§8.1, fila «Caudal sostenido», columna «Mecanismo de medición»**: el NFR gana el mecanismo que lo hace cumplir —límite de tasa por persona autenticada o por dirección de origen, `Contratos-REST.md` **1.9** §4.1— y la batería que lo verifica. Tarea `BT-00029`. **La cifra del NFR no cambia y sigue provisoria**: los umbrales del límite son múltiplos de ella y se ajustan por configuración cuando `PT-05` mida. Una celda. Sube minor. |
| 3.14 | 2026-09-13 | **§8.1, fila «Caudal sostenido», columna «Mecanismo de medición»**: las cifras del límite pasan a 60 por persona, 1200 por origen, 600 canjes por origen y 10 fallos por cuenta cada 900 segundos, con remisión a [`ADR-00011`](Adrs/ADR-00011-El-Canje-Se-Protege-Por-Cuenta-Y-El-Origen-Tolera-Una-Comision.md). Corrige el defecto de `BT-00029`: los topes por origen dejaban afuera a una comisión detrás del front o del NAT de la facultad (`RN-B1`). **La cifra del NFR no cambia y sigue provisoria.** Una celda. Sube minor. |
| 3.15 | 2026-09-13 | **Migración normativa 13.7 → 13.16, fase M4, fila PM-06** (`Root-Rules.md` 8.6 → 8.7 §12.2 punto 5; `Migracion-Rules.md` 3.20 §4.9). Las **31** filas de ítems diferidos de este documento suman la columna **Ciclo de origen**, **derivada del historial y no escrita a mano**: 23 derivadas —el commit que dio de alta la fila en el archivo de su proyecto, con su asunto literal como fase, el proyecto de la sección como unidad y el commit como base— y 8 `no derivable — anterior al mecanismo`, porque la fila ya existía con otro texto. Comando y resultado por fila en `SDD/Expedientes/0001-Migracion-Normativa-A-13.16/evidencia/E-006g`. **Ninguna otra celda cambia.** Sube minor. Estado anterior en `_legacy/2026-09-13/Arquitectura-Unidad-Entrega-v3.14.md` |
| 3.12 | 2026-09-12 | **`BT-00030` · §9.1, fila del riesgo de CORS/navegador: la celda de mitigación declara explícitamente por qué CORS no aplica hoy**, con la razón ([`ADR-00009`](Adrs/ADR-00009-La-Api-Autentica-Personas-No-Aplicaciones.md): ningún cliente propio es JavaScript de navegador desde otro origen; el front es Blazor Server y llama servidor a servidor; MAUI no es un navegador), el hecho medido (0 ocurrencias de `cors` en `src/`; `OPTIONS /salud` con `Origin` ajeno → `405` sin `Access-Control-Allow-Origin`, contra producción y contra el contenedor) y la condición que lo reabre (un cliente propio en navegador desde otro origen → política por origen explícito, nunca `AllowAnyOrigin` con credenciales). Entra **§9.1.1** con los comandos, su salida y la forma que tendría la política si la condición se diera. **Ninguna otra fila, sección ni ancho de tabla cambia**; el riesgo no se retira, porque la probabilidad sigue siendo la misma: lo que cambia es que la mitigación deja de ser sólo una ausencia y pasa a ser una declaración medida. Sin cambio en `src/`. Sube minor. |
