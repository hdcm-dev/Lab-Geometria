...
| --- | --- | --- |
| `Modelo-Datos-Logico.md` | **Omitido** | La regla de la categoría lo omite para `library` sin estado. `tiene_persistencia` es false y el intake declara «no aplica directamente» en §17.1.P.4 · GeometriaFactory-Application. Lo que esta capa sí decide sobre persistencia es el **alcance** de la unidad de trabajo, y eso vive en [`ADR-04005`](../../Adrs/ADR-04005-Un-Caso-De-Uso-Una-Unidad-De-Trabajo.md). El modelo lógico le corresponde a la categoría 05 de `GeometriaFactory-Infrastructure` |
