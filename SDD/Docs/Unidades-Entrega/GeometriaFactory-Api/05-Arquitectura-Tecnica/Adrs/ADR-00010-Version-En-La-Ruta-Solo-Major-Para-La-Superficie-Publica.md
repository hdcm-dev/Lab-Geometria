...
- **Remitido, no decidido acá:** el evento de etiqueta y la herramienta (`BT-00033`, `Estrategia-Versionado.md`).
- **Decidido por `BT-00035`, en [`../../09-Devops/Estrategia-Versionado.md`](../../09-Devops/Estrategia-Versionado.md) 6.0 §6.b, sobre `D-02`:** `/v{N+1}/` se abre sólo por un cambio Mayor de `Contratos-REST.md` §6; `/v{N}/` convive **un cuatrimestre como mínimo desde que `/v{N+1}/` entra a producción**, contado como cuatro meses calendario, prorrogable y nunca acortable; toda respuesta de `/v{N}/` lleva `Deprecation` (`draft-ietf-httpapi-deprecation-header`) y `Sunset` (RFC 8594) y `changelog.md` lo anuncia; al vencer, `/v{N}/` responde `410` —código que entra a `Contratos-REST.md` §4 por la tarea del retiro, no hoy—; `/salud`, `/openapi/v1.json` y `/documentacion` no se deprecan. **Hoy sólo existe `/v1/` y nada está deprecado.**
...
| 1.2 | 2026-09-13 | **§7 deja de remitir a `BT-00035` el plazo de convivencia y la cabecera `Deprecation`, y los asienta como decididos** en [`../../09-Devops/Estrategia-Versionado.md`](../../09-Devops/Estrategia-Versionado.md) **6.0** §6.b: un cuatrimestre desde que `/v{N+1}/` entra a producción (`D-02`), `Deprecation` y `Sunset` en toda respuesta de `/v{N}/`, `410` al retirar, `/salud` y el explorador exentos, y hoy nada deprecado. El ítem «Remitido» de §7 conserva lo que sigue remitido (`BT-00033`). **El cuerpo de la decisión (§2), las alternativas, los trade-offs y las métricas no cambian.** Sube minor. |
