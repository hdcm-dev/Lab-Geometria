...
- **Y desde el 2026-09-01, `Pooling=False` en la cadena de conexión, que es lo que hace que la evaluación no mienta.** Con el agrupamiento activo SQLite conserva el descriptor sobre el inodo, de modo que un almacén **borrado** se sigue leyendo y la salud contesta `ready`. **Medido sobre el binario**: `200 {"ready":true}` a los 2, 7, 12 y 17 segundos con el archivo eliminado. La convención de arriba estaba escrita y **no se cumplía**. Es `R-2` de la mesa del 2026-09-01, y su lección de método: **las cuatro pruebas que la respaldaban llamaban `ClearAllPools()` antes de romper el almacén, algo que el servicio corriendo jamás hace** — la forma más cara de verde, la que se consigue poniendo en la prueba la condición que le falta al producto.
...
| 1.2 | 2026-09-01 | **La convención que la 1.1 declaró NO SE CUMPLÍA, y hubo que medirla sobre el binario para verlo.** `/salud` evaluaba el almacén en cada consulta, sí — pero con el agrupamiento de conexiones activo SQLite conserva el descriptor sobre el inodo, de modo que **con el archivo borrado la evaluación seguía dando verde**: `200 {"ready":true}` a los 2, 7, 12 y 17 segundos. Entra **`Pooling=False`**, y con él la misma prueba da **503**. **La lección de método es peor que el defecto**: las cuatro pruebas que respaldaban la 1.1 llamaban `SqliteConnection.ClearAllPools()` antes de romper el almacén, **algo que el servicio corriendo jamás hace**. Pasaban, y el defecto seguía vivo. Es la forma más cara de verde: **la que se consigue poniendo en la prueba la condición que le falta al producto.** Entra `HealthEndpointStoreTests`, que entra por el **punto de acceso** y **no** llama a `ClearAllPools`; verificada en las dos direcciones —sin el arreglo falla con «Expected ServiceUnavailable / Actual OK»—. `R-2` de la mesa del 2026-09-01. | Orquestador SDD |
