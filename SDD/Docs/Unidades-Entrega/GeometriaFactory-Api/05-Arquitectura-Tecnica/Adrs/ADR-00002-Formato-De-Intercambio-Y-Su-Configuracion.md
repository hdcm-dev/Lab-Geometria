...
| --- | --- | --- |
| Una sola configuración declarada, con nombres literales, conjuntos por nombre, nulos emitidos, números sin cultura, lectura estricta y límite único (**adoptada**) | Cierra el trade-off que `Contracts ADR-00001` aceptó por escrito; ninguna de las seis reglas de formato depende de que los dos extremos se configuren igual **por separado** | Obliga a que un cambio de configuración se coordine entre dos proyectos de código, y la lectura estricta rompe ante un extremo desactualizado, aunque de forma ruidosa |
| No fijar nada y dejar que cada extremo se configure | Ningún acoplamiento de configuración | **Descartada.** Es exactamente el riesgo que `Contracts ADR-00001` declaró aceptado y que las dos categorías 05 destinatarias tenían que cerrar. Además `GeometriaFactory-Web` ya declaró que no lo decide de un solo lado |
...
| Truncar el cuerpo que excede el límite | El servicio nunca rechaza un envío | **Descartada, y es la peor de todas.** Rompe `RN-00008` sin que nada falle: el trabajo queda guardado con el texto mutilado y el alumno lo descubre al ver el dibujo |
...
3. `RN-00008` queda protegida en el único lugar del backend donde el texto puede alterarse por transporte, con una prueba byte a byte.
4. `RN-00009` queda protegida al traducir: la posición y el campo cruzan la frontera sin recortarse.
...
- La configuración de intercambio se declara **en la composición de raíz** y en ningún otro lado ([`ADR-00006`](ADR-00006-Composicion-De-Raiz-Ciclos-De-Vida-Y-Configuracion.md)).
...
- [`../../../GeometriaFactory-Web/05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md`](../../../GeometriaFactory-Web/05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md) §11 `PA-03`, que reasigna la decisión a esta categoría.
...
| 1.2 | 2026-09-12 | **§2 regla 5 deja de fundarse en «no hay clientes de terceros a los que romper»** (tarea `BT-00027`). La premisa quedó superada por [`ADR-00010`](ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) (hay aplicaciones propias que no compilan contra el ensamblado, `ADR-00009`). **La regla de lectura estricta no cambia**: la nota aclara que a esos clientes los protege `/v{MAJOR}/`, no la tolerancia. Sube minor. |
