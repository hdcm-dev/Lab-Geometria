...
| **Por origen** | La petición no trae acceso válido: los cuatro puntos anónimos (`A-01`, `A-02`, `A-03`, `A-17`), la forma sin sesión de `A-05`, y también una petición a un punto bajo la guardia con acceso ausente, vencido o mal firmado. **La petición que la guardia rechaza con `401` también gasta cuota**: el limitador corre antes de autorizar | **1200 por minuto** | `RateLimiting__PermitsPerAddress` |
| **El canje, por origen** | `A-01` (`POST /v1/auth/token`) reemplaza la cuota anterior por una **propia**, siempre por dirección y contra cuentas cualesquiera: cada intento cuesta una derivación anclada ([`ADR-06004`](Adrs/ADR-06004-Derivacion-De-Clave-Anclada-Con-Parametros-Versionados.md)), se acierte o no. **Es un tope holgado, no la defensa de una cuenta** | **600 por minuto** | `RateLimiting__CredentialExchangePermitsPerAddress` |
| **Los fallos, por cuenta** | `A-01` y la forma sin sesión de `A-05`, **desde cualquier origen**: la partición es el correo normalizado como lo compara el dominio ([`ADR-06003`](Adrs/ADR-06003-Comparacion-De-Correos-Y-El-Indice-Que-La-Sostiene.md)), exista o no una cuenta con él. **Cuentan los intentos fallidos, no los ingresos.** Agotada, el punto responde `429` antes de buscar la cuenta y antes de derivar, aunque la contraseña sea la correcta ([`ADR-00011`](Adrs/ADR-00011-El-Canje-Se-Protege-Por-Cuenta-Y-El-Origen-Tolera-Una-Comision.md)) | **10 cada 900 segundos** | `RateLimiting__CredentialFailuresPerAccount`, `RateLimiting__CredentialFailureWindowSeconds` |

La ventana es de **un minuto** (`RateLimiting__WindowSeconds`, 60) —quince minutos para los fallos por cuenta— y **deslizante**, no fija: con ventana fija quien agota la cuota en el último segundo la tiene entera en el primero siguiente y el pico real es el doble del declarado. No hay cola: la petición que excede se rechaza y el consumidor decide cuándo volver.
...
**Por origen, una comisión entera detrás de una sola dirección** ([`ADR-00011`](Adrs/ADR-00011-El-Canje-Se-Protege-Por-Cuenta-Y-El-Origen-Tolera-Una-Comision.md)). `GeometriaFactory.Web` habla con esta superficie servidor a servidor y **no reenvía la dirección del navegador**, y en la facultad los alumnos salen por un mismo NAT: para la partición por origen, la clase es **una** dirección. El tamaño de la comisión no tiene cifra —`D5` se cerró por incognoscible el 2026-08-20—; se toma la mayor que el producto midió, **334 alumnos** (`Audit/Medicion-Volumen-De-Comision-2026-08-31.md`), redondeada a trescientos: **600 canjes** (uno con un error de tipeo por alumno) y **1200 peticiones anónimas** (cuatro por alumno: sonda de aprovisionamiento, registro, cambio de la provisoria y un reintento). **Hasta el 2026-09-13 eran 30 y 120** y dejaban afuera a la clase, que choca con `RN-B1`; se corrigieron y la ficha `BT-00029` lo registra.

**La defensa contra la fuerza bruta es por cuenta** (OWASP Authentication Cheat Sheet, *login throttling*): **diez fallos cada quince minutos**, que acotan un diccionario a cuarenta contraseñas por hora por cuenta, cualquiera sea la cantidad de orígenes. Una ventana de un minuto devolvería los diez intentos cada minuto. **El rechazo no dice si la cuenta existe**: un correo desconocido gasta cuota igual que una contraseña equivocada y recibe el `429` en el mismo intento. Cuando `PT-05` mida el uso real, las seis cifras se ajustan **por configuración**, sin tocar el código; `appsettings.json` las trae escritas para que quien despliega las vea.

**La respuesta `429` lleva `Retry-After` en segundos enteros, redondeados hacia arriba, y no lleva cuerpo**, sea cual sea la cuota que se agotó. La ventana deslizante del marco no informa la espera, así que en la práctica el valor es la ventana entera: 60, o 900 para la cuota por cuenta. No lleva código del contrato porque el conjunto cerrado no declara ninguno para una cuota y esta capa no inventa códigos ([`ADR-00004`](Adrs/ADR-00004-Dos-Traducciones-Con-Tabla-Unica-Y-Sin-Codigos-Inventados.md)): es la tercera respuesta sin código de §5.1. Lo que el consumidor necesita —cuándo volver— lo dice la cabecera.
...
**Lo verifica `RateLimitingTests`** (batería de integración) **con los umbrales por omisión y no con unos bajados para la prueba**: la sexagésima primera petición de una persona, la mil doscientas una de un origen y el canje seiscientos uno de un origen —con seiscientas cuentas distintas— reciben `429` con `Retry-After`; cuarenta ingresos correctos de cuentas reales desde un mismo origen entran; el undécimo intento sobre una cuenta, desde orígenes distintos y con mayúsculas cambiadas, recibe `429` aunque traiga la contraseña correcta, y otra cuenta sigue entrando; el `429` por cuenta llega en el mismo intento y con la misma forma para una cuenta existente y una inexistente; los ingresos correctos no gastan la cuota por cuenta; los fallos de la forma sin sesión de `A-05` la comparten con el canje; otra persona y otro origen siguen en `200`; la misma persona desde otra dirección sigue en `429`; `/salud` responde `200` más veces que cualquier cuota; `X-Forwarded-For` se honra desde una red declarada y se ignora desde cualquier otra; una ráfaga concurrente de sesenta escrituras de una persona llega entera al único escritor sin ningún `5xx` y la sexagésima primera no llega; y un umbral en cero detiene el arranque nombrando la llave.
...
| 1.11 | 2026-09-13 | **§4.1 corrige el límite del canje, que dejaba afuera a una clase** ([`ADR-00011`](Adrs/ADR-00011-El-Canje-Se-Protege-Por-Cuenta-Y-El-Origen-Tolera-Una-Comision.md)). La cuota del canje **por origen** de `BT-00029` —30 por minuto— la agotaba la comisión entera, porque el front no reenvía la dirección del navegador y la facultad sale por un NAT (`RN-B1`). Entra la fila **«Los fallos, por cuenta»**: diez fallos cada 900 segundos por correo normalizado, desde cualquier origen, en `A-01` y en la forma sin sesión de `A-05`, sin revelar si la cuenta existe. Los topes por origen suben a **600** canjes y **1200** anónimas por minuto, dimensionados para trescientos alumnos detrás de una dirección. El párrafo de umbrales se parte en tres y el de la batería se actualiza. **Ningún punto, verbo, papel ni código de respuesta cambia.** Sube minor. |
