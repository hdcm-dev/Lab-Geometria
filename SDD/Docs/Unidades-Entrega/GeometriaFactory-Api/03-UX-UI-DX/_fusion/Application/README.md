...
| `DX-Portal-Developers.md` | `tiene_portal_developers` == false. No hay portal hospedado ni integradores externos: este proyecto de código no se publica en ningún feed y sus dos consumidores son proyectos de código del mismo producto (`PRODUCT-INTAKE` §17.1.P.7 · GeometriaFactory-Application, §17.1.P.3 · GeometriaFactory-Application) |
| `DX-Operability.md` | Es obligatorio para `worker-service` y este proyecto de código es `library`. No hay nada que operar: no atiende peticiones, no abre conexiones y no registra ni instrumenta. Sus únicos NFR son el tiempo del caso de uso más pesado y la exclusión de los componentes en las consultas de listado (§17.1.P.10 · GeometriaFactory-Application) |
