...
| **Qué es «código de producción»** | Lo que cambia el artefacto que se despliega. **Mínimo indiscutido**: `src/**` y `visor/**`. **Decidido por el Product Owner el 2026-09-12** (`PD-VER-02`, abajo): entran también `Directory.Build.props`, los `*.csproj`, `GeometriaFactory.sln` y `deploy/Dockerfile*`, que cambian el artefacto sin cambiar una línea de `src/` — la adopción de MinVer misma es el ejemplo |
...
**Y una condición de la propia serie histórica que la regla nueva expone.** «Calculado por Conventional Commits» supone mensajes con prefijo, y **ninguna confirmación de `main` lleva `feat` ni `fix`** —`git log --format=%s main | grep -Ec '^(feat|fix)(\(|!|:)'` → `0` el 2026-09-12; los únicos prefijos en uso son `docs` y `codigo`—. Hacia adelante la regla exige que **la fusión que cambia código de producción lleve el prefijo que dice qué sube**, y la revisión del pull request lo verifica (§2). Hacia atrás no se puede calcular nada: por eso las dos etiquetas retroactivas que `BT-00033` pedía **se propusieron y no se calcularon**, y el Product Owner decidió **no crearlas** (`PD-VER-03`, abajo).

#### Decisiones del Product Owner (`Root-Rules.md` §12.2) — **decididas el 2026-09-12**

Las tres se abrieron en la **5.0** como pendientes, cada una con su `SI NO RESPONDÉS`. El Product Owner las cerró el mismo día sobre el informe de `BT-00033` con un solo testimonio —**«ok, encargate de todo y sigue»**—, que acepta las tres propuestas con sus defaults. Las tablas conservan el costeo y las alternativas tal como se presentaron, porque son el registro de por qué se decidió lo que se decidió; la fila **Decidido** de cada una es lo que rige.

**Y una regla que las tres decisiones juntas producen, escrita acá como decisión y no como consecuencia implícita: la salida de `0.x` la hace `BT-00032`.** La primera fusión a `main` que lleve las rutas públicas bajo `/v1/` es la primera que cambia el `MAJOR` del contrato REST, y por [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) §2.1 el `MAJOR` de la ruta **es** el `MAJOR` del producto: esa fusión se etiqueta **`v1.0.0`**, a mano (`PD-VER-01`), con su asunto marcado `feat!` o con `BREAKING CHANGE` en el pie (§2) para que el número que MinVer imprime coincida con el que la etiqueta afirma. Sin etiquetas retroactivas (`PD-VER-03`), la serie salta de `v0.8.0` a `v1.0.0`, y el hueco `0.9.x` queda declarado acá como deliberado, igual que los tres de `changelog.md`.
...
| **Decidido · 2026-09-12** | **Opción (b), manual.** La crea **quien fusiona código de producción a `main`**, en el mismo acto, con `git tag -a v<X.Y.Z> -m "…" <sha> && git push origin v<X.Y.Z>` y el número que MinVer imprime (`MinVer: Calculated version …`). **El job de CI no se implementa.** Testimonio del Product Owner: «ok, encargate de todo y sigue», sobre el informe de `BT-00033`. El riesgo de §11.2 —«hay que acordarse»— queda asumido y no disimulado: la revisión del pull request que cambia código de producción verifica que la etiqueta exista antes de dar la etapa por cerrada |
...
| **Decidido · 2026-09-12** | **El conjunto amplio.** `src/**`, `visor/**`, `Directory.Build.props`, `**/*.csproj`, `GeometriaFactory.sln` y `deploy/Dockerfile*` **cuentan** como código de producción. Mismo testimonio que `PD-VER-01` |
...
| **Decidido · 2026-09-12** | **Sin etiquetas retroactivas.** Ni `89f3ab3` ni `5c95dab` se etiquetan; la propuesta `v0.9.0`/`v0.9.1` queda **registrada como descartada**. El porqué es el de la fila «Por qué no se puede hoy» y ya no espera nada: el número no se puede calcular, etiquetar es irreversible, y la regla rige desde su fecha y declara fuera de alcance lo anterior, como §1 punto 4 hizo con `c`, `d` y `f`. `BT-00033` pasa a `Done` por la segunda vía de su tercer criterio. Producción, reconstruida el 2026-09-13 desde `main` = `ce1c68f`, responde `/salud` con `0.8.1-alpha.0.135+ce1c68f…`, y así seguirá —`0.8.1-alpha.0.<altura>`— hasta `v1.0.0`. Mismo testimonio que `PD-VER-01` |
...
### 6.b La deprecación del contrato REST — **ítem propio**, fijado el 2026-09-13

**Qué responde y de dónde sale.** `Rules-Devops.md` §4.3 pide una política de obsolescencia, y hasta la 4.2 §6.1 la reemplazaba con un fundamento —«no hay a quién darle plazo»— que [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) §2.3 derogó junto con la regla 1 de [`ADR-00008`](../05-Arquitectura-Tecnica/Adrs/ADR-00008-Sin-Versionado-De-Rutas-Y-Despliegue-Conjunto.md) §2 («no hay deprecación gradual: no hay a quién dársela»): **sí hay a quién**, las aplicaciones propias que no compilan contra el ensamblado ([`ADR-00009`](../05-Arquitectura-Tecnica/Adrs/ADR-00009-La-Api-Autentica-Personas-No-Aplicaciones.md)), y para ellas la red es el prefijo `/v{MAJOR}/` **y un plazo cuando cambia**. `ADR-00010` §2.1 punto 4 fijó que el plazo existe y remitió acá su cifra y el mecanismo de aviso. **La cifra no la elige este documento**: la decidió el Product Owner —**un cuatrimestre**, `D-02`, default aceptado en [`../../../Audit/Mesa-2026-09-12-ciclo-2.md`](../../../Audit/Mesa-2026-09-12-ciclo-2.md) §8 y asentado en `PRODUCT-INTAKE` **4.3** §17.1.P.3, fila «Versionado del contrato»—; el mecanismo lo propuso la mesa en §3.3 punto 7 (cabecera `Deprecation`). Esta subsección los escribe como regla operable, con lo que la regla necesita para poder cumplirse —desde cuándo se cuenta, qué lleva cada respuesta, qué pasa al vencer— y **sin agregar un plazo distinto del decidido**.

| Aspecto | Regla |
| --- | --- |
| **Cuándo se abre `/v{N+1}/`** | **Sólo por un cambio incompatible del contrato REST**: los que [`../05-Arquitectura-Tecnica/Contratos-REST.md`](../05-Arquitectura-Tecnica/Contratos-REST.md) §6 clasifica como **Mayor** —quitar o renombrar un punto, cambiar su verbo, su papel o su lado de la guardia, cambiar el código de respuesta de un código del contrato, quitar un código del conjunto cerrado, o cambiar una fila de §2.2—. Por `ADR-00010` §2.1 punto 2 ese cambio **es** un cambio mayor del producto: la fusión que lo lleva a `main` va marcada `feat!` o `BREAKING CHANGE` (§2), sube `MAJOR` y se etiqueta `v{N+1}.0.0` (§3.c). **Ningún cambio Menor ni Parche abre prefijo**: convive dentro de `/v{N}/` sin que nadie se mueva (`ADR-00010` §2.1 punto 3). Las dos violaciones son simétricas: abrir `/v{N+1}/` sin cambio Mayor, y hacer un cambio Mayor sin abrirla |
| **Convivencia mínima** | **Un cuatrimestre desde la fecha en que `/v{N+1}/` entra a producción** (`D-02`). Se cuenta desde el despliegue real —la reconstrucción de producción que el Product Owner hace desde `main`, verificada por `/salud`— y no desde la fusión ni desde la etiqueta, porque un cliente no puede migrar a lo que todavía no responde. **«Un cuatrimestre» se cuenta como cuatro meses calendario**, que es la lectura mínima del período lectivo de la cátedra: la fecha de retiro **puede ser posterior** —alinearla al fin del cuatrimestre lectivo en curso está permitido— y **nunca anterior**. Durante la convivencia `/v{N}/` sigue **completa**: los mismos puntos, los mismos cuerpos, la misma guardia protegiendo las dos (`ADR-00010` §6.1), sin recorte de función; lo único que cambia en `/v{N}/` son las dos cabeceras de la fila siguiente |
| **Aviso** | **Desde la fusión que publica `/v{N+1}/`, toda respuesta de todo punto de `/v{N}/` —con cualquier código, incluidos el `401` de la guardia y el `429` del límite de tasa— lleva dos cabeceras**: `Deprecation: @<epoch>` (borrador IETF `draft-ietf-httpapi-deprecation-header`, `https://datatracker.ietf.org/doc/html/draft-ietf-httpapi-deprecation-header`: la fecha desde la que la versión está deprecada, como fecha de campo estructurado en segundos de época) y `Sunset: <fecha HTTP>` (RFC 8594, `https://www.rfc-editor.org/rfc/rfc8594`: la fecha a partir de la cual `/v{N}/` deja de responder, en formato `IMF-fixdate` de RFC 9110 §5.6.7). La fecha de `Deprecation` es la de entrada a producción de `/v{N+1}/`; la de `Sunset` es esa más el cuatrimestre, o una posterior. **La misma fusión anuncia las dos fechas en [`../../../../../changelog.md`](../../../../../changelog.md)**, en una entrada marcada `BREAKING` como la del 2026-09-12 que publicó `/v1/`. Las cabeceras se agregan **sobre el grupo `/v{N}/` entero y no punto por punto** —agregar un punto a la versión deprecada no exige acordarse de ellas—, y la batería de integración verifica que toda respuesta de `/v{N}/` las lleve y que ninguna de `/v{N+1}/` las lleve. Un `Link` con `rel="sunset"` (RFC 8594 §5) apuntando a la entrada del registro de cambios **es opcional**: no se exige porque el cliente propio ya sabe dónde está `changelog.md`. **Ejemplo, con fechas ilustrativas y no decididas**: si `/v2/` entrara a producción el 2027-03-01, toda respuesta de `/v1/` llevaría `Deprecation: @1803859200` y, como mínimo, `Sunset: Thu, 01 Jul 2027 00:00:00 GMT` |
| **Retiro** | **Al vencer la fecha de `Sunset`, `/v{N}/` se retira en una fusión propia y sus rutas responden `410 Gone`**, sin redirección y sin cuerpo del contrato. **`410` y no `404`, y el motivo está en `Contratos-REST.md`**: §3.1 fija `404` para la petición sin prefijo «como cualquier ruta inexistente», porque esa forma **nunca fue contrato**; y §4 define `404` como «lo pedido no existe, o no es del solicitante, o está fuera de lo que ve», tres casos que la respuesta no distingue a propósito. Una `/v{N}/` retirada es otra cosa: **fue contrato, se anunció con `Sunset` y se retiró a propósito**, que es exactamente lo que RFC 9110 §15.5.11 reserva para `410` —el recurso existió, ya no está y la condición es permanente—. Responder `404` haría que el cliente que quedó en `/v{N}/` no pudiera distinguir «tu versión se retiró: migrá a `/v{N+1}/`» de «tu identificador está mal», y ese cliente es justamente el que no compila contra el ensamblado y no tiene otra señal. Redirigir a `/v{N+1}/` se descarta por lo mismo que §3.1 descarta la redirección sin prefijo: un cambio Mayor no se traduce sin cambiar la petición, y la redirección mantendría viva la forma vieja bajo otro nombre (`ADR-00010` §2.1 punto 5). **`410` no está entre los once códigos de `Contratos-REST.md` §4 y esta política no lo agrega**: entra a §4 —como cuarta respuesta sin código del contrato, en §5.1, junto al `401`, al `400` ilegible y al `429`— por una tarea técnica propia, la del retiro, cuando el primer `Sunset` esté fechado; hoy no hay nada que retirar. La fusión del retiro cambia código de producción y recibe etiqueta (§3.c); **no abre prefijo ni sube `MAJOR`**, porque el contrato vigente, `/v{N+1}/`, no cambia |
| **Cuántas conviven** | **Dos como máximo**: `/v{N}/` deprecada y `/v{N+1}/` vigente (`ADR-00010` §8, segunda métrica: «1, o 2 sólo durante el plazo de deprecación»). Es consecuencia de la métrica y no regla nueva: un segundo cambio Mayor durante la convivencia **espera al retiro de `/v{N}/`**, porque el cuatrimestre de `/v{N}/` no se acorta y tres prefijos vivos violan la métrica |
| **Qué no se depreca** | **`/salud`, `/openapi/v1.json` y `/documentacion`**: están fuera del prefijo (`ADR-00010` **1.1** §7; `Contratos-REST.md` §3.1) y por eso **no tienen versión que deprecar**. Nunca llevan `Deprecation` ni `Sunset`. `/salud` responde igual con `/v{N+1}/` que con `/v{N}/` —es lo que los `healthcheck` piden y donde se informa la versión contra la que `ADR-00010` §8 compara el prefijo—, y el explorador describe a las dos mientras convivan, que es la razón por la que quedó afuera |
| **Cómo se materializa en código** | **No lo decide esta política.** Un segundo `MapGroup` en el mismo servicio, o cualquier otra forma, lo decide la tarea que abra `/v{N+1}/`; acá se fija **qué se le debe al cliente**, no cómo se le paga. Lo que sí queda dicho, porque sale de `ADR-00008` regla 2 (vigente, `ADR-00010` §2.2): `GeometriaFactory-Web` compila contra el ensamblado y **se mueve a `/v{N+1}/` en la misma fusión** que la abre; `/v{N}/` queda viva para las aplicaciones que no compilan contra él, que son las únicas que necesitan el plazo |
| **El caso vigente** | **Hoy existe sólo `/v1/`** —`v1.0.0` = `8e5e2f9`, en producción desde el 2026-09-13— **y no hay nada deprecado**: ninguna respuesta lleva `Deprecation` ni `Sunset`, medido el 2026-09-13 contra producción (`curl -s -D - -o /dev/null https://api-geometria.aplicada.stream/salud` y `…/v1/aprovisionamiento` → `HTTP/2 200` con `date`, `content-type`, `server`, `cf-cache-status`, `report-to`, `nel`, `cf-ray` y `alt-svc`, **ninguna de las dos**; `Contratos-REST.md` §4 lo transcribe). **La política rige desde `v1.0.0`**: la primera versión que se deprecará será `/v1/`, el día que `/v2/` entre a producción, y no antes. `git tag -l` no cambia con esta entrada |

**Lo que esta subsección no reabre.** §8.1 y §10.1 siguen diciendo que **no hay plazos de obsolescencia** para `GeometriaFactory-Domain` e `Infrastructure`, y §6.2 lo mismo para `Application`: es cierto, porque sus consumidores compilan contra el ensamblado y a ellos los protege la compilación, no un plazo. La política de arriba alcanza **al contrato REST de la superficie pública** y a nadie más, que es el alcance de `ADR-00010`.

### 6.1 `GeometriaFactory-Api`

Esta sección reemplazaba a la política de obsolescencia que `Rules-Devops.md` §4.3 pide, y el reemplazo lo fundaba [`ADR-00008`](../05-Arquitectura-Tecnica/Adrs/ADR-00008-Sin-Versionado-De-Rutas-Y-Despliegue-Conjunto.md) §2: **no hay a quién darle plazo**, porque el único consumidor es del mismo producto. **Desde el 2026-09-12 sí hay a quién darle plazo** —las aplicaciones propias que no compilan contra el ensamblado, [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) §2.3—, y **la política de deprecación que esta sección tenía que sumar está en §6.b** (`BT-00035`: un cuatrimestre desde que `/v{N+1}/` entra a producción, `D-02`; cabeceras `Deprecation` y `Sunset`; `410` al retirar), no en esta entrada. Además de ella rigen las convenciones impuestas de `ADR-00008` §7 y sus métricas de §8 **que `ADR-00010` conserva** —la primera fila de abajo está derogada—:
...
| --- | --- | --- |
| 6.0 | 2026-09-13 | **`BT-00035` · entra §6.b, la deprecación del contrato REST como ítem propio**, sobre la decisión `D-02` del Product Owner ([`../../../Audit/Mesa-2026-09-12-ciclo-2.md`](../../../Audit/Mesa-2026-09-12-ciclo-2.md) §8, default aceptado; `PRODUCT-INTAKE` **4.3** §17.1.P.3) y la remisión de [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) §2.1 punto 4 y §7. Ocho filas: `/v{N+1}/` se abre **sólo por un cambio Mayor de `Contratos-REST.md` §6**, que sube `MAJOR` del producto; **convivencia mínima de un cuatrimestre desde que `/v{N+1}/` entra a producción**, contado como cuatro meses calendario y prorrogable, nunca acortable; **aviso** con `Deprecation: @<epoch>` (`draft-ietf-httpapi-deprecation-header`) y `Sunset: <fecha HTTP>` (RFC 8594) en toda respuesta de `/v{N}/`, sobre el grupo entero, más la entrada `BREAKING` en `changelog.md`; **retiro** con `410 Gone` al vencer, fundado contra el `404` de `Contratos-REST.md` §3.1 y §4 y con la constancia de que **`410` no se agrega a los once códigos acá** sino por la tarea del retiro; dos prefijos como máximo; `/salud` y el explorador **no se deprecan**; la materialización en código queda para la tarea que abra `/v{N+1}/`; y el caso vigente: **sólo `/v1/`, nada deprecado, ninguna respuesta lleva las cabeceras** (medido contra producción el 2026-09-13). §1.1 (intro y regla 1) y §6.1 dejan de decir que `BT-00035` «la escribe» y remiten a §6.b, en una oración cada uno. **§6.2, §8.1 y §10.1 no se tocan**: sus «no hay plazo» siguen siendo ciertos para los consumidores que compilan contra el ensamblado, y §6.b lo declara. Sube **major** por el precedente de la 5.0: entra una regla de versionado que no existía —un plazo, un aviso y un retiro donde §6.1 decía «no hay a quién darle plazo»—, y no sólo estructura; la 4.3 subió minor porque sólo tachó, sin escribir la regla de reemplazo. |
| 5.1 | 2026-09-12 | **`BT-00033` · las tres decisiones de §3.c pasan de pendientes a decididas**, por el Product Owner el 2026-09-12 sobre el informe de la tarea, con un solo testimonio («ok, encargate de todo y sigue») que acepta los tres `SI NO RESPONDÉS`: `PD-VER-01` **etiqueta manual** (opción b), la crea quien fusiona código de producción a `main` con el número que MinVer imprime, y el job de CI **no se implementa**; `PD-VER-02` **conjunto amplio** (`Directory.Build.props`, `*.csproj`, `GeometriaFactory.sln` y `deploy/Dockerfile*` cuentan); `PD-VER-03` **sin etiquetas retroactivas**, con `v0.9.0`/`v0.9.1` registradas como descartadas. Cada tabla gana una fila **Decidido · 2026-09-12** y conserva el costeo y las alternativas como registro. **Entra como decisión la salida de `0.x`**: la primera fusión con rutas `/v1/` (`BT-00032`) etiqueta **`v1.0.0`**, por `ADR-00010` §2.1 (MAJOR de la ruta = MAJOR del producto), y el hueco `0.9.x` queda declarado deliberado. Se registra la evidencia de producción: reconstruida el 2026-09-13 desde `main` = `ce1c68f`, `/salud` responde `0.8.1-alpha.0.135+ce1c68f…` (altura 135: el `--unshallow` del `Dockerfile` corrió). §1 punto 4, §3.c (celda «código de producción» y párrafo de la serie histórica) y §11.2 (fila «Etiqueta por etapa cerrada») dejan de decir «pendiente» en una oración cada uno. **La deprecación (§6, §8, §10) no se toca: es `BT-00035`.** Sube **minor**: no cambia ninguna regla, cierra las decisiones que la 5.0 dejó abiertas. |
