...
| `cobertura` | Recolector de cobertura, con informe **por componente** | `QG-00003`: **75 %** de líneas y **70 %** de ramas | 75 / 70 **[ASUNCIÓN del intake §17.1.P.6 · GeometriaFactory-Api, asunción `A-3` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
| `cobertura` | Recuento de pruebas por clase en el informe (`TC-00037`) | `QG-04`: pirámide de **60 %** integración y **40 %** unitarias | 60 / 40 **[ASUNCIÓN del mismo origen]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`); la **inversión** no es asunción. Ver §2.2 |
| `imagen` | Construcción con `deploy/Dockerfile` **multietapa** y arranque desde el contenedor de desarrollo; `TC-00033` | `QG-00013`: el arranque en frío aplica las transformaciones y responde salud en menos de **30 segundos** | 30 s **[ASUNCIÓN del intake §17.1.P.10 · GeometriaFactory-Api, asunción `A-5` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
...
| `test` (batería de integración) | `TC-00034` | `QG-00014`: percentil 99 del listado por debajo de **500 ms** medido en el servidor, y caudal sostenido de **20 peticiones por minuto** | 500 ms y 20 por minuto **[ASUNCIÓN del intake §17.1.P.10 · GeometriaFactory-Api]** | **Bloqueante en su percentil** desde el 2026-08-26 (`D1`), y **condicionado en su caudal**: las **20 peticiones por minuto** quedaron **fuera de la confirmación** —su fundamento se cayó con `D5` y su valor sale de `PT-05`—. **El gate frena por el p99 y no frena por el caudal**, y se dice en lugar de darlo por entero |
...
**Quince gates, y ninguno movido de lugar.** Los que salen del intake §17.1.P.8 · GeometriaFactory-Api son **`QG-00001`, `QG-00002`, `QG-00003` y `QG-00013`**, uno por cada stage de su tabla que lleva un gate de `08` detrás —la quinta fila, `despliegue`, no lleva ninguno, y la de `imagen` lleva además la puerta técnica `PT-04`, que no es gate de `08`—. **`QG-00004` sale de §17.1.P.6 · GeometriaFactory-Api** y **`QG-00005` de `05` §8 y de `RN-00013`**; los demás salen de una fila de `05` §8, que declara los **diecisiete** NFR de este proyecto de código. Se enumeran por identificador y no por posición en la lista, porque el orden de `Estrategia-Calidad.md` §3 no es el de la fuente.
...
| `test` | Recolector de cobertura, con informe **por componente** | `QG-02003`: **90 %** de líneas y **85 %** de ramas | 90 / 85 **[ASUNCIÓN del intake §17.1.P.6 · GeometriaFactory-Domain, asunción `A-3` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
| `test` | Duración total reportada por el ejecutor | `QG-02007`: la batería completa termina en menos de **10 segundos** | 10 s **[ASUNCIÓN del intake §17.1.P.10 · GeometriaFactory-Domain, asunción `A-5` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
...
**Los dos gates dejaron de ser condicionados el 2026-08-26**, al confirmarse sus asunciones (`D1`), y **pasaron a bloqueantes**. Mientras lo fueron, se midieron igual: condicionado no era opcional: `Estrategia-Calidad.md` §3.1 declara que la medición se hace y el resultado se registra, y lo que queda en suspenso es la consecuencia automática. El pipeline **emite el número y no lo silencia**; su incumplimiento entra como hallazgo del punto de control de la etapa, no como rechazo de la fusión. Los dos dependen de valores rotulados **[ASUNCIÓN]** en el intake §22, y `BT-02015` es la tarea que los eleva al Product Owner.
...
| `build` | Inspección del archivo de proyecto (`TC-04027`) | `QG-02005`: exactamente **1** referencia a otro proyecto de código del producto —`GeometriaFactory-Domain`— y **0** a bibliotecas de persistencia, transporte, serialización o marco web | 1 y 0 | **Bloqueante** |
| `test` | `scripts/test.sh` | `QG-02`: la batería pasa entera, **0** pruebas rojas y **0** deshabilitadas sin motivo escrito | 0 y 0 | **Bloqueante** |
| `test` | Prueba de inspección `TC-04026`, más la revisión del pull request | `QG-04004`: **ninguna** prueba de esta capa toca la base de datos real | 0 | **Bloqueante.** Es la puerta propia del intake §17.1.P.8 · GeometriaFactory-Application. Ver §2.2 |
...
| `test` | Recolector de cobertura, con informe **por componente** | `QG-04003`: **85 %** de líneas y **80 %** de ramas | 85 / 80 **[ASUNCIÓN del intake §17.1.P.6 · GeometriaFactory-Application, asunción `A-3` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
| `test` | Medición sobre la batería unitaria con doble del puerto de validación | `QG-04010`: el caso de uso más pesado resuelve en menos de **500 ms** para el texto semilla de **3** piezas de `E-1`, **sin acceso a base** | 500 ms **[ASUNCIÓN del intake §17.1.P.10 · GeometriaFactory-Application, asunción `A-5` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
...
| `test` | Recolector de cobertura, con informe **por componente** | `QG-06005`: **85 %** de líneas y **80 %** de ramas | 85 / 80 **[ASUNCIÓN del intake §17.1.P.6 · GeometriaFactory-Infrastructure, asunción `A-3` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
| `test` | Informe de cobertura **acotado a los dos motores** | `QG-06`: **95 %** de líneas en el validador de figuras | 95 **[ASUNCIÓN del mismo origen]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
| `test` | `TC-06015` | `QG-06014`: la interpretación del texto de **3** piezas de `E-1` termina en menos de **200 ms**, medida **sin almacén** | 200 ms **[ASUNCIÓN del intake §17.1.P.10 · GeometriaFactory-Infrastructure, asunción `A-5` de §22]** | **Bloqueante** desde el 2026-08-26, confirmada su asunción (`D1`) |
...
| Pull request que **cambia el ensamblado de contratos** | Todo lo anterior. Es donde se mide el `QG-08005` de `GeometriaFactory-Contracts`, que exige **100 %** de los tipos de transferencia ejercitados por al menos una prueba de integración | La fusión, también para aquel proyecto de código |
...
| Apertura o actualización del pull request de la etapa | Los tres stages, más la revisión de `QG-02008` | **La fusión**, por los gates bloqueantes de §2.1 |
...
| --- | --- | --- |
| Confirmación empujada a la rama de una etapa | `restore` → `build` → `test` completos | Nada por sí solo: es la señal temprana para quien construye |
| Apertura o actualización del pull request de la etapa | Los tres stages, más la revisión de `QG-04004` y de `QG-04011` | **La fusión**, por los gates bloqueantes de §2.1 |
...
| Fusión a la rama principal | Los tres stages sobre el estado fusionado | El cierre de la etapa si algo se rompió al fusionar |
| Cierre de la etapa | Lo anterior, más `TC-04011` y la matriz de comprobaciones (`QG-04007`) | La declaración de etapa cerrada |
| Etiqueta de cierre de etapa | Los tres stages sobre el estado etiquetado | La declaración de etapa cerrada |
...
| Pull request que **toca el validador de figuras** | Todo lo anterior, con `QG-06003` sobre los **diez** casos y `QG-06007` sobre el caso testigo | La fusión |
...
| **Prohibición explícita de caché** | **El almacén de la batería de integración y el del stage `imagen` no se cachean.** Se crean vacíos en cada ejecución | `QG-00011` mide **0** peticiones atendidas con la preparación del almacén incompleta, y `PT-04` exige aplicar las transformaciones **sobre un almacén vacío**. Un almacén cacheado dejaría de ser vacío |
| Artefacto del stage `build` | El ensamblado compilado, con los tres proyectos de código referenciados | Intake §13 |
| Artefacto del stage `test` | La salida de la batería, **con el recuento de pruebas por clase** para `QG-00004`, y los recuentos de las inspecciones | `Estrategia-Calidad.md` §3 |
| Artefacto del stage `cobertura` | El informe **por componente** | El mismo, columna de verificación de `QG-00003` |
| Artefacto del stage `imagen` | **La imagen**, más el registro de su arranque: transformaciones aplicadas, salud respondida y **el tiempo de arranque en frío** | `QG-00013`; `PT-04` |
...
| Artefacto del stage `test` | El **informe de cobertura por componente**, la salida de la batería y **el número de `QG-04010`** | `Estrategia-Calidad.md` §3, que exige el informe por componente |
| Artefactos de inspección | Los **recuentos** de `QG-04004`, `QG-04005`, `QG-04006`, `QG-04008` y `QG-04009` | Los cinco están escritos como recuentos en `Estrategia-Calidad.md` §3 |
...
**El informe de cobertura no se emite como número global**, y no es preferencia de esta categoría: `QG-04003` se verifica sobre el informe **por componente**, y este proyecto de código tiene **8** componentes (`05` §3.1). Un promedio único podría cerrar el 85 % dejando un componente muy por debajo.
...
| Invalidación | Al cambiar cualquier archivo de proyecto. **Sin expiración por tiempo** | Un plazo en días no lo da ninguna fuente |
| **Prohibición explícita de caché** | **El almacén del stage `verificar-transformaciones` no se cachea entre ejecuciones.** Se crea inexistente en cada una | `QG-06004` mide la aplicación **sobre un almacén inexistente**; un almacén cacheado dejaría de ser inexistente y el gate mediría otra cosa |
...
| Artefacto del stage `test` | El **informe de cobertura por componente**, el **informe acotado a los dos motores**, la salida de la batería y el número de `QG-06014` | `Estrategia-Calidad.md` §3, columnas de verificación de `QG-06005`, `QG-06006` y `QG-06014` |
| Artefacto del stage `verificar-transformaciones` | El registro del linaje aplicado y la constancia de que **no hubo paso manual** | `QG-06004` |
| Artefactos de inspección | Los **recuentos** de `QG-06008` a `QG-06013`, cada uno con la condición en que se midió | `Estrategia-Calidad.md` §3 |
| Retención | Mientras dure el punto de control de la etapa; se adjuntan al **informe de cierre** | Intake §15, regla de delivery 3 |

**Los dos informes de cobertura son dos y no uno**, y no es redundancia: `QG-06005` mide el conjunto del proyecto de código y `QG-06006` mide **sólo los dos motores**, con un piso más alto. Un informe único no permitiría verificar el segundo, que es donde el intake §17.1.P.6 · GeometriaFactory-Infrastructure puso el número más alto del producto porque es el criterio que más veces se rompe.
...
| **Cambio incompatible del contrato → producto desplegado** | Sólo con **las dos unidades desplegables desplegadas desde el mismo estado del repositorio** | El `QG-08008` de `GeometriaFactory-Contracts`, que bloquea la **publicación de la etapa** | El mismo, con constancia escrita |
...
| El registro del stage `imagen` | Las transformaciones aplicadas, la salud respondida y el **tiempo de arranque en frío** | `QG-00013`; `PT-04` |
...
| El informe de cierre de la etapa | La medición de `QG-02003` y `QG-02007` con su distancia al umbral, y la constancia de los gates bloqueantes | Definition of Done §1.3 y `Criterios-Validacion.md` §6 |
...
| El informe de cierre de la etapa | La medición de `QG-04003` y `QG-04010` con su distancia al umbral, la constancia de los gates bloqueantes y el cierre de `QG-04007` sobre las cuatro comprobaciones | Definition of Done §1.3 |
...
| El registro del stage `verificar-transformaciones` | El linaje aplicado y la constancia de que no hubo paso manual | `QG-06004` |
| El informe de cierre de la etapa | La medición de `QG-06005`, `QG-06006` y `QG-06014` con su distancia al umbral, y la constancia de los gates bloqueantes | Definition of Done §1.3 |
| El registro de cambios del producto | Toda fila de cambio mayor de esta biblioteca | [`Estrategia-Versionado.md`](Estrategia-Versionado.md) §6 |

**No se declara ningún canal de mensajería ni ningún tablero**: ninguna fuente lo declara y `equipo_n` es 1.

**Y una regla de notificación propia de este proyecto de código**: `QG-06013` mide **0** mensajes o trazas con un secreto, la ruta del almacén o el texto del alumno. **Eso alcanza también a la salida del pipeline**: un registro de ejecución que imprimiera la ruta del almacén de prueba o un fragmento del texto de un escenario estaría produciendo, en la canalización, exactamente lo que el gate prohíbe en el producto.
...
| Id | Punto abierto | Quién lo cierra | En qué evento se cierra (artefacto y sección) | Estado | Ciclo de origen |
| --- | --- | --- | --- | --- | --- |
| PD-01 | La **herramienta concreta** de cada stage —ejecutor de pruebas, recolector de cobertura y reglas de análisis estático— y su anclaje de versión | El equipo, en el punto de control de la etapa `a` | `scripts/build.sh`, `scripts/test.sh` y `.github/workflows/deploy-front-ftp.yml` | **Cerrado** el 2026-08-20 · **A2b, por lectura**: las herramientas están ancladas en los guiones — `dotnet build`, `dotnet test`, `npm ci`, `webpack` y `playwright` | «Fase F de los tres proyectos de nivel 0, e intake 1.21» (2026-08-11) · GeometriaFactory-Domain · base `1f5df1a` |
| PD-02 | La **confirmación de los dos valores rotulados [ASUNCIÓN]** que hoy dejan condicionados a `QG-02003` y `QG-02007`. Confirmados, los dos pasan a bloqueantes sin ningún otro cambio de este documento | El Product Owner, sobre el intake §22 | [`../../../../../changelog.md`](../../../../../changelog.md), etapa `d` § «Decidido en esta etapa, y elevado al punto de control» | **Cerrado** el 2026-08-26 · **decisión del Product Owner**, sobre [`../../../Audit/D1-Confirmacion-De-Asunciones.md`](../../../Audit/D1-Confirmacion-De-Asunciones.md) 1.0 y el intake §22: **confirma los valores rotulados `[ASUNCIÓN]`** —los umbrales de latencia y arranque de `A-5`, las coberturas y la forma de la pirámide de `A-3`, los gates que no son de líneas de `A-4` y las métricas de negocio de `A-2`—. **Con una excepción declarada y no confirmada: el caudal de 20 peticiones por minuto**, cuyo fundamento se cayó al cerrarse `D5` —el volumen de la comisión, incognoscible— y **cuyo valor definitivo sale de `PT-05`**, no de esta confirmación. Sigue **provisorio** y con su fila propia en `05` §8. **Confirmar no es medir**: los umbrales se eligieron altos donde la fuente señala criticidad, y ninguna fuente los fija | «Fase F de los tres proyectos de nivel 0, e intake 1.21» (2026-08-11) · GeometriaFactory-Domain · base `1f5df1a` |
| PD-03 | Si el mutation score entra alguna vez al pipeline. Hoy `CV-02019` se reporta «sin medir» y su hueco está declarado | La categoría 08, si la herramienta se elige | **Decisión del Product Owner, 2026-08-20** · `Audit/A3-Decisiones-Del-Product-Owner.md` `D8` | **Cerrado con un NO** el 2026-08-20 · El mutation score **no entra al pipeline**. `CV-02019` queda declarado **sin medir**, con su motivo, y deja de ser un pendiente | «Fase F de los tres proyectos de nivel 0, e intake 1.21» (2026-08-11) · GeometriaFactory-Domain · base `1f5df1a` |
...
| Id | Punto abierto | Quién lo cierra | En qué evento se cierra (artefacto y sección) | Estado | Ciclo de origen |
| --- | --- | --- | --- | --- | --- |
| PD-01 | La **herramienta concreta** de cada stage —ejecutor de pruebas, recolector de cobertura y reglas de análisis estático— y su anclaje de versión | El equipo, en el punto de control de la etapa `a` | `scripts/build.sh`, `scripts/test.sh` y `.github/workflows/deploy-front-ftp.yml` | **Cerrado** el 2026-08-20 · **A2b, por lectura**: las herramientas están ancladas en los guiones — `dotnet build`, `dotnet test`, `npm ci`, `webpack` y `playwright` | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Application · base `87fbb3b` |
| PD-02 | La **confirmación de los dos valores rotulados [ASUNCIÓN]** que hoy dejan condicionados a `QG-04003` y `QG-04010`. Confirmados, los dos pasan a bloqueantes sin ningún otro cambio de este documento | El Product Owner, sobre el intake §22, por `BT-04018` | [`../../../../../changelog.md`](../../../../../changelog.md), etapa `d` § «Decidido en esta etapa, y elevado al punto de control» | **Cerrado** el 2026-08-26 · **decisión del Product Owner**, sobre [`../../../Audit/D1-Confirmacion-De-Asunciones.md`](../../../Audit/D1-Confirmacion-De-Asunciones.md) 1.0 y el intake §22: **confirma los valores rotulados `[ASUNCIÓN]`** —los umbrales de latencia y arranque de `A-5`, las coberturas y la forma de la pirámide de `A-3`, los gates que no son de líneas de `A-4` y las métricas de negocio de `A-2`—. **Con una excepción declarada y no confirmada: el caudal de 20 peticiones por minuto**, cuyo fundamento se cayó al cerrarse `D5` —el volumen de la comisión, incognoscible— y **cuyo valor definitivo sale de `PT-05`**, no de esta confirmación. Sigue **provisorio** y con su fila propia en `05` §8. **Confirmar no es medir**: los umbrales se eligieron altos donde la fuente señala criticidad, y ninguna fuente los fija | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Application · base `87fbb3b` |
| PD-03 | La **herramienta que calcula la versión** a partir de las convenciones de mensaje de confirmación. Es `PA-06` de `05` §11 y esta categoría **la declara por su función y no la elige** | El equipo en la fase `i` | [`../../../00-Contexto/Roadmap-Producto.md`](../../../00-Contexto/Roadmap-Producto.md) §2.1, fase `i` · Despliegue real, que es cuando aparece el primer release real que hay que versionar | **Vigente.** Evento **reasignado a la fase `i`** el 2026-08-30 por el Product Owner, cerrando la escalada `E-02` de [`../../../Audit/Mesa-2026-08-27.md`](../../../Audit/Mesa-2026-08-27.md) §9. Estaba **vencido** desde el cierre de la etapa `a` el 2026-08-13 | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Application · base `87fbb3b` |
...
| Id | Punto abierto | Quién lo cierra | En qué evento se cierra (artefacto y sección) | Estado | Ciclo de origen |
| --- | --- | --- | --- | --- | --- |
| PD-01 | La **herramienta concreta** de cada stage —ejecutor de pruebas, recolector de cobertura, reglas de análisis estático y la herramienta de transformaciones como herramienta local del repositorio— y su anclaje de versión | El equipo, en el punto de control de la etapa `a` | `scripts/build.sh`, `scripts/test.sh` y `.github/workflows/deploy-front-ftp.yml` | **Cerrado** el 2026-08-20 · **A2b, por lectura**: las herramientas están ancladas en los guiones — `dotnet build`, `dotnet test`, `npm ci`, `webpack` y `playwright` | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Infrastructure · base `87fbb3b` |
| PD-02 | La **confirmación de los tres valores rotulados [ASUNCIÓN]** que hoy dejan condicionados a `QG-06005`, `QG-06006` y `QG-06014`. Confirmados, los tres pasan a bloqueantes sin ningún otro cambio de este documento | El Product Owner, sobre el intake §22, por `BT-06023` | [`../../../../../changelog.md`](../../../../../changelog.md), etapa `d` § «Decidido en esta etapa, y elevado al punto de control» | **Cerrado** el 2026-08-26 · **decisión del Product Owner**, sobre [`../../../Audit/D1-Confirmacion-De-Asunciones.md`](../../../Audit/D1-Confirmacion-De-Asunciones.md) 1.0 y el intake §22: **confirma los valores rotulados `[ASUNCIÓN]`** —los umbrales de latencia y arranque de `A-5`, las coberturas y la forma de la pirámide de `A-3`, los gates que no son de líneas de `A-4` y las métricas de negocio de `A-2`—. **Con una excepción declarada y no confirmada: el caudal de 20 peticiones por minuto**, cuyo fundamento se cayó al cerrarse `D5` —el volumen de la comisión, incognoscible— y **cuyo valor definitivo sale de `PT-05`**, no de esta confirmación. Sigue **provisorio** y con su fila propia en `05` §8. **Confirmar no es medir**: los umbrales se eligieron altos donde la fuente señala criticidad, y ninguna fuente los fija | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Infrastructure · base `87fbb3b` |
| PD-03 | **Cuál de las dos funciones de derivación de clave se ancla, y con qué parámetros.** El intake §17.1.P.1 · GeometriaFactory-Infrastructure declara «PBKDF2 o Argon2» y **no elige**; `PA-03` de `05` §11 deja la forma y el criterio fijados por `ADR-06004` y la elección concreta en la regla de anclaje. **Esta categoría no la elige**, y declara su efecto de canalización: es una dependencia externa del artefacto del servidor propio y entra en su inventario | El equipo, en la etapa `a` | `src/GeometriaFactory.Infrastructure/Security/PasswordDerivation.cs` | **Cerrado** el 2026-08-20 · **A2b, por lectura**: elige **PBKDF2** y escribe su criterio; el parámetro entra por `PasswordDerivation:Iterations` | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Infrastructure · base `87fbb3b` |
| PD-04 | La **frecuencia del respaldo del almacén**, que el intake §17.1.P.4 · GeometriaFactory-Infrastructure declara «a definir por el docente» y `PA-07` de `05` §11 dirige a esta categoría. **No se inventa un número**: ver [`Entornos-Deploy.md`](Entornos-Deploy.md) §4 | El Product Owner | [`../../../00-Contexto/Roadmap-Producto.md`](../../../00-Contexto/Roadmap-Producto.md) §2.1, **punto de control de la fase `i`**, que `PRODUCT-INTAKE` §10 y §15 declaran bloqueante y es el último que queda | **Vigente.** Evento **declarado el 2026-08-30** por el Product Owner, cerrando la escalada `E-04` de [`../../../Audit/Mesa-2026-08-27.md`](../../../Audit/Mesa-2026-08-27.md) §9. Antes decía «falta declarar el evento», que es peor que estar vencido: sin evento **nada lo podía vencer nunca** **CAMBIA DE CONTENIDO el 2026-08-31, y no de dueño: ya no dice «no hay respaldo».** El mecanismo existe —`scripts/respaldo-almacen.sh` y `scripts/restaurar-almacen.sh`, con el diario en WAL fijado y comprobado por `StorePreparation`—. **Lo que falta es la política**: cada cuánto, dónde y cuánto se conserva. Sigue siendo del Product Owner, y mientras siga abierto **la copia existe si alguien la corrió**. Hallazgo `MI-01` de [`../../../Audit/Mesa-2026-08-31-B.md`](../../../Audit/Mesa-2026-08-31-B.md) | «Fase F de los cuatro restantes» (2026-08-11) · GeometriaFactory-Infrastructure · base `87fbb3b` |
| PD-05 | **El ADR que `Rules-Devops.md` §2.2 pide para apartarse del modelo de canales `preview` / `stable`, y que este proyecto de código no tiene.** Las otras tres bibliotecas lo anclan en su `ADR-06003`; las siete ADR de acá no cubren publicación ni canales, de modo que el apartamiento de [`Entornos-Deploy.md`](Entornos-Deploy.md) §1.1 se apoya hoy en el intake §17.1.P.7 · GeometriaFactory-Infrastructure y §13 y no en el instrumento que la regla nombra. **El apartamiento no está en duda; falta el instrumento** | La categoría 05 de este proyecto de código, emitiendo la ADR correspondiente | `05-Arquitectura-Tecnica/Arquitectura-Unidad-Entrega.md`, su próxima emisión | **Vigente.** No hubo emisión nueva de la 05 | «propagación del intake 1.22 y cierre de la Fase F» (2026-08-11) · GeometriaFactory-Infrastructure · base `aa154ef` |
...
| Sus gates corren **antes** de que la imagen del backend se construya | Un fallo de `QG-02001`, `QG-02002`, `QG-02004`, `QG-02005` o `QG-02006` detiene la cadena en el punto más barato posible, sin llegar al empaquetado | `05` §5 y la tabla de §2.1 |
...
| Sus gates corren **antes** de que la imagen del backend se construya | Un fallo de `QG-04001`, `QG-04002`, `QG-04004`, `QG-04005`, `QG-04006`, `QG-04008` o `QG-04009` detiene la cadena en el punto más barato posible, sin llegar al empaquetado | `05` §5 y la tabla de §2.1 |
...
| **Recibe la clave de firma, no la busca** | La canalización de la unidad desplegable tiene que proveerla por variable de entorno o archivo montado; **si no llega, el arranque falla con su condición declarada** en lugar de generar una al vuelo | `05` §5, fila de secretos; `QG-06012` |
