...
**Y el segundo artefacto que `Rules-Devops.md` §2.2 admite para este tipo no existe acá.** La tabla nombra un contrato de servicio versionado como artefacto secundario; el intake §17.1.P.3 · GeometriaFactory-Api declaraba hasta su versión 4.2 que **el versionado del contrato es el del ensamblado de contratos** y que **no hay versionado de rutas porque no hay clientes de terceros**; **desde el 2026-09-12 la superficie pública se versiona en la ruta por `/v{MAJOR}/`** ([`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md)), pero eso **no crea un artefacto de contrato publicable**: la versión viaja en la ruta de la misma imagen, y la descripción OpenAPI se genera de los puntos que existen. Una guía de publicación de contrato seguiría describiendo una entrega que nadie recibe por separado.
...
| Una transformación de esquema quedó mal | **Volver a la etiqueta no deshace el esquema del almacén**: el esquema sobrevive al reemplazo. Se corrige con otra transformación, y lo único que restituye datos es el **respaldo** — que **desde el 2026-08-31 existe**: `scripts/respaldo-almacen.sh` lo toma y `scripts/restaurar-almacen.sh` lo restituye, con el servicio detenido. **Hasta ese día esta fila remitía a un mecanismo que no existía en ningún árbol** (`MI-01` de [`../../../Audit/Mesa-2026-08-31-B.md`](../../../Audit/Mesa-2026-08-31-B.md)) | [`../../GeometriaFactory-Infrastructure/09-Devops/Estrategia-Versionado.md`](Estrategia-Versionado.md) §4 |
...
| 1.3 | 2026-09-12 | **§1 deja de afirmar «no hay versionado de rutas porque no hay clientes de terceros»** (tarea `BT-00027`). Remite a [`ADR-00010`](../05-Arquitectura-Tecnica/Adrs/ADR-00010-Version-En-La-Ruta-Solo-Major-Para-La-Superficie-Publica.md) (`/v{MAJOR}/`) y declara por qué la omisión del segundo artefacto se sostiene igual: la versión viaja en la ruta de la misma imagen. Un párrafo. Sube minor. |
