...
| **Apartamiento que lo autoriza** | [`ADR-14004`](../../../Producto/Adrs/ADR-14004-Item-Obligatorio-Sin-Objeto-Se-Declara-No-Aplica.md), que declara «no aplica» como tercera salida frente a un ítem obligatorio **sin objeto** en este producto. `Rules-Devops.md` §4.4 punto 2.b sólo ofrece dos, y omitir el ADR dejaba el apartamiento sin declarar |
| **Estado de ese apartamiento** | **`Aceptado`** — el Product Owner lo aprobó el **2026-08-26**, sin modificar su contenido. Hasta esa fecha esta fila declaraba que la figura se apoyaba en un instrumento sin aprobar; **ya no hace falta**, y con la aceptación el apartamiento **cuenta como decisión y no como omisión** (`Root-Rules.md` §11) |
...
| Modo de diario | **WAL, declarado por la fuente y FIJADO Y COMPROBADO POR EL PRODUCTO desde el 2026-08-31.** `StorePreparation.PrepareAsync` ejecuta `PRAGMA journal_mode=WAL`, **lee lo que el motor contesta** y detiene el arranque si no dice `wal` | Intake §17.1.P.4 · GeometriaFactory-Infrastructure. **Antes estaba declarado y no fijado**: `UseSqlite(cadena)` a secas deja el motor en `delete`, con lo cual la condición de consistencia del respaldo no se cumplía. Hallazgo `MI-01` de [`../../../Audit/Mesa-2026-08-31-B.md`](../../../Audit/Mesa-2026-08-31-B.md) |
...
| Condición | Con qué se cumple, desde el 2026-08-31 | Fundamento |
| --- | --- | --- |
| Se copia **el archivo con el diario activo**, y la copia es consistente. No se copia el archivo a mano mientras el proceso escribe | **`scripts/respaldo-almacen.sh`**, con `VACUUM INTO` y no `cp`: toma una instantánea transaccionalmente consistente **sin detener el servicio**. Copiar con el proceso escribiendo produce una copia que *casi siempre* funciona y falla el día que hace falta | Intake §17.1.P.4 · GeometriaFactory-Infrastructure |
| **El respaldo es el único mecanismo del producto para volver atrás sobre datos.** Volver a una etiqueta revierte el código y no el almacén, y el guion de restablecimiento **deja el almacén vacío** | **`scripts/restaurar-almacen.sh`**, que es la otra mitad. Exige el servicio detenido y **lo comprueba por el archivo** —`-wal` con contenido— y no por el proceso | `05` §5; [`Pipeline-CI-CD.md`](Pipeline-CI-CD.md) §7 |
| La copia **vive fuera del volumen que respalda**, o no protege del modo de falla más probable de un servidor domiciliario | El guion **se niega** si el destino es el directorio del almacén | **Decisión de esta categoría**, declarada como tal |
| **La copia sirve, y eso se comprueba y no se supone** | `PRAGMA integrity_check` sobre **la copia**, y si no da `ok` **la borra y sale 1**. Una copia corrupta es peor que ninguna: ocupa el lugar de la que falta | **Decisión de esta categoría**, incorporada el 2026-08-31 |
| **Restituir no puede causar una segunda pérdida** | El guion **no borra el almacén anterior: lo aparta con sello**, y verifica la copia **antes** de instalarla. Restituir se hace con miedo y de apuro; un guion que además destruye el estado actual convierte un error de elección de archivo en una segunda pérdida — la lección del **2026-08-15**, que `scripts/store-path.sh` dejó escrita | **Decisión de esta categoría**, incorporada el 2026-08-31 |
| **No se declara ninguna frecuencia, ninguna retención y ningún destino concreto** | **Sigue sin declararse, y es deliberado.** Los guiones exigen el destino como argumento y no traen ninguno por omisión | Ninguna fuente los da, y el intake §10 declara «sin plazo». Un número puesto acá se propagaría como si fuera del producto |
