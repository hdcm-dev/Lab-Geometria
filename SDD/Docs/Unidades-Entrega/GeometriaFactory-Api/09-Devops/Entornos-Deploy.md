...
| **Estado de ese apartamiento** | **`Propuesto`, esperando la aprobación del Product Owner.** Hasta que se acepte, esta declaración **se apoya en un instrumento que todavía no fue aprobado**, y se dice acá en lugar de escribirse como si ya autorizara. El precedente del destino es `ADR-14003`, que pasó a `Aceptado` por un acto explícito |
