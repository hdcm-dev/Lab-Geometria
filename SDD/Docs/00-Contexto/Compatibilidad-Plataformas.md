...
- [2. Matriz de compatibilidad](#2-matriz-de-compatibilidad)
  ...
  - [2.6 Resultado medido sobre el hosting real (PT-01.a, y la corrección a §2.4)](#26-resultado-medido-sobre-el-hosting-real-pt-01a-y-la-corrección-a-24)
    ...
    - [2.6.2 Por qué el hosting no ofrece WebSocket, y el entorno del sitio verificado en el panel](#262-por-qué-el-hosting-no-ofrece-websocket-y-el-entorno-del-sitio-verificado-en-el-panel)
...
#### 2.6.2 Por qué el hosting no ofrece WebSocket, y el entorno del sitio verificado en el panel

**§2.6 dejó medido que la negociación del circuito no ofrece WebSocket. Lo que faltaba era la causa, y la causa cambia qué se puede hacer al respecto.** Lo que sigue se **verificó el 2026-08-13 entrando al panel de la cuenta del hosting**; las credenciales viven en la ficha de proveedores externos y **no aparece acá ningún valor, usuario, host ni ruta**.

**El hallazgo, en una línea: WebSocket no es una carencia técnica del proveedor, es una característica que vende con el plan.**

| Qué se miró en el panel | Qué se encontró |
|---|---|
| La matriz de características por paquete | Tiene una fila literal **`WebSocket`**, marcada como **no incluida en el plan gratuito** e **incluida en los tres planes pagos** |
| Control de sanidad de esa lectura | La fila de **publicidad forzada** está **invertida**: presente en el gratuito, ausente en los pagos. Es lo que tiene que ocurrir si la matriz se está leyendo en el sentido correcto, y por eso se declara |
| Interruptor de WebSocket en el panel | **No hay ninguno.** El formulario del grupo de aplicaciones **no menciona WebSocket ni una vez**, y sus **tres** opciones ajustables están **bloqueadas con el aviso de mejorar el plan** |
| Base de conocimiento del proveedor | **Ningún artículo sobre WebSocket ni sobre SignalR** |

**Qué explica esto de lo que ya estaba documentado.** Las medidas que sostienen la ausencia estaban registradas y no tenían causa; ahora la tienen.

| Medida ya documentada | Qué la explica |
|---|---|
| El servidor de información **ignora** el pedido de ascenso y responde **200** en vez de **101** (intake §17.2.P.9 · GeometriaFactory-Web) | El módulo de WebSocket **no está habilitado para el sitio**, porque el plan no lo incluye. Un servidor que no tiene el módulo activo no completa el ascenso: atiende la petición como una petición corriente |
| Agregar `<webSocket enabled="true" />` al archivo de configuración del sitio **no cambió la oferta de la negociación** | Porque es una **decisión del proveedor por encima de la configuración de la aplicación**: nada que el artefacto declare habilita lo que el plan no incluye |
| La negociación devuelve **dos** transportes y no tres (§2.6) | Es la consecuencia visible de las dos filas de arriba, medida desde afuera |

**Qué lo habilitaría, y qué no.** Lo habilitaría **contratar cualquiera de los tres planes pagos de este proveedor, o mudarse a un anfitrión que ofrezca WebSocket**. **No** lo habilita ninguna línea del archivo de configuración del sitio, ninguna opción del panel —no existe— ni ningún cambio de código. Coincide con la salida que el intake §17.2.P.9 · GeometriaFactory-Web declara para el caso de que la decisión se reabra: un anfitrión que ofrezca WebSocket, y SignalR pasa a usarlo solo, **sin tocar una línea de código**.

**La línea de configuración se revirtió, y hay que decirlo.** `<webSocket enabled="true" />` está **probado que no hace nada** en este hosting y **daba la falsa impresión de que el asunto estaba resuelto**, así que **ya no está en el archivo de configuración del sitio**: el artefacto publicado declara el manejador, el ensamblado de la pieza pública y el modelo in-process, y **nada sobre WebSocket**. Lo que sostiene la sesión interactiva en producción no es una preparación del artefacto: es que **SignalR negocia y repliega solo**, que es la decisión registrada en el intake §17.2.P.9 · GeometriaFactory-Web.

**El entorno del sitio, verificado en el mismo panel.** Ninguno de estos datos se podía leer desde la dirección pública, y todos condicionan a la pieza pública.

| Qué | Valor verificado en el panel | Qué significa para el producto |
|---|---|---|
| Versión de .NET del sitio | **«.NET Core, todas las versiones»**, genérico: el panel no pide elegir un número | La ficha del producto del proveedor declara soporte **hasta .NET 10**, de modo que **`net10.0` está explícitamente soportado**. **Confirma** lo que §2.6 midió contra el hosting real, ahora también desde el lado declarativo |
| Sistema operativo | **Windows Server 2022** | La pieza pública es el **único** artefacto del producto que no corre sobre Linux. `CP-01` acota Linux a los **seis proyectos de código de servidor**, y esta fila es la contraparte medida de esa acotación |
| Servidor de información | **IIS 10** | Es quien está delante del proceso de la pieza pública y quien fija las capacidades del transporte, que es exactamente lo que §2.6 midió |
| Arquitectura del proceso | **32 bits**, y **no se puede cambiar en este plan** | Techo de memoria direccionable para el proceso de la pieza pública. Hoy el front es el esqueleto de la etapa `a` y no lo roza; conviene tenerlo escrito **antes** de que entre la etapa de visualización |
| Canalización | **Integrada**, tampoco modificable | Es la que corresponde al manejador que el artefacto publicado declara, y por eso no hubo que ajustar nada |
| HTTPS | **Configurado con certificado propio**, aunque el resumen del panel liste las direcciones como `http://` | La dirección pública que este documento interroga es `https://…` y responde **200** (§2.6). **El listado del panel no es evidencia de que no haya HTTPS**, y no debe leerse como tal |
| Cuotas del plan | **150 MB** de almacenamiento y **5 GB** de transferencia mensual | El paquete de la visualización viaja **dentro** de la publicación (`CP-05`), así que el almacenamiento es el límite que conviene vigilar cuando esa etapa entre. Hoy no hay medición de cuánto ocupa la publicación |
| Actividad mínima exigida | El plan gratuito **exige al menos cinco visitas por mes** o **da de baja el sitio** | **Riesgo nuevo que ninguna fuente preveía**, y no de plataforma sino de continuidad: un laboratorio que se usa por ráfagas puede pasar un mes entero sin actividad. Queda registrado como **`RG-07`** en [`Vision-Producto.md`](Vision-Producto.md) §8 |

**Qué no cambia por este hallazgo.** El semáforo de `PT-01.b` sigue siendo **amarillo estable** y por el mismo motivo: el repliegue a long polling está **ejercido y funcionando** (§2.4) y **medido sobre el hosting real** (§2.6.1). **Ninguna decisión de plataforma se modifica.** Lo que cambia es que la ausencia de WebSocket deja de ser un dato sin explicación y pasa a ser una **condición del plan contratado**, con una salida conocida y de costo comercial, no de rediseño.
...
| El hosting no sostiene el transporte de sesión interactiva por WebSockets | **Es el caso real, no una contingencia** (§2.6): el hosting **no ofrece WebSockets**, sólo `ServerSentEvents` y `LongPolling`. La alternativa declarada —repliegue a long polling— **es lo que corre en producción**, y es **aceptable**: no es motivo de rediseño. Está **ejercida y funcionando** desde §2.4. **La causa está verificada en §2.6.2: WebSocket es una característica que el proveedor vende con el plan, no una carencia técnica**, de modo que la única vía de habilitarlo es comercial —plan pago o anfitrión que lo ofrezca— y **ninguna configuración del artefacto lo consigue**. **La latencia percibida sobre el hosting real sigue sin medir**, y la de §2.4 es de bucle local y no se puede citar como la de producción | PT-01.b |
...
| Plataforma del hosting público | Web, Contracts | **Verificada sobre el hosting real el 2026-08-13: soporta `net10.0`.** La marca `[A VERIFICAR]` queda **resuelta** y la versión objetivo del front **no se bajó**. `PT-01.a` **pasa**: la ruta pública responde **200**. **Confirmado también desde el panel** (§2.6.2): la versión declarada del sitio es «.NET Core, todas las versiones» y la ficha del producto declara soporte hasta .NET 10 | Etapa `a`, PT-01.a, **medida**. Evidencia en §2.6, y el entorno del sitio en §2.6.2 |
| Transporte de sesión interactiva del hosting | Web | **Verificada sobre el hosting real el 2026-08-13: el hosting NO ofrece WebSockets.** La negociación devuelve sólo `ServerSentEvents` y `LongPolling`, de modo que **en producción la sesión interactiva va por long polling**. El repliegue está **ejercido y funcionando** desde §2.4, así que el escenario es el declarado aceptable y no obliga a rediseño. **La causa está verificada: es característica de plan** (§2.6.2). **La latencia percibida en producción sigue sin medir** | Etapa `a`, PT-01.b, **medida sobre la dirección pública** con la negociación del circuito. Evidencia y comandos en §2.6; la causa y el entorno del sitio, en §2.6.2 |
...
| 1.6 | 2026-08-14 | **Documenta la causa de la ausencia de WebSocket, que §2.6 había medido sin explicar, y el entorno del sitio.** Agrega **§2.6.2**, verificada el 2026-08-13 entrando al panel de la cuenta y sin exponer ningún dato del canal: **WebSocket no es una carencia técnica del proveedor, es una característica que vende con el plan** —hay una fila literal `WebSocket` en su matriz de características por paquete, **no incluida en el gratuito** e **incluida en los tres pagos**, con el control de sanidad de que la fila de publicidad forzada está invertida—, **en el panel no hay ningún interruptor de WebSocket** —el formulario del grupo de aplicaciones no lo menciona y sus tres opciones ajustables están bloqueadas con el aviso de mejorar el plan— y **la base de conocimiento del proveedor no tiene artículos sobre WebSocket ni sobre SignalR**. Con eso quedan **explicadas** las medidas ya documentadas: el servidor **ignora** el pedido de ascenso y responde **200** en vez de **101** porque el módulo no está habilitado para el sitio, y `<webSocket enabled="true" />` no cambió nada porque es **decisión del proveedor por encima de la configuración de la aplicación**; **esa línea se revirtió** y se deja escrito, porque estaba probado que no hace nada y daba la falsa impresión de que el asunto estaba resuelto. Registra **qué lo habilitaría**: plan pago o anfitrión que lo ofrezca, **nunca configuración ni código**. Agrega el **entorno del sitio verificado**: **Windows Server 2022**, **IIS 10**, proceso de **32 bits** no modificable en este plan, canalización **integrada** tampoco modificable, **HTTPS con certificado propio** pese a que el resumen del panel liste las direcciones como `http://`, versión de .NET declarada como **«.NET Core, todas las versiones»** con soporte hasta **.NET 10** en la ficha del producto —que **confirma** `net10.0` desde el lado declarativo—, y cuotas de **150 MB** de almacenamiento y **5 GB** de transferencia mensual. Deja registrada la **exigencia de al menos cinco visitas mensuales** del plan gratuito bajo pena de baja del sitio, y remite al riesgo **`RG-07`** que abre por eso `Vision-Producto.md` §8. **§4**, **§5** y **§6** reciben la remisión a §2.6.2 en las filas que ya hablaban del transporte y de la plataforma del hosting. **Ningún semáforo, ninguna plataforma y ninguna decisión de plataforma cambia**: `PT-01.b` sigue **amarillo estable**. Sube minor. | Ingeniero DevOps Senior + Deploy Engineer (AG-09) |
