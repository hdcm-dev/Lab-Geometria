...
>
> Copia archivada de `Alcance-Producto.md` en su versión **1.1**, tomada el 2026-08-09 por el orquestador SDD **antes** de despachar la propagación de la Fase B2 (`Master-Prompt.md` §8).
>
> - **Estado:** `Superado`
...
Están en el alcance declarado del producto, con prioridad explícita, y **no están comprometidas para el tramo `a` a `h`**. El intake las ubica en la etapa `i` y siguientes, que se planifican con la plantilla completa cuando `h` esté cerrada y demostrada (PRODUCT-INTAKE §4 y §15).
