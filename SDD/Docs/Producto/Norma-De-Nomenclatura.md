...
| Correo escrito como confirmación | `ConfirmationEmail` | Miembro de `AccountDeletionRequest` y de `AccountDeletionForm` | `Contracts CU-02` FA-01; `RN-07`, que hace de la confirmación escrita la condición de la baja **[el alcance se amplía en la 1.13, con el campo del diálogo de baja del panel; es el mismo concepto y por eso no lleva fila nueva]** |
...
| Dar de baja una cuenta desde la pieza pública | `DeleteAccountAsync` | Operación de `DataServiceClient` | `Definicion-Superficie-HTTP.md` §3, punto `A-08`; `RN-07`. El concepto es el `DeleteAsync` de §6.15.2 —y así lo conserva la **operación del panel**, que sí se llama `DeleteAsync`—; **acá el nombre se califica** por el mismo motivo que `ListAccountsAsync`: en este tipo conviven cinco salidas sobre el mismo recurso y una `DeleteAsync` desnuda no diría sobre qué |
| Resetear la contraseña de una cuenta | `ResetAccountPasswordAsync` | Operación de `DataServiceClient` | `Definicion-Superficie-HTTP.md` §3, punto `A-09`; `RN-12` |
...
| `Aprobar` | `Approve` | «Aprobar» | Valor de `WorkOutcome` | `Domain CU-10` §4 paso 5, que lo lleva a `Approved`; `RN-10` |
| `Rechazar` | `Reject` | «Rechazar» | Valor de `WorkOutcome` | `Domain CU-10` §4 paso 5, que lo lleva a `Rejected`; `RN-10` |
...
| Estado fuera del alcance del administrador | `StatusOutsideAdministratorScope` | Miembro de `Work` | `Domain CU-11` §4 paso 3 y §10, `RN-11`. **[decisión de la etapa `e`, declarada]**: el predicado de alcance se expone **como dato y no como método** para que la consulta del adaptador lo use tal cual; un método no lo traduce el motor de datos y terminaría copiado en la consulta, que es el segundo lugar donde `RN-11` podría decir otra cosa |
| Constituir el trabajo | `Create` | Operación de `Work` | `Domain CU-05` §4, el único camino de alta de un trabajo |
| Reeditar | `Edit` | Operación de `Work` **y** valor de `WorkOperation` | `Domain CU-05` FA-01; `RN-04`, que la acota al borrador. **Un concepto, un nombre** (corolario 1 de §6.1): la operación y el valor con el que se la consulta son lo mismo |
| Enviar | `Submit` | Operación de `Work` | `Domain CU-08` §4; `RN-05`, que decide entre `Draft` y `Submitted` **por las observaciones y no por la operación** |
| Aplicar el desenlace | `ApplyOutcome` | Operación de `Work` | `Domain CU-10` §4; `RN-10`. **Se escribe en la etapa `e` por `Domain BT-12` y NO se expone**: el punto `A-15` es de la etapa `h` |
| Resolver el acceso del alumno | `ResolveStudentAccess` | Operación de `Work` | `Domain CU-09`; `RN-03`, `RN-04`, `INV-02`, `INV-03` |
| Resolver el alcance del administrador | `ResolveAdministratorScope` | Operación de `Work` | `Domain CU-11`; `RN-11`, `RN-04`. Es el **contrato simétrico** del anterior, y no comparte ningún motivo con él |
...
| --- | --- | --- |
| `V-1` | **Verificable con la precisión de la 1.4** | La población es la del instrumento de §2.1 —tokens de región de código— y el rango es el glosario entero: **dieciséis tablas desde la 1.16** —eran doce en la 1.12, trece con §6.16 en la 1.13, catorce con §6.17 en la 1.14 y quince con §6.18 en la 1.15—. El único residuo es distinguir el identificador **declarado** del **citado**, y esa distinción ya está escrita: es la de §4.1 entre uso propio y reporte de fuente |
