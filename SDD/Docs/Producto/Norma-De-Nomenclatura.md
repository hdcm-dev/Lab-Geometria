...
| --- | --- | --- | --- |
| 1.8 | 2026-08-14 | **Agrega tres filas a §6.13.2 por la rebanada del cambio forzado de contraseña, y no renombra nada.** La trae la resolución del Product Owner de `PRODUCT-INTAKE` **1.34** —**con qué se autentica el cambio forzado**—, que hizo alcanzable una pantalla que dos fuentes contradictorias dejaban fuera de alcance, y que al construirse escribió **tres identificadores** que ninguna de las diez tablas contaba: `ExecuteWithCurrentCredentialAsync`, la segunda operación del caso de uso de cambio; y `PasswordChangeEmail` y `BeginPasswordChange`, los dos del custodio de la sesión de la pieza pública. Entran por el **corolario 4 de §6.1**, **fuera de los 155** y con su marca. **§6.13.2** pasa de **75 a 78** miembros, **§6.13** de **106 a 109** filas y **§6.2** actualiza la fila del recuento. **El correo de la solicitud de cambio de contraseña NO lleva fila nueva**: `Email` ya está en §6.13.2 declarado como propiedad de los contratos de cuenta, y las dos clases de prueba nuevas tampoco, por la regla de §6.13.1 —una clase de prueba nombra el concepto que ya tiene fila, acá el curso de cambio forzado de `OwnCredentialForcedChange`, que está en §6.12—. **Ningún código de condición lleva fila nueva, otra vez: los que la rebanada usó ya estaban todos.** No renombra nada del corpus, no toca las nueve tablas anteriores y no mueve ninguna cifra de §2 ni el total de 155. | Etapa `c` (rebanada del cambio forzado) |
