...
> | Columna `Estado` de la tabla de §3.1 | El mismo enum, acotado por `Rules-Documentacion.md` §3 punto 3 a `Planificado`, `Vigente` y `Potencialmente desactualizado` | Lo mismo, para cada artefacto del plan | `Vigente` |
...
Estados admitidos en la columna `Estado` de §3.1 —y en la tabla de artefactos de cada README de proyecto de código—: `Planificado`, `Vigente` y `Potencialmente desactualizado` (`Rules-Documentacion.md` §3 punto 3). Es el **mismo eje** que el `status` del frontmatter y **no** el de la cabecera de prosa, que declara aprobación; la tabla del encabezado de este documento separa los dos.

**Hoy están en `Planificado` todos los artefactos salvo los ocho `README.md` de la categoría, que son los únicos redactados y por eso los únicos `Vigente`.** Ningún `Planificado` tiene fecha de última revisión, porque ninguno se redactó.
...
**Tres de los siete ya están emitidos, y los cuatro que faltan tienen ahora un consumidor esperándolos.** La bitácora nace con siete eventualidades reales de la implementación de los samples, y **sus destinos de triaje son `Guia-Contenedor`, `Guia-Contribucion`, `Runbook-Operacion` y `Conceptos-Fundamentales` del visor** — tres de los cuatro todavía `Planificado`. Es lo que hace que la propagación quede **pendiente y no omitida**: el destino está asignado, que es lo que `Rules-Documentacion.md` §0.6 exige, y el documento que lo recibe es trabajo de esta misma categoría.
...
Una fila por proyecto de código, con el detalle desplegado en el README de su propia categoría 11, bajo `Unidades-Entrega/<Nombre-Unidad-Entrega>/11-Documentacion/`.
...
El recuento incluye el `README.md` de cada categoría 11, que es obligatorio siempre. Siete de esos ocho README —los de los proyectos de código— se emiten con este mismo plan. **Los ocho quedan `Vigente` en el eje de contenido** —están redactados— **y `Propuesto` en el eje de aprobación**, que son los dos ejes que el encabezado de este documento declara; los **64** artefactos restantes quedan en `Planificado`.
...
| --- | --- | --- |
| ~~Cuántas aristas de compilación tiene el producto, siete u ocho~~ **CERRADO el 2026-08-31: ocho, de dos clases** —siete referencias de proyecto y un activo de construcción—. El diagrama de contenedores puede dibujarlas sin salvedad | `Vision-General-Sistema`, en su diagrama de contenedores | — |
...
| --- | --- | --- |
| 1.0 | 2026-08-11 | Emisión inicial en la Fase H. Es el **Momento 1** del modelo de documentación viva: el índice del cuerpo documental de entrega, **sin contenido redactado**. Declara la matriz de ruteo con **quince** entradas de actor por intención; los **siete** artefactos de nivel producto y los **sesenta y cinco** repartidos entre los siete proyectos de código, **setenta y dos** en total, todos en estado `Planificado` salvo los ocho README de categoría, que quedan `Vigente`; el gating aplicado con sus **cinco** clases de omisión, cada una contra el flag o el tipo D8 que la produce; el orden de lectura para **cuatro** roles; la cadencia de actualización anclada a las etapas del intake; y los **seis** puntos abiertos heredados que este plan **no resuelve**, con el documento de 11 que va a tener que absorber cada uno. **Autor:** Technical Writer / Documentation Lead (AG-11) |
...
| 1.2 | 2026-08-17 | **Migración normativa 8.11 → 9.9, fase M4**, fila 7 del `Audit/Plan-Migracion-8.11-a-9.9.md`. La ruta de §3.2 pasa de `Proyectos/<Nombre-Proyecto-Codigo>/11-Documentacion/` a `Unidades-Entrega/<Nombre-Unidad-Entrega>/11-Documentacion/`, el layout vigente desde la 8.0. Ningún artefacto planificado, rol ni tipo D8 de la tabla cambia. Estado anterior archivado en `_legacy/2026-08-17/`. Sube **minor**. |
| 1.3 | 2026-08-30 | **Tres artefactos de nivel producto pasan de `Planificado` a `Vigente`**: `Bitacora-Eventualidades.md` 1.0, `Contrato-Agentes.md` 1.0 y el `AGENTS.md` de la raíz, derivado del contrato. Cierran los hallazgos **`I-01`** y **`I-02`** del incremento `I-1` de la Fase I —el segundo era **el único bloqueante**: sin el contrato, el paso 4 de la fase no se podía ejecutar en ninguna corrida—. **La bitácora nace con siete entradas reales y no vacía**, de la implementación de los dieciséis samples entre el 2026-08-27 y el 2026-08-30: emitir un archivo vacío habría cerrado el hallazgo sin capitalizar nada, que es el instrumento ceremonial que `Rules-Documentacion.md` §0.6 evita. §3.1 suma la nota de que **los destinos de triaje de esas siete son cuatro documentos, tres de ellos todavía `Planificado`**, de modo que la propagación queda pendiente y no omitida. **Se corrige además la cabecera, que declaraba `1.1` mientras la última fila de esta tabla ya era la `1.2`.** Sube **minor**: emite artefactos que el plan ya declaraba y no cambia ninguna decisión ni ningún recuento. |
| 1.4 | 2026-08-31 | **Cierra el primero de los tres puntos abiertos heredados que esta categoría listaba: las aristas de compilación son OCHO, de dos clases.** Verificado el 2026-08-31 contra los seis `.csproj` y `scripts/build-visor.sh`; `PRODUCT-MANIFEST` §2 tenía razón desde la emisión 1.0. **`Vision-General-Sistema` puede dibujar su diagrama de contenedores sin salvedad**, rotulando `Visor → Web` como activo de construcción y no como referencia de proyecto. **Y se corrige de paso un defecto de esta cabecera**: declaraba versión **1.2** mientras el control de cambios ya tenía fila **1.3**. El número de la cabecera se había quedado atrás en la emisión anterior. Se emite con `PRODUCT-MANIFEST` **5.4** y `Vista-Producto.md` **1.9**. **Ningún artefacto de esta categoría cambia de estado.** |
