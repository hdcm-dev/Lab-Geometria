...
| `CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` | Una cuenta con contraseña provisoria sin cambiar —producida **por la habilitación (RN-08016) o por el reseteo (F-26)**— intenta ingresar **o cualquier otra operación** | Texto neutro con motivo, sin detalles. Handoff al contrato de cambio de contraseña de CU-08008. **Es un solo código para todas las operaciones bloqueadas y para los dos orígenes de la marca**, porque el trabajo que le queda al consumidor es siempre el mismo (RN-08013, RN-08016, INV-09) |
| `CONTRATO_RESETEO_NO_APLICABLE_A_LA_CUENTA_DE_ADMINISTRADOR` | Se pide el reseteo de contraseña sobre la cuenta con papel `Administrador` | Texto neutro, sin detalles. Terminación controlada: el contrato no ofrece camino alternativo, y el cambio de la propia contraseña es CU-08002 FA-02 |
| `CONTRATO_OPERACION_EXCLUSIVA_DEL_ADMINISTRADOR` | Quien pide una operación reservada al administrador **fuera del desenlace de un trabajo** —gobernar las cuentas de la comisión (CU-08002), ver el listado de la comisión (CU-08004) o resetear la contraseña de un alumno (CU-08008)— no tiene ese papel | Texto neutro, sin detalles. Terminación controlada **sin escritura** |
| `CONTRATO_ESTADO_NO_PERMITE_MODIFICAR` | Se pide enviar o reeditar un trabajo que está en `Pendiente`, `Finalizado` o `Rechazado`, y que por lo tanto es de sólo lectura (CU-08003) | Texto neutro que declara el estado actual, sin detalles. Terminación controlada **sin escritura** |
...
- **El conjunto cerrado tiene diecisiete códigos.** Es la unión de los que declaran los ocho casos de uso. Los dos últimos en entrar son `CONTRATO_OPERACION_EXCLUSIVA_DEL_ADMINISTRADOR` —declarado por CU-08002, CU-08004 y CU-08008 además de acá— y `CONTRATO_ESTADO_NO_PERMITE_MODIFICAR` —declarado por CU-08003—, incorporados por decisión del Product Owner (`PRODUCT-INTAKE` **1.29** §17.4 P.3) y **emitidos formalmente** por [`../../05-Arquitectura-Tecnica/Contratos-Abstractions.md`](Contratos-Abstractions.md) §5.1. **Ninguno de los dos recicla un identificador retirado**, y los dos códigos vecinos no cambian de enunciado: `CONTRATO_DESENLACE_EXCLUSIVO_DEL_ADMINISTRADOR` sigue acotado a aprobar y a rechazar, y `CONTRATO_ESTADO_NO_PERMITE_ELIMINAR` sigue acotado a la eliminación.
- **La emisión anterior fue la única en la que el conjunto se achicó**, de diecisiete a quince. Salieron dos y no entró ninguno: `CONTRATO_CONTRASENA_NO_ESTABLECIDA`, de CU-08001 y CU-08002, y `CONTRATO_RESETEO_NO_APLICABLE_A_CUENTA_SIN_CONTRASENA`, de CU-08008. **Fue la primera y única vez que el conjunto se achicó**, y el motivo no fue una simplificación sino la imposibilidad de sus causas: con **RN-08016** (`PRODUCT-INTAKE` 1.13 §4.1) habilitar produce y fija la contraseña provisoria, de modo que ninguna cuenta llega a estar habilitada sin contraseña y el reseteo sobre una cuenta sin contraseña la fija en lugar de rechazarla. **Ninguno de los dos identificadores se recicla.** Y sigue sin entrar el que podría haberse esperado: la cuenta no habilitada no rechaza el reseteo, porque el Product Owner decidió que resetear no exige que la cuenta esté habilitada (RN-08015), de modo que esa causa no existe y no recibe código. **Las señales declaradas siguen siendo tres**, las de §6.1 de CU-08003, CU-08004 y CU-08005: el cambio de contraseña pendiente **sí** impide la operación pedida, de modo que es un error transportado y no una señal, y el conjunto de señales no se toca.
