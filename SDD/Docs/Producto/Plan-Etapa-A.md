...
1. Los **tres puertos que el intake sí declara** son `IRepositorioTrabajos`, `IValidadorFiguras` e `IRelojDelSistema` (intake §13, §14 y §17.2.P.1). Están en castellano y llevan `I`. **Sigue siendo cierto como reporte de la fuente, y ya no es fundamento:** el glosario de la norma §6.3 los lleva a `IWorkRepository`, `IFigureValidator` e `ISystemClock` en el tramo `R-2`, que toca el intake por ser la fuente (norma §4.1 punto 1).
2. Los **campos del modelo de datos que el intake nombra literalmente** son `HashContrasena` —sin eñe— (§17.1.P.5) y `JsonOriginal` (§17.3.P.4). Están en castellano, sin tildes, en `PascalCase`. **Ídem:** la norma §6.5 los lleva a `PasswordHash` y `OriginalJson`, también en `R-2`.
