...
| --- | --- | --- |
| `R-01` | ~~**`X-1`: siete u ocho aristas de compilación.** El manifiesto declara ocho en §2, dibuja siete en §3 y valida siete en §4~~ **CERRADO el 2026-08-31: son ocho.** | ~~Decide si el archivo de proyecto de `Api` declara la referencia a `Application` o la recibe transitivamente. **Se materializa el primer día de la etapa `a`** y hoy no tiene desenlace~~ **Se materializó y nadie lo registró**: `GeometriaFactory.Api.csproj` declara la referencia **directa**, tal como §2 del manifiesto pide. El riesgo se resolvió el primer día de la etapa `a` —cerrada el **2026-08-13**— y la fila siguió abierta **dieciocho días** porque el desenlace vivía en un archivo de proyecto y no en un documento |
