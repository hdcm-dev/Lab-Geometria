...
- [5. Cómo se verifica el cierre de la etapa](#5-cómo-se-verifica-el-cierre-de-la-etapa)
  ...
  - [5.2 El estado de cierre completo, y qué falta para medir `PT-01.a`](#52-el-estado-de-cierre-completo-y-qué-falta-para-medir-pt-01a)
...
| 3 | `PT-01.a`: la dirección pública responde correctamente | Publicación del front al hosting y comprobación de la dirección pública. **Si no pasa, la salida declarada es bajar la versión objetivo del front y no la del backend**. **Sin medir al 2026-08-13**, y es el único de los ocho que lo está: §5.2 dice qué falta, quién lo provee y qué parte del camino sí quedó comprobada | Flujo de publicación; `V-4` |
| 4 | `PT-01.b`: el transporte de la sesión interactiva está **medido y su resultado documentado, incluido el repliegue si ocurre** | Medición con semáforo. **Sólo el peor resultado obliga a cambiar el modelo de front; un repliegue de transporte no es motivo de rediseño**. **Medido en el entorno de desarrollo el 2026-08-13, semáforo verde**: el resultado y su evidencia están en [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §2.4. Sobre el hosting sigue sin medir, y §5 de ese documento dice con qué comando | `Web BT-04` |
| 5 | `PT-01.c`: **veinte minutos** de navegación continua sin que el proceso recicle la sesión, y **reconexión funcional** al cortar y restablecer la red | Recorrido cronometrado sobre el front publicado. Es el peor escenario y **no tiene mitigación en el código**. **Medido en el entorno de desarrollo el 2026-08-13: las dos mitades pasaron** —20 min 45 s sobre un único circuito, y reconexión al **mismo** circuito tras cortar la red—, con la evidencia en [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §2.5. **No queda cerrado**: sobre el hosting sigue sin medir, y el reciclado del proceso sólo se puede observar ahí | `Web BT-04` |
...
| 8 | Está verificado que **la sesión interactiva no llega al servicio de datos** | Inspección de red desde el navegador durante el recorrido: **cero** peticiones del navegador hacia el servicio de datos. Es `RA-01`. **Verificado corriendo el 2026-08-13**: §5.1 dice con qué método y con qué control | Intake §17.5.P.3, fila de WebSockets, que lo declara criterio de aceptación de la etapa `a` |
...
| --- | --- | --- |
| 4 · `PT-01.b`, transporte de la sesión interactiva | **Medido en el entorno de desarrollo contenido, con navegador real. Semáforo verde** | [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §2.4, con la oferta de la negociación, el transporte elegido, el repliegue provocado y la latencia de los dos |
| 8 · `RA-01`, la sesión interactiva no llega al servicio de datos | **Verificado corriendo, con instrumento y con control** | Acá abajo |
| 5 · `PT-01.c`, estabilidad de la sesión y reconexión | **Medido en el entorno de desarrollo contenido, con navegador real. Las dos mitades del criterio pasaron. Parcial: sobre el hosting sigue sin medir** | [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §2.5, con la duración real de la corrida, la identidad del circuito, el corte de red y la reconexión |
| 3 · `PT-01.a` | **Sin medir, y no se puede medir sin el hosting.** Depende de la publicación, que la etapa todavía no hizo | §5.2, que declara qué falta, de quién depende y qué tramo del camino sí quedó comprobado corriendo |
...
**Qué queda fuera del alcance de lo medido para `PT-01.c`, y por qué importa.** El criterio vigila un riesgo del **hosting gratuito**: que el proceso recicle la sesión. Eso **no se puede observar en el entorno de desarrollo contenido**, donde el proceso es propio y nadie lo recicla. Lo que la medición del 2026-08-13 sí cierra es la otra mitad del riesgo: **ni el producto ni su montaje reciclan la sesión por su cuenta a lo largo de veinte minutos**, y **la reconexión ante un corte de red real funciona sobre el mismo circuito**. `R-07` de §7 sigue en pie sin ablandarse.

### 5.2 El estado de cierre completo, y qué falta para medir `PT-01.a`

**Siete de los ocho criterios propios están medidos; uno no.** Esta sección deja el estado entero en un solo lugar, con dónde vive la evidencia de cada uno, y desarrolla el único que falta.
...
| 3 | **`PT-01.a`: la dirección pública responde correctamente** | **SIN MEDIR** | No hay hosting publicado. Ver el resto de esta sección |
| 4 | `PT-01.b`: transporte medido, repliegue incluido | **Medido en el entorno de desarrollo contenido** | [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §2.4 |
| 5 | `PT-01.c`: veinte minutos y reconexión | **Medido en el entorno de desarrollo contenido. Parcial**: el reciclado del proceso sólo se observa en el hosting | [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §2.5 |
...
**Qué es exactamente `PT-01.a`, leído en las fuentes y no interpretado.** El intake §17.6.P.10 le da criterio y umbral: «el front publicado arranca y sirve la página inicial», umbral **200 en la URL pública**, y si no pasa, la salida es **bajar la versión objetivo del front y no la del backend**. §17.6.P.9 dice **qué incógnita resuelve**: la versión de plataforma que soporta el hosting está **[A VERIFICAR]** en la fuente, «es PT-01.a», y **se resuelve midiendo, no decidiendo**. **Quién la mide**: el paso **8** del flujo de publicación, que es el propio flujo y no una persona ([`../Proyectos/GeometriaFactory-Web/09-Devops/Pipeline-CI-CD.md`](../Proyectos/GeometriaFactory-Web/09-Devops/Pipeline-CI-CD.md) §2.1 y `QG-03`). **Sobre qué se mide**: §9 de ese mismo documento lo declara sin ambigüedad —sobre el front **ya publicado** en el hosting, **no sobre una ejecución local**—, y agrega la consecuencia de orden: `PT-01` exige que **el flujo de publicación exista y haya corrido** antes de que se pueda medir.
...
**Qué tiene que existir para poder medirla, quién lo provee y en qué orden.** Nada de esto lo puede aportar quien construye el código:

| # | Qué tiene que existir | Quién lo provee | Por qué, con su fuente |
| --- | --- | --- | --- |
| 1 | **Cuenta en el hosting público** con servidor de información, transporte seguro y dominio | El Product Owner. Se contrata y se configura **por fuera del repositorio**: no hay infraestructura declarativa | Intake §17.6.P.9; [`../Proyectos/GeometriaFactory-Web/09-Devops/Entornos-Deploy.md`](../Proyectos/GeometriaFactory-Web/09-Devops/Entornos-Deploy.md) §3 |
| 2 | **Los secretos del repositorio que el flujo consume**, nombrados por su función: la dirección base del servicio de datos, las credenciales y el destino del canal de publicación, y la dirección pública que el paso 8 interroga. **Ningún valor vive en el repositorio ni en esta cadena de documentos** | El mismo, cargándolos en el almacén de secretos del repositorio | Intake §17.6.P.5; [`../Proyectos/GeometriaFactory-Web/09-Devops/Guia-Publicacion-Front-Ftp.md`](../Proyectos/GeometriaFactory-Web/09-Devops/Guia-Publicacion-Front-Ftp.md) §1 |
| 3 | **Una corrida del flujo de publicación entero**, por fusión a la rama principal con cambios bajo las rutas del filtro o por disparo manual | El mismo, que es quien fusiona y quien aprueba | Intake §17.6.P.7; `Guia-Publicacion-Front-Ftp.md` §2 |
| 4 | **Que la dirección que el paso 8 interroga corresponda a una ruta que el front sirva.** En la etapa `a` la pieza pública tiene **una sola ruta**, la página de estado: la raíz **no está servida todavía**, y las rutas navegables son de la etapa `b` (§6) | El mismo, al fijar el valor del secreto de la dirección pública | Comprobado corriendo, más abajo; [`../Proyectos/GeometriaFactory-Web/09-Devops/Guia-Publicacion-Front-Ftp.md`](../Proyectos/GeometriaFactory-Web/09-Devops/Guia-Publicacion-Front-Ftp.md) §3 |

**El orden es el de la tabla y no es arbitrario**: sin cuenta no hay destino, sin secretos el flujo se detiene en el paso 6 —y lo hace a propósito—, y sin corrida no hay nada que interrogar. **La medición de `PT-01.a` es la salida del paso 8 de esa corrida**, y su registro, con la hora, es lo que el informe de cierre adjunta.

**Y hay dos criterios más que quedan a la espera de lo mismo.** `PT-01.b` y `PT-01.c` están medidos en el entorno de desarrollo contenido y **sobre el hosting siguen sin medir**; [`../00-Contexto/Compatibilidad-Plataformas.md`](../00-Contexto/Compatibilidad-Plataformas.md) §5 declara con qué comando se completarían. Los tres dependen del mismo insumo, de modo que **la publicación no destraba un criterio sino tres**.

**Qué se comprobó corriendo el 2026-08-13, sin hosting y sin ningún secreto.** No mide `PT-01.a`; mide que **el camino que la llevará hasta ahí está bien formado**. Todo corrió sobre una copia de trabajo del árbol, con las dos cadenas de herramientas dentro de contenedores, y el árbol del repositorio quedó intacto:
