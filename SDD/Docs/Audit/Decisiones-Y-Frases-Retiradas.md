...
| `DF-09` | **El reset de `box-sizing` no alcanza a la mayoría de la página**: medido en el navegador, **450 de 533 elementos** computan `content-box` con el reset global presente y correcto en `app.css` §Base, y la cadena de ancestros en `border-box` hasta el punto del corte | Cambiar el reset a lo ancho del producto **puede mover cualquier disposición**, y eso no se hace de paso. La causa **no está establecida** y afirmarla sin medirla sería el error que este producto viene persiguiendo | Que aparezca un segundo defecto visible de la misma familia, o que se toque la hoja por otro motivo | Equipo, con la categoría 03 |
| `DF-08` | ~~**Los tramos `failed` y `rejected` del aviso de reconexión quedan sin texto**~~ **REABIERTO Y ACOTADO el 2026-09-01: la decisión de no mostrarlos era un DEFECTO, no una prudencia.** El marco cubría ese caso y se lo quitamos; el resultado fue el «vencimiento silencioso» que el wireframe prohíbe (`R-1` de la mesa). Los dos tramos **ya muestran** un texto derivado del principio de §5, con su salida. **Lo que sigue diferido es el texto EXACTO** — cuando los reintentos se agotan o el servidor rechaza el circuito, no se muestra nada del producto y el marco tampoco puede | La maqueta aprobó **un solo** texto, el de reintento. Mostrarlo cuando los reintentos se agotaron **diría algo falso**, y escribir otro sería inventar castellano de producto | Que la maqueta apruebe el texto de los otros dos tramos | Product Owner, con la categoría 03 |
