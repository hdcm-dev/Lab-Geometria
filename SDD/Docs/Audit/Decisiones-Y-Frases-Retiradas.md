...
| `DF-08` | **Los tramos `failed` y `rejected` del aviso de reconexión quedan sin texto** — cuando los reintentos se agotan o el servidor rechaza el circuito, no se muestra nada del producto y el marco tampoco puede | La maqueta aprobó **un solo** texto, el de reintento. Mostrarlo cuando los reintentos se agotaron **diría algo falso**, y escribir otro sería inventar castellano de producto | Que la maqueta apruebe el texto de los otros dos tramos | Product Owner, con la categoría 03 |
