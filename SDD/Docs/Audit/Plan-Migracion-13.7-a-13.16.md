...
**Estado:** **Aprobado por el Product Owner el 2026-09-13** (EXP-0001 actuación 011, punto A; el lote de la actuación 010 lo nombró «1.2» y la aprobación asienta el 1.3, que era el vigente en el árbol desde `0f26c2b`: la diferencia son `M6-14` y `M6-15`, cuyos efectos —puntos D y E— sí estaban en el lote). **Aplicado** en `c14bf3b` (M2 a M4, E-022) y **cerrado** con M5 (procedencia a 13.16, manifiesto 7.1). Lleva los veredictos de [`Mesa-2026-09-13.md`](Mesa-2026-09-13.md) y el estado al cierre de cada fila en §4.1
...
|---|---|---|---|---|---|---|---|---|
| **PM-01** | Intake 4.6 → **5.0** | Plantilla 3.5 → 3.6; `Intake-Rules` 4.2 → 4.3 | §13.2 marca y pertenencia por la arista; grafo y orden con dos clases; §13.3 perfil por ecosistema y puntos 2 y 3 como decisión del destino; §16.1 una fila por sample por el D8 de la unidad, filas anteriores transpuestas; §16, §17.2, §19, §23 y trazabilidad sin «excepción» ni D8 por proyecto; cabecera 3.6. **No** toca `visor.bundle.js` (lote) | Revisar, con escritura estructural, caso (b), major | Documento de origen (§16 para la forma de entrada de cada sample); documento hermano (manifiesto §2.A–§2.C, `Web ADR-10008`, y el `ejemplo-*.md` de `10-Examples` que gobierna cada sample, para su nivel); **para los tres samples de `Contracts` no existe `ejemplo-*.md` (`DD-3`) y el nivel sale del `README.md` del propio sample (`samples/contracts/*/README.md`, «Nivel:»), que es la única declaración que hay: se declara como fuente fuera de las tres de §2.1 (`M6-25`, 1.4)** | `GeometriaFactory.sln` y `samples/*/*.csproj` (E-014), E-016 | M2 | `P-01` |
...
| **PM-06** | 8 documentos, **118** filas de ítems diferidos; más los **8** ítems verticales de `CU-08001` a `CU-08008` (`M6-01`): universo **126**. Comando ampliado (`M6-15`): `git grep -n "4 · En qué evento se cierra" b9675d8 -- SDD/Docs ':!*/_legacy/*'` → 12: los 8 `CU`, `Audit/Mesa-2026-08-27.md` (PM-11), dos inventarios de `Audit/` (registro) y **`Producto/Adrs/ADR-14005…md` :185**, que no entra al universo: el ADR está **RETIRADO** desde el 2026-08-29 y su ítem quedó superado por el tramo `R-4` (`Audit/Inventario-Renumerado-R-4-2026-08-29.md`); es registro | `Root-Rules` 8.7 §12.2 punto 5; `Migracion-Rules` §4.8/§4.9 | Sexta columna «Ciclo de origen» en las 118 filas: **95 derivadas** —32 de ellas elegidas por la regla de linaje entre más de un candidato, E-020— y **23 no derivables**; campo 5 en los 8 bloques verticales, derivados del alta `aa3abd3`. Clasificación §4.8 de las **20 abiertas** (98 cerradas son registro) —**y, en 1.4, de los 8 ítems verticales de los `CU`, todos abiertos y con ciclo `aa3abd3`: hueco del ciclo, E-015b (`M6-22`); universo abierto 28: 23 del ciclo, 0 de norma posterior, 3 sin ciclo, 2 elevadas**—, con el tema anclado a inicio de palabra (`M6-14`): 15 del ciclo, 0 de norma posterior, 3 sin ciclo, **2 elevadas** (`Product-Backlog.md` de Api :749 `PA-09` y `Pipeline-CI-CD.md` de Api :597 `PD-04`: van al lote, puntos D y E) | Revisar | Documento de origen | E-006g/h (y las cuentas anteriores E-006, E-006e), E-015, E-020 | M4 | `P-06`, `P-06b` |
...
| **PM-11** | Deuda de `Mesa-2026-08-27.md` §8 (cabeceras con versión citada) | `Root-Rules` §10 R1 | Evento: la M4 de esta migración. **Esta M4 no escribe** (D-1): el evento no ocurre; P-03 y P-04 aplican la corrección a sus cabeceras y E-011 mide el resto. **Cierre (1.4, `M6-19`):** con la aprobación de la actuación 011 esta M4 **sí escribió** y el evento **ocurrió**; se cierra en forma parcial y declarada (veredicto 21 de la mesa): 2 de 118 cabeceras corregidas, las 116 restantes en `DD-8` de `Mesa-2026-09-13.md` §8, con evento reasignado, y ofrecidas al Product Owner en el lote de cierre (actuación 013, punto F) | — | — | E-011 | M4 | `P-03`, `P-04` |
| **PM-12** | Resto del corpus | Reglas sin cambio o de proceso | — | No tocar | — | — | — | — |

### 4.1 Estado de cada fila al cierre (1.4, 2026-09-13)

| Fila | Estado al cierre | Evidencia |
|---|---|---|
| PM-01 | **Escrita**: intake 5.0 (`c14bf3b`), snapshot `Intake/_legacy/2026-09-13/…-v4.6.md` | E-022 (1), (2), (4) |
| PM-02 | **Escrita**: manifiesto 7.0 re-derivado del intake 5.0 (`c14bf3b`); 7.1 por M5 | E-022 (6) |
| PM-03, PM-04 | **Escritas**: `Vista-Producto.md` 1.11, `Pipeline-Producto.md` 1.9; las dos conductas del build con evidencia (E-012, E-023) | E-022, E-023 |
| PM-05 | No tocar, cerrada con evidencia | E-014 |
| PM-06 | **Escrita**: 118 filas con ciclo de origen y 8 bloques verticales con campo 5; 28 abiertas clasificadas, 2 elevadas con default aplicado (lote 010 D y E sin respuesta) | E-006g, E-015, E-015b, E-020 |
| PM-07 | No tocar, cerrada con evidencia | E-013 |
| PM-08 | **Escrita**: `ADR-14001` a `14004`, sólo contador, campo 7 y fila de control; campos 1 a 5 idénticos al snapshot | E-022 (4); audit de cierre §4 |
| PM-09 | Cerrada: vivas re-expresadas; ocurrencia 9 en `DD-6`; 15 ocurrencias de registro intactas | E-022 (9) |
| PM-10 | **Hecha (M5)**: procedencia del manifiesto a 13.16 con la cadena completa; manifiesto 7.1 | E-002; manifiesto §1.1 |
| PM-11 | **Cerrada en forma parcial y declarada**: 2 de 118; `DD-8` | E-011; `Mesa-2026-09-13.md` §8 |
| PM-12 | No tocar | — |

**Ninguna fila queda sin resolver.** Es la condición del paso 1 de `Master-Prompt-Migracion.md` §9 para escribir la procedencia.
...
| 9 | `11-Documentacion/README.md` :196 | Fila cerrada en tabla viva | **Viva (D-3)**: corresponde agregar el nombre vigente; **no está en ninguna propuesta** porque ese documento es de una regla sin cambio: queda en `DD-6` de `Mesa-2026-09-13.md` §8 |
...
|---|---|---|---|
| 1.4 | 2026-09-13 | **Cierre.** Estado: aprobado (actuación 011), aplicado (`c14bf3b`) y cerrado con M5. §4.1 nuevo con el estado de cada fila al cierre. PM-11 declara que el evento de la deuda de cabeceras ocurrió y se cierra en forma parcial (`M6-19`, `DD-8`); PM-06 suma la clasificación de los ocho ítems de los `CU` (`M6-22`, E-015b); PM-01 declara la fuente del nivel de los tres samples de `Contracts` (`M6-25`). | Orquestador de migración normativa SDD |
| 1.3 | 2026-09-13 | **Correcciones de la segunda ronda de M6**: la clasificación §4.8 ancla el tema a inicio de palabra y `PD-04` de Api pasa a elevada (`M6-14`, 15/0/3/2); el comando del universo de huecos se amplía a `SDD/Docs` y declara el ítem de `ADR-14005` como registro de un ADR retirado (`M6-15`). | Orquestador de migración normativa SDD |
| 1.2 | 2026-09-13 | **Correcciones de la primera ronda de M6.** PM-06 suma los ocho ítems diferidos verticales de `CU-08001` a `CU-08008` que el inventario de cabecera tabular no había visto (`M6-01`), la clasificación §4.8 con su paso 2 corrido sobre las 20 filas abiertas (`M6-02`: 16 del ciclo, 1 elevada) y las 32 filas elegidas por linaje (`M6-07`); PM-01 nombra la fuente del nivel y de la entrada de cada sample (`M6-09`); la ocurrencia 9 queda en `DD-6` (`M6-08`). | Orquestador de migración normativa SDD |
