...
Acá vive el conjunto cerrado de códigos del contrato: **17 vivos sobre 20 emitidos**, con 3 retirados que no se reciclan. Los dos últimos en entrar son `OPERATION_ADMIN_ONLY` y `STATE_FORBIDS_UPDATE`, por decisión del Product Owner en `PRODUCT-INTAKE` **1.29** §17.4 P.3.
...
| `BT-02` | Tarea técnica | ~~Anclar la versión de la biblioteca de componentes de interfaz~~ **SIN OBJETO: no hay biblioteca, y es una decisión declarada en el `.csproj`** | — | Criterio en `Backlog-Tecnico.md` | `05` §11 `PA-01`, **cerrado el 2026-08-20**; intake §17.2.P.1, **marca retirada el 2026-08-31** |
...
~~De nivel producto, [`README.md`](README.md) §8 declara **7 filas, una de ellas tachada por cerrada** —el rechazo del informe de Fase B de `GeometriaFactory-Api`—, de modo que **6 siguen vivas**.~~ **CORREGIDO el 2026-08-31: §8 quedó en NUEVE filas y UNA sola viva.** Las ocho restantes están tachadas con su desenlace, y **seis de las ocho se habían cerrado sin que nadie tocara esa tabla**. La única viva es **`D5`, el volumen de la comisión**, y es el único pendiente del producto **sin evento de cierre**: no hay medición que lo conteste. ~~Y el intake §22 declara **4 asunciones vivas** (`A-2` a `A-5`): `A-1` está resuelta en el propio documento.~~ **CORREGIDO el 2026-08-31: queda viva UNA, y es un fragmento.** El Product Owner **confirmó `A-2`, `A-3`, `A-4` y los umbrales de `A-5` el 2026-08-26** con la decisión `D1`, que cerró doce filas vencidas y volvió bloqueantes diez puertas. Sobrevive **sólo el caudal de 20 peticiones por minuto** de `A-5`, excluido expresamente: sale de `PT-05`, que se mide en la fase `i`, y depende de `D5`. **El recuento de esta fila no estaba mal cuando se escribió**: `D1` se propagó a los documentos que usan los umbrales y no a la fuente que los declara, y durante cinco días el §22 siguió diciendo cuatro. *(El recuento del §8 del README que abre este párrafo se corrige aparte, con el propio §8.)*
...
| `V-4` | ~~**La versión de plataforma que soporta el hosting** del front, `[A VERIFICAR]` en la fuente. Es `PT-01.a`~~ **RESUELTA el 2026-08-13** | `Web/05` §11 `PA-02`, **cerrado el 2026-08-31** | Se resolvió **midiendo**, en la etapa `a`, **tal como esta fila pedía**: `PT-01.a` pasa con **200** y el hosting soporta `net10.0` | ~~Si no pasa, la salida declarada es bajar la versión objetivo del front y no la del backend~~ **Pasó, y no hizo falta bajar nada.** Y la advertencia de esta fila —«es una medición que hay que hacer **antes** de escribir el front, no después»— **se cumplió**: se midió el primer día del despliegue. Lo que no se hizo fue retirar la marca |
| `V-5` | ~~**La versión exacta de la biblioteca de componentes de interfaz**, `[A VERIFICAR]`~~ **SIN OBJETO el 2026-08-31** | `Web/05` §11 `PA-01`, **cerrado por lectura el 2026-08-20** | — | **CERRADO el 2026-08-31 por lectura, con el desenlace que `PA-01` de `05` §11 ya tenía desde el 2026-08-20**: **no hay biblioteca de componentes, y su ausencia es una decisión declarada** — `GeometriaFactory.Web.csproj` lleva escrito que *«la etapa `b` decide NO INTRODUCIR MudBlazor»*, con tres razones medidas sobre la maqueta aprobada. **Una incógnita sobre la versión de algo que no existe no se resuelve midiendo: no tiene objeto** (`ADR-14004`). **Condición de reapertura declarada:** si una etapa introduce comportamiento sobre las superficies, la pregunta vuelve, y vuelve mejor planteada — entra tematizada contra los tokens de la maqueta, o no entra. |
...
| `GeometriaFactory-Visor` | [`Proyectos/GeometriaFactory-Visor/08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md`](Unidades-Entrega/GeometriaFactory-Web/08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md) | **15** | **Las 15 en `Sin verificar`** |
...
| `GeometriaFactory-Domain` | **9** | Integrador (obligatorio), mantenedor (obligatorio), operador no aplica | 1 `Vigente` + **8** `Planificado` | [`Proyectos/GeometriaFactory-Domain/11-Documentacion/README.md`](Unidades-Entrega/GeometriaFactory-Api/11-Documentacion/README.md) |
...
| `GeometriaFactory-Visor` | **11** | Integrador (obligatorio), mantenedor (obligatorio, **con `Guia-Extension`**), operador no aplica | 1 `Vigente` + **10** `Planificado` | [`Proyectos/GeometriaFactory-Visor/11-Documentacion/README.md`](Unidades-Entrega/GeometriaFactory-Web/11-Documentacion/README.md) |
| `GeometriaFactory-Application` | **9** | Integrador (obligatorio), mantenedor (obligatorio), operador no aplica | 1 `Vigente` + **8** `Planificado` | [`Proyectos/GeometriaFactory-Application/11-Documentacion/README.md`](Unidades-Entrega/GeometriaFactory-Api/11-Documentacion/README.md) |
...
| `GeometriaFactory-Infrastructure` | **9** | Integrador (obligatorio), mantenedor (obligatorio), operador no aplica | 1 `Vigente` + **8** `Planificado` | [`Proyectos/GeometriaFactory-Infrastructure/11-Documentacion/README.md`](Unidades-Entrega/GeometriaFactory-Api/11-Documentacion/README.md) |
...
| 1.6 | 2026-08-29 | **Tramo `R-3d` del renombre `F-03`, que lo cierra.** **4 línea(s)** pasan los códigos de condición de la forma castellana a la vigente, con el mapeo de [`Producto/Norma-De-Nomenclatura.md`](Producto/Norma-De-Nomenclatura.md) **§6.8** —101 pares— y **sin elegir ninguno acá**. Se respeta **§4.1**: no se tocan las filas de control de cambios, ni lo que está entre «…», ni **la prosa que narra el renombre** —una línea que trae la forma vieja y su par vigente está reportando, no usando—. **Ninguna palabra de prosa cambia**, verificado con el control de diff del tramo. | AG-00030 |
| 1.7 | 2026-08-31 | **Cierra `X-1`, la discrepancia más vieja del checkout: son OCHO aristas de compilación.** Verificado el 2026-08-31 contra los seis `.csproj`, `GeometriaFactory.sln` y `scripts/build-visor.sh`. **El recuento de esta fila era el correcto**: contó ocho sobre la columna «Dependencias» de §2 del manifiesto, y §2 es la única de las tres secciones que acertaba. Son **siete referencias de proyecto y un activo de construcción** —`Visor → Web`, el bundle copiado a `wwwroot/js/`, que ningún `.csproj` puede expresar—, y **rotular esa clase es lo que faltaba** para que ocho y siete dejaran de ser defendibles al mismo tiempo. **La consecuencia que la fila anticipaba no se materializó, y conviene registrarlo**: advertía que quien armara el archivo de proyecto de `Api` leyendo §3 omitiría la referencia a `Application`; `GeometriaFactory.Api.csproj` **la declara**. Quien lo escribió leyó §2. Se emite junto con `PRODUCT-MANIFEST` **5.4** y `Vista-Producto.md` **1.9**, que corrigen las fuentes. **Ninguna magnitud, ningún proyecto de código y ninguna decisión de arquitectura cambia.** | Orquestador SDD |
| 1.8 | 2026-08-31 | **§6: la cuenta de asunciones vivas del intake pasa de cuatro a una, y la que queda es un fragmento.** El Product Owner **confirmó `A-2`, `A-3`, `A-4` y los umbrales de `A-5` el 2026-08-26** por la decisión `D1`, que cerró **doce filas vencidas** en cinco documentos y volvió **bloqueantes** diez puertas de calidad. Sobrevive **únicamente el caudal de 20 peticiones por minuto**, excluido expresamente de la confirmación: su valor sale de `PT-05` —fase `i`— y depende de `D5`, el volumen de la comisión, que sigue sin decidirse. **Este documento no se equivocó al escribirlo**: `D1` se propagó a los documentos que **usan** los umbrales y no a `PRODUCT-INTAKE` §22, que es la fuente que los **declara**, y de ahí lo leyó este checkout. Se emite junto con el intake **3.2**, que estampa la confirmación en la fuente. **El recuento del §8 del README que abre el mismo párrafo queda deliberadamente sin tocar**: se corrige con el propio §8, para no dejar dos ediciones a medias sobre la misma frase. | Orquestador SDD |
| 1.9 | 2026-08-31 | **Cierre de las dos incógnitas `[A VERIFICAR]` que ya no tenían pregunta**, sobre el inventario [`Inventario-Marcas-A-Verificar-2026-08-31.md`](Audit/Inventario-Marcas-A-Verificar-2026-08-31.md), que clasificó las **71** apariciones vivas del corpus en **cinco** incógnitas. **(a) La versión de plataforma del hosting quedó RESUELTA el 2026-08-13, midiendo**: `PT-01.a` pasa con **200** y el hosting soporta `net10.0`, confirmado desde el panel; no hizo falta bajar la versión objetivo del front. **(b) La versión de la biblioteca de componentes queda SIN OBJETO**: la biblioteca nunca se introdujo y su ausencia es una decisión declarada en el `.csproj` — `PA-01` de `Web/05` §11 **ya lo había cerrado por lectura el 2026-08-20** y el desenlace no bajó. **Ninguna de las dos se decide acá: las dos se leen.**  **Ningún umbral, ningún contrato y ninguna decisión cambian.** | Orquestador SDD |
| 1.10 | 2026-08-31 | **Cierra la mitad de la frase de §6 que la emisión 1.8 dejó deliberadamente sin tocar**, ahora que el §8 del README se rehizo. De **«7 filas, 6 vivas»** a **nueve filas y UNA viva**: `D5`, el volumen de la comisión. Se registra lo que el rehecho destapó: **seis de las ocho filas de §8 se habían cerrado sin que nadie tocara esa tabla**. Se emite con [`README.md`](README.md) **2.4**. | Orquestador SDD |
