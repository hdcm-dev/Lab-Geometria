...
**Trazabilidad upstream:** los **siete** casos de uso de [`../02-Especificacion-Funcional/Casos-De-Uso/`](../02-Especificacion-Funcional/Casos-De-Uso/); [`../02-Especificacion-Funcional/Definicion-Contrato-De-Fachada.md`](../02-Especificacion-Funcional/Definicion-Contrato-De-Fachada.md) 1.1 §3.2, §4, §5 y §6; [`../02-Especificacion-Funcional/Especificacion-Funcional.md`](../02-Especificacion-Funcional/Especificacion-Funcional.md) 1.2 §6; las **catorce** historias de [`../06-Backlog-Tecnico/Product-Backlog.md`](../06-Backlog-Tecnico/Product-Backlog.md) §3; los **ocho** NFR de [`../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md`](../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md) §8; [`../03-UX-UI-DX/DX-Error-Messages.md`](../03-UX-UI-DX/DX-Error-Messages.md) §3; [`../../../../Intake/PRODUCT-INTAKE-Fabrica-De-Geometria.md`](../../../../Intake/PRODUCT-INTAKE-Fabrica-De-Geometria.md) **1.19** §15, §17.7.P.8, §18, §20 y §21
...
#### TC-01 — Crear-Instancia-Y-Aislarla-De-Las-Demas

| Campo | Valor |
| --- | --- |
| Tipo | Integración y extremo a extremo en página |
| Cubre | `CU-01`; garantías `G-1`, `G-3`, `G-4`, `G-7`; `US-01`; `BT-04`, `BT-05`, `BT-08` |
...
#### TC-02 — Sin-Capacidad-Grafica-Y-Sin-Elemento-Valido-No-Hay-Instancia

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-01`; garantía `G-7`; `US-03`; códigos `CAPACIDAD_GRAFICA_AUSENTE` y `ELEMENTO_DE_DIBUJO_INVALIDO` **curso C-1**; entradas `E-VIS-01` y `E-VIS-02` de `03` |
...
#### TC-03 — El-Estado-Inicial-De-Los-Dos-Movimientos-Llega-Por-Parametro

| Campo | Valor |
| --- | --- |
| Tipo | Integración e inspección del artefacto generado |
| Cubre | `CU-01`, `CU-07`; garantías `G-2`, `G-3`; `US-02`; `BT-11` |
...
#### TC-04 — Destruir-Libera-Los-Recursos-Y-Corta-El-Bucle

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-05`; garantías `G-4`, `G-7`; `US-11`; NFR «Liberación de recursos»; `BT-05`, `BT-12`, `BT-14` |
...
#### TC-05 — Dibujar-Las-Tres-Figuras-De-E1-Y-Los-Seis-Tipos-De-E7

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-02`; `US-04`; `PT-02` en su tramo de dibujo; `BT-07`, `BT-08`, `BT-09`, `BT-14` |
...
#### TC-06 — Leer-Las-Dimensiones-Con-Las-Variantes-De-Clave-Del-Emisor

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-02`; `US-05`; `BT-07` |
...
#### TC-07 — Ninguna-Pieza-Desaparece-Sin-Quedar-Enumerada

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-02`; garantía `G-5`; `US-06`; NFR «Ausencia de fallo silencioso»; códigos `TIPO_NO_DIBUJABLE` y `DIMENSION_NO_LEGIBLE`; entradas `E-VIS-09` y `E-VIS-10` de `03` |
...
#### TC-08 — Devolver-La-Estructura-Del-Texto-Para-El-Arbol

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-02`; `US-07`; `BT-04`, `BT-07` |
...
#### TC-09 — La-Disposicion-Se-Deriva-Del-Indice-Y-Es-Determinista

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-02`; garantía `G-6`; `US-08`; NFR «Disposición determinista»; `BT-10` |
...
#### TC-10 — Un-Texto-Que-No-Da-Piezas-Deja-La-Instancia-Viva-Y-Vacia

| Campo | Valor |
| --- | --- |
| Tipo | Integración |
| Cubre | `CU-02`; garantía `G-7`; código `TEXTO_NO_LEGIBLE`; entrada `E-VIS-08` de `03` |
...
#### TC-11 — Resaltar-En-Exclusiva-La-Pieza-Del-Indice-Indicado

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-03`; garantías `G-4`, `G-7`; `US-09`; `PT-02` en su tramo de sincronización; código `INDICE_FUERA_DE_RANGO`; entradas `E-VIS-11` y `E-VIS-12` de `03` |
...
#### TC-12 — Ajustar-La-Escena-Y-Sobrevivir-A-Un-Elemento-Sin-Tamano

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-04`; garantía `G-7`; `US-10`; código `ELEMENTO_DE_DIBUJO_INVALIDO` **curso C-2**; entrada `E-VIS-07` de `03` |
...
| Salida esperada | Un ajuste, una condición sin pérdida de instancia y un ajuste posterior exitoso. **Es el mismo código que `TC-02` con otro curso**, y la diferencia está en el efecto sobre la instancia: allá no se crea, acá sigue viva |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-13 — Gobernar-Los-Dos-Movimientos-En-Vivo-Sin-Reconstruir

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-07`; garantía `G-7`; `US-12`; `BT-11` |
...
#### TC-14 — Los-Movimientos-Se-Detienen-Sin-Cambiar-El-Estado-Gobernado

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-07`; `US-13`; `BT-11` |
...
#### TC-15 — Ejercitar-Las-Seis-Funciones-Sin-Backend

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | `CU-06`; las **seis** propiedades transversales de `02` §6; `US-14`; NFR «Se ejercita sin backend»; sample **S-1**; `BT-15` |
...
#### TC-16 — Cero-Red-Con-Los-Dos-Movimientos-Prendidos
...
#### TC-18 — La-Superficie-Del-Bundle-Son-Seis-Funciones-Y-Nada-Mas

| Campo | Valor |
| --- | --- |
| Tipo | Inspección del artefacto generado |
| Cubre | NFR «Superficie pública del bundle»; garantía `G-1` en su tramo de inspección; `QG-04`, `QG-06`; `BT-16` |
...
#### TC-19 — Puerta-PT-03-El-Motor-Queda-Dentro-Del-Bundle

| Campo | Valor |
| --- | --- |
| Tipo | Inspección del artefacto generado y extremo a extremo en página |
| Cubre | Puerta técnica **`PT-03`** del intake §15 y §17.7.P.8; NFR «Dependencias traídas de una red de distribución externa»; `QG-02`; `BT-13` |
...
#### TC-20 — Puerta-PT-02-El-Bundle-En-Una-Pagina-Del-Anfitrion

| Campo | Valor |
| --- | --- |
| Tipo | Extremo a extremo en página |
| Cubre | Puerta técnica **`PT-02`** del intake §15 y §17.7.P.8; `US-01`, `US-04`, `US-09`, `US-11`; `QG-03`; `BT-14` |
...
#### TC-21 — Los-Codigos-Son-Siete-Y-Ninguno-Nace-Aguas-Abajo

| Campo | Valor |
| --- | --- |
| Tipo | Inspección del artefacto generado |
| Cubre | Los **siete** códigos de `Definicion-Contrato-De-Fachada.md` §6; `QG-08`; `BT-06`; `05` §9, sexto riesgo |
...
| Magnitud | Valor |
| --- | --- |
| Casos de prueba declarados | **21**, `TC-01` a `TC-21`, serie contigua |
