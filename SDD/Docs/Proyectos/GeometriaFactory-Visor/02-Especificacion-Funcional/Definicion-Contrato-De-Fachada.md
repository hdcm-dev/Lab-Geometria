...
Las cinco primeras son las que declara PRODUCT-INTAKE §17.7 P.3. La sexta, `establecerMovimiento` (§4.6), la **acuña este documento** por decisión del Product Owner del 2026-08-09, tomada al cerrar la validación visual de la Fase B2: prender y apagar los dos movimientos de la capacidad **F-25** con la escena andando exigía, dentro de las cinco, reconstruir la instancia, lo que **pierde la selección de pieza** y produce un parpadeo. El intake declara cinco y corresponde consolidarlo allá; hasta que eso ocurra, la fuente de la sexta es esta sección.
...
| G-5 · Sin fallo silencioso | Toda pieza que la fachada no dibuja queda enumerada en el resultado de dibujo con su índice. Ninguna desaparece sin dejar registro | `Vision-Producto.md` §9 (fallo silencioso), NB-06 §4 |
...
Consecuencia verificable: cargar dos veces el mismo texto produce la misma disposición, comparable pieza por pieza. Es lo que le permite a quien mira dos previsualizaciones del mismo trabajo compararlas sin confundirse (NB-06 §4).
...
**Punto abierto resuelto.** La versión anterior de esta sección elevó al Product Owner que el cambio en vivo, dentro de las cinco funciones del intake, obligaba a reconstruir la instancia y a que el anfitrión repusiera la selección. **El Product Owner lo resolvió el 2026-08-09 agregando la sexta función de gobierno**, `establecerMovimiento` (§4.6), con su contrato de uso en `CU-07`. Lo que queda es de consolidación y no de decisión: el intake §17.7 P.3 sigue declarando cinco funciones y corresponde que las declare **seis**. La fuente de la sexta, hasta esa consolidación, es §4.6 de este documento.
...
| --- | --- |
| Necesidad de negocio | `NB-06` (visualización del trabajo dentro del producto); `NB-04` en su parte de piezas efectivamente dibujadas |
...
| Casos de uso que lo desarrollan | `CU-01`, `CU-02`, `CU-03`, `CU-04`, `CU-05`, `CU-06`, `CU-07` |
