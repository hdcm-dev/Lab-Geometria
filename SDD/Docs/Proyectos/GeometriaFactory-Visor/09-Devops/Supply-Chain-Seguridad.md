...
**Trazabilidad upstream:** [`../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md`](../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md) 1.0 §5, §8 y §9; [`../05-Arquitectura-Tecnica/Adrs/ADR-04-Motor-De-Dibujo-Empaquetado-Y-Aislado.md`](../05-Arquitectura-Tecnica/Adrs/ADR-04-Motor-De-Dibujo-Empaquetado-Y-Aislado.md); [`../05-Arquitectura-Tecnica/Adrs/ADR-03-Visualizador-Puro-Sin-Red-Ni-Identidad.md`](../05-Arquitectura-Tecnica/Adrs/ADR-03-Visualizador-Puro-Sin-Red-Ni-Identidad.md); [`../08-Calidad-Y-Pruebas/Estrategia-Calidad.md`](../08-Calidad-Y-Pruebas/Estrategia-Calidad.md) 1.1 §3 y §5; [`../08-Calidad-Y-Pruebas/Plan-Pruebas.md`](../08-Calidad-Y-Pruebas/Plan-Pruebas.md) 1.0 §4; [`../../../../Intake/PRODUCT-INTAKE-Fabrica-De-Geometria.md`](../../../../Intake/PRODUCT-INTAKE-Fabrica-De-Geometria.md) **1.20** §10, §13, §14, §15, §17.7.P.1, §17.7.P.4, §17.7.P.5, §17.7.P.8 y §17.7.P.11
...
| El artefacto es reproducible | Métrica de `ADR-06` §8: dos construcciones desde el mismo estado producen el mismo artefacto |

**Firmar lo que se regenera en cada publicación no agrega garantía**: el receptor —el proceso del hosting— no verifica firmas de recursos estáticos, y el productor y el consumidor del archivo son el mismo flujo de trabajo. La firma tendría sujeto si el bundle se distribuyera por un canal a terceros, que es justamente lo que `ADR-06` §4 descartó.
...
| --- | --- | --- |
| Construcción **automatizada y reproducible por guion** | **Cumplido**, y con una exigencia extra que los otros dos proyectos de código no tienen: la instalación de dependencias es **reproducible desde el archivo de bloqueo** y `ADR-06` §8 exige que dos construcciones desde el mismo estado produzcan el mismo artefacto | Intake §17.7.P.8; `ADR-06` §8 |
| **Procedencia** emitida del artefacto | **No cumplido.** Hoy no se emite ninguna | Decisión de esta categoría: se declara la brecha |

**La reproducibilidad exigida acá es más fuerte que la del nivel objetivo**, y no por ambición de esta categoría: `ADR-06` la fija como métrica de validación. Es lo que hace que la resolución de `PA-05` sea segura: si el artefacto no fuera reproducible, regenerarlo en cada publicación en lugar de guardarlo sería un riesgo y no una propiedad.
...
| **0** dependencias traídas de una red de distribución externa en tiempo de ejecución | 0 | `TC-19`, medición de `PT-03` | **Bloqueante, y detiene la planificación de la etapa `g`** |
| **0** peticiones originadas por el bundle, **incluidas las que una dependencia haga por dentro** | 0, con los dos movimientos prendidos | `TC-16` y `TC-18`, sobre el **bundle generado** | **Bloqueante, sin gradación** |
