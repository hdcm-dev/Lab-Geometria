...
| Fusión a la rama principal con cambios bajo `visor/` | Además, el flujo de trabajo de publicación del front, que el intake §17.6.P.7 declara restringido a cambios bajo `src/GeometriaFactory.Web/` y `visor/` | La publicación del front |
| Etiqueta de cierre de etapa | Todos los stages sobre el estado etiquetado | La declaración de etapa cerrada |

**La anteúltima fila es propia de este proyecto de código**, y es la única de los tres de nivel topológico 0 cuyo cambio dispara por sí mismo una publicación: el filtro de rutas de ese flujo de trabajo incluye `visor/`.
...
| Un cambio bajo `visor/` **dispara** ese flujo | Es el único de los tres proyectos de código de nivel 0 cuyo cambio dispara por sí mismo una publicación | Intake §17.6.P.7 |
