---
doc_id: DOC-INFRASTRUCTURE-README-01
doc_type: plan-documental
title: Plan documental — GeometriaFactory-Infrastructure
status: Planificado
...
# Plan documental — GeometriaFactory-Infrastructure
...
**Tipo de proyecto de código (D8):** `library` · nivel topológico 2
**Momento:** 1 — plan documental, sin contenido redactado
**Trazabilidad upstream:** [`../../../Producto/11-Documentacion/README.md`](../../../Producto/11-Documentacion/README.md) 1.0; [`../05-Arquitectura-Tecnica/`](../05-Arquitectura-Tecnica/) con sus **7** ADR; [`../02-Especificacion-Funcional/`](../02-Especificacion-Funcional/) con sus **10** casos de uso
**Tiempo estimado de lectura:** 4 min

## Resumen ejecutivo

Los adaptadores de los cuatro puertos, los dos mecanismos de seguridad y el validador de figuras. Es la pieza con más reglas verificadas del producto y la que sostiene la única base de datos. Su cuerpo documental sirve al integrador que la conecta desde la composición de raíz y al mantenedor que la extiende.
...
| --- | --- |
| ¿Dónde se compara un correo con otro? | `Recorrido-Codigo`, y `Conceptos-Fundamentales` para el criterio |
| ¿Cómo corro la batería del validador sola? | `Guia-Contribucion`, que cita la estrategia de la categoría 08 |
| ¿Qué pasa si una transformación de esquema queda mal? | `Troubleshooting`, con la asimetría de que el código vuelve atrás y el almacén no |

**El validador de figuras es lo que este cuerpo documental tiene que explicar mejor.** Su batería tiene **10** casos sobre los **8** escenarios de datos del intake, y es donde vive el hallazgo que le da sentido al producto: el par de valor declarado y valor derivado que hace visible el error de fórmula del alumno.
...
| --- | --- |
| `guia-integracion-servicio-de-datos.md` | La arista `Infrastructure → Api`: el único consumidor, que la conecta desde su composición de raíz |
...
| --- | --- | --- |
| Cuál de las dos funciones de derivación de clave se ancla, y con qué parámetros | `Conceptos-Fundamentales`, `Guia-Contribucion` | El equipo, en la etapa que la ancla |
| Hasta dónde llega el conjunto de tipos reconstruibles: ninguna fuente enumera las clases | `Conceptos-Fundamentales` | Product Owner |
| La frecuencia del respaldo, que la fuente declara «a definir por el docente» | `Recorrido-Codigo`, y el runbook del servicio de datos | Product Owner |
