...
#### TC-01 — Ortoedro-Con-Clave-Sinonima

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **1** de la batería (`T1`); `CU-01`; `RN-09`; `US-01`; paso `P-3` del flujo |
...
#### TC-02 — Texto-Con-Comas-Finales

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **2** de la batería (`T2`); `CU-01`; `US-01`; paso `P-2` |
...
| Salida esperada | Dos lecturas exitosas y un rechazo con el código correcto. La distinción entre **texto ilegible** y **motor no disponible** es la que `TC-13` desarrolla |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-03 — Cubo-Con-Caras-Cuadrado

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **3** de la batería (`T3`); `CU-01`; `US-01`; paso `P-4` |
...
| Salida esperada | Seis caras interpretadas, con la lectura del largo. Es la mitad de `T3` que viene del primer ejemplo; la otra mitad es `TC-04` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-04 — Cubo-Con-Caras-Rectangulo

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **4** de la batería (`T3`); `CU-01`; `US-01`; paso `P-4` |
| Setup | Fixture con el texto literal de `E-4` |
| Pasos | Given el mismo cubo de lado 3, emitido por el **otro** ejemplo de la cátedra, cuyas caras declaran el otro tipo, When se lo interpreta, Then las caras **se interpretan igual que las de `TC-03`**: las dos traen el largo, que es lo que se usa |
| Salida esperada | Resultado equivalente al de `TC-03` en cuanto a la reconstrucción. **El contraste entre los dos ejemplos es lo que hace visible el defecto** (§20.E-4, contexto) |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-05 — Area-Del-Cubo-Declarada-Contra-Derivada

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **5** de la batería; `CU-02`; `RN-09`; `US-05`, `US-07`; paso `P-6` |
| Setup | Fixture con el texto literal de `E-3`, ya interpretado por `TC-03` |
...
#### TC-06 — Volumen-Del-Ortoedro-Declarado-Contra-Derivado

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **6** de la batería; `CU-02`; `RN-09`; `US-05`, `US-07`; paso `P-6` |
...
#### TC-07 — Dimension-En-Cero-Que-No-Descarta-La-Figura

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **7** de la batería; `CU-01`, `CU-02`; `US-03`; pasos `P-4` y `P-6` |
...
#### TC-08 — Tipo-Desconocido-Con-Posicion-Y-Campo

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **8** de la batería; `CU-01`; `RN-09`; `US-04`; paso `P-3` |
...
| Salida esperada | Un error localizado, la pieza válida reconstruida y tres tratamientos equivalentes. Es el tramo principal de `RN-09` |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-09 — Texto-Semilla-Completo

| Campo | Valor |
| --- | --- |
| Tipo | Unit, **y medición de la tolerancia** |
| Cubre | Caso **9** de la batería; `CU-01`, `CU-02`; `US-02`, `US-06`; NFR de tolerancia; pasos `P-1` a `P-7` |
...
#### TC-10 — Dimension-No-Legible

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | Caso **10** de la batería; `CU-01`; `RN-09`; `US-04`; paso `P-4` |
...
#### TC-11 — Los-Seis-Tipos-Reconstruibles

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-01`; `US-03`; **cobertura adicional declarada** de `05` §10.5 |
| Setup | Fixture con el texto literal de `E-7` |
| Pasos | Given el texto de `E-7`, When se lo interpreta, Then se reconstruyen **seis** piezas, una por cada tipo reconstruible, con las figuras planas **como piezas del conjunto raíz** y no sólo como componentes. Then el ortoedro se lee por su clave alternativa, igual que en `TC-01` |
...
#### TC-12 — La-Cantidad-De-Figuras-Del-Conjunto-Raiz-Y-La-Posicion-Reservada

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-01`; `RN-09`; `US-02`, `US-03`; `RC-02` |
...
#### TC-13 — Texto-Ilegible-No-Es-Motor-No-Disponible

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-01`; `US-01`; el segundo riesgo de `05` §9 |
...
#### TC-14 — Cero-Peticiones-De-Red-De-Los-Dos-Motores

| Campo | Valor |
| --- | --- |
| Tipo | Unit, **prueba de inspección** |
| Cubre | NFR de peticiones de red (`05` §8); `QG-08`; `CU-01` CA-11 |
...
#### TC-15 — Tiempo-De-Interpretacion-Del-Texto-Semilla

| Campo | Valor |
| --- | --- |
| Tipo | Unit, **medición** |
| Cubre | NFR de tiempo de interpretación (`05` §8); `QG-14`; `US-01` |
...
#### TC-16 — Texto-Original-Literal-Y-Escritura-Que-Lo-Reemplaza-Rechazada

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-03`; `RN-08`; `RC-01`; `US-08`; NFR de escrituras que reemplazan el texto |
...
| Salida esperada | Comparación byte a byte sin diferencias y un rechazo sin efecto. Es el **tramo principal de `RN-08`**: ésta es la capa donde el texto se escribe, y por lo tanto donde puede perderse |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-17 — Materializacion-Del-Trabajo-En-Una-Unidad-De-Trabajo
...
#### TC-18 — Consulta-Con-El-Recorte-Ya-Trasladado-Y-Sin-Recorte-No-Resuelta

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-03`; `RN-03`, `RN-11`; `US-10` |
...
| Salida esperada | Dos consultas acotadas y un rechazo. **Esta capa no comprueba pertenencia**: lo que hace es **no ofrecer el camino** por el que `RN-03` y `RN-11` se romperían |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-19 — Listado-Sin-Componentes-Y-Sin-Texto-Original

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-03`; `US-11`; NFR de componentes en listados; `QG-10` |
...
#### TC-20 — Retiro-Fisico-Todo-O-Nada

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-04`; `RN-04`; `RC-05`; `US-12` |
...
#### TC-21 — Arrastre-De-La-Baja-Interrumpido-Sin-Retiro-Parcial

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna, **con el almacén interrumpido a mitad de operación** |
| Cubre | `CU-04`; `RN-07`; `RC-05`; `US-13`; NFR de retiros parciales; `QG-11` |
...
#### TC-22 — Unicidad-Del-Correo-Y-Del-Administrador-En-El-Almacen

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-05`; `RN-01`, `RN-02`; `INV-01`, `INV-05`; `US-14` |
...
#### TC-23 — Las-Dos-Preguntas-Sobre-El-Conjunto

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-05`; `RN-01`, `RN-02`; `US-15` |
...
#### TC-25 — Derivacion-Sin-Guardar-Ni-Registrar-En-Claro
...
#### TC-26 — Verificacion-Que-Distingue-El-Derivado-Ilegible
...
#### TC-27 — Provisoria-No-Adivinable-Y-Sin-Repetirse

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-07`; **`RN-14`, su tramo principal y único**; `RN-16`; `US-19`; NFR de provisorias repetidas; `QG-09` |
...
| Salida esperada | **0** provisorias iguales y **0** derivables de un dato conocido. Es la delegación explícita que las tres capas de arriba le hacen a ésta: `RN-14` es la única de las dieciséis reglas cuyo tramo principal **y único** vive acá |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-28 — Sin-Aleatoriedad-No-Se-Produce-Valor

| Campo | Valor |
| --- | --- |
| Tipo | Unit, con la fuente de material impredecible doblada |
| Cubre | `CU-07`; `RN-14`; `US-20`; el tercer riesgo de `05` §9 |
...
#### TC-29 — Acceso-Firmado-Con-Sus-Cuatro-Reclamos

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-08`; `RN-01`; `US-21` |
...
#### TC-30 — Sin-Clave-De-Firma-No-Hay-Emision

| Campo | Valor |
| --- | --- |
| Tipo | Unit |
| Cubre | `CU-08`; `US-22`; `QG-12`; el cuarto riesgo de `05` §9 |
...
#### TC-31 — El-Sello-Del-Reloj-Entra-Por-Puerto
...
#### TC-32 — Transformaciones-Aplicadas-Sobre-Almacen-Inexistente

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-10`; `US-24`; NFR de aplicación de transformaciones; `QG-04` |
...
#### TC-33 — Arranque-Detenido-En-Lugar-De-Operar-Sobre-Un-Almacen-Dudoso

| Campo | Valor |
| --- | --- |
| Tipo | Integración interna |
| Cubre | `CU-10`; `US-25`; el quinto y el sexto riesgo de `05` §9 |
...
#### TC-34 — Catalogo-De-17-Condiciones-En-Las-Dos-Direcciones
...
#### TC-35 — Ningun-Mensaje-Ni-Traza-Con-Un-Secreto-La-Ruta-Del-Almacen-O-El-Texto-Del-Alumno
...
| --- | --- | --- |
| Casos de prueba de este catálogo | **35**, `TC-01` a `TC-35` | Contar los encabezados de §2 |
| Casos de la batería del validador | **10 de 10**, `TC-01` a `TC-10`, en el mismo orden que `05` §10.5 | §2.1 y [`Estrategia-Testing.md`](Estrategia-Testing.md) §6.1 |
...
| Escenarios del intake §20 usados como texto literal | **8 de 8** | `E-1` en `TC-06`, `TC-09`, `TC-12`, `TC-15`; `E-2` en `TC-01`, `TC-02`, `TC-06`, `TC-16`; `E-3` en `TC-03`, `TC-05`; `E-4` en `TC-04`; `E-5` en `TC-08`, `TC-12`; `E-6` en `TC-07`; `E-7` en `TC-11`, `TC-19`; `E-8` en `TC-10` |
| Casos de prueba de inspección estructural | **3** — `TC-14`, `TC-34`, `TC-35` | §2.2 y §2.6 |
| Casos de integración interna | **11** — `TC-16` a `TC-24`, `TC-32`, `TC-33` | §2.3 y §2.5 |
