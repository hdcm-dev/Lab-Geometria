...
**La tercera fila de la primera tabla es la que hace corta a la segunda.** Este proyecto de código **no habla por red con nada**: el intake §17.3.P.3 lo declara —«no expone endpoints; consume el sistema de archivos donde vive el archivo y nada más», y **el validador de figuras no hace red**—. Un ambiente se distingue de otro por qué servicios alcanza; acá no hay ninguno que alcanzar.
...
| Concurrencia de escritura | **Escritor único.** No es una configuración: es una propiedad del motor que el producto acepta como trade-off | Intake §17.3.P.4 y §17.3.P.12; [`ADR-02`](../05-Arquitectura-Tecnica/Adrs/ADR-02-Un-Archivo-Escritor-Unico-Y-Una-Unidad-De-Trabajo-Por-Operacion.md) |
