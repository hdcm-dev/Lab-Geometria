...
Se adopta el **versionado semántico 2.0.0**, con el formato `MAJOR.MINOR.PATCH[-PRERELEASE][+BUILDMETADATA]`. El intake §17.3.P.7 declara la estrategia **idéntica a la de §17.1.P.7** —versionado semántico, convenciones de mensaje, sin publicación en feed, una rama y una etiqueta por etapa— **y le agrega una obligación propia**, que es el eje de este documento: **cada transformación de esquema se versiona con el código de su etapa, y no se editan transformaciones ya fusionadas**.
...
**Se declara por su función, y esta categoría no la elige**: el intake §17.3.P.7 remite a §17.1.P.7, que la ancla en la etapa `a`, y ninguna fuente la nombra.
...
| --- | --- | --- |
| La **herramienta de transformaciones de esquema**, instalada como **herramienta local del repositorio** para que su versión quede versionada junto al código | El archivo de herramientas del repositorio, anclado en la etapa `a` | Intake §17.3.P.1 |
| El **motor de almacenamiento en su versión embebida** por el proveedor de acceso a datos | El archivo de proyecto, anclado en la etapa `a` | Intake §17.3.P.9 |
...
**Y una consecuencia que el producto ya declaró y esta categoría no reabre**: el intake §17.3.P.4 declara el respaldo como **copia del archivo con el diario activo**, consistente, con **frecuencia a definir por el docente**. Es el único mecanismo declarado para volver atrás sobre datos, y su cadencia **no la fija esta categoría**.

## 5. Modelo de ramas

El del producto, heredado entero y sin variantes: **una rama por etapa** a partir de la principal, con etiqueta al fusionar; **un pull request por etapa, que es el punto de control**; **etapas en serie**; y sin OK explícito no se avanza (intake §10, §15 y §17.1.P.7).
...
**No hay canales de publicación.** El intake §17.3.P.7, por remisión a §17.1.P.7, declara que no se publica en ningún feed, y §13 lo generaliza al producto. `05` §5 lo repite en su última fila. `Rules-Devops.md` §2.2 fija para el tipo `library` el modelo `preview` / `stable` sobre feed único; el apartamiento queda desarrollado en [`Entornos-Deploy.md`](Entornos-Deploy.md) §1. **Tampoco se usan sufijos de anticipo**: no hay canal donde publicar uno ni integrador que lo consuma.
