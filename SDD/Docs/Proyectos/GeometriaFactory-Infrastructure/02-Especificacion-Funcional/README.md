...
| 1.3 | 2026-08-10 | **Cierra el hallazgo `C-02` (P0) del informe de auditoría `SDD/Docs/Audit/Coherencia-Corpus-r1.md` 1.0 en las declaraciones vivas de este archivo que el informe no registra, contra `PRODUCT-INTAKE` 1.14.** La fila de `Reglas-De-Negocio/` decía **quince** reglas, «las quince con archivo propio allá», y desglosaba «trece con tramo, dos sin él». Las reglas son **dieciséis**, `RN-01` a `RN-16`, contadas sobre los archivos de `GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/`, y el desglose pasa a **catorce con tramo y dos sin él**, recontado sobre la tabla de §6 del índice maestro, que en la misma tanda incorporó la fila de `RN-16` que le faltaba. Las tres con tramo principal acá —RN-08, RN-09 y RN-14— no cambian. **Ningún documento de la sección, ningún caso de uso y ninguna omisión declarada cambia.** Sube minor. |
