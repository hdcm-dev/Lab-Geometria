...
| Correo normalizado | Texto | No | Ninguno | **Derivado del anterior, y nunca editado por separado.** Es lo que decide la identidad ([`ADR-03`](Adrs/ADR-03-Comparacion-De-Correos-Y-El-Indice-Que-La-Sostiene.md)) |
...
| Credencial derivada | Texto | **Sí** | Ninguno | **Nula mientras la cuenta está `Pendiente`**; toma valor en el acto de habilitación, con la provisoria que el sistema produce (`RN-16`). Lleva consigo los parámetros con los que se produjo ([`ADR-04`](Adrs/ADR-04-Derivacion-De-Clave-Anclada-Con-Parametros-Versionados.md)) |
| Marca de cambio de contraseña pendiente | Valor de verdad | No | Falso | **Atributo propio, que no es un estado de cuenta** (`RC-07`) |
| Momento de alta | Momento | No | Ninguno | En tiempo universal coordinado, aportado por el puerto de reloj ([`ADR-02`](Adrs/ADR-02-Un-Archivo-Escritor-Unico-Y-Una-Unidad-De-Trabajo-Por-Operacion.md)) |

**No hay columna de momento de última modificación de la cuenta**, y el motivo está en `RC-06`: el modelo del dominio no la declara y el consumidor no la registra. Sigue como punto abierto del Product Owner.
...
| Fecha declarada | Texto | No | Ninguno | **La escribe el alumno y no es un sello** (`RC-06`). Se guarda tal como la escribió y **no se convierte de zona** |
| Texto original | Texto | No | Ninguno | El texto que el alumno pegó, **conservado literal** (`RC-01`). Se guarda como texto y **no se consulta por su contenido** |
| Cantidad de figuras del conjunto raíz | Entero | Sí | Ninguno | **Incluidas las no reconstruidas.** Es el rango de posiciones válidas (`RC-02`). Nula mientras el texto no se interpretó |
| Estado | Texto | No | Ninguno | Conjunto cerrado de **4** valores, guardado por su nombre |
| Comentario del administrador | Texto | Sí | Ninguno | **Campo, no entidad, y sin historial** (`RC-07`) |
...
| Momento de creación | Momento | No | Ninguno | Sello del sistema, en tiempo universal coordinado (`RC-06`) |
| Momento de última modificación | Momento | No | Ninguno | Sello del sistema, en tiempo universal coordinado (`RC-06`) |

**Los tres tiempos son distintos y no se confunden**: la fecha declarada la escribe el alumno, los dos momentos los produce el sistema por el puerto de reloj. Es `RC-06`, y el intake lo declara con rótulo de decisión del Product Owner.
...
| Posición en el conjunto raíz | Entero | No | Ninguno | **Es su identidad de dominio, y no se compacta** (`RC-02`) |
...
| Área derivada | Real | Sí | Ninguno | La que el producto calcula (`RC-03`). **Los dos por separado** |
| Volumen declarado | Real | Sí | Ninguno | Nulo en las piezas planas |
| Volumen derivado | Real | Sí | Ninguno | Nulo en las piezas planas |

**La familia plana o volumétrica no se guarda** (`RC-04`): se deriva del tipo, y guardarla crearía un segundo lugar donde puede decir otra cosa.
...
| Posición de la figura | Entero | No | Ninguno | La ubicación que el mensaje declara (`RN-09`) |
| Campo | Texto | No | Ninguno | El campo que el mensaje declara (`RN-09`) |
| Valor declarado | Real | Sí | Ninguno | **Sólo en la advertencia**, y junto con el siguiente (`RC-03`) |
...
| --- | --- | --- | --- | --- |
| IX-01 | Cuenta | Correo normalizado | **Único** | Es **la segunda línea de `RN-02` e `INV-01`**: la consulta previa del consumidor no es garantía por sí sola. La colisión que la consulta no vio termina en `CORREO_YA_REGISTRADO` ([`ADR-03`](Adrs/ADR-03-Comparacion-De-Correos-Y-El-Indice-Que-La-Sostiene.md)) |
| IX-02 | Cuenta | Papel | **Único parcial**, sólo sobre las filas con papel `Administrador` | Sostiene `INV-05` y `RN-01` en el almacén: la materialización que dejaría dos administradores termina en `UNICIDAD_DE_ADMINISTRADOR_VIOLADA`. Es **parcial** porque las cuentas de alumno no son únicas por papel |
...
| IX-04 | Pieza | Trabajo, Posición en el conjunto raíz | **Único compuesto** | Sostiene `RC-02`: dos piezas del mismo trabajo no pueden ocupar la misma posición, que es la identidad de dominio de la pieza |
...
| RE-04 | Cuenta | Correo normalizado único (IX-01) | `RN-02`, `INV-01`, [`ADR-03`](Adrs/ADR-03-Comparacion-De-Correos-Y-El-Indice-Que-La-Sostiene.md) |
| RE-05 | Cuenta | A lo sumo **una** fila con papel `Administrador` (IX-02) | `RN-01`, `INV-05` |
| RE-06 | Trabajo | Clave primaria sobre la identidad, y clave foránea de dueño hacia Cuenta **con arrastre del retiro** | `RN-07`, `RC-05` |
| RE-07 | Trabajo | Estado pertenece al conjunto cerrado de **4** valores | `Modelo-Conceptual.md` §4 |
| RE-08 | Trabajo | El texto original no admite ser reemplazado por uno distinto en una fila existente | `RN-08`, `RC-01`, `ESCRITURA_QUE_REESCRIBE_EL_TEXTO_ORIGINAL` |
| RE-09 | Pieza | Clave foránea hacia Trabajo **con arrastre del retiro**, y posición única por trabajo (IX-04) | `RC-02`, `RC-05` |
| RE-10 | Pieza | La posición es mayor o igual que cero y menor que la cantidad de figuras del conjunto raíz del trabajo | `RC-02` |
| RE-11 | Componente | Clave foránea hacia Pieza **con arrastre del retiro** | `RC-05` |
| RE-12 | Observación | Clave foránea hacia Trabajo **con arrastre del retiro**; y **ninguna** hacia Pieza | `Modelo-Conceptual.md` §3.1, `RC-05` |
| RE-13 | Observación | Especie pertenece al conjunto cerrado de **2** valores | `Modelo-Conceptual.md` §4 |
| RE-14 | Observación | El valor declarado y el valor derivado están los dos presentes o los dos ausentes, y sólo están presentes en la advertencia | `RC-03` |
| RE-15 | Todas | **Ninguna columna de borrado lógico y ninguna de pertenencia a instancia** | `RC-05`, `INV-05`, `PRODUCT-INTAKE` §17.3.P.4 |

**Los cuatro arrastres del retiro —RE-06, RE-09, RE-11 y RE-12— son lo que hace que el retiro físico sea comprobable por ausencia**, que es exactamente el criterio con el que `RN-07` se verifica: no queda ningún trabajo del alumno dado de baja. La unidad de trabajo los cubre a los cuatro de una vez ([`ADR-02`](Adrs/ADR-02-Un-Archivo-Escritor-Unico-Y-Una-Unidad-De-Trabajo-Por-Operacion.md)).
...
| Cuándo se aplica | **Al arrancar**, automáticamente, antes de que el servicio atienda su primera petición ([`ADR-07`](Adrs/ADR-07-Transformaciones-Al-Arrancar-Con-Linaje-Inmutable.md)) |
...
| Qué pasa si no se puede aplicar | **El arranque se detiene.** No se aplica un esquema por aproximación y **no se descarta el almacén** ([`ADR-07`](Adrs/ADR-07-Transformaciones-Al-Arrancar-Con-Linaje-Inmutable.md)) |
...
| --- | --- | --- | --- |
| Cuenta | Cuenta (`Modelo-Conceptual.md` §3.2) | CU-04, CU-05 | `RC-06`, `RC-07` |
| Trabajo | Trabajo (§3.3) | CU-03, CU-04 | `RC-01`, `RC-02`, `RC-05`, `RC-06`, `RC-07` |
| Pieza | Pieza (§3.4) | CU-01, CU-03, CU-04 | `RC-02`, `RC-03`, `RC-04`, `RC-05` |
| Componente | Componente (§3.5) | CU-01, CU-03, CU-04 | `RC-04`, `RC-05` |
| Observación | Observación (§3.6) | CU-01, CU-02, CU-03, CU-04 | `RC-02`, `RC-03`, `RC-05` |

**Las cinco entidades conceptuales tienen tabla y ninguna tabla existe sin entidad conceptual.** Las **siete** reglas conceptuales están todas representadas: `RC-01` en RE-08, `RC-02` en IX-04 y RE-10, `RC-03` en RE-14, `RC-04` por la **ausencia** de columna de familia, `RC-05` en los cuatro arrastres y en RE-15, `RC-06` en las tres columnas de tiempo de Trabajo y en la ausencia de la cuarta en Cuenta, y `RC-07` en la marca como columna propia y en el comentario como campo sin historial.

| Dimensión | Referencia |
| --- | --- |
| CU que lo consumen | CU-01 a CU-05 y CU-10 de [`../02-Especificacion-Funcional/Casos-De-Uso/`](../02-Especificacion-Funcional/Casos-De-Uso/) |
| RN que sostiene en el almacén | RN-01, RN-02, RN-04, RN-07, RN-08, RN-09, RN-11, RN-12, RN-13, RN-15, RN-16 |
| Invariantes que sostiene | INV-01, INV-05, INV-07, INV-09 |
| ADR que lo gobiernan | ADR-02, ADR-03, ADR-04, ADR-07 |
...
| La **forma exacta del valor derivado de una credencial**, con sus parámetros | [`ADR-04`](Adrs/ADR-04-Derivacion-De-Clave-Anclada-Con-Parametros-Versionados.md) fija que los parámetros viajan junto al valor; cuál función se ancla se decide en la etapa `a`, y con ella la forma concreta de esa columna | El equipo en la etapa `a` |
