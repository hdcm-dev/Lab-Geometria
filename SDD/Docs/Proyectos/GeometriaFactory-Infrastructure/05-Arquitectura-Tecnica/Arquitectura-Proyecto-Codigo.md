...
| PA-05 | El **límite de tamaño del texto que se acepta.** Esta categoría **no lo fija acá y declara por qué**: el corte pertenece al borde del proceso, y fijar dos límites distintos —uno en el borde y otro en el motor— haría que un texto pudiera entrar y no interpretarse. [`ADR-06`](Adrs/ADR-06-Lectura-Tolerante-Y-Tabla-De-Derivacion-Por-Tipo.md) §2 decide que el motor **no impone límite propio**; el valor y su forma de rechazo los fija la categoría 05 de `GeometriaFactory-Api` | La categoría 05 de `GeometriaFactory-Api`, y el Product Owner si quisiera un valor propio | Al emitirse |
...
**Once filas: diez abiertas y una resuelta.** `PA-08` se conserva en la tabla como fila resuelta, con su desenlace y su fecha, en lugar de retirarse: el identificador está citado desde §10.5 y desde el README de la sección, y retirarlo dejaría un hueco en la numeración sin declarar.
