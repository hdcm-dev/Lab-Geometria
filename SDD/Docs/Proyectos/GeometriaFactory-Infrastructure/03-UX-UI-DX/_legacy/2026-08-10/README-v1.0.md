...
- **Las reglas de negocio no viven acá.** Las **quince** del producto están en `GeometriaFactory-Domain` y esta capa las **ejerce**; `Especificacion-Funcional.md` §6 dice, regla por regla, dónde se ejerce cada una: **trece con tramo acá, dos sin él y tres con su tramo principal acá**.
...
- **Puntos abiertos que esta sección roza y no reabre.** Los **diez** que `Especificacion-Funcional.md` §11 declara: cinco propios de la capa —**qué devuelve el validador ante el texto de `E-8`**, hasta dónde llega el conjunto de tipos reconstruibles, **cómo se sostiene que la provisoria no se repite**, la longitud y el alfabeto de la provisoria y la vigencia exacta del acceso— y cinco heredados de aguas arriba, entre ellos cuál función de derivación se ancla y el criterio con el que dos correos son el mismo.
