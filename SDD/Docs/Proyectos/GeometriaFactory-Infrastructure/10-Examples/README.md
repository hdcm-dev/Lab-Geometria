...
Tres markdown explicativos, uno por sample, con sus **diez** secciones obligatorias de `Rules-Examples.md` §4.2, y este índice. Cada markdown apunta a una carpeta ejecutable de `/samples/infrastructure/` del repositorio, que esta pasada deja **esqueletada**: con su README local y su comando previsto, y sin corrida hecha.
...
| --- | --- | --- | --- | --- |
| [`ejemplo-01-basico.md`](ejemplo-01-basico.md) | Básico | < 5 min | CU-01, CU-02 | `/samples/infrastructure/01-basico/` |
| [`ejemplo-02-intermedio.md`](ejemplo-02-intermedio.md) | Intermedio | 10-15 min | CU-03, CU-04, CU-05 | `/samples/infrastructure/02-intermedio/` |
| [`ejemplo-03-avanzado.md`](ejemplo-03-avanzado.md) | Avanzado | 10-15 min | CU-06, CU-07, CU-08, CU-09, CU-10 | `/samples/infrastructure/03-avanzado/` |
...
**Cobertura de los diez casos de uso: 10 de 10.** Verificación uno por uno: `CU-01` y `CU-02` en el 01; `CU-03`, `CU-04` y `CU-05` en el 02; `CU-06`, `CU-07`, `CU-08`, `CU-09` y `CU-10` en el 03. Sin repeticiones y sin huecos: 2 + 3 + 5 = 10.
...
| --- | --- | --- | --- | --- | --- |
| `VER-01` | [`ejemplo-01-basico.md`](ejemplo-01-basico.md) | CU-01, CU-02; US-01 a US-07 | `dotnet run --project samples/infrastructure/01-basico` | No verificado — sin código | — |
| `VER-02` | [`ejemplo-02-intermedio.md`](ejemplo-02-intermedio.md) | CU-03, CU-04, CU-05; US-08 a US-16 | `dotnet run --project samples/infrastructure/02-intermedio` | No verificado — sin código | — |
| `VER-03` | [`ejemplo-03-avanzado.md`](ejemplo-03-avanzado.md) | CU-06, CU-07, CU-08, CU-09, CU-10; US-17 a US-25 | `dotnet run --project samples/infrastructure/03-avanzado` | No verificado — sin código | — |
...
**Y hay una relación que conviene decir con precisión, porque es la más fácil de confundir.** Los **diez** casos de la batería obligatoria del producto —los diez primeros de [`../08-Calidad-Y-Pruebas/Casos-Prueba-Referenciales.md`](../08-Calidad-Y-Pruebas/Casos-Prueba-Referenciales.md)— **no los reemplaza el sample 01**, aunque los dos usen los mismos ocho escenarios. La batería vive en `tests/` y corre en el pipeline; el sample corre a mano y muestra el resultado por consola, para que quien integra vea lo que el validador produce sin escribir una prueba. `VER-01` sensa que **el sample siga produciendo lo que la batería exige**, no que la batería exista.
...
**Y hay un tercer motivo, propio de este proyecto de código: el intake ya le había asignado un sample en otra sección.** El `PRODUCT-INTAKE` §18 declara la muestra **S-3** —«juego de datos de los ocho escenarios de la Parte D, en archivos sueltos»— y en su columna de proyecto de código que ilustra dice **`GeometriaFactory-Infrastructure` (validador)**. **Ésa es una divergencia interna del intake, y se declara en lugar de resolverla por cuenta propia**: §16.1 dice que este proyecto de código no produce samples y §18 le asigna uno con nombre y contenido. Esta categoría adopta la lectura de §18 —emitir—, con el fundamento de que §18 es la sección que la propia guía nombra como estrategia de samples del producto (`Rules-Examples.md` §8, insumos del prompt de la categoría) y de que los dos términos del argumento anterior se cumplen igual. **Está elevado al Product Owner para que alinee §16.1 con §18 S-3.**
...
**Lo que queda abierto:** la consolidación de `PRODUCT-INTAKE` §16.1 y su alineación con §18 **S-3**. Corregirlas es del Product Owner sobre su propio documento. **Hasta que se consolide, la fuente vinculante de la estructura de `/samples/infrastructure/` es §5 de este README.**
