...
| --- | --- | --- | --- |
| Bibliotecas de guion agregadas que consulten servicios por su cuenta | **0** | `QG-06`, con `TC-30`, inspección del árbol de fuentes y de las dependencias de guion | **Bloqueante** |
...
| Versión de la biblioteca de componentes de interfaz | **Anclada y registrada** al crear el andamiaje | `BT-02`, etapa `a`. Está **[A VERIFICAR]** en la fuente y **no se inventa acá** | Bloqueante como tarea de la etapa `a` |
...
| Exposición de la dirección del servidor propio | Rotación del secreto, republicación y **revisión de por dónde se filtró**: `QG-08` mide **0** mensajes que la expongan, sobre los **quince** códigos vivos y el camino de ausencia de respuesta | Es `RA-03`; `RI-05` de [`../../../Producto/Vista-Producto.md`](../../../Producto/Vista-Producto.md) §7 |
...
| **`RA-03`** · nada expone direcciones de servicios internos | Un mensaje de error que incluya la dirección, la ruta del almacén o una traza | `QG-08` (**0** sobre los **quince** códigos vivos **y** sobre el camino de ausencia de respuesta) | `RI-05`: la violación directa de `RA-03`, en el último tramo antes de llegar a la persona |
