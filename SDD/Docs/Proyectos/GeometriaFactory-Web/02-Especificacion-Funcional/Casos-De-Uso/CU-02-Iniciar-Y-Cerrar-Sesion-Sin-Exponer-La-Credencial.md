...
| `CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` | La cuenta fue reseteada por el administrador y todavía no cambió la provisoria | Handoff a FA-07: **no se otorga sesión** y la persona queda derivada al cambio forzado. Es el paralelo de `CONTRATO_CONTRASENA_NO_ESTABLECIDA`, y la diferencia con `CONTRATO_CUENTA_NO_HABILITADA` es el destino: allá la persona vuelve a la ruta de ingreso sin camino propio, acá tiene uno y es el cambio |
