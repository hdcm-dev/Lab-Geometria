...
| Documento a retroalimentar | `PRODUCT-INTAKE` 1.7 lo consolidó. Pendientes de propagar, y **no los propaga esta bitácora**: `Roadmap-Producto.md` §3 punto 4 y `NB-06` §5, que siguen escritos contra el intake 1.5 y afirman que ubicar F-25 en la etapa `g` «no la compromete» y que es `Should Have` diferible. Son de nivel producto, suben versión con archivado y su propagación es del orquestador |
...
| **La propagación del intake 1.7 a `00-Contexto` y a `01-Necesidades-Negocio`** | `Roadmap-Producto.md` §3 punto 4 y `NB-06` §5 y §9 siguen escritos contra el intake 1.5 y afirman que `F-25` es `Should Have` y que ubicarla en la etapa `g` «no la compromete». El intake 1.7 la subió a **`Must Have`**, con lo que esas afirmaciones dejaron de ser ciertas. Son documentos de nivel producto: suben versión con archivado y la propagación es del orquestador, no de esta fase |
