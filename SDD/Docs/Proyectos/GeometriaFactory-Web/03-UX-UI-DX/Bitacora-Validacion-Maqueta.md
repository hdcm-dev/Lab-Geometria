...
**Variante:** UX/UI
**Trazabilidad upstream:** `SDD/Maquetas/GeometriaFactory-Web/` íntegra, en sus cuatro iteraciones; `Maqueta-Rules.md` §3.5 y §3.6; `PRODUCT-INTAKE-Fabrica-De-Geometria.md` 1.5, §4 (F-25), §17.7 P.10 y su control de cambios; `../../GeometriaFactory-Visor/02-Especificacion-Funcional/Definicion-Contrato-De-Fachada.md`
...
Las cuatro iteraciones produjeron seis hallazgos. La columna de destino es la lectura de la matriz de propagación de `Maqueta-Rules.md` §3.6.
...
| `H-6` | El **giro de las figuras no existe** en el visualizador previo: el original mueve la cámara y deja las piezas quietas. Es comportamiento no declarado en la especificación, incorporado por decisión del Product Owner durante la validación visual | Alcance funcional que la maqueta mostró faltante · destino 00, 01, 02, 03, 06, 07 | El `PRODUCT-INTAKE` lo absorbió como **F-25** en su versión 1.5. En 03: `Wireframes-Vista-De-Trabajo.md`, `Experiencia-De-Uso.md` y `Glosario-UX.md` de este proyecto de código, y la categoría 03 del proyecto de código del visor. En 02: `Definicion-Contrato-De-Fachada.md` §5.5, que declara el gobierno de los dos movimientos. **`00-Contexto` y `01-Necesidades-Negocio` quedaron sin tocar y con su alcance informado**, según la regla de corte de `Maqueta-Rules.md` §3.6: ver §5 |

Además de los seis hallazgos, la iteración 3 produjo una corrección que no se emitió como hallazgo numerado y que sí se propagó: **el cero como dimensión legible**, ausente de toda la documentación del visor y necesaria para que el escenario `E-6` del intake §20 no se contradiga.

## 5. Lo que quedó sin resolver, y por qué

| Qué | Por qué no se resolvió acá |
| --- | --- |
| **`H-6` alcanza a `00-Contexto` y a `01-Necesidades-Negocio`** | Son de nivel producto y ya fueron auditadas en la Fase A. La regla de corte de `Maqueta-Rules.md` §3.6 obliga a detenerse, informar el alcance real y pedir confirmación: un hallazgo de maqueta de un proyecto de código no reescribe la visión del producto sin que el humano lo sepa. **Alcance concreto**: `Alcance-Producto.md` §4.1 no lista `F-25` entre las capacidades comprometidas y su §4 declara dieciséis capacidades `Must Have` sin contemplar la `Should Have` nueva; `Roadmap-Producto.md` §2.1, §3 y §5.2 no le asignan etapa; y `Necesidades-Negocio.md` §3, §5.1 y su tabla de trazabilidad de capacidad a necesidad no la vinculan a ninguna `NB-XX`, siendo `NB-06`, visualización del trabajo dentro del producto, la candidata natural |
| **`H-5`, la condición sin escenario de datos propio** | Es punto abierto declarado en el `PRODUCT-INTAKE` 1.5 y su resolución es del Product Owner |
...
| **El punto abierto del gobierno del movimiento con la instancia viva** | `Definicion-Contrato-De-Fachada.md` §5.5 lo declara: el intake §17.7 P.3 fija la superficie pública en cinco funciones y la incorporación de `F-25` no la extendió. La vía declarada cumple el contrato tal como está; una sexta función de gobierno es decisión del intake |
