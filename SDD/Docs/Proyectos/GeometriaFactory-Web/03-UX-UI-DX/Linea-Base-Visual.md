...
| `SUP-04` | `Credencial-Propia` | `Credencial-Propia.html` | `Wireframes-Credencial-Propia.md` | `CU-03` | Elegir la contraseña propia la primera vez —con la provisoria como vigente— y cambiarla después |
...
| **La regla `RN-16`** —habilitar una cuenta produce su contraseña provisoria— en lo que tiene de visible: la operación **habilitar** de la fila de `SUP-09` pasa a **mostrar la contraseña provisoria para comunicarla, una sola vez**, y deja la cuenta con cambio de contraseña pendiente | **Después** de la aprobación de la maqueta, en la ronda que el `PRODUCT-INTAKE` absorbió en su versión **1.13** §4.1; [`Wireframes-Panel-De-Cuentas.md`](Wireframes-Panel-De-Cuentas.md) lo absorbió en su **1.5** reescribiendo la fila de comportamiento de habilitar | **Habilitar, sin provisoria.** Es una de las **cuatro** operaciones de la fila de cuentas que el Product Owner miró y aprobó el 2026-08-09: la transición de situación admitida, su insignia y la orientación posterior. Lo que se validó es el comportamiento **anterior** a RN-16 | **La exhibición de la contraseña provisoria y la marca de cambio pendiente.** La maqueta **no muestra ninguna provisoria en ninguna parte** —`grep -r provisoria` sobre `SDD/Maquetas/GeometriaFactory-Web/` devuelve **cero**—, de modo que ningún `CMP-XX` ni `EST-XX` de esta línea de base la cubre, y **no se le asigna uno acá**, por el mismo motivo que en la fila de F-26: un identificador de línea de base afirma que alguien lo miró y lo aprobó. **Lo que distingue a esta fila de las otras dos**: acá no falta una capacidad nueva sin validar, sino que **cambió el comportamiento de una operación ya validada**, y ésa es exactamente la clase de desfase que una línea de base no puede callar |
...
**Qué significa para quien construya, en el caso de RN-16.** Lo que hay que construir es lo que especifican [`Wireframes-Panel-De-Cuentas.md`](Wireframes-Panel-De-Cuentas.md) en su versión vigente, `CU-04` y `US-09`; lo que esta línea de base **no** puede afirmar es que se haya visto en pantalla. **Y hay una consecuencia sobre el sensado que no se puede dejar implícita**: como habilitar sí fue validada, su sonda existe —`SD-09` en [`../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md`](../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md)— y **daría conforme sobre un panel que todavía no cumple RN-16**, porque sensa el comportamiento aprobado el 2026-08-09. Por eso `SD-09` acota su alcance en lugar de ampliarlo por decreto, y **no se le inventa un identificador de línea de base a la provisoria**. La vía es la misma que la de F-26: **iteración 5** de maqueta y reemisión de esta línea de base. Los recuentos de §1 —11 superficies, 73 componentes, 74 estados y 24 rutas— **no cambian**.
...
| --- | --- | --- |
| 1.5 | 2026-08-11 | **Cierra los hallazgos `NB2-03` (P2) y `NB2-04` (P3)** del informe [`../../../Audit/B2-Maqueta-GeometriaFactory-Web-r2.md`](../../../Audit/B2-Maqueta-GeometriaFactory-Web-r2.md) 1.0. **§6.1**: entra una **tercera fila**, la de **`RN-16`** —habilitar una cuenta produce su contraseña provisoria y la superficie la muestra una sola vez para comunicarla—, que entró con el `PRODUCT-INTAKE` **1.13**, es decir **después** de la aprobación de la maqueta del 2026-08-09, y que hasta hoy vivía en este documento en una sola línea del control de cambios 1.4. A diferencia de `establecerMovimiento` y de `F-26`, **RN-16 no agrega una capacidad nueva: cambia el comportamiento de `habilitar`, que es una de las cuatro operaciones que la maqueta sí validó**, y por eso su desfase es de otra clase y se declara aparte. **No se le asignan `CMP-XX` ni `EST-XX`**, por el mismo motivo declarado para F-26, y **los cuatro recuentos de §1 no se inflan**: siguen en 11 superficies, 73 componentes, 74 estados y 24 rutas. La prosa suma el caso de RN-16 y deja escrita la consecuencia sobre el sensado —`SD-09` sensa el comportamiento aprobado y **daría conforme sobre un panel que no cumple RN-16**—, con la **iteración 5** de maqueta como vía, la misma que ya rige para F-26. **§6.1**, fila de `establecerMovimiento`: la afirmación «no aparece en ningún archivo de la maqueta» era **exacta en su intención e inexacta en su letra** —el nombre aparece **tres** veces y a propósito, en `README.md`, en el comentario de bloque de `Maqueta.js` y en el texto que el panel muestra en pantalla, siempre para declarar su ausencia— y pasa a «**no está implementada** en ningún archivo de la maqueta: la maqueta la nombra sólo para declarar que no la ejerce». Es el tipo de enunciado absoluto que el corpus verifica por `grep`. **Búsqueda de propagación hecha con `grep` sobre todo el corpus vivo**: el enunciado absoluto no se repite en ningún otro documento, y el hueco de RN-16 se declara además en el párrafo de §4 de [`../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md`](../08-Calidad-Y-Pruebas/Matriz-Sensado-Deriva.md) y en el alcance de su `SD-09`, corregidos en la misma tanda. **Ninguna superficie, componente, estado, ruta ni identificador de esta línea de base se agrega, se retira ni se renumera.** Sube minor. |
| 1.4 | 2026-08-10 | Alineación con `PRODUCT-INTAKE` **1.13** §4.1 (**RN-16**) y la precisión de **F-04**. La fila de `SUP-04` reescribe su descripción: el primer uso de la superficie deja de ser un establecimiento sin credencial y pasa a ser un cambio con la provisoria como vigente. **Las superficies, sus archivos de maqueta y su recuento no cambian.** Sube minor. (DX Lead (AG-03)). |
