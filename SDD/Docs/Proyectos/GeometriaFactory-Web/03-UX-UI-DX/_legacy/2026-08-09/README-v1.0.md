...
| [`Wireframes-Credencial-Propia.md`](Wireframes-Credencial-Propia.md) | Establecer y cambiar la contraseña, con sus dos cursos y sus dos shells | Propuesto |
...
| [`Wireframes-Panel-De-Cuentas.md`](Wireframes-Panel-De-Cuentas.md) | Las cuatro operaciones sobre las cuentas, con la confirmación escrita de la baja | Propuesto |
...
**El modelo UX-UI de `Devs/Modelos-UX-UI/` no está elegido a esta fecha**: esa elección es del paso 1 de la Fase B2 y la toma el humano.
...
- **Autoridad.** Esta sección no origina ninguna capacidad, prioridad ni exclusión. Todo se deriva de los diez casos de uso de `../02-Especificacion-Funcional/`, de sus **once restricciones transversales** `RT-01` a `RT-11`, del contrato de fachada del visualizador y del `PRODUCT-INTAKE`, y traza a su sección de origen.
...
- **Capacidades que el producto no tiene y que ninguna superficie inventa.** No hay recuperación de contraseña, no hay notificaciones por correo, no hay calificación con nota, no hay múltiples administradores, no hay papeles configurables y no hay canal de soporte. Las ausencias que la persona podría buscar están **declaradas en la superficie** —la nota de la contraseña olvidada en `Ingreso`, el subtítulo de expectativa en `Registro-De-Cuenta`—, porque una ausencia silenciosa se busca y una declarada no.
