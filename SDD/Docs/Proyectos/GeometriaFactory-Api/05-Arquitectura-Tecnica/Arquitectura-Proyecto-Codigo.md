...
| PA-06 | **El alcance de la colección de peticiones, que la fuente declara en dos lugares con alcances distintos.** Verificado contra el texto vivo de `PRODUCT-INTAKE` **1.18**: §16.1 dice «Colección de peticiones HTTP reproducible con los escenarios **E-1 a E-8** como cuerpo» —**los ocho**— y §18 **S-2** dice «con los cuerpos de **E-2 y E-5**» —**dos**—. **No es un recuento envejecido: los dos textos están al día y describen alcances distintos**, ocho contra dos, y la fuente no declara cuál manda. La categoría 02 adopta **los ocho** con su fundamento —`E-8` es el modo de falla que el propio intake llama el más probable— y esta categoría **hereda esa lectura y no la reabre** | **Product Owner**, para declarar cuál de los dos alcances rige y alinear §16.1 con §18 S-2 | Sin fecha comprometida |
...
**Diez filas: nueve abiertas —`PA-01` a `PA-09`— y una resuelta, `PA-10`.** `PA-06` sigue abierto y su enunciado cambió por completo: lo que subsiste es una **divergencia de alcance entre dos textos vivos**, no un recuento envejecido. `PA-10` se conserva en la tabla como fila resuelta en lugar de retirarse, porque retirarlo dejaría un hueco de numeración sin declarar.
