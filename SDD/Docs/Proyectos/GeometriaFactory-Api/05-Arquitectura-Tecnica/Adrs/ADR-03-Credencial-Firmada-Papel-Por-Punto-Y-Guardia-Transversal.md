# ADR-03 — Credencial firmada, papel exigido por punto y una guardia transversal sin excepciones sueltas

**Proyecto de código:** GeometriaFactory-Api
**Documento:** ADR-03-Credencial-Firmada-Papel-Por-Punto-Y-Guardia-Transversal.md
**Versión:** 1.0
**Estado:** Aprobado
**Fecha:** 2026-08-10
...
**Y una ausencia que esta ADR sostiene explícitamente: ningún punto de acceso fija una contraseña sobre una cuenta existente sin credencial.** Es `RN-16` vista desde la superficie, y se comprueba sobre los cuatro puntos que no exigen acceso: uno canja credenciales, uno registra una cuenta **sin** contraseña, uno sólo procede mientras no exista administrador y uno es de sólo lectura. El identificador `A-04`, que era la excepción, **quedó retirado y no se recicla**.
...
1. `RN-13` e `INV-09` quedan sostenidos por una propiedad **contable**: cuatro puntos fuera de la guardia, once dentro, y una prueba que lo recorre en las dos direcciones.
2. `RN-16` queda sostenida como **ausencia comprobable**: ningún punto fija una contraseña sobre una cuenta existente sin credencial.
3. La autorización real queda donde la capa de aplicación la puso, y `RN-04` conserva su criterio de verificación —forzar la petición contra esta superficie— probando la regla y no una copia del borde.
...
2. **Se acepta que el registro de cuenta sea anónimo**, y se declara que **debe seguir siéndolo**: es como el alumno entra al laboratorio. La escritura anónima que `RN-16` eliminó es la **de contraseña**, no toda escritura anónima.
...
- La vigencia se toma de configuración y llega por la composición de raíz ([`ADR-06`](ADR-06-Composicion-De-Raiz-Ciclos-De-Vida-Y-Configuracion.md)).
...
- `PRODUCT-INTAKE-Fabrica-De-Geometria.md` **1.17** §4.1 (RN-13, RN-16), §17.5.P.5 y §17.5.P.6.
...
- [`../../02-Especificacion-Funcional/Casos-De-Uso/CU-02-Admitir-La-Peticion-Acceso-Papel-Y-Marca.md`](../../02-Especificacion-Funcional/Casos-De-Uso/CU-02-Admitir-La-Peticion-Acceso-Papel-Y-Marca.md).
- [`../../../GeometriaFactory-Application/05-Arquitectura-Tecnica/Adrs/ADR-04-Orden-Fijo-De-Las-Cuatro-Comprobaciones.md`](../../../GeometriaFactory-Application/05-Arquitectura-Tecnica/Adrs/ADR-04-Orden-Fijo-De-Las-Cuatro-Comprobaciones.md), que es la comprobación que esta guardia garantiza que se ejerza siempre.
- ADR relacionadas: [`ADR-01`](ADR-01-Host-Delgado-Con-Composicion-De-Raiz-Unica.md), [`ADR-04`](ADR-04-Dos-Traducciones-Con-Tabla-Unica-Y-Sin-Codigos-Inventados.md).

## 10. Control de cambios

| Versión | Fecha | Descripción |
| --- | --- | --- |
| 1.0 | 2026-08-10 | Emisión inicial. Convierte la garantía de `RN-13` e `INV-09` en una propiedad contable —cuatro puntos fuera de la guardia, once dentro, una excepción declarada— verificada por una prueba de inspección en las dos direcciones, y sostiene como ausencia comprobable la de `RN-16`. Evalúa cinco alternativas, declara cuatro trade-offs —incluido el registro del riesgo aceptado del tramo sin cifrar, que no se reabre— y fija siete métricas de validación. |
