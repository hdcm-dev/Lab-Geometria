...
**Esta categoría no cambia nada, y eso es el resultado esperado.** `CU-12` §10 había adoptado **los ocho** con su fundamento —«`E-8` es el modo de falla que el propio intake llama el más probable de todos», porque lo produce la configuración regional de la máquina del alumno y no un error de programación— y esta categoría heredaba esa lectura. **La decisión la confirma**: el sample 02 conserva sus ocho cuerpos, `VER-02` conserva sus aserciones de recuento y la fila `SD-02` de la matriz de sensado no se toca. El punto abierto que la registraba, `PA-06` de [`../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md`](../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md) §11, queda **cerrado con su desenlace y su fecha**.
