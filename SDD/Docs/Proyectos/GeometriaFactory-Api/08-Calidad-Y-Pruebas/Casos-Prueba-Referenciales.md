...
#### TC-01 — Canje-De-Credenciales-Por-Un-Acceso-Firmado

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-01`; punto `A-01`; `US-01` |
...
#### TC-02 — Credenciales-Invalidas-Sin-Declarar-Que-Campo-Fallo

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-01`, `CU-09`; punto `A-01`; `US-02`; **segunda familia empobrecida** |
...
#### TC-03 — Cuenta-Pendiente-O-Bloqueada-Con-Motivo

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-01`; `RN-06`; `INV-06`; punto `A-01`; `US-03` |
...
| Salida esperada | Dos motivos distinguibles entre sí y del rechazo genérico de `TC-02`. **La situación de la cuenta sí se dice; cuál campo falló, no** |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-04 — Rechazo-Sin-Acceso-Vencido-O-Con-Firma-Ajena

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-02`; los **once** puntos bajo la guardia; `US-04` |
...
#### TC-05 — El-Papel-Que-Cada-Punto-Declara

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-02`; `RN-01`; `US-05` |
...
#### TC-06 — La-Guardia-Del-Cambio-Pendiente-Sobre-Todos-Los-Puntos-Salvo-Uno

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-02`; `RN-13`; `INV-09`; `US-06` |
...
#### TC-07 — Exactamente-Cuatro-Puntos-Fuera-De-La-Guardia

| Campo | Valor |
| --- | --- |
| Tipo | **Inspección con umbral exacto** |
| Cubre | NFR de puntos fuera de la guardia (`05` §8); `QG-05`; `RN-13`; los **quince** puntos de `05` §3.4 |
...
| Salida esperada | 4 y 11, en las dos direcciones, y **0** puntos que fijen contraseña sin credencial. Es el primer riesgo de `05` §9: **un punto nuevo fuera de la guardia hace que `RN-13` deje de valer y nada falla** |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

### 2.2 Cuentas de la comisión

#### TC-08 — Registro-De-Cuenta-Sin-Campo-De-Contrasena

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-03`; `RN-02`; `INV-01`; punto `A-02`; `US-07` |
...
#### TC-09 — Administrador-Solo-Mientras-No-Exista-Ninguna

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-03`; `RN-01`; `INV-05`, `INV-08`; punto `A-03`; `US-08` |
...
#### TC-10 — Cambio-De-Contrasena-Propia-En-Sus-Dos-Formas

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo, más inspección |
| Cubre | `CU-03`; `RN-16`, `RN-13`; punto `A-05`; `US-09`, `US-10`; NFR de puntos que fijan contraseña sin credencial |
...
#### TC-11 — Listado-De-Cuentas-Con-Su-Situacion-Y-Su-Marca

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-04`; punto `A-06`; `US-11` |
...
#### TC-12 — Cambio-De-Situacion-Con-Verificacion-De-Papel

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-04`; `RN-01`, `RN-06`, `RN-16`; punto `A-07`; `US-12` |
...
#### TC-13 — Baja-Transportando-El-Correo-Escrito-Como-Confirmacion

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-04`; `RN-07`; punto `A-08`; `US-13` |
...
#### TC-14 — Reseteo-Que-Devuelve-La-Provisoria-Una-Sola-Vez

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-05`; `RN-12`, `RN-14`; punto `A-09`; `US-14` |
...
#### TC-15 — El-Reseteo-No-Exige-Ni-Comprueba-La-Situacion-De-La-Cuenta

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-05`; `RN-15`; punto `A-09`; `US-15` |
...
| Salida esperada | Tres reseteos sobre tres estados distintos, y **0** motivos por situación. Es `RN-15` verificada **por ausencia** |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-16 — La-Provisoria-No-Aparece-En-Ninguna-Traza

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo, más inspección del registro del servidor |
| Cubre | `CU-05`; `RA-03`; punto `A-09`; `US-16` |
| Setup | El registro del servidor de la ejecución de `TC-14` |
...
#### TC-17 — Envio-Que-Responde-Con-Exito-Transportando-El-Estado-Decidido

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-06`; `RN-05`; `INV-04`; punto `A-10`; `US-17` |
...
#### TC-18 — Reenvio-De-Un-Borrador-Con-El-Texto-Vuelto-A-Pegar

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-06`; `RN-08`, `RN-04`; punto `A-11`; `US-18` |
...
#### TC-19 — Texto-Original-Sin-Normalizar-En-El-Borde-Y-Sin-Truncar

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo, con comparación byte a byte |
| Cubre | `CU-06`; `RN-08`; punto `A-10`; `US-19`; NFR de textos alterados; `QG-09` |
...
| Salida esperada | **0** caracteres de diferencia y **0** truncamientos silenciosos. Es el tercer riesgo de `05` §9: truncar **rompe `RN-08` en silencio** y el alumno lo descubre al ver el dibujo |
| Salida observada | Sin ejecutar |
| Estado | `Pendiente` |

#### TC-20 — Eliminacion-Con-Los-Dos-Alcances-Forzando-La-Peticion

| Campo | Valor |
| --- | --- |
| Tipo | **Forzando la petición**, integración por protocolo |
| Cubre | `CU-06`; `RN-04`, `RN-03`, `RN-11`; `INV-02`, `INV-03`; punto `A-12`; `US-20`; NFR de eliminaciones fuera de alcance; `QG-12` |
...
#### TC-21 — Listado-Sin-Parametro-Para-Pedir-Borradores-Ajenos

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo, más inspección |
| Cubre | `CU-07`; `RN-11`, `RN-03`; punto `A-13`; `US-21` |
...
#### TC-22 — Detalle-Con-Piezas-Componentes-Observaciones-Y-Comentario

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-07`; `RN-03`, `RN-11`; punto `A-14`; `US-22` |
...
#### TC-23 — Desenlace-Desde-Pendiente-Con-Su-Terminalidad

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-08`; `RN-10`, `RN-11`; `INV-07`; punto `A-15`; `US-23` |
...
#### TC-25 — Las-Tres-Familias-Empobrecidas-Son-Indistinguibles

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-09`; `RN-03`, `RN-02`; `INV-02`; `US-24`; NFR de respuestas indistinguibles; `QG-07` |
...
#### TC-26 — Ninguna-Respuesta-Expone-Direccion-Ruta-Secreto-Ni-Traza

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo, más inspección del registro del servidor |
| Cubre | `CU-09`; `RA-03`; `US-25`; NFR de respuestas que exponen; `QG-08` |
...
#### TC-27 — Cero-Codigos-Inventados-Y-Cero-Renombrados

| Campo | Valor |
| --- | --- |
| Tipo | Unit, **inspección con umbral exacto** |
| Cubre | `CU-09`; NFR de códigos con traducción; `QG-06` |
...
#### TC-28 — Cuatro-Puertos-Conectados-A-Su-Adaptador

| Campo | Valor |
| --- | --- |
| Tipo | Unit, **inspección con umbral exacto**, con fallo en construcción |
| Cubre | `CU-10`; `US-26`; NFR de puertos conectados; `QG-10` |
...
#### TC-29 — Una-Sola-Configuracion-De-Intercambio-Compartida

| Campo | Valor |
| --- | --- |
| Tipo | Unit, **inspección** |
| Cubre | `CU-10`; NFR de configuraciones de intercambio; `QG-10` |
...
#### TC-30 — Transformaciones-De-Esquema-Al-Arrancar
...
#### TC-31 — Arranque-Detenido-Y-Cero-Peticiones-Atendidas

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo, con la preparación forzada a fallar |
| Cubre | `CU-11`; `US-28`; NFR de peticiones con preparación incompleta; `QG-11` |
...
#### TC-32 — Salud-Que-Responde-Sin-Exigir-Acceso

| Campo | Valor |
| --- | --- |
| Tipo | Integración por protocolo |
| Cubre | `CU-11`; punto `A-16`; `US-29` |
...
#### TC-33 — Arranque-En-Frio-En-Menos-De-Treinta-Segundos
...
#### TC-34 — Latencia-Del-Listado-Y-Caudal-Sostenido
...
#### TC-35 — La-Coleccion-De-Peticiones-Reproducible

| Campo | Valor |
| --- | --- |
| Tipo | Ejecución de la colección, en la demostración de etapa |
| Cubre | `CU-12`; `US-30`; NFR de pasos de la colección; `QG-15` |
...
#### TC-36 — Sin-Canal-De-Sesion-Interactiva-Y-Sin-Intercambio-De-Origen-Cruzado
...
#### TC-37 — Forma-De-La-Piramide-De-Pruebas
...
| --- | --- | --- |
| Casos de verificación de este catálogo | **37**, `TC-01` a `TC-37` | Contar los encabezados de §2 |
...
| Inspecciones con umbral exacto | **5** — `TC-07`, `TC-24`, `TC-27`, `TC-28`, `TC-36` | §2, columna de tipo |
| Casos que verifican **forzando la petición** | **1** — `TC-20`, y es el único que la fuente exige así | §2.3 |
| Escenarios del intake §20 usados como cuerpo de petición | **8 de 8** | `E-1` en `TC-17`, `TC-19`; `E-2` en `TC-19`; `E-5` y `E-8` en `TC-17`; `E-7` en `TC-22`; `E-3`, `E-4` y `E-6` en `TC-35` y en la batería del validador que corre desde acá |
