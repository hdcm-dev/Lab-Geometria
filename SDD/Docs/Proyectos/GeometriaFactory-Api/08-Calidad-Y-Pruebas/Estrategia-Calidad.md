...
El intake §17.5.P.8 declara que el guion de pruebas de este proyecto de código pasa **«incluidas las nueve pruebas del validador»**. Esa batería es de `GeometriaFactory-Infrastructure` y **tiene diez casos**, no nueve: el intake §21 los cruza en una tabla de **diez** filas, la décima incorporada con `E-8` bajo el rótulo **[DECISIÓN 2026-08-09]**, y la Fase C de ese proyecto de código ya resolvió la lectura en diez.

**Esta categoría aplica diez y no baja la batería a nueve para que coincida con la redacción de la puerta.** La redacción es **anterior** a la incorporación del décimo caso y quedó sin propagar en **dos** lugares —§17.3.P.8 y §17.5.P.8—; corregirla es del Product Owner sobre su propio documento. Queda declarado como hueco en [`Matriz-Cobertura-Pruebas.md`](Matriz-Cobertura-Pruebas.md) §8, y la categoría 08 de `GeometriaFactory-Infrastructure` lo declara del mismo modo desde su lado.
