...
**Y el segundo artefacto que `Rules-Devops.md` §2.2 admite para este tipo no existe acá.** La tabla nombra un contrato de servicio versionado como artefacto secundario; el intake §17.5.P.3 declara que **el versionado del contrato es el del ensamblado de contratos** y que **no hay versionado de rutas porque no hay clientes de terceros**. Una guía de publicación de contrato describiría una entrega que nadie recibe.
...
| **Que el motor de contenedores del destino resuelva la referencia al repositorio**, con credenciales si es privado | **[A VERIFICAR]** en la fuente. Ver §2.1 | Intake §17.5.P.11 punto 5 |
| **Volumen persistente** para el almacén, **fuera de la imagen** | Si el almacén quedara dentro de la imagen, cada reemplazo de versión borraría el trabajo de la comisión | Intake §17.5.P.4; Definition of Done §1.4 |
| **Clave de firma del acceso**, nombrada por su función | Provista por variable de entorno o archivo montado, **fuera del repositorio y fuera de la imagen**. El archivo de composición la referencia; **nunca la contiene** | Intake §17.5.P.5 |
| **Un puerto publicado hacia el enrutador** | Es **el único punto de entrada al servidor propio** | `05` §5 |
| **La etiqueta de la etapa cerrada** | Es lo que se despliega, y lo que permite volver a cualquier demostración ya aprobada | Intake §17.5.P.7; Definition of Done §1.4 |
...
| 2 | **El arranque en frío no se estira** | `TC-33` | Menos de **30 segundos** [ASUNCIÓN del intake §17.5.P.10]. **Condicionado**: se mide y se registra |
...
| --- | --- | --- |
| El servicio desplegado está roto | Levantar la composición **desde la etiqueta de la etapa anterior**. El motor reconstruye en destino | Intake §17.5.P.8; [`ADR-08`](../05-Arquitectura-Tecnica/Adrs/ADR-08-Sin-Versionado-De-Rutas-Y-Despliegue-Conjunto.md) §6, trade-off 3 |
...
| Un cambio incompatible del contrato llegó a las dos unidades | **Se revierten las dos juntas**, desde el mismo estado del repositorio | Intake §17.4.P.3; [`Pipeline-CI-CD.md`](Pipeline-CI-CD.md) §7 |
| Una transformación de esquema quedó mal | **Volver a la etiqueta no deshace el esquema del almacén**: el esquema sobrevive al reemplazo. Se corrige con otra transformación, y lo único que restituye datos es el **respaldo** | [`../../GeometriaFactory-Infrastructure/09-Devops/Estrategia-Versionado.md`](../../GeometriaFactory-Infrastructure/09-Devops/Estrategia-Versionado.md) §4 |
...
Las **seis** de [`ADR-08`](../05-Arquitectura-Tecnica/Adrs/ADR-08-Sin-Versionado-De-Rutas-Y-Despliegue-Conjunto.md) §8, que esta categoría adopta **sin agregar ninguna**, con la columna de dónde se observa cada una:
