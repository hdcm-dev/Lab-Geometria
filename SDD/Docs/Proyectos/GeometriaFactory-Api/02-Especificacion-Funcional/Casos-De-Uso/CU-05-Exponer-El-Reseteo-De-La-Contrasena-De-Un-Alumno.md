...
| ~~`CONTRATO_RESETEO_NO_APLICABLE_A_CUENTA_SIN_CONTRASENA`~~ | — | **Retirado del conjunto cerrado** por `PRODUCT-INTAKE` 1.13 §4.1 (**RN-16**). Existía porque el camino declarado para una cuenta sin contraseña era el primer ingreso anónimo; suprimido ese camino, **el reseteo procede** y fija la provisoria en lugar de reemplazarla. El retiro cierra además la tensión con **RN-15**, que declara que el reseteo procede sobre `Pendiente`. **El identificador no se recicla** |
