# CU-00003 — Exponer el alta de cuenta y la credencial propia

**Producto:** Fábrica de Geometría
**Proyecto de código:** GeometriaFactory-Api
**Documento:** CU-00003-Exponer-El-Alta-De-Cuenta-Y-La-Credencial-Propia.md
**Versión:** 1.5
**Estado:** Aprobado
**Fecha:** 2026-08-15
**Autor:** Analista Funcional + API Designer (AG-02)
**Trazabilidad upstream:** [`NB-00001`](../../../../01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00001-Control-De-Admision-Al-Laboratorio.md), [`NB-00002`](../../../../01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00002-Identidad-Propia-Del-Alumno-Sin-Correo.md); `PRODUCT-INTAKE-Fabrica-De-Geometria.md` **1.34** (§4.1 RN-00013 y su fila de control de cambios 1.34: **con qué se autentica el cambio forzado**), **1.26** §4 (F-01, F-02, **F-04** precisada, F-05), §4.1 (RN-00001, RN-00002, RN-00006, RN-00013, **RN-00016**), §6 (flujo 1), §9 (X-1), §14 (RA-01, RA-03), §17.5.P.5; `Proyectos/GeometriaFactory-Contracts/.../CU-00002-Contrato-De-Administracion-De-Cuentas.md`; `Proyectos/GeometriaFactory-Application/.../CU-00001-Registrar-El-Alta-De-Una-Cuenta.md`, `.../CU-00010-Configurar-La-Cuenta-De-Administrador.md` y `.../CU-00003-Resolver-El-Ingreso-Y-La-Credencial-Del-Alumno.md`
...
| **A-17** | Responder **si el laboratorio ya tiene administrador**, y nada más. De **sólo lectura** | La pieza pública, **antes de que nadie se haya identificado**: es lo que el guardián 1 de `Web ADR-00003` §2 necesita saber para decidir a dónde mandar a quien pidió una ruta |

**A-17 es hermano de A-03 y por eso está en este contrato: es la misma ventana de alta, mirada desde afuera.** A-03 la cierra; A-17 dice si sigue abierta. Existe porque el **guardián 1** de `Web ADR-00003` §2 —«mientras no exista la cuenta de administrador, cualquier ruta pedida desvía al aprovisionamiento inicial; una vez que existe, esa ruta deja de armar formulario para siempre y desvía de forma neutra»— **nunca se pudo construir**: ninguno de los quince puntos anteriores servía para que un anónimo preguntara eso. `A-03` configura —es escritura—, `A-16` responde por la salud del servicio y `A-06` exige ser `Administrador`. El detalle de la decisión, lo que el punto revela y por qué no se le metió el dato a `A-16` están en [`../Definicion-Superficie-HTTP.md`](../Definicion-Superficie-HTTP.md) §3.

**El punto A-04 quedó retirado por `PRODUCT-INTAKE` 1.13, y su capacidad no.** Exponía el establecimiento de la contraseña en el primer ingreso y era **la única escritura de contraseña de esta superficie que ocurría sin credencial** —A-02 y A-03 también se ejercen sin credencial, y siguen haciéndolo: el **registro** es anónimo por diseño (`PRODUCT-INTAKE` **1.15** §4.1)—: su forma de identificación era el punto abierto más importante de esta categoría. **RN-00016** hace que habilitar produzca la contraseña provisoria, con lo cual la persona llega a elegir la suya **ya autenticada**, por **A-05**. El alumno sigue eligiendo su contraseña; lo que desapareció es el punto anónimo. **El identificador `A-04` no se recicla.**
...
- Para **A-05**, la petición llega de **una de dos maneras**, que `PRODUCT-INTAKE` **1.34** declara: con un **acceso firmado válido**, que atravesó la guardia de CU-00002 con su excepción declarada; o **sin acceso**, con el correo y la contraseña vigente, que es el **cambio forzado**. La segunda existe porque RN-00013 **no le emite sesión de trabajo** a la cuenta marcada: exigirle acceso firmado dejaba su pantalla inalcanzable. **En las dos, la vigente es obligatoria y no se emite ningún acceso.**
- Para **A-02**, **A-03** y **A-17**, la petición **no** trae acceso y **no** lo necesita. En **A-17** eso no es una concesión sino el requisito: quien pregunta es el guardián, y el guardián corre **antes** de que haya con qué identificarse.
...
5. Llega una petición a **A-17**, sin cuerpo y sin acceso. Se consulta si existe cuenta con papel `Administrador` y se responde `200` con **ese único dato**. **No se escribe nada y no hay estado del laboratorio que vuelva inválida la petición**: «todavía no» es una respuesta legítima y no un fallo, y por eso este punto tiene **un solo código de respuesta**.
...
**Ninguna de las cinco devuelve una contraseña, en claro ni derivada, y ninguna la registra.** Los únicos valores de credencial que esta superficie devuelve alguna vez son las dos provisorias que el sistema produce —en CU-00004 al habilitar y en CU-00005 al resetear—, y ninguna de las dos se registra.

## 7. Postcondiciones

- **A-02 con éxito:** existe una cuenta en situación `Pendiente`, sin credencial, que **todavía no obtiene acceso** (RN-00006). La credencial la recibe al ser habilitada, por A-07 (CU-00004).
...
| CA-10 | Una instancia **sin** administrador y la misma instancia **con** administrador ya configurado | Se invoca A-17 **sin acceso** en cada estado | Responde `200` en los dos, con el dato que corresponde; y el cuerpo lleva **exactamente un dato**: **0 apariciones** de correo, nombre, fecha o cantidad de cuentas |

## 9. Trazabilidad

| Dimensión | Referencia |
| --- | --- |
| Necesidad de negocio | NB-00001 por el alta y por la configuración inicial; NB-00002 por la credencial propia sin canal de correo |
| Reglas de negocio aplicables | [RN-02001](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02001-Administrador-Unico-Y-Papeles-Fijos.md), en el punto de configuración y su negativa. [RN-02002](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02002-Correo-Del-Alumno-Unico.md), en la traducción del correo ocupado. [RN-02006](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02006-Cuenta-Pendiente-O-Bloqueada-Sin-Acceso.md), por la situación inicial que el alta fija. [RN-02013](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02013-Cambio-Forzado-Antes-De-Toda-Otra-Capacidad.md), porque **A-05 es la única excepción de la guardia**, y desde el intake **1.34** también el único punto que se puede ejercer **sin sesión de trabajo presentando la contraseña vigente**, que es lo que vuelve alcanzable el cambio forzado |
| Regla de arquitectura del producto | **RA-03** en las cinco condiciones de §6; **RA-01** en los cuatro puntos, que la pieza pública ejerce servidor a servidor aunque **tres** de ellos no exijan acceso. **A-17 es el caso donde RA-01 más importa**: lo consulta un intermediario del servidor de la pieza pública y **ningún guion del navegador lo alcanza** |
| Puntos de acceso | A-02, A-03, A-05, A-17 |
| Contrato de uso que transporta | `GeometriaFactory-Contracts` `CU-00002`, incluida su reutilización por el cambio obligatorio |
| Historias de usuario a generar en 06 | US-00007, US-00008, US-00009, US-00010 |
| Componentes esperados en 05 | Tres puntos de acceso. **El punto abierto de identificación de A-04 quedó cerrado** por `PRODUCT-INTAKE` 1.13 §4.1 (RN-00016), con el retiro del punto |
...
| 1.5 | 2026-08-15 | **Agrega el punto de acceso `A-17` a este contrato, y con él se cierra el hueco que impedía construir el guardián 1 de `Web ADR-00003` §2.** `A-17` responde **si el laboratorio ya tiene administrador, y nada más**: es de **sólo lectura**, **no exige acceso** y tiene **un solo código de respuesta**, `200`. Entra en este contrato y no en el del punto de salud porque es **hermano de `A-03`** —la misma ventana de alta, mirada desde afuera: `A-03` la cierra, `A-17` dice si sigue abierta—; el detalle de la decisión, lo que el punto revela y **por qué no se le metió el dato a `A-16`** están en [`../Definicion-Superficie-HTTP.md`](../Definicion-Superficie-HTTP.md) §3, que sube a 1.7 por lo mismo. **§1**: fila del punto y prosa con el motivo por el que su ausencia no era un olvido sino un hueco funcional. **§3**: la precondición de «sin acceso» pasa a nombrar los **tres** puntos que lo son, con la constancia de que en `A-17` eso **es el requisito** y no una concesión. **§4**: paso **5**, con la constancia de que no escribe nada y de que «todavía no» es una respuesta legítima. **§8**: entra **CA-10**, que ejerce el punto en los dos estados del laboratorio y cuadra **0 apariciones** de correo, nombre, fecha o cantidad de cuentas en el cuerpo. **§9**: los puntos de acceso pasan a cuatro y `RA-01` pasa a nombrarlos, con la constancia de que **ningún guion del navegador alcanza `A-17`**. **Ningún flujo alternativo, ninguna condición de §6 y ninguna postcondición cambia**: el punto no escribe. **La decisión la tomó el orquestador, con el Product Owner avisado, y QUEDA A RATIFICACIÓN.** Sube minor. |
