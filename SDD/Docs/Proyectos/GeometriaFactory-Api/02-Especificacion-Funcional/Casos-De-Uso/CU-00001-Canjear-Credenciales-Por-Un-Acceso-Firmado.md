# CU-00001 — Canjear credenciales por un acceso firmado

**Producto:** Fábrica de Geometría
**Proyecto de código:** GeometriaFactory-Api
**Documento:** CU-00001-Canjear-Credenciales-Por-Un-Acceso-Firmado.md
**Versión:** 1.4
**Estado:** Aprobado
**Fecha:** 2026-08-14
**Autor:** Analista Funcional + API Designer (AG-02)
**Trazabilidad upstream:** [`NB-00002`](../../../../01-Necesidades-Negocio/Necesidades-De-Negocio/NB-00002-Identidad-Propia-Del-Alumno-Sin-Correo.md); `PRODUCT-INTAKE-Fabrica-De-Geometria.md` **1.34** (§4.1 RN-00013 y su fila de control de cambios 1.34: **con qué se autentica el cambio forzado**), **1.26** §4 (F-05, **F-04** precisada), §4.1 (RN-00006, RN-00013, **RN-00016**), §14 (RA-01, RA-03), §17.5.P.3, §17.5.P.5 y su nota de seguridad, §11 (RN-B5); `Proyectos/GeometriaFactory-Contracts/.../CU-00001-Contrato-De-Canje-De-Credenciales-Y-Sesion.md` completo; `Proyectos/GeometriaFactory-Application/.../CU-00003-Resolver-El-Ingreso-Y-La-Credencial-Del-Alumno.md`; `Proyectos/GeometriaFactory-Infrastructure/.../CU-00008-Emitir-El-Acceso-Firmado.md` y `CU-00006-Derivar-La-Contrasena-Y-Verificar-Una-Credencial.md`
...
- La clave de firma está provista por configuración (CU-00010). Sin ella **no se emite ningún acceso**.
...
| ~~`CONTRATO_CONTRASENA_NO_ESTABLECIDA`~~ | — | **Retirado del conjunto cerrado** por `PRODUCT-INTAKE` 1.13 §4.1 (**RN-00016**): habilitar produce y fija la contraseña provisoria, de modo que ninguna cuenta llega a estar habilitada sin contraseña. **El identificador no se recicla**, y el desvío del primer ingreso lo cubre hoy el código de la fila siguiente. Se conserva la fila tachada para que una cita vieja no quede sin respuesta |
| `CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` | `403` | La cuenta tiene una provisoria sin cambiar, **producida por la habilitación (RN-00016) o por el reseteo (F-26)**: desde el intake 1.13 es también el código del **primer ingreso**. Con motivo; la pieza pública lo convierte en el desvío a **A-05**, que desde el intake **1.34** se alcanza **con la contraseña actual** y sin acceso firmado. **No se emite acceso** |
...
**Que este punto no emita acceso a la cuenta marcada no la deja sin salida, y hasta el intake 1.34 sí la dejaba.** La contradicción era entre dos documentos vivos: acá se declara que la cuenta con la marca **no obtiene acceso**, y [`Definicion-Superficie-HTTP.md`](../Definicion-Superficie-HTTP.md) §3 ponía **A-05** bajo la guardia de sesión, que exige exactamente el acceso que esta tabla niega. Sin acceso emitido, **la pantalla del cambio quedaba inalcanzable y el alumno reseteado quedaba fuera del laboratorio**. El Product Owner lo resolvió declarando que la operación de cambio admite **dos formas de autenticarse**: con **sesión de trabajo**, el cambio corriente que cualquier cuenta puede hacer cuando quiera —incluida la del administrador—, y con **la contraseña actual**, el cambio forzado, donde **la provisoria que el administrador comunicó es la que autentica**. **`RN-00013` no se afloja y esta tabla no cambia**: la cuenta marcada se reconoce, **sigue sin obtener sesión de trabajo**, y la obtiene recién cuando vuelve a canjear con la contraseña que eligió.

**Por qué dos motivos distintos comparten el `403` y uno solo tiene el `401`.** Los dos del `403` describen **la situación de una cuenta que existe** y la persona necesita saber cuál es, porque de eso depende qué tiene que hacer después: esperar la habilitación o cambiar la provisoria. Eran tres hasta que **RN-00016** suprimió el establecimiento de la contraseña como camino propio. El `401` describe que **el par no corresponde a ninguna cuenta**, y ahí el producto deliberadamente no dice más: distinguir el correo desconocido de la contraseña equivocada permitiría averiguar por tanteo qué correos están registrados.
...
| CA-05 | Una cuenta con la contraseña reseteada, con la provisoria sin cambiar, presentando **la provisoria correcta** | Se canjea | Responde `403` con el motivo de cambio requerido y **0 accesos emitidos**. La credencial se reconoce y **no se admite** |
...
| Reglas de negocio aplicables | [RN-02006](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02006-Cuenta-Pendiente-O-Bloqueada-Sin-Acceso.md), con su tramo de traducción acá: la respuesta **con motivo**. [RN-02013](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02013-Cambio-Forzado-Antes-De-Toda-Otra-Capacidad.md): la cuenta marcada **se autentica y no obtiene sesión de trabajo**, que en esta superficie significa que la petición se reconoce y **no devuelve acceso**. [RN-02001](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-02001-Administrador-Unico-Y-Papeles-Fijos.md), por el papel que viaja en el acceso |
...
| Contrato de uso que transporta | `GeometriaFactory-Contracts` `CU-00001` |
| Historias de usuario a generar en 06 | US-00001, US-00002, US-00003 |
...
- **Este punto no aplica la guardia de `CU-00002`**, y es correcto: la guardia verifica un acceso, y acá todavía no hay ninguno. Lo que sí hace es **rechazar por sí mismo** a la cuenta marcada, que es el único caso en que las dos cosas se parecen.
- **El desvío tiene a dónde ir, y es la única forma que tiene de tenerla.** Desde `PRODUCT-INTAKE` **1.34**, **A-05** no exige acceso firmado: admite además autenticarse con la contraseña actual, que es lo que hace alcanzable el cambio forzado. Es **la misma forma que ya tenía el establecimiento de la primera contraseña** desde RN-00016 —el primer ingreso recorre este mismo camino—, y **no abre ninguna escritura de contraseña sin credencial**: la vigente sigue siendo obligatoria, y sin ella no se cambia nada.
...
| --- | --- | --- |
| 1.4 | 2026-08-14 | **Reconcilia con `Definicion-Superficie-HTTP.md` §3 la contradicción que dejaba una pantalla inalcanzable, con la resolución de `PRODUCT-INTAKE` 1.34.** Esta §6 declara que la cuenta con la marca **no obtiene acceso**, y §3 de la superficie ponía **A-05** bajo la guardia de sesión: sin acceso emitido, el alumno reseteado no podía llegar al cambio y quedaba fuera del laboratorio. El Product Owner resolvió que la operación de cambio admite **dos formas de autenticarse** —con sesión de trabajo, el cambio corriente; con la contraseña actual, el cambio forzado, donde la provisoria que el administrador comunicó es la que autentica—. **§6**: la fila de `CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` declara que el desvío a **A-05** se alcanza sin acceso firmado, y la prosa posterior explica la contradicción y su resolución. **§10**: nota nueva con la forma del desvío y con la constancia de que no aparece ninguna escritura de contraseña sin credencial. **Cabecera**: pasa a citar el intake **1.34**. **Ninguna fila de la tabla de errores cambia de código ni de respuesta, ningún criterio de aceptación cambia, y `RN-00013` no se afloja**: la cuenta marcada sigue sin obtener sesión de trabajo hasta que la contraseña se cambie. Sube minor. |
