...
| [`Definicion-Superficie-HTTP.md`](Definicion-Superficie-HTTP.md) | Documento de concepto central: los **dieciséis** puntos de acceso, los **diez** códigos de respuesta, las dos traducciones y qué está declarado por una fuente y qué es derivación de esta categoría |
...
| CU-02 | [Admitir la petición: acceso, papel y marca](Casos-De-Uso/CU-02-Admitir-La-Peticion-Acceso-Papel-Y-Marca.md) | La guardia transversal de los quince puntos restantes, y la excepción única | Propuesto |
...
| US-09 | Establecer la contraseña propia en el primer ingreso efectivo | CU-03 |
...
| --- | --- | --- |
| `Reglas-De-Negocio/RN-XX-<Nombre>.md` | **Omitido** | **Las quince reglas del producto viven en `GeometriaFactory-Domain`**, las quince con archivo propio allá, y acá se **referencian** por identificador y con enlace. §6 declara, regla por regla, dónde se ejerce cada una en esta capa. Es el mismo criterio que aplican `GeometriaFactory-Application` §9 y `GeometriaFactory-Infrastructure` §9 |
...
**Doce, y ninguno bloqueante. Ocho son propios de esta categoría y cuatro vienen declarados de aguas arriba y no se reabren.** Los tres primeros son huecos de la superficie que esta categoría **encontró y no resolvió**, y los tres se elevan al Product Owner.

| Punto | Situación | Quién lo resuelve |
| --- | --- | --- |
| **Cómo se identifica la cuenta al establecer la contraseña del primer ingreso** | **Propio, y es el más importante.** Es la única escritura de la superficie que ocurre **sin acceso firmado**, porque la persona todavía no puede obtener uno: el ensamblado de contratos declara la solicitud de establecimiento con «la contraseña elegida» y **no declara cómo viaja la identidad de la cuenta**. Un punto de acceso anónimo que acepte correo y contraseña nueva permitiría fijarle la contraseña a cualquier cuenta habilitada que todavía no la tenga. `CU-03` §10 deja escritas las dos salidas —transportar también la identidad con alguna prueba de posesión, o emitir un acceso de alcance acotado— y **no elige**, porque es una decisión de seguridad y no de forma | **Product Owner**, y `05-Arquitectura-Tecnica` |
| **Qué código del contrato recibe una operación de administrador pedida por un alumno** | **Propio.** El conjunto cerrado de diecisiete códigos tiene `CONTRATO_DESENLACE_EXCLUSIVO_DEL_ADMINISTRADOR`, que por su enunciado sólo cubre el desenlace. La capa de aplicación emite un motivo de facultad requerida en cuatro de sus casos de uso —el gobierno de cuentas, la revisión de la comisión, el desenlace y el reseteo—, y **para los otros tres caminos el contrato no declara ningún código**. Verificado recorriendo la §6 de los ocho contratos de uso. `CU-09` §10 lo declara y **no inventa un código**: los códigos son del ensamblado de contratos | **Product Owner**, y `GeometriaFactory-Contracts` si decide ampliar su conjunto cerrado |
| **Qué código del contrato recibe un envío o una reedición forzados fuera de `Borrador`** | **Propio.** El conjunto cerrado tiene `CONTRATO_ESTADO_NO_PERMITE_ELIMINAR`, cuyo enunciado vigente está **acotado a la eliminación y al camino del alumno**. La capa de aplicación emite motivos propios para el envío y para la reedición fuera de `Borrador`, y el contrato no tiene dónde ponerlos. La salida disponible sin cambiar nada es el código genérico, que es peor de lo que el producto ya sabe hacer en el caso vecino | **Product Owner**, y `GeometriaFactory-Contracts` |
| **Las rutas y los verbos definitivos** | **Propio.** Las **dos** únicas cosas que una fuente declara de la superficie son el punto de canje de credenciales y la existencia de un punto de salud, cuya ruta la fuente **no da**. Las dieciséis filas de `Definicion-Superficie-HTTP.md` §3 son una **propuesta derivada** de esta categoría, rotulada como tal fila por fila, y su forma definitiva se fija en 05 y se valida en el punto de control de la primera etapa | `05-Arquitectura-Tecnica` |
...
| **El alcance de la colección de peticiones** | **Propio.** El intake la declara en dos lugares que **no dicen lo mismo**: §16.1 la describe con «los escenarios E-1 a E-7 como cuerpo» y §18 S-2 nombra «los cuerpos de E-2 y E-5». Las dos son anteriores a que el escenario **E-8** entrara al intake, y ninguna de las dos se actualizó. `CU-12` adopta los **ocho** y lo declara como derivación, con el fundamento de que E-8 es el modo de falla que el propio intake llama **el más probable** de todos | **Product Owner**, para confirmar el alcance y actualizar §16.1 y §18 |
| **Vigencia exacta del acceso firmado** | **Propio.** El intake declara «corta» y «sin token de refresco», y no fija un número. Es el mismo punto que `GeometriaFactory-Infrastructure` §11 declara abierto, y esta categoría **no lo reabre ni lo resuelve**: lo hereda como condición de su guardia | `05-Arquitectura-Tecnica`, y Product Owner si quisiera fijarlo |
