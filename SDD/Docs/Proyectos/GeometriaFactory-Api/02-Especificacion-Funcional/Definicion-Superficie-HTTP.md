...
**Trazabilidad upstream:** `PRODUCT-INTAKE-Fabrica-De-Geometria.md` **1.34** (§4.1 RN-13 y su fila de control de cambios 1.34: **con qué se autentica el cambio forzado**, que es lo que reconcilia §3 con `CU-01` §6), **1.26** §17.5.P.3 (protocolo, consumidor, ausencia de CORS y de WebSockets, punto de canje y punto de salud), §17.5.P.5 (flujo, reclamos, respuestas, autorización, secretos), §17.5.P.2, §17.5.P.6, §17.5.P.9, §17.5.P.10, §17.5.P.11, §9 (X-9), §14 (**RA-01, RA-02, RA-03**), §4.1 (RN-03, RN-06, RN-09, RN-13, **RN-16**), §4 (**F-04** precisada, F-03); `Proyectos/GeometriaFactory-Contracts/02-Especificacion-Funcional/` completo, y en particular la §6 de sus ocho contratos de uso y `CU-06` §6 y §10, de donde sale el **conjunto cerrado de diecisiete códigos**; `Proyectos/GeometriaFactory-Application/02-Especificacion-Funcional/Especificacion-Funcional.md` §4 y §6; `Proyectos/GeometriaFactory-Infrastructure/03-UX-UI-DX/DX-Error-Messages.md` §1.3 y §2.3, que declara que la traducción de sus condiciones hacia afuera del proceso pertenece a este proyecto de código
...
| 1.6 | 2026-08-14 | **Reconcilia con `Api CU-01` §6 la contradicción que dejaba una pantalla inalcanzable, con la resolución de `PRODUCT-INTAKE` 1.34.** §3 ponía **A-05** bajo la guardia de sesión y `CU-01` §6 declara que la cuenta con la marca **no obtiene acceso**: sin acceso emitido, el alumno reseteado no podía llegar al cambio y quedaba fuera del laboratorio. **§3**, fila de **A-05**: la intención declara las **dos formas de autenticarse** —con sesión de trabajo y con la contraseña actual—, la columna de papel exigido las separa y la de códigos suma `403`, que la cuenta que dejó de estar habilitada entre el desvío y el envío ya podía producir. **§3**, recuento: pasa de «cuatro sin acceso firmado más once bajo la guardia» a **cuatro más A-05 con las dos formas más diez bajo la guardia**; cuatro más uno más diez siguen siendo **quince**, y **ningún punto de acceso se agrega, se retira ni se recicla**. **§3**, prosa nueva con la contradicción y su resolución, y la constancia de que la clase «escritura de contraseña sin credencial» **sigue ausente**, porque la forma sin sesión exige la vigente y sólo procede sobre una cuenta marcada. **§7**, séptima ausencia: la misma constancia, en la celda del porqué. **Cabecera**: pasa a citar el intake **1.34**. **`RN-13` no se afloja**: A-05 no emite acceso en ninguna de sus dos formas. Sube minor. |
