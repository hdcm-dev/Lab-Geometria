...
2. **Lo que está acá de más, no lo protege nadie.** Cada punto de acceso agrega una superficie donde la guardia puede faltar y donde una regla puede romperse. La lista completa es la única forma de comprobar que **los quince puntos que exigen acceso están los quince bajo la misma guardia**.
...
| A-04 | Establecer la contraseña propia en el primer ingreso efectivo | `POST` | `/cuentas/contrasena-inicial` **[derivado, y con su identidad abierta: §9]** | Ver §9 | `200`, `400`, `403`, `404` | CU-03 |
...
| A-07 | Cambiar la situación de una cuenta: habilitar, bloquear, rehabilitar | `POST` | `/cuentas/{id}/situacion` **[derivado]** | `Administrador` | `200`, `400`, `401`, `403`, `404` | CU-04 |
...
**Dieciséis puntos de acceso.** El recuento que importa y que se puede comprobar sobre la tabla: **cuatro no exigen acceso firmado** —A-01, A-02, A-03 y A-16—, **uno tiene su forma de identificación abierta** —A-04— y **los once restantes exigen acceso firmado y quedan bajo la guardia de `CU-02`**. Cuatro más uno más once son dieciséis.

**Por qué A-04 se cuenta aparte y no entre los que exigen acceso.** Porque no puede exigirlo: es la escritura que ocurre **antes** de que la persona pueda obtener uno. Es el único punto de la superficie con esa propiedad, y por eso su forma es un punto abierto y no una fila más.
...
El conjunto cerrado lo declara `GeometriaFactory-Contracts` `CU-06` §10 y es de **diecisiete** códigos, unión de los que declaran sus ocho contratos de uso. **Ninguno se agrega, ninguno se renombra y ninguno se traduce a texto acá.**
...
| `CONTRATO_CONTRASENA_NO_ESTABLECIDA` | `403` | Con motivo. La pieza pública lo convierte en el desvío al establecimiento de la contraseña |
| `CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` | `403` | Con motivo. **Es un solo código para todas las operaciones bloqueadas**, y acá es un solo código de respuesta para todas |
...
| `CONTRATO_RESETEO_NO_APLICABLE_A_CUENTA_SIN_CONTRASENA` | `409` | Ídem: no hay contraseña que reemplazar. **No se confunde con el establecimiento**, que es otro punto y otro solicitante |
...
Cada ausencia es una decisión declarada de una fuente, no un pendiente. Se enumeran porque **una superficie se audita también por lo que no expone**, y porque cuatro de estas seis se reintroducen fácil por comodidad.
...
Los seis primeros son propios de este documento y están recogidos en el índice maestro; el séptimo se hereda. **Ninguno es bloqueante y ninguno se resuelve acá.**

1. **La identidad en el establecimiento de la contraseña (A-04).** Es la única escritura que ocurre sin acceso firmado. El ensamblado de contratos declara la solicitud con la contraseña elegida y **no declara cómo viaja la identidad**. Las dos salidas visibles son transportar también la identidad con alguna prueba de posesión, o emitir para ese único paso un acceso de alcance acotado. **Esta categoría no elige**: es una decisión de seguridad.
2. **El código del contrato para una operación de administrador pedida por quien no lo es**, fuera del desenlace. Verificado recorriendo la §6 de los ocho contratos de uso: el único código de facultad del conjunto cerrado está acotado al desenlace, y el gobierno de cuentas, el reseteo y el listado de la comisión no tienen ninguno.
3. **El código del contrato para un envío o una reedición forzados fuera de `Borrador`.** El conjunto cerrado tiene el código análogo **acotado a la eliminación**.
4. **Las rutas y los verbos definitivos**, que en §3 son propuesta derivada salvo el canje.
5. **La distinción entre `500` y `503` bajo un único código de contrato**, que §6 adopta y declara. Si el Product Owner quisiera que la pieza pública pudiera distinguirlas por el contrato y no por el número, haría falta un código nuevo en el conjunto cerrado, que es decisión del ensamblado de contratos.
6. **El límite de tamaño del cuerpo de una petición.** Ninguna fuente lo declara. Es el mismo hueco que la capa que interpreta el texto declaró abierto para el texto, y acá reaparece con una consecuencia propia: **un límite mal elegido trunca el texto de un alumno y rompe RN-08 sin que nada falle**.
7. **La vigencia exacta del acceso firmado**, declarada «corta» y sin número por el intake, y ya declarada abierta por `GeometriaFactory-Infrastructure`. **No se reabre acá.**
