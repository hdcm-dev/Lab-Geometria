...
**Trazabilidad upstream:** [`Casos-Prueba-Referenciales.md`](Casos-Prueba-Referenciales.md) 1.0; [`../02-Especificacion-Funcional/Especificacion-Funcional.md`](../02-Especificacion-Funcional/Especificacion-Funcional.md) 1.9 §3 y §4; [`../02-Especificacion-Funcional/Definicion-Modelo-De-Dominio.md`](../02-Especificacion-Funcional/Definicion-Modelo-De-Dominio.md) §4; [`../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md`](../05-Arquitectura-Tecnica/Arquitectura-Proyecto-Codigo.md) 1.0 §3.1 y §8
...
| --- | --- | --- | --- | --- |
| CU-01 Registrar el alta de un alumno | Given los datos obligatorios y la unicidad declarada comprobada, When se constituye el alumno, Then la cuenta nace `Pendiente` y sin credencial; con un dato ausente, con la unicidad no declarada, con credencial aportada, con estado pedido o con papel `Administrador`, Then se rechaza | `TC-01`, `TC-02` | Unit | `Pendiente` |
| CU-02 Gobernar el ciclo de vida de la cuenta | Given una cuenta de alumno y la provisoria derivada, When se la habilita, bloquea o rehabilita, Then la transición procede y la habilitación fija la credencial y pone la marca; When se la da de baja con confirmación coincidente, Then arrastra sus trabajos | `TC-03`, `TC-04`, `TC-05` | Unit e integración interna | `Pendiente` |
| CU-03 Fijar y reemplazar la credencial derivada | Given la credencial vigente declarada verificada y un valor nuevo ya derivado, When se reemplaza, Then el valor se reemplaza; y si la marca estaba puesta, Then además se levanta | `TC-08`, `TC-09`, `TC-03` | Unit e integración interna | `Pendiente` |
| CU-04 Evaluar la admisibilidad de la cuenta | Given una cuenta en cada uno de sus tres estados, con y sin la marca, When se evalúa la admisibilidad, Then devuelve admisible o no admisible **con su motivo**, y nunca lanza | `TC-10`, `TC-09` | Unit | `Pendiente` |
| CU-05 Crear y reeditar un trabajo | Given dueño, nombre, fecha declarada y texto original, When se constituye, Then nace en `Borrador` con el texto íntegro; When se reedita fuera de `Borrador`, Then se rechaza | `TC-11`, `TC-12` | Unit | `Pendiente` |
| CU-06 Reconstruir el conjunto de piezas | Given un conjunto de piezas con su posición en el conjunto raíz, When se lo adopta, Then las posiciones se conservan sin renumerar y la familia se deriva del tipo; una figura no adoptada **reserva su posición** | `TC-13`, `TC-14` | Integración interna | `Pendiente` |
| CU-07 Registrar las observaciones del trabajo | Given una advertencia con su par de valores y un error con índice de figura y campo, When se los registra, Then se adoptan; sin el par o sin la ubicación, Then se rechazan | `TC-15`, `TC-16`, `TC-14` | Unit e integración interna | `Pendiente` |
| CU-08 Gobernar el estado del trabajo en el envío | Given un trabajo en `Borrador` cuyo resultado no trae errores, When se lo envía, Then pasa a `Pendiente` aunque tenga advertencias; con al menos un error, Then queda en `Borrador` | `TC-17`, `TC-18`, `TC-19` | Integración interna | `Pendiente` |
| CU-09 Resolver el acceso del alumno a un trabajo | Given un trabajo ajeno y uno inexistente, When se resuelve el acceso, Then los dos resultados son idénticos; ver procede en los cuatro estados y reeditar y eliminar sólo en `Borrador` | `TC-20`, `TC-12` | Unit | `Pendiente` |
| CU-10 Resolver el desenlace del trabajo | Given un trabajo en `Pendiente` y el papel `Administrador`, When se lo aprueba o se lo rechaza, Then alcanza su estado terminal con comentario opcional; en otro estado o con otro papel, Then se rechaza | `TC-22`, `TC-19` | Unit | `Pendiente` |
| CU-11 Resolver el alcance del administrador | Given un trabajo en `Borrador`, When el administrador consulta su alcance, Then queda fuera; en los otros tres estados, Then entra y admite eliminación | `TC-21` | Unit | `Pendiente` |
...
| Dependencias salientes del proyecto de código | Exactamente **0** y **0** | `TC-24` | Inspección del archivo de proyecto, y revisión del pull request | `Pendiente` |
| Cobertura del catálogo de condiciones | **100 %** de las **42** condiciones alcanzadas, y **0** emitidas fuera del catálogo | `TC-23` | Prueba de inspección que compara los dos conjuntos en las dos direcciones | `Pendiente` |
| Ejercicio de los invariantes | **100 %** de los **nueve** con prueba de violación rechazada, **sin dobles** | `TC-26`, sobre la tabla de §5 | Prueba de inspección sobre la matriz, revisada al cerrar cada etapa | `Pendiente` |
| Advertencias de construcción | Exactamente **0** | Gate `QG-01`, **no un caso de prueba** | Etapa `build` del pipeline | `Pendiente` |

**Los dos valores rotulados [ASUNCIÓN] se citan con su rótulo y no se convierten en compromiso.** Su confirmación está pendiente del Product Owner en el intake §22 —asunción `A-3` para la cobertura y `A-5` para el tiempo— y su conversión en trabajo es `BT-15`. Hasta entonces sus gates son **condicionados** ([`Estrategia-Calidad.md`](Estrategia-Calidad.md) §3.1): se miden y se registran, y no bloquean la fusión.
...
| --- | --- | --- | --- | --- |
| RN-01 Administrador único y papeles fijos | INV-05 | `TC-02`, `TC-05`, `TC-06`, `TC-21`, `TC-22` | Unit | `Pendiente` |
| RN-02 El correo del alumno es único | INV-01 | `TC-01`, `TC-02`, `TC-06` | Unit | `Pendiente` |
| RN-03 Trabajo ajeno indistinguible de inexistente | INV-02 | `TC-11`, `TC-20` | Unit | `Pendiente` |
| RN-04 Eliminación acotada al borrador para el alumno | INV-03 | `TC-04`, `TC-12`, `TC-20`, `TC-21` | Unit e integración interna | `Pendiente` |
| RN-05 No se pasa a estado `Pendiente` con errores de validación | INV-04 | `TC-17`, `TC-18` | Integración interna | `Pendiente` |
| RN-06 Cuenta `Pendiente` o `Bloqueado` sin acceso | INV-06 | `TC-03`, `TC-08`, `TC-10` | Unit | `Pendiente` |
| RN-07 Baja con arrastre y confirmación escrita | Ninguno | `TC-04` | Integración interna | `Pendiente` |
| RN-08 Texto original conservado íntegro | Ninguno | `TC-11`, `TC-18` | Unit e integración interna | `Pendiente` |
| RN-09 Observación de error con posición y campo | Ninguno | `TC-13`, `TC-14`, `TC-16` | Unit e integración interna | `Pendiente` |
| RN-10 Desenlace exclusivo del administrador y terminalidad | INV-07 | `TC-19`, `TC-22` | Unit | `Pendiente` |
| RN-11 El administrador no ve los borradores | Ninguno | `TC-21`, `TC-22` | Unit | `Pendiente` |
| RN-12 El reseteo conserva la cuenta y sus trabajos | INV-09 | `TC-07`, `TC-09` | Integración interna | `Pendiente` |
| RN-13 Cambio forzado antes de toda otra capacidad | INV-09 | `TC-09`, `TC-10` | Unit e integración interna | `Pendiente` |
| RN-14 La provisoria la produce el sistema | Ninguno | `TC-03`, `TC-07` | Unit e integración interna | `Pendiente` |
| RN-15 Resetear no exige cuenta habilitada | Ninguno | `TC-05`, `TC-07` | Unit e integración interna | `Pendiente` |
| RN-16 Habilitar produce la provisoria | INV-09 | `TC-03`, `TC-09`, `TC-10` | Unit e integración interna | `Pendiente` |

**Dieciséis de dieciséis reglas con al menos un caso de prueba.** El reparto de la columna de invariante es **diez con invariante y seis sin él**, que es exactamente el que declaran `02` §4 y `05` §10.2; esta matriz lo refleja y no lo redefine. **RN-12, RN-13 y RN-16 comparten INV-09**, con la lectura que la categoría 02 adoptó de la columna del propio invariante en el intake §17.1.P.2, declarando que la prosa de esa sección es ambigua; esta categoría hereda esa lectura y **no afirma que la prosa del intake la respalde**.

**RN-14 tiene caso de prueba aunque `05` §10.2 declare que ningún componente de este proyecto de código la gobierna.** Lo que se verifica acá no es la producción de la provisoria —que ocurre afuera— sino su consecuencia sobre la superficie: que la habilitación y el reseteo **exigen el valor ya derivado** y lo rechazan vacío.

## 5. Trazabilidad invariante ↔ tests

Nueve filas, una por invariante de [`../02-Especificacion-Funcional/Definicion-Modelo-De-Dominio.md`](../02-Especificacion-Funcional/Definicion-Modelo-De-Dominio.md) §4. Es la tabla que `TC-26` recorre.

| Invariante | Componente que lo sostiene (`05` §10.3) | Test de violación rechazada | Usa dobles | Estado |
| --- | --- | --- | --- | --- |
| INV-01 El correo del alumno es único | Núcleo de entidades | `TC-02` (alta sin la unicidad declarada), `TC-06` | No | `Pendiente` |
| INV-02 Un alumno sólo accede a sus propios trabajos | Máquina de estados del trabajo | `TC-20` (trabajo ajeno), `TC-11` | No | `Pendiente` |
| INV-03 El alumno elimina sólo en `Borrador` y sobre trabajo propio | Máquina de estados del trabajo | `TC-20`, `TC-12` | No | `Pendiente` |
| INV-04 Un trabajo `Finalizado` no tiene errores de interpretación | Máquina de estados del trabajo | `TC-18` (envío con error no transiciona) | No | `Pendiente` |
| INV-05 Existe exactamente un administrador | Guardas de cuenta | `TC-06` (segunda configuración rechazada) | No | `Pendiente` |
| INV-06 Cuenta `Pendiente` o `Bloqueado` sin acceso | Evaluador de admisibilidad | `TC-10`, `TC-08` | No | `Pendiente` |
| INV-07 Un estado terminal no cambia de estado ni de contenido | Máquina de estados del trabajo | `TC-19` (cuatro operaciones sobre los dos estados terminales) | No | `Pendiente` |
| INV-08 La cuenta de administrador está siempre `Habilitado` | Guardas de cuenta | `TC-05` (las cinco operaciones rechazadas), `TC-02`, `TC-06` | No | `Pendiente` |
| INV-09 Con la marca puesta la cuenta no ejerce ninguna otra capacidad | Evaluador de admisibilidad | `TC-10`, `TC-09` (la marca se levanta sólo con el cambio efectivo) | No | `Pendiente` |
