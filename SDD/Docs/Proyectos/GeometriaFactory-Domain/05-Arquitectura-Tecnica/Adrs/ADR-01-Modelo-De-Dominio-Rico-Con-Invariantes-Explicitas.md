# ADR-01 — Modelo de dominio rico con invariantes explícitas y cero dependencias

**Proyecto de código:** GeometriaFactory-Domain
**Documento:** ADR-01-Modelo-De-Dominio-Rico-Con-Invariantes-Explicitas.md
**Versión:** 1.0
**Estado:** Propuesto
**Fecha:** 2026-08-10
...
Lo que motiva la decisión: `PRODUCT-INTAKE` §17.1.P.2 exige que los invariantes y las transiciones de estado se puedan probar **sin infraestructura**, y §17.1.P.1 declara la condición estructural que lo permite, «`Domain` sin dependencias». Aguas abajo, la regla de no-regresión acumulativa del producto (`PRODUCT-INTAKE` §15) hace que el costo de ejercer esas pruebas se pague en cada etapa, de modo que su velocidad es una propiedad arquitectónica y no un detalle.

Motivación upstream: NB-01, NB-02, NB-03, NB-04, NB-05 y NB-09, que son las seis necesidades a las que trazan los trece casos de uso de la categoría 02; RN-01 a RN-16; INV-01 a INV-09.
...
**Propuesto** desde 2026-08-10. Deriva de una decisión ya tomada aguas arriba (`PRODUCT-INTAKE` §17.1.P.2), de modo que lo que esta ADR agrega es su registro formal, la partición en dos agregados y la puerta de verificación.
...
| Modelo anémico, con la lógica en los servicios de aplicación | Menos tipos; toda la lógica junta y fácil de recorrer | Los invariantes se irían al nivel 1, y probarlos exigiría la capa de aplicación: se pierde exactamente la propiedad que el intake declara como motivo (§17.1.P.2) |
| Entidades del proveedor de persistencia como modelo de dominio | Un solo juego de tipos entre dominio y base; menos mapeo | Obliga a referenciar una biblioteca de persistencia desde el nivel 0 y viola la regla de dependencias hacia adentro (`PRODUCT-INTAKE` §17.1.P.2) |
...
1. Los **nueve** invariantes son verificables con pruebas unitarias puras y sin dobles, que es lo que `PRODUCT-INTAKE` §17.1.P.6 declara como estrategia de este proyecto de código.
...
1. **Se acepta la duplicación aparente entre entidades y tipos de transferencia.** Es deliberada y es lo que impide que un cambio de dominio rompa el contrato que cruza la frontera de proceso (`PRODUCT-INTAKE` §17.1.P.12).
...
4. **Dos agregados obligan a que la atomicidad la establezca el consumidor** cuando una operación toca los dos, como la baja de una cuenta que arrastra sus trabajos (RN-07).
...
- Los nombres de tipos y de espacios de nombres **no se fijan acá**: quedan abiertos hasta el punto de control de la etapa `a` (`PRODUCT-INTAKE` §17.1.P.11).
...
| Tiempo de la batería del dominio | Menos de **10 segundos** [ASUNCIÓN del intake, §17.1.P.10] | Duración total reportada en la etapa de `test` |
| Cobertura | **90 %** de líneas y **85 %** de ramas [ASUNCIÓN del intake, §17.1.P.6] | Informe de cobertura del pipeline |
...
- `PRODUCT-INTAKE-Fabrica-De-Geometria.md` 1.15 §17.1.P.1, §17.1.P.2, §17.1.P.6, §17.1.P.10, §17.1.P.11 y §17.1.P.12; §14; §15.
...
- ADR relacionadas: [`ADR-02`](ADR-02-Superficie-Publica-De-Guardas-Y-Resultados-Tipados.md), [`ADR-05`](ADR-05-Guarda-Unica-De-Admisibilidad.md), [`ADR-06`](ADR-06-El-Dominio-No-Lee-El-Reloj-Ni-El-Conjunto.md).
