...
**Trazabilidad upstream:** `00-Contexto/Vision-Producto.md` §1, §3, §9; `00-Contexto/Alcance-Producto.md` §4.1, §5, §8; `01-Necesidades-Negocio/Necesidades-Negocio.md` §2, §4 y §5.3, y las necesidades NB-01 a NB-06 y NB-09; `PRODUCT-INTAKE-Fabrica-De-Geometria.md` §17.1 íntegro —en particular §17.1.P.2 con los **nueve** invariantes, INV-08 adoptado e INV-09 nuevo—, §4 (capacidad **F-26**), §4.1 (las **trece** reglas, con RN-12 y RN-13 nuevas), §7 (CL-7 reescrito), §9 (X-2 retirada) y §4.2 (modelo de estados del trabajo), §13 y §14 (composición), §6, §7, §12, §20 y §21
...
| `Reglas-De-Negocio/RN-XX-<Nombre>.md` | Trece reglas de negocio, una por archivo |
...
Las trece reglas del producto, con el invariante que expresa a cada una como condición permanente sobre los datos. La correspondencia es de PRODUCT-INTAKE §17.1.P.2: **los invariantes no son reglas distintas, son las mismas vistas desde el dominio.**
...
Las cuatro reglas sin invariante asociado lo están por un motivo declarado: RN-07, RN-08 y RN-09 describen comportamientos y no condiciones permanentes sobre el estado, y RN-11 es una regla de alcance de consulta (PRODUCT-INTAKE §17.1.P.2). **RN-12 y RN-13 comparten invariante**, INV-09, y no es un descuido de la tabla: el intake lo declara así en §17.1.P.2, porque son las dos mitades de la misma condición —qué conserva el reseteo y qué no puede la cuenta hasta cambiar la provisoria—. El fundamento está en `Definicion-Modelo-De-Dominio.md` §4.3.

Los nueve invariantes no llevan archivo propio: son propiedades permanentes del modelo y viven enunciados en [`Definicion-Modelo-De-Dominio.md`](Definicion-Modelo-De-Dominio.md) §4.1. **INV-08, que esta categoría había propuesto como candidato, está adoptado** por `PRODUCT-INTAKE` §17.1.P.2 y se cuenta entre los vigentes; §4.2 conserva el registro de su recorrido. **INV-09 es nuevo del intake 1.7** y es el que sostiene a las dos reglas del reseteo.
...
| --- | --- | --- | --- |
| NB-01 · Control de admisión y de bajas del laboratorio | CU-12, CU-01, CU-02, CU-04, CU-13 | RN-01, RN-02, RN-06, RN-07, RN-12 | US-01, US-02, US-04, US-05, US-08, US-24, US-25, US-26 |
| NB-02 · Identidad propia del alumno sin canal de correo | CU-01, CU-03, CU-04, CU-13 | RN-01, RN-02, RN-06, RN-12, RN-13 | US-01, US-03, US-06, US-07, US-08, US-26, US-27 |
...
| **Cómo llega la cuenta reseteada al cambio de contraseña** | RN-13 dice que la cuenta con la marca puesta «**ingresa**, y lo único que puede hacer es cambiar la contraseña». Esta categoría lo modela **sin ingreso**: CU-04 devuelve no admisible con el motivo `CAMBIO_DE_CONTRASENA_PENDIENTE`, y el reemplazo de CU-03 procede igual, porque exige la credencial vigente verificada y no una sesión previa. Es exactamente la forma con la que el producto ya resuelve el primer ingreso efectivo, donde `CREDENCIAL_NO_ESTABLECIDA` tampoco produce acceso y encamina al establecimiento. **La lectura es de esta categoría** y la diferencia es observable sólo desde la capa que emite el acceso: si el Product Owner quiso literalmente que la cuenta obtenga acceso y quede confinada a una única ruta, la guarda dejaría de ser un motivo de no admisibilidad y pasaría a ser una comprobación por capacidad, con impacto en `GeometriaFactory-Api` y en `GeometriaFactory-Web` | Product Owner, con una precisión en `PRODUCT-INTAKE` §4.1 RN-13 o §17.1.P.2 |
