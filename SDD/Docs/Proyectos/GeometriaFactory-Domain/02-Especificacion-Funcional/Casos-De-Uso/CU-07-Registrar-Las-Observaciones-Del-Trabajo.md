...
- **El trabajo conoce la cantidad de figuras de su conjunto raíz**, que CU-06 incorporó al reconstruirlo. Es el rango contra el que se valida la posición de cada observación, y es lo que hace comprobable el borde anterior: una observación puede ubicar una figura que no se reconstruyó, pero no una que el texto no trae.
- La posición que trae cada observación es la de la figura **en el conjunto raíz del texto**, no la de una pieza adoptada: una figura que no se pudo reconstruir conserva su posición reservada y sigue siendo ubicable (CU-06 FA-03).
...
4. El dominio verifica que toda observación de especie error de validación indique la posición de la figura y el campo (RN-09), y que esa posición pertenezca al rango del conjunto raíz interpretado, esté o no adoptada la pieza correspondiente.
...
7. El dominio deja disponible, para el consumidor, si el trabajo tiene al menos una observación de especie error de validación, que es la condición que gobierna el envío: con ella el trabajo queda en `Borrador` y sin ella pasa a estado `Pendiente` (CU-08).
...
| Invariantes | INV-04, en cuanto la especie de cada observación es lo que después decide el envío y sostiene que un trabajo `Finalizado` tenga el texto interpretado sin errores. **RN-08 y RN-09 no tienen invariante asociado** (PRODUCT-INTAKE §17.1.P.2) |
...
- Una advertencia nunca impide que el trabajo pase a estado `Pendiente`. Esa consecuencia es de RN-05 y se ejerce en CU-08.
...
- **La observación no es el comentario del administrador.** La observación la emite el producto al interpretar el texto y hay tantas como defectos; el comentario lo escribe una persona, es un atributo del trabajo y hay a lo sumo uno. El comentario se registra por CU-10 y no por este caso de uso.
