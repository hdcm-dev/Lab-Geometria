...
| Credencial derivada | Valor derivado de la contraseña del alumno, que el dominio recibe ya derivado y nunca en claro. En el auto-registro, **sin valor mientras la cuenta está `Pendiente` y con valor desde el acto de habilitación** (RN-16); con valor desde el alta en la configuración del administrador | `Definicion-Modelo-De-Dominio.md`, CU-02, CU-03, CU-04, CU-12, RN-06, RN-16 | «Hash de contraseña» en las fuentes técnicas |
...
| Baja de la cuenta | Operación destructiva e irreversible que elimina la cuenta y todos sus trabajos, cualquiera sea el estado de cada uno. No es un estado de cuenta, y **no es el reseteo de contraseña**: ver esa entrada | `Definicion-Modelo-De-Dominio.md`, CU-02, RN-01, RN-07, RN-12 | «Baja física» en las fuentes |
| Contraseña provisoria | Contraseña que **el sistema produce** al **habilitar** una cuenta de alumno (RN-16) o al **resetear su contraseña** (RN-14), que el administrador le comunica por fuera del producto y que la cuenta está obligada a reemplazar antes de ejercer cualquier otra capacidad. El dominio nunca la conoce ni la produce: llega ya derivada | `Definicion-Modelo-De-Dominio.md`, CU-02, CU-13, CU-03, CU-04, RN-12, RN-13, RN-16 | — |
| Marca de cambio de contraseña pendiente | Atributo del alumno, puesto o levantado, que declara que su credencial vigente es una contraseña provisoria. La ponen **las dos** operaciones que producen una contraseña provisoria —la **habilitación** (CU-02, RN-16) y el **reseteo** (CU-13, RN-14)— y la levanta **únicamente** el reemplazo hecho por la propia cuenta. Se nombra **siempre con la palabra «marca»**, porque `Pendiente` a secas ya nombra un estado de cuenta y un estado de trabajo: ver §3.3 | `Definicion-Modelo-De-Dominio.md`, CU-02, CU-13, CU-03, CU-04, RN-12, RN-13, RN-16 | «Cambio de contraseña pendiente» como nombre del atributo. **No** se dice «cuenta pendiente» para nombrarla |
| Reseteo de contraseña | Operación **conservadora** del administrador sobre una cuenta de alumno: **el sistema le produce** una contraseña provisoria (RN-14) y el dominio le pone la marca, sin cambiar su estado de cuenta, su papel, su identidad ni ninguno de sus trabajos, y **cualquiera sea el estado de esa cuenta**. **Resetear no es dar de baja** | `Definicion-Modelo-De-Dominio.md`, CU-13, CU-02, RN-12, RN-14, RN-15 | — |
...
| 1.4 | 2026-08-09 | Absorbe `PRODUCT-INTAKE` **1.7**, que incorpora la capacidad **F-26**, las reglas **RN-12** y **RN-13** y el invariante **INV-09**. §2 da de alta tres términos: **«contraseña provisoria»**, **«marca de cambio de contraseña pendiente»** —con la exigencia de nombrarla siempre con la palabra «marca», porque `Pendiente` ya colisiona en §3.3— y **«reseteo de contraseña»**, con la advertencia de que **no es la baja de la cuenta**, que es la confusión que la capacidad viene a cerrar. La entrada «baja de la cuenta» remite a ella. Las celdas de ocurrencias que contaban «los doce CU» pasan a trece. |
| 1.5 | 2026-08-09 | Absorbe las dos decisiones del Product Owner sobre **F-26**. §2 corrige dos definiciones sin dar de alta ni de baja ningún término: **contraseña provisoria** deja de atribuirle al administrador la elección del valor —**la produce el sistema**, y el dominio no la conoce ni la produce—, y **reseteo de contraseña** explicita que procede **cualquiera sea el estado de la cuenta**, que es lo que CU-13 FA-02 y RN-12 §3 ya declaraban. Ningún alias se agrega ni se prohíbe y los recuentos de artefactos no cambian. |
| 1.6 | 2026-08-09 | Absorbe el `PRODUCT-INTAKE` **1.10**. La entrada **«reseteo de contraseña»** de §2 decía que el administrador «le fija una contraseña provisoria», que es la formulación anterior a la decisión del Product Owner sobre quién la produce y que **RN-14** invierte: pasa a decir que **el sistema la produce** y el dominio pone la marca. La columna de dónde se usa suma **RN-14** y **RN-15**, las dos reglas que el intake 1.10 incorpora sobre esta operación; la viñeta que declara que el reseteo procede **cualquiera sea el estado de la cuenta** ya era correcta y ahora tiene regla propia. **Ningún término se acuña ni se retira, ninguna forma obligatoria cambia y el recuento de términos no cambia.** Sube minor. |
| 1.7 | 2026-08-10 | Absorbe el `PRODUCT-INTAKE` **1.13** y su regla **RN-16**. La entrada **«credencial derivada»** de §2 decía que en el auto-registro queda sin valor «hasta el primer ingreso efectivo», y eso dejó de ser cierto: la credencial se fija **en el acto de habilitación**, con la contraseña provisoria que el sistema produce. La columna de dónde se usa suma **CU-02** y **RN-16**. **Ningún término se acuña ni se retira**: la provisoria y la marca ya estaban declaradas y RN-16 no cambia lo que nombran, sólo agrega un acto que las produce. Sube minor. |
| 1.8 | 2026-08-10 | **Cierra el hallazgo `C-08` (P2) del informe de auditoría `SDD/Docs/Audit/Coherencia-Corpus-r1.md` 1.0.** La cabecera de trazabilidad declaraba derivarse del `PRODUCT-INTAKE` **1.7**, versión archivada, y pasa a declarar la **1.14**, vigente. La **1.7** es la versión cuya letra sobre **RN-13** e **INV-09** fue precisada en la 1.8 y corregida en la 1.14, que es exactamente el punto donde el corpus más se equivocó. Se revisó el cuerpo antes de mover la cabecera y **no arrastra ninguna decisión de las versiones intermedias**: no queda en él ningún recuento de «quince reglas» ni de «diecisiete códigos», ninguna cita a la exclusión **X-2** como vigente y ninguna afirmación de que la marca de cambio de contraseña pendiente la ponga únicamente el reseteo. **Cierra además el hallazgo `C-01` (P0) en dos entradas de §2 que el informe no registra**: «Marca de cambio de contraseña pendiente» declaraba que «la pone **únicamente** el reseteo» —lo contrario de lo que declaran `Definicion-Modelo-De-Dominio.md` §4.1, **RN-16** e **INV-09** en el intake 1.14— y «Contraseña provisoria» la definía como la que el sistema produce «al **resetear**» una cuenta. Las dos pasan a declarar los **dos** actos que producen provisoria y ponen la marca —la habilitación (CU-02, RN-16) y el reseteo (CU-13, RN-14)—, con la levantada únicamente por el reemplazo hecho por la propia cuenta, y las dos columnas de procedencia suman **CU-02** y **RN-16**. Es la misma frase que el informe levanta en §2.1 de `Definicion-Modelo-De-Dominio.md`, replicada en el glosario del mismo proyecto de código. **Ningún otro término, ninguna forma calificada y ninguna colisión declarada cambia.** Sube minor. |
