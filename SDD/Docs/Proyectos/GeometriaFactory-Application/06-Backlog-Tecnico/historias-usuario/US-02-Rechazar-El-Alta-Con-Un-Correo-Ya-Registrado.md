# US-02 — Rechazar el alta con un correo ya registrado

**Producto:** Fábrica de Geometría
**Proyecto de código:** GeometriaFactory-Application
**Documento:** US-02-Rechazar-El-Alta-Con-Un-Correo-Ya-Registrado.md
**Versión:** 1.0
**Estado:** Propuesta
**Fecha:** 2026-08-10
...
`RN-02` fija que el correo del alumno es único, y `02` §6 declara que **la verificación sobre el conjunto de cuentas es de esta capa, en los dos caminos de alta**. El contrato de uso es [`CU-01`](../../02-Especificacion-Funcional/Casos-De-Uso/CU-01-Registrar-El-Alta-De-Una-Cuenta.md). Sin esta historia, la colisión llegaría al consumidor como una excepción del almacén, que es exactamente lo que [`ADR-06`](../../05-Arquitectura-Tecnica/Adrs/ADR-06-Resultado-Tipado-Y-Catalogo-Cerrado-De-Condiciones.md) descarta.
...
| CU cubiertos | CU-01 |
| RN e invariantes que ejerce | RN-02; INV-01 |
| Componente de `05` §3.1 | Orquestación del alta de cuentas |
| Puertos que consume | Repositorio de cuentas |
| Comprobación de `02` §4 que la alcanza | Ninguna: el solicitante es anónimo, igual que en US-01 |
| BT derivadas | BT-08, BT-12, BT-21 |
...
`Must` porque `RN-02` es una de las dieciséis reglas del producto y porque sin esta negativa el alta duplicaría cuentas, con lo que el ingreso dejaría de ser determinista (`05` §9, último riesgo de `GeometriaFactory-Infrastructure` sobre la misma regla).
...
El **criterio de comparación de dos correos** —tal cual o normalizados— condiciona cuándo se produce esta negativa y **no se decide acá**: `05` §11 `PA-03` lo derivó a la categoría 05 de `GeometriaFactory-Infrastructure`. BT-21 lo acompaña con su plazo.
