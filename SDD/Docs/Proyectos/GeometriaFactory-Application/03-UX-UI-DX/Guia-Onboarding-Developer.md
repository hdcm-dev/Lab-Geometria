...
| Se busca dónde se enuncia una regla de negocio y no está | Las **trece** reglas viven en `GeometriaFactory-Domain` y acá se **ejercen**, no se redactan. **RN-12 y RN-13 todavía no tienen archivo allá**: entraron con el `PRODUCT-INTAKE` 1.7 y acá se citan contra el intake | `Especificacion-Funcional.md` §6 dice, regla por regla, dónde se ejerce cada una en esta capa |
