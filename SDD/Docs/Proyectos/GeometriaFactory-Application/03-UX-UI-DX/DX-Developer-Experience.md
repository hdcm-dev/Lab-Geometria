...
| Reglas de negocio relevantes | RN-01 a RN-15, **las quince con archivo en `GeometriaFactory-Domain`**, con el lugar donde se ejerce cada una declarado en `Especificacion-Funcional.md` §6 |
...
| 1.3 | 2026-08-09 | **Cierra los hallazgos `F26-14` y las dos filas de este archivo del `F26-20`** del informe de auditoría `SDD/Docs/Audit/F26-Propagacion-r1.md` 1.0, contra `PRODUCT-INTAKE` **1.10**. **`F26-14`**: **§1.4** se titulaba «Las **tres** negativas» y su propio cuerpo decía, dos líneas más abajo, que las cuatro comprobaciones transversales «producen **cuatro** negativas distintas»; la tabla tenía tres filas y `CAMBIO_DE_CONTRASENA_PENDIENTE` **no aparecía ni una vez en el archivo**, de modo que el marco DX de la capa no nombraba la negativa que **corta primero**. Entra la cuarta fila, con lo que oculta —nada—, con su excepción declarada —el reemplazo de `CU-03` FA-05, que es lo que la levanta— y con su traducción obligatoria, que es derivar al cambio de contraseña sin emitir sesión (RN-13). El título pasa a decir **cuatro**. **`F26-20`**: **§6** hablaba de «las **treinta y cuatro** condiciones del catálogo», que son **36** desde `CU-11`, y el mismo archivo declara 36 más abajo; y **§8** citaba «RN-01 a RN-13», que son **RN-01 a RN-15** desde el intake 1.10, las quince con archivo en `GeometriaFactory-Domain`. **Ningún caso de uso, puerto ni condición del catálogo cambia.** Sube minor. |
