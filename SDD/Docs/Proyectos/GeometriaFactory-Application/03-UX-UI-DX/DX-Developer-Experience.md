...
3. **Esta capa orquesta y decide quién puede, pero no declara reglas.** Las **trece** reglas del producto viven en `GeometriaFactory-Domain` —dos de ellas, RN-12 y RN-13, todavía sin archivo allá y citadas contra el intake— y acá se **ejercen** sobre el pedido concreto (`Especificacion-Funcional.md` §6). Un caso de uso que enunciara una regla nueva estaría mal ubicado; el procedimiento de decisión está en [`Guia-Onboarding-Developer.md`](Guia-Onboarding-Developer.md) §7.3.
...
| Reglas de negocio relevantes | RN-01 a RN-13, las once primeras con archivo en `GeometriaFactory-Domain` y las dos últimas citadas contra el `PRODUCT-INTAKE` 1.7 §4.1, con el lugar donde se ejerce cada una declarado en `Especificacion-Funcional.md` §6 |
