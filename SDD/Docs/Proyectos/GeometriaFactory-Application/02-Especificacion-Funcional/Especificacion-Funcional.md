...
**Trazabilidad upstream:** `00-Contexto/Vision-Producto.md` §1, §3 y §9 (glosario raíz de la cadena); `00-Contexto/Alcance-Producto.md` §4.1 y §5; `01-Necesidades-Negocio/Necesidades-Negocio.md` §2, §4 y §5.3, y las necesidades NB-01 a NB-07 y NB-09; `PRODUCT-INTAKE-Fabrica-De-Geometria.md` **1.7**, §17.2 íntegro —en particular §17.2.P.2 (inversión de dependencias), §17.2.P.5 (verificación de pertenencia), §17.2.P.10 y §17.2.P.11—, §4 (con F-26), §4.1 (**las trece reglas**), §4.2 (modelo de estados del trabajo), §6, §7 (CL-7), §9 (X-2 retirada), §12, §14 y §17.1.P.2 (INV-09); `Proyectos/GeometriaFactory-Domain/02-Especificacion-Funcional/` completo, cuyos **doce** casos de uso esta categoría orquesta
...
| **Cambio de contraseña pendiente** | Que la cuenta solicitante **no** esté marcada por un reseteo del administrador. Es la comprobación que hace exigible el invariante **INV-09** del intake 1.7 §17.1.P.2 | `CAMBIO_DE_CONTRASENA_PENDIENTE`. No lee ni escribe nada: la cuenta ingresó, y lo único que puede hacer es cambiar su contraseña (RN-13) | **Todos**, con una sola excepción declarada: el reemplazo de CU-03 FA-05, que es lo único que la levanta. Ver la precisión 5 |
...
| CU-11 | [Resetear la contraseña de un alumno](Casos-De-Uso/CU-11-Resetear-La-Contrasena-De-Un-Alumno.md) | Contraseña provisoria fijada por el administrador, con marca de cambio pendiente, conservando la cuenta y **todos sus trabajos** | Propuesto |

Once casos de uso, sobre un mínimo de cinco para el tipo `library`.

## 6. Reglas de negocio que esta capa hace cumplir

**Las reglas del producto viven en `GeometriaFactory-Domain` y acá se referencian, no se redactan.** Lo que esta tabla declara es dónde se ejerce cada una en esta capa, que es una cosa distinta de dónde está enunciada. **Las trece tienen tramo acá**, y en dos el tramo principal está en otra capa: RN-05, que resuelve el dominio sobre el conjunto de observaciones que esta capa le entrega, y RN-09, cuyo mensaje ubicado lo produce el validador detrás del puerto. Las dos filas lo declaran.

**Dos de las trece —RN-12 y RN-13— entraron con el `PRODUCT-INTAKE` 1.7 y todavía no tienen archivo en `GeometriaFactory-Domain`.** Se referencian contra el intake §4.1 hasta que esa categoría las redacte, con el enlace pendiente declarado en §11. **Esta categoría no las redacta**: hacerlo crearía dos enunciados de la misma regla en la misma cadena documental, que es exactamente lo que §9 evita.
...
| **RN-12** (`PRODUCT-INTAKE` 1.7 §4.1, sin archivo aguas arriba todavía) | El reseteo de contraseña conserva la cuenta y sus trabajos, y no es una baja | CU-11: la postcondición que deja intactos estado de habilitación, papel, identidad y **todos los trabajos con sus estados y comentarios**, y la ausencia deliberada de todo retiro |
| **RN-13** (`PRODUCT-INTAKE` 1.7 §4.1, sin archivo aguas arriba todavía) | Mientras la provisoria no se cambie, la cuenta no llega a ninguna otra parte del sistema | La cuarta comprobación transversal de §4, en **todos** los casos de uso; CU-03 FA-05, que es el único lugar donde la marca se levanta; CU-11, que es el único donde se pone |
...
| CU-09 | CU-09, CU-11 |
| CU-11 | [CU-03](../../GeometriaFactory-Domain/02-Especificacion-Funcional/Casos-De-Uso/CU-03-Fijar-Y-Reemplazar-La-Credencial-Derivada.md), en su camino de reemplazo |

**CU-03 del dominio queda orquestado desde dos casos de uso de esta capa**, y no es una duplicación: el reemplazo que ejerce CU-03 de acá lo pide la propia cuenta declarando verificada su credencial vigente, y el que ejerce CU-11 lo pide el administrador por facultad, sin conocerla. Son dos sujetos y dos autorizaciones distintas sobre la misma operación de dominio.
...
| Archivo de RN-12 y RN-13 en `GeometriaFactory-Domain` | Las dos reglas entraron con el `PRODUCT-INTAKE` 1.7 y esta capa las ejerce en CU-11 y en la cuarta comprobación de §4, pero **no tienen todavía archivo aguas arriba**, de modo que §6 las referencia contra el intake y no por enlace. **No es ambigüedad de esta categoría**: es un archivo que falta en la que las redacta | `GeometriaFactory-Domain`, categoría 02 |
| Marca de cambio de contraseña pendiente en el modelo del dominio | INV-09 la exige como atributo de la cuenta, y esta capa la escribe en CU-11 y la lee en toda la categoría. **Si el modelo del dominio no la declara**, corre la misma suerte que los sellos: se leería como dato de esta capa, y no debería, porque de ella depende un invariante del producto | `GeometriaFactory-Domain`, y `05-Arquitectura-Tecnica` al fijar el nombre |
