# ADR-05 — Un caso de uso, una unidad de trabajo: el alcance lo fija esta capa

**Proyecto de código:** GeometriaFactory-Application
**Documento:** ADR-05-Un-Caso-De-Uso-Una-Unidad-De-Trabajo.md
**Versión:** 1.0
**Estado:** Propuesto
**Fecha:** 2026-08-10
...
El caso testigo es `RN-07`: la baja arrastra **todos** los trabajos de la cuenta, en cualquier estado, y exige la confirmación escrita del correo. Son tres efectos —comparar, retirar, cambiar la situación— que o pasan juntos o no pasa ninguno.

Motivación upstream: NB-01, NB-03, NB-07, NB-09; RN-04, RN-07, RN-11, RN-12; INV-03; `PRODUCT-INTAKE` §17.2.P.4 y §17.2.P.10.
...
| Que el alcance lo fije el adaptador, una transacción por llamada al repositorio | El adaptador es quien conoce el motor y podría optimizar | La baja podría quedar a medias, y `RN-07` dejaría de valer sin que ninguna prueba de esta capa lo notara. El límite de consistencia dejaría de ser una decisión de diseño y pasaría a ser un efecto secundario |
...
1. `RN-07` se materializa como propiedad estructural y no como cuidado del programador: la baja es atómica porque el caso de uso es la unidad.
2. `RN-12` se verifica por el mismo camino y en sentido contrario: el reseteo **no** dispara ningún retiro, y su postcondición declara todos los trabajos conservados.
...
5. El predicado de alcance trasladado a la consulta hace que un borrador **no se traiga**, en lugar de traerse y filtrarse: `RN-11` deja de depender de un filtro en memoria.
...
| Trabajos retirados por un reseteo | Exactamente **0** | Prueba de `RN-12`, con la cuenta y todos sus trabajos conservados |
...
- `PRODUCT-INTAKE-Fabrica-De-Geometria.md` **1.16** §17.2.P.4, §17.2.P.10 y §4.1 (RN-04, RN-07, RN-11, RN-12).
...
- [`../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Definicion-Modelo-De-Dominio.md`](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Definicion-Modelo-De-Dominio.md) §7, donde el dominio deriva la atomicidad al puerto de repositorio que esta capa declara.
- [`../../../GeometriaFactory-Contracts/05-Arquitectura-Tecnica/Adrs/ADR-05-Proyeccion-De-Listado-Separada-Del-Detalle.md`](../../../GeometriaFactory-Contracts/05-Arquitectura-Tecnica/Adrs/ADR-05-Proyeccion-De-Listado-Separada-Del-Detalle.md), la decisión equivalente del otro lado de la frontera.
- ADR relacionadas: [`ADR-01`](ADR-01-Casos-De-Uso-Con-Inversion-De-Dependencias.md), [`ADR-02`](ADR-02-Cuatro-Puertos-Y-La-Frontera-Que-Declaran.md).
