...
- [ ] `BT-01` está cerrada: el proyecto de código y su proyecto de pruebas existen, la batería corre aunque sea vacía, y el archivo de proyecto declara **1** dependencia saliente.
- [ ] `BT-02` está cerrada: los nombres de tipos, de espacios de nombres y **el del cuarto puerto** están fijados y validados en el punto de control de la etapa `a`. Sin esto los dobles de puerto se escriben contra un nombre que va a cambiar.
...
- [ ] `TC-26` da **0** en sus tres recuentos: ninguna prueba de la etapa abrió el almacén real.
- [ ] `TC-27` sigue dando **1** dependencia saliente y **0** de las prohibidas.
- [ ] `TC-28` cierra en las dos direcciones sobre las condiciones que la etapa incorporó.
- [ ] `TC-11` y la tabla de [`Matriz-Cobertura-Pruebas.md`](Matriz-Cobertura-Pruebas.md) §5 cierran sobre las comprobaciones que la etapa toca: **cada una con prueba de su negativa sin base de datos**.
...
| --- | --- | --- | --- | --- |
| RQ-01 | Que un caso de uso consulte el almacén por su cuenta y deje de ser probable con dobles | Alto | Media | `TC-26` y `TC-27` corren en **cada** etapa, no sólo en la `a`; `QG-04` y `QG-05` bloquean la fusión |
| RQ-02 | Que aparezca un camino que ejerza una capacidad **sin** resolver antes la marca de cambio de contraseña pendiente | **Muy alto** | Media | `TC-11` como prueba de orden en cada etapa a partir de la `d`; umbral de **100 %** de líneas y ramas en la guarda de autorización; criterio 5 de la Definition of Ready |
| RQ-03 | Que la negativa por pertenencia y la negativa por facultad se confundan, y un trabajo ajeno responda «no autorizado» | Alto | Media | `TC-12`, que verifica la indistinguibilidad **en los dos sentidos** contra la tabla de traducciones prohibidas de `03` §2.4 |
| RQ-04 | Que un caso de uso reparta su efecto entre dos unidades de trabajo y la baja deje trabajos huérfanos | Alto | Baja | `TC-29`, con la baja de cuenta como caso testigo y el recuento de aperturas instrumentado |
| RQ-05 | Que el consumidor trate el resultado tipado como excepción y descarte los rechazos | Medio | Media | `TC-31` verifica que ninguna de las 36 condiciones lance; el efecto sobre el consumidor se verifica en `GeometriaFactory-Api` |
| RQ-06 | Que el nombre del cuarto puerto se fije sin punto de control y haya que renombrar los dobles | Bajo, de retrabajo | Alta | Criterio de entrada de §2: `BT-02` cerrada **antes** de escribir los casos de prueba que usan ese doble |
| RQ-07 | **Que un escenario del intake §20 se sustituya por un resultado de interpretación inventado** «porque es más cómodo de armar» | Alto | Media | [`Estrategia-Testing.md`](Estrategia-Testing.md) §6 lo prohíbe; el criterio de salida exige que los ocho escenarios sigan siendo el material de `TC-15` a `TC-17` y de `TC-22` |
...
| --- | --- | --- | --- | --- |
| `a` | EP-01 Esqueleto ambulante y verificación de viabilidad | Ninguna capacidad funcional. Se ponen en pie las pruebas de inspección estructural y la batería vacía | `TC-26`, `TC-27` | Batería que corre; `QG-01`, `QG-02`, `QG-04` y `QG-05` medidos por primera vez; `BT-02` cerrada con el nombre del cuarto puerto |
| `c` | EP-02 Identidad del administrador y sesión | Configuración del administrador, admisibilidad con su motivo y reemplazo de credencial | `TC-03`, `TC-08`, `TC-09`, `TC-31` | Matriz con `CU-03` y `CU-10` cerrados; primera medición de `QG-11` |
| `d` | EP-03 Ciclo de vida de la cuenta de alumno | Auto-registro, las cuatro operaciones de admisión, la provisoria, el reseteo y **la comprobación transversal de la marca** | `TC-01`, `TC-02`, `TC-04`, `TC-05`, `TC-06`, `TC-07`, `TC-10`, `TC-11`, `TC-29` | `INV-09` ejercido con la prueba de orden; la baja como caso testigo de la unidad de trabajo; `BT-18` cerrada o elevada |
| `e` | EP-04 Gestión del trabajo | Constitución y reedición del trabajo, las dos consultas con su predicado y la eliminación en sus dos alcances | `TC-12`, `TC-13`, `TC-14`, `TC-20`, `TC-21`, `TC-22`, `TC-25`, `TC-30` | Matriz con `CU-04`, `CU-06`, `CU-07` y `CU-09` cerrados; `QG-09` medido |
| `f` | EP-05 Interpretación y verificación del dato del alumno | El envío por el puerto, con los **ocho** escenarios del intake como resultado de interpretación, y la terminación controlada | `TC-15`, `TC-16`, `TC-17`, `TC-18`, `TC-19`, `TC-28` | Los ocho escenarios ejercitados; catálogo de **36** condiciones cerrado en las dos direcciones; `BT-19` mide los 500 ms sobre `E-1` |
| `h` | EP-06 Desenlace de la entrega | Aprobar y rechazar desde `Pendiente`, con terminalidad, y la lectura del desenlace por el alumno | `TC-23`, `TC-24`, y reejecución de `TC-22` y `TC-25` | Matriz completa: 11 de 11 casos de uso, 16 de 16 reglas, 4 de 4 comprobaciones y 9 de 9 invariantes |

**La suma cubre los treinta y un casos de prueba.** `TC-22` y `TC-25` aparecen dos veces porque la etapa `h` los reejecuta con el desenlace ya construido, que es cuando el comentario y la terminalidad se pueden verificar de verdad.
