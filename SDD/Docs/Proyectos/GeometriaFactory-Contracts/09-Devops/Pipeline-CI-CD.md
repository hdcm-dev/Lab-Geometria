...
**Hallazgo que esta categoría eleva y no resuelve por su cuenta.** El intake §17.6.P.7 declara que el flujo de trabajo que publica el front está **restringido a cambios bajo `src/GeometriaFactory.Web/` y `visor/`**. Un cambio de este ensamblado vive en `src/GeometriaFactory.Contracts/` y, por esa restricción, **no dispara la publicación del front**, aunque `QG-08` exija que las dos unidades salgan juntas. La regla no queda incumplida —el mismo §17.6.P.7 declara que el flujo también se dispara manualmente—, pero **el despliegue conjunto queda apoyado en que alguien se acuerde de dispararlo**, que es exactamente lo que un gate escrito como recuento viene a evitar. Queda como `PD-01` en §10, para la categoría 09 de `GeometriaFactory-Web`, que es la dueña de ese flujo de trabajo.
...
| --- | --- | --- | --- |
| PD-01 | La **restricción de rutas del flujo de trabajo del front** (`src/GeometriaFactory.Web/` y `visor/`, intake §17.6.P.7) deja fuera a `src/GeometriaFactory.Contracts/`, de modo que un cambio de contrato no dispara la publicación del front por fusión, aunque `QG-08` exija el despliegue conjunto | La categoría 09 de `GeometriaFactory-Web`, que es la dueña de ese flujo de trabajo, con elevación al Product Owner si cambia lo que el intake declara | Al emitirse la categoría 09 de `GeometriaFactory-Web` |
