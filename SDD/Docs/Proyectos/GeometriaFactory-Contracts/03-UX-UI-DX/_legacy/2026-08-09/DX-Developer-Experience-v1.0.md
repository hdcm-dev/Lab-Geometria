...
>
> Esta es una **copia archivada** del documento `DX-Developer-Experience.md` en su versión **1.0**, tomada el 2026-08-09 por el orquestador SDD antes de que la versión vigente la superara (`Master-Prompt.md` §5 y §5.1).
>
> - **Estado:** `Superado`
...
**Autor:** DX Lead (AG-03)
**Variante:** DX
**Trazabilidad upstream:** `02-Especificacion-Funcional/Especificacion-Funcional.md` §1, §2, §6 (RT-01 a RT-07); los seis contratos de uso `CU-01` a `CU-06`, con sus §17 de compatibilidad de versión pública y con `CU-01` §10; `02-Especificacion-Funcional/Glosario-Funcional.md` §2, §3.1 y §3.2; `00-Contexto/Vision-Producto.md` §3.2, §7, §9; `00-Contexto/Alcance-Producto.md` §2.2, §4.3, §8; `01-Necesidades-Negocio/Necesidades-Negocio.md` §2 y las necesidades NB-02, NB-03, NB-04, NB-08; `PRODUCT-INTAKE` §17.4 P.1 a P.12, §14 (RA-03), §15 (etapas y puntos de control), §16 y §16.1
...
- **La proyección de listado no arrastra el texto original ni los componentes de las piezas** (`RT-04`, `PRODUCT-INTAKE` §17.4 P.10). El listado existe precisamente para no ser el detalle.
...
| **30 minutos** | Ubicar cualquier tipo de transferencia en su familia y saber qué transporta y qué no | Responder sin abrir el código: dónde vive el texto original completo (detalle de CU-05, y sólo ahí), por qué la proyección de listado de CU-04 no lo trae (`RT-04`), y qué campos tiene la respuesta de error (`CU-06` CA-01: cuatro, y cero que puedan transportar una dirección de servicio) |
| **1 hora** | Clasificar un cambio propuesto sobre la superficie pública como compatible o incompatible, y saber cuál es la respuesta operativa | Clasificar correctamente los tres cambios de control de [`Guia-Onboarding-Developer.md`](Guia-Onboarding-Developer.md) §3.3, y enunciar que ante un cambio incompatible las dos piezas desplegables se despliegan juntas (`RT-06`), sin versionar rutas |
...
| **Reference** | Información | Los seis contratos de uso `CU-01` a `CU-06` de `02-Especificacion-Funcional/`, que son la descripción normativa de qué viaja en cada familia de tipos; `Especificacion-Funcional.md` §6 para las siete restricciones transversales | Lo consultable punto por punto. **Esta categoría no lo duplica**: lo referencia |
...
**Clase T — errores transportados.** Son las respuestas de error de la pieza de datos, expresadas como tipo de transferencia. No los produce el ensamblado: los transporta. Su forma única es la de `CU-06`: código de un conjunto cerrado, texto neutro, colección de detalles de ubicación y momento. El diagnóstico tiene que quedar accionable **sin filtrar nada**: con índice de figura y campo señalado cuando corresponde, y nunca con la dirección de un servicio interno (`RT-02`).
...
| **TTFV** (time-to-first-value) | Desde el mismo punto de partida hasta clasificar correctamente un cambio propuesto como compatible o incompatible con su acción operativa | <= 1 hora | Los tres cambios de control de [`Guia-Onboarding-Developer.md`](Guia-Onboarding-Developer.md) §3.3, resueltos sin abrir el código |
...
| Superficie pública que documenta | El ensamblado de contratos completo: las cinco familias de tipos de transferencia de `CU-01` a `CU-05` y el tipo único de error de `CU-06` |
| CU origen | `CU-01` a `CU-06` de `02-Especificacion-Funcional/`, con sus §17 de compatibilidad de versión pública |
| Reglas de negocio relevantes | Ninguna en este proyecto de código; ver `Especificacion-Funcional.md` §5. Rigen en su lugar las restricciones transversales `RT-01` a `RT-07` §6 y la regla de arquitectura RA-03 de `PRODUCT-INTAKE` §14 |
| Wireframes asociados | N/A. Variante DX sin UI final; el mínimo de la regla para `library` es cero |
| US a generar en 06 | US-01 a US-16, previstas en `Especificacion-Funcional.md` §4. Esta categoría aporta criterios de ergonomía de superficie a US-02, US-05, US-08, US-11, US-14, US-15 y US-16 |
| Tests previstos en 08 | Inspección de superficie pública para `RT-01` y `RT-04`; verificación de ausencia de referencia hacia el dominio para `RT-05`; cobertura del 100 % de tipos de transferencia por prueba de integración (`RT-07`) |
