...
**Clase T — errores transportados.** Son las respuestas de error de la pieza de datos, expresadas como tipo de transferencia. No los produce el ensamblado: los transporta. Su forma única es la de `CU-06`: código de un conjunto cerrado de **diecisiete**, texto neutro, colección de detalles de ubicación y momento. Fuera de ese conjunto quedan **tres señales declaradas** que no son error y que se catalogan aparte, porque tratarlas como fallo es un defecto tan caro como no representar un fallo real. El diagnóstico tiene que quedar accionable **sin filtrar nada**: con índice de figura y campo señalado cuando corresponde, y nunca con la dirección de un servicio interno (`RT-02`).
...
| Reglas de negocio relevantes | Ninguna en este proyecto de código; ver `Especificacion-Funcional.md` §5. Rigen en su lugar las once restricciones transversales `RT-01` a `RT-11` §6 y la regla de arquitectura RA-03 de `PRODUCT-INTAKE` §14. Las reglas del circuito de revisión, `RN-10` y `RN-11`, viven en `GeometriaFactory-Domain` |
| Wireframes asociados | N/A. Variante DX sin UI final; el mínimo de la regla para `library` es cero |
| US a generar en 06 | US-01 a US-22, previstas en `Especificacion-Funcional.md` §4. Esta categoría aporta criterios de ergonomía de superficie a US-02, US-05, US-08, US-11, US-14, US-15, US-16, US-17, US-18 y US-19 |
