...
**Autor:** DX Lead (AG-03)
**Variante:** DX
**Trazabilidad upstream:** `02-Especificacion-Funcional/Especificacion-Funcional.md` §1, §2, §3.1, §4.2 y §6 (`RT-01` a `RT-09`); los **siete** contratos de uso `CU-01` a `CU-07`, con sus §17 de compatibilidad de versión pública, con `CU-01` §10, `CU-06` §10 y `CU-07` §10; `02-Especificacion-Funcional/Glosario-Funcional.md` §2, §3.1, §3.2 y §3.3; `00-Contexto/Vision-Producto.md` §3.2, §7, §9.1 y §9.2; `00-Contexto/Alcance-Producto.md` §2.2, §4.3, §8; `01-Necesidades-Negocio/Necesidades-Negocio.md` §2 y las necesidades NB-02, NB-03, NB-04, NB-08, **NB-09**; `PRODUCT-INTAKE` 1.3 §4 (F-21 a F-24), §4.1 (RN-10, RN-11), §4.2 (modelo de estados), §17.4 P.1 a P.12, §14 (RA-03), §15 (etapas y puntos de control), §16 y §16.1
...
| **Reference** | Información | Los siete contratos de uso `CU-01` a `CU-07` de `02-Especificacion-Funcional/`, que son la descripción normativa de qué viaja en cada familia de tipos; `Especificacion-Funcional.md` §6 para las nueve restricciones transversales | Lo consultable punto por punto. **Esta categoría no lo duplica**: lo referencia |
...
**Clase T — errores transportados.** Son las respuestas de error de la pieza de datos, expresadas como tipo de transferencia. No los produce el ensamblado: los transporta. Su forma única es la de `CU-06`: código de un conjunto cerrado de **catorce**, texto neutro, colección de detalles de ubicación y momento. Fuera de ese conjunto quedan **tres señales declaradas** que no son error y que se catalogan aparte, porque tratarlas como fallo es un defecto tan caro como no representar un fallo real. El diagnóstico tiene que quedar accionable **sin filtrar nada**: con índice de figura y campo señalado cuando corresponde, y nunca con la dirección de un servicio interno (`RT-02`).
...
| Superficie pública que documenta | El ensamblado de contratos completo: las seis familias de tipos de transferencia de `CU-01` a `CU-05` y `CU-07`, y el tipo único de error de `CU-06` |
| CU origen | `CU-01` a `CU-07` de `02-Especificacion-Funcional/`, con sus §17 de compatibilidad de versión pública |
