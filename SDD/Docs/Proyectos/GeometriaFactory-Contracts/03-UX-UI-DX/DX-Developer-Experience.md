...
**Clase T — errores transportados.** Son las respuestas de error de la pieza de datos, expresadas como tipo de transferencia. No los produce el ensamblado: los transporta. Su forma única es la de `CU-06`: código de un conjunto cerrado de **quince**, texto neutro, colección de detalles de ubicación y momento. Fuera de ese conjunto quedan **tres señales declaradas** que no son error y que se catalogan aparte, porque tratarlas como fallo es un defecto tan caro como no representar un fallo real. El diagnóstico tiene que quedar accionable **sin filtrar nada**: con índice de figura y campo señalado cuando corresponde, y nunca con la dirección de un servicio interno (`RT-02`).
