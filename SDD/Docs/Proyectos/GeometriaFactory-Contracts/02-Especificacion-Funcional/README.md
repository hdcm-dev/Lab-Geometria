...
| CU-06 | Contrato de respuesta de error | La forma única con la que un fallo cruza la frontera de servicio: texto neutro, conjunto cerrado de **diecisiete** códigos, ubicación del defecto y ninguna dirección de servicio | NB-04, NB-08, NB-09, NB-02 | [CU-06](Casos-De-Uso/CU-06-Contrato-De-Respuesta-De-Error.md) |
...
| 1.6 | 2026-08-10 | **Absorbe la corrección de `PRODUCT-INTAKE` 1.15 §4.1 (RN-16)**: lo que la regla elimina es la escritura anónima **de credencial**, no toda escritura anónima —el **registro de cuenta** es anónimo por diseño y debe seguir siéndolo—. Único cambio acá: **la fila 1.4** de este control de cambios pasa a decir «de credencial». Ningún artefacto se agrega ni se omite y el orden de lectura no cambia. | Analista Funcional + API Designer (AG-02) |
