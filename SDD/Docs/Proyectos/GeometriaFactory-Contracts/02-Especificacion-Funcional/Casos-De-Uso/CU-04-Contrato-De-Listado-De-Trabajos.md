...
- El contrato ya declara los tipos de trabajo de CU-03 y el conjunto cerrado de cuatro estados: `Borrador`, `Pendiente`, `Finalizado` y `Rechazado`.
...
| FA-03 | El administrador abre un trabajo en estado `Pendiente` para resolverlo | El listado le entrega el identificador, y el desenlace se ejerce con el contrato de CU-07. El elemento de listado no transporta ningún campo de decisión | El flujo vuelve al paso 1, con el trabajo ya en un estado terminal |
...
| --- | --- |
| Necesidad de negocio | NB-03, NB-07, y NB-09 por el estado de desenlace que el listado transporta |
| Reglas de negocio aplicables | Ninguna propia: este proyecto de código no las redacta. Aplican [`RN-11`](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-11-El-Administrador-No-Ve-Los-Borradores.md) sobre CA-06 y [`RN-03`](../../../GeometriaFactory-Domain/02-Especificacion-Funcional/Reglas-De-Negocio/RN-03-Trabajo-Ajeno-Indistinguible-De-Inexistente.md) sobre el alcance del listado propio, las dos de `GeometriaFactory-Domain`. Ver `Especificacion-Funcional.md` §5 |
...
- **El comentario del administrador no viaja en el listado.** El elemento transporta el estado, que es lo que expresa el desenlace y lo que el alumno ve en su lista; el texto del comentario viaja en el detalle de CU-05, que es el único tipo del ensamblado que lo transporta, y el alumno lo alcanza abriendo el trabajo desde su listado. Es coherente con el requisito estructural de CA-01, que existe para que el listado no arrastre texto libre de cada trabajo.
