...
| Historias de usuario a generar en 06 | US-14 tipo de error con texto neutro; US-15 detalle de ubicación con campo señalado e índice de figura; US-16 conjunto cerrado de dieciséis códigos de error |
...
- **El conjunto cerrado tiene dieciséis códigos.** Es la unión de los que declaran los ocho casos de uso. Respecto de la versión anterior entran dos —`CONTRATO_CAMBIO_DE_CONTRASENA_REQUERIDO` y `CONTRATO_RESETEO_NO_APLICABLE_A_LA_CUENTA_DE_ADMINISTRADOR`, los dos de CU-08 y de la capacidad **F-26**— y no sale ninguno. **Las señales declaradas siguen siendo tres**, las de §6.1 de CU-03, CU-04 y CU-05: el cambio de contraseña pendiente **sí** impide la operación pedida, de modo que es un error transportado y no una señal, y el conjunto de señales no se toca.
