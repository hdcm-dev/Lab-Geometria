...
**Trazabilidad upstream:** `00-Contexto/Vision-Producto.md` §3, §7 (R-03, R-04), §9; `00-Contexto/Alcance-Producto.md` §2.2, §4.1, §4.2, §5, §8; `01-Necesidades-Negocio/Necesidades-Negocio.md` 1.1 §2, §4, §5.3 y las nueve `NB-XX`; `PRODUCT-INTAKE` 1.3 §4, §4.1 (las once reglas) y §4.2 (modelo de estados), §17.4 íntegro (P.1 a P.12), §13 y §14 (composición, contratos entre proyectos de código y reglas RA-01 a RA-03), §17.5 P.3 y P.5 (qué existe del otro lado del contrato), §4, §6, §7 y §20 (escenarios de instancia)
...
| Se hizo transversal CU-06 | Los otros seis casos de uso comparten sus caminos de error. Es el caso de uso transversal de manejo de errores que sugiere §5.2 de las reglas, y además concentra en un solo tipo la superficie donde RA-03 se puede violar |
...
La numeración de esta sección es contigua de `CU-01` a `CU-07`, sin huecos.

## 4. Matriz NB→CU→RN→US

| NB | CU | RN | US a generar en 06 |
| --- | --- | --- | --- |
| NB-01 | CU-01, CU-02 | — | US-01, US-02, US-03, US-04, US-05 |
| NB-02 | CU-01, CU-02, CU-06 | — | US-01, US-02, US-03, US-04, US-05, US-14 |
...
Las veinte historias de usuario previstas son una previsión de esta categoría; las confirma la categoría 06 al redactarlas.
...
| --- | --- | --- |
| NB-01 | Parcial: transporta el listado de cuentas, la orden de cambio de situación y la confirmación escrita de la baja | La unicidad del administrador y el arrastre de trabajos en la baja son invariantes de `GeometriaFactory-Domain`; el panel es de `GeometriaFactory-Web` |
| NB-02 | Parcial: declara que ninguna forma de la contraseña almacenada cruza la frontera y que la situación de la cuenta viaja explicada | La derivación de clave es de `GeometriaFactory-Infrastructure`; el circuito de alta es de `GeometriaFactory-Web` |
...
Las reglas de dominio viven en la especificación funcional de `GeometriaFactory-Domain`. Los casos de uso de esta sección las **refieren por identificador**, con enlace relativo al archivo de ese proyecto de código, en la fila «Reglas de negocio aplicables» de su §9: `RN-01` y `RN-06` en CU-01; `RN-01`, `RN-02`, `RN-06` y `RN-07` en CU-02; `RN-03`, `RN-04`, `RN-05` y `RN-08` en CU-03; `RN-03` y `RN-11` en CU-04; `RN-03`, `RN-09` y `RN-10` en CU-05; `RN-03`, `RN-09`, `RN-10` y `RN-11` en CU-06 —`RN-09` es la que `PRODUCT-INTAKE` §17.4 P.5 ancla al tipo de respuesta de error—; y `RN-10` y `RN-11` en CU-07. Los siete casos de uso refieren ahora al menos una regla por identificador. Donde una invariante no tiene identificador nombrable, la fila la nombra en lenguaje natural y declara su proyecto de código destino. **Ninguna `RN-XX` se redacta ni se inventa en esta sección.**
...
Valen para los siete casos de uso y no se repiten en cada uno más allá de su criterio de aceptación correspondiente.
...
| RT-05 | El ensamblado no declara ninguna referencia hacia `GeometriaFactory-Domain` | `PRODUCT-INTAKE` §17.4 P.8 (quality gate bloqueante) | Precondición de los siete casos de uso; su verificación pertenece a 09 |
