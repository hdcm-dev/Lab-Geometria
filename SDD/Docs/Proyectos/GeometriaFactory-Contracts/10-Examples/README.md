...
**Un proyecto de código sin comportamiento igual tiene qué demostrar.** Estos tres samples no ejercitan lógica: recorren la superficie pública de las **ocho** familias de tipos, componen cuerpos con los datos reales del `PRODUCT-INTAKE` §20 y **cuentan lo que no está**. La forma característica de aserción es el recuento —cero campos, cuatro campos, diecisiete códigos—, que es la que [`../08-Calidad-Y-Pruebas/Estrategia-Testing.md`](../08-Calidad-Y-Pruebas/Estrategia-Testing.md) §4 declara propia de este proyecto de código.
...
| --- | --- | --- | --- | --- |
| [`ejemplo-01-basico.md`](ejemplo-01-basico.md) | Básico | < 5 min | CU-08001, CU-08002 | `/samples/contracts/01-basico/` |
| [`ejemplo-02-intermedio.md`](ejemplo-02-intermedio.md) | Intermedio | < 5 min | CU-08003, CU-08004, CU-08005 | `/samples/contracts/02-intermedio/` |
| [`ejemplo-03-avanzado.md`](ejemplo-03-avanzado.md) | Avanzado | 10-15 min | CU-08006, CU-08007, CU-08008 | `/samples/contracts/03-avanzado/` |

**Tres samples, el piso que `Rules-Examples.md` §2.2 fija para `library`.**

**Cobertura de los ocho contratos de uso: 8 de 8**, sin repeticiones. Uno por uno: `CU-08001` y `CU-08002` en el 01; `CU-08003`, `CU-08004` y `CU-08005` en el 02; `CU-08006`, `CU-08007` y `CU-08008` en el 03.
...
**Qué queda deliberadamente fuera.** `US-08010`, el resumen por alumno y por estado, no se ilustra en ningún sample: su caso de prueba `TC-08011` está declarado **fuera del tramo comprometido** en [`../08-Calidad-Y-Pruebas/Casos-Prueba-Referenciales.md`](../08-Calidad-Y-Pruebas/Casos-Prueba-Referenciales.md), y un sample que la ilustrara comprometería lo que la categoría 08 declaró no comprometido.

## 3. Contratos de verificación

| Sonda | Sample | Verifica | Comando | Estado | Última corrida |
| --- | --- | --- | --- | --- | --- |
| `VER-08001` | [`ejemplo-01-basico.md`](ejemplo-01-basico.md) | CU-08001, CU-08002; US-08001 a US-08005 | `dotnet run --project samples/contracts/01-basico` | No verificado — sin código | — |
| `VER-08002` | [`ejemplo-02-intermedio.md`](ejemplo-02-intermedio.md) | CU-08003, CU-08004, CU-08005; US-08006, US-08007, US-08008, US-08011, US-08012, US-08013, US-08018, US-08019 | `dotnet run --project samples/contracts/02-intermedio` | No verificado — sin código | — |
| `VER-08003` | [`ejemplo-03-avanzado.md`](ejemplo-03-avanzado.md) | CU-08006, CU-08007, CU-08008; US-08014, US-08015, US-08016, US-08017, US-08021, US-08022 | `dotnet run --project samples/contracts/03-avanzado` | No verificado — sin código | — |
...
**Qué gates de la categoría 08 quedan ejercidos desde afuera del pipeline.** `QG-04` —diecisiete códigos vivos y ninguno fuera del conjunto— por `VER-08003`; `QG-07` —cuatro campos en la respuesta de sesión y cero que impidan operar— por `VER-08001`; `QG-06` —proyección de listado sin texto original, componentes ni comentario— por `VER-08002`, con su carácter **condicionado** conservado; `QG-02` —cero referencias hacia el dominio— por `VER-08003`; y `QG-09` —ningún tipo que permita salir de un estado terminal ni que habilite al navegador— por `VER-08003`. **`QG-05` no queda ejercido**: el 100 % de los tipos ejercitados por integración exige la batería que vive en `GeometriaFactory-Api`, y ningún sample la sustituye.
