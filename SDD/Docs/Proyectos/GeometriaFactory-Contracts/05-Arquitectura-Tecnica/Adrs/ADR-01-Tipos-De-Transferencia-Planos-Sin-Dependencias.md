# ADR-01 — Ensamblado de tipos de transferencia planos, sin comportamiento y sin dependencias
...
Motivación upstream: NB-01 a NB-09, las nueve, en el grado que declara [`../../02-Especificacion-Funcional/Especificacion-Funcional.md`](../../02-Especificacion-Funcional/Especificacion-Funcional.md) §4.1; RN-01 a RN-16, todas transportadas y ninguna redactada acá; RA-01 y RA-03.
...
**Propuesto** desde 2026-08-10. Las dos alternativas principales las descartó el intake en §17.4.P.2; lo que esta ADR agrega es el registro formal, la definición operativa de «plano» y la puerta de verificación.
...
| Compartir las entidades de dominio entre las dos unidades | Cero duplicación de forma | Acopla la unidad pública a cambios internos del dominio y **filtra campos que no le corresponden**, empezando por la credencial derivada. Descartada por `PRODUCT-INTAKE` §17.4.P.2 |
| Generar el cliente desde una descripción formal del servicio | Contrato explícito, verificable por herramienta y con clientes generados | Costo de cadena de herramientas frente a **dos** consumidores de la misma solución compilados juntos. Descartada por `PRODUCT-INTAKE` §17.4.P.2 |
...
1. **Un cambio incompatible es un error de compilación, no un fallo en producción**, que es la propiedad que el intake declara en §17.4.P.3.
...
1. **Se acepta duplicar forma** entre las entidades del dominio y los tipos de transferencia. Es deliberado y es lo que impide que un cambio de dominio rompa el contrato (`PRODUCT-INTAKE` §17.1.P.12 y §17.4.P.12).
...
- ADR relacionadas: [`ADR-03`](ADR-03-Versionado-Por-Compilacion-Compartida.md), [`ADR-04`](ADR-04-Regla-De-Exposicion-De-La-Frontera.md).
