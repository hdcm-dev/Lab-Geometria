...
pedir POST /v1/auth/token "{\"email\":\"$GF_SEED_ADMIN_CORREO\",\"password\":\"$clave_admin\"}"
...
alta_de_alumno() {
  ...
  pedir POST /v1/cuentas "{\"email\":\"$correo\",\"firstName\":\"$nombre\",\"lastName\":\"$apellido\"}"
...
pedir GET /v1/trabajos "" "$acceso_admin"
...
pedir GET "/v1/trabajos/${trabajo_de[E2]}" "" "$acceso_alumno1"
...
observaciones_de() { pedir GET "/v1/trabajos/${trabajo_de[$1]}" "" "$acceso_alumno1"; }
